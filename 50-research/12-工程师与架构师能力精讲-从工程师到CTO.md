...
> **另一组轴线（治理 vs 管理，与 08 号 §九 一句话核心判据一致）**：本节是「管理 vs 技术领导」，不要与「治理 vs 管理」混淆——治理就是跨层级的远程校正机制，专职守护不变量（标准/原则/基线），而管理只在框架内调变量、达成运行目标。架构师对目的的「守护责」属治理一侧（跨层级校正、守不变量），团队的自愿执行属管理一侧（框架内达成运行目标）；两组轴线正交，不可互相替代。
