...
> **日期**：2026-08-23（v1.3：同日补充 §12 架构师与架构治理，待办顺延 §13）
...
**分形不对称**（模块级比系统级**少**的东西，不是遗漏而是结构）：

| 模块级没有 | 原因 |
|---|---|
| 模块架构 | 模块的"架构"就是上层架构的分配 |
| 模块确认 | 确认（validation）必须发生在真实使用环境（系统级） |
| 模块部署 | 部署是系统级行为 |
| 模块集成方案/报告 | 集成主要发生在系统级（把模块组装起来） |

**模块工程师的职责**（模块级局部闭环）：模块需求（细化上层分配的派生需求）→ 模块设计（架构边界内，内聚/耦合，Parnas 意义上的模块化设计）→ 模块实现（可独立编译/组合/加载）→ 模块验证（证明模块满足模块需求——**注意：模块验证证明"模块做对了"，系统级验证/确认仍是系统工程师和架构师的**）。
...
**结构洞察**：对照 15288 的"系统 → 系统元素"递归层级，清单恰好两层；且模块级**没有**架构/确认/部署/集成——正是 §6.3 的"分形不对称"实证。
