...
> **文档版本**：v1.10（v1.0 = 2026-08-17 整理初版；v1.1 = 2026-09-04 定义升格 2022、与 06 号 v4.7 同步；v1.2 = 2026-09-21 与 30 号对齐；v1.3 = 2026-09-21 第二轮收口·其余术语按 GB/T 45630-2025 统一；v1.4 = 2026-09-21 依 29 号对齐架构治理口径；v1.5 = 2026-09-21 治理侧术语按 28 号统一·定义句三职能作「担责」；v1.6 = 2026-09-21 依 24 号对齐架构与设计边界口径（过程≠阶段、种类≠粒度、元素三层、判据精确化）；v1.7 = 2026-09-21 第七轮·15288 版本锚定与「架构粒度」措辞收口；v1.8 = 2026-09-21 第十轮·依 23 号对齐；v1.9 = 2026-09-21 第十一轮·依 22 号对齐（术语权威层 · 两哲学 · X／Z 块 · 清单化定位 · 译名统一）；**v1.10 = 2026-09-24 第十二轮·依库内标准正本结清挂账（G-10／G-30／G-32／G-33／G-35／G-36 与 §5.2.2「未逐字确认」）**）
> **整理日期**：2026-08-17
> **修订日期**：2026-09-21
...
>
> **修订记录（2026-09-24 · v1.9 → v1.10｜依库内标准正本结清挂账）**：依 `90-治理登记/02-标准条款号核对表.md` §五（G-10／G-30／G-32／G-33／G-35／G-36）与库内 42010:2022、15288:2023 正本，就地改 7 处，**结构、结论、节号、表格行数与 mermaid 图结构一处未动**——①**§12.1 撤回「架构视角是一种工作产品（work product）」（G-10）**：42010:2022 **§3.8 定义句是裸名词短语**、**句中无 work product**——定义换成正本逐字 *set of conventions for the creation, interpretation and use of an architecture view (3.7) to frame one or more concerns (3.10)*；**保留**标准明写的两条：架构描述 AD ＝ work product（§3.3）、架构视图 ＝ **information part**（§3.7）。②**§12.1「架构视图」的英文括注 `information item` 改 `information part`（G-32／G-11）**——42010:2022 用 `information part`（§3.14），`information item` 是 15288:2023 §3.18 的术语；引 15288／42020 原文处照原文不改。③**§7.1.1 章题、引文与 2×2 矩阵栏名按正本派名校正（G-36）**：「Conception 派／Perception 派」改按 **42010:2022 附录 A.2** 逐字派名 **"Architecture as concept"／"Architecture as property"**（全文检索 `Perception` 零命中；旧派名括注标「2011 口径的二手转述派名」）；原标 2011 的引文块明标「**42010:2011 口径，库内无 2011 全文故不可核**」，并注明该句在 42010:**2022** 英文版与中英对照版**零命中**、不得以现行版身份引用；§7.3 的「Conception／Perception」同改。④**§7.5 撤回「标准用 architecting 这个词但没有给它立词条定义」（G-30）**——42010:2022 **§3.1 architecting 就是词条**（同节前半已引其七步动作集，句内自相矛盾），改正面表述。⑤**§8.5「15288:2023 界定系统是 man-made」改正（G-33）**——正本全文 `man-made` 仅 1 命中且在 **§3.21 life cycle** 的 *human-made entity*，**不在 system 定义里**；改按「人造实体（*human-made entity*）」并注明 §3.46 未作人造物限定。⑥**§7.4 的「条款 5.2.2，未逐字确认」结清**——正本 §5.2.2 节题 *Architectures and architecture descriptions* 已逐字核对（列举语 *The architecture of an entity of interest can pertain to any or all of the entity's:*），**撤「未逐字确认」**，并把该条从「AD 的内容要求」正位为「该组要件的正本落点」。⑦**G-35 属概念纪律**：本文 §7.1.1 与 §12.1 不涉「artefact 与 work product 不可互译」的化用，本轮仅随 §12.1 的属概念更正一并核过，**无改动**（该条落在 10／12 号）。**未改**：§1～§6、§7.1、§7.2、§7.4 其余行、§8.1～§8.4、§8.6～§11、§12.2～§14；全部修订记录中的旧条款号与旧表述一律保留原样（历史不改写）。
...
#### 7.1.1 定义背后的两种哲学：Architecture as concept 与 Architecture as property

"concepts **or** properties" 这个析取式不是修辞，而是**哲学兼容设计**。标准正本 42010:2022 附录 A.2 逐字给出两派派名与表态：
...
〔按：下引 *Under either philosophy, an architecture is abstract — not an artifact* 与其后的 AD 段，出处为 42010:**2011** 版配套解释（经 S-6 二手转述），**42010:2011 口径、库内无 2011 全文故不可核**，且该句在库内 42010:**2022** 英文版与中英对照版均**零命中**——**不得以现行版身份引用**。〕2011 版配套发布页（iso-architecture.org／"Defining architecture"）原文：
...
>
> - **架构作为构思（Architecture as concept；2011 口径作 Conception 派）**：架构是某个**存在于人脑中**的系统概念（**架构师中心**）；
> - **架构作为属性（Architecture as property；2011 口径作 Perception 派）**：架构是**所关注实体**（entity of interest）的一项属性或特质（**观察者中心**）。
>
> 无论哪一种哲学，**架构都是抽象的——不是人工制品**（**42010:2011 口径，库外不可核**）；本标准使用另一术语——**架构描述（architecture description）**——来指代用于表达和记录架构的**工作产品**。

两派的分工，可排成一张 2×2 矩阵（依 22 号 §8.1；栏名按正本 42010:2022 附录 A.2 派名，括注为 2011 口径的二手派名）：

| | **Architecture as concept 侧**（2011 口径：Conception 派） | **Architecture as property 侧**（2011 口径：Perception 派） |
|---|---|---|
| **架构（abstract，抽象物）** | 架构师脑中的 concepts：理念、思路、意图 | 观察者感知到的 properties：系统的客观特征 |
| **架构描述 AD（work product，工作产品）** | 描述／记录出来的 concepts：术语表、定义文档、原则说明 | 描述／记录出来的 properties：视图、模型、规格表、数据字典 |
...
- **"or"（原文）**：你可以用**概念角度**或**属性角度**来描述架构——两种视角都是有效的，任选其一即可。这两个角度各有其名：**概念角度＝Architecture as concept**（构想说；架构是**人脑中**的实体概念，架构师中心），**属性角度＝Architecture as property**（属性说；架构是**所关注实体**的属性或特质，观察者中心）——正本派名见 42010:2022 附录 A.2；「Conception 派／Perception 派」为 **2011 口径的二手转述派名**（库外不可核）。两派的分野、*without prejudice* 表态与哲学底，见 §7.1.1；并读 22 号 §5.6／§8.1。
...
另外，定义正文的 "embodied in" 在 2022 版**已删去**——2022 不再以元素与关系界定架构；该组要件在正本中的落点是 **42010:2022 §5.2.2**（节题 *Architectures and architecture descriptions*，**已逐字核对**），列举语逐字为：*The architecture of an entity of interest can pertain to any or all of the entity's:* — constituent elements；interactions or interrelationships among its elements；interactions or interrelationships with its environment, including with other entities in that environment；behaviour and structure；principles governing its design, use, operation and evolution。〔此处与下条〔按〕是**两件不同的事**：本条给的是该组要件的**正本落点**（42010:2022 §5.2.2，**已核**）；下条给的是该层**术语含义的新出处**（42020:2019 §3.3 Note 1）。二者不是同一个去向，不可并读。〕
...
**architecting（42010:2022 §3.1）** 是七步动作集：*conceiving, **defining**, expressing, documenting, communicating, certifying proper implementation of, maintaining and improving*——**构思／定义／表达／记录／沟通／认证实现／维护改进**。清单化的定义住在其中的 **`defining`** 一环，是这一步的产物；它不是"绕过标准"的做法。须注意两点：`defining`（动词）产出的是一批 definition（名词），二者不是一回事（见 §7.1 的 definition 定位）；`architecting` 本身**是 42010:2022 §3.1 的正式词条**（上引七步动作集即其定义句），**不是「用了词却无定义」**。
...
注意这个问句的后半必须挂到目的上：**人造系统的同一性是目的论的，不是实体论的**（15288:2023 未在 system 定义句 §3.46 里作人造物限定；「人造实体」*human-made entity* 一词见 §3.21 life cycle 定义句）。忒修斯之船是最好的思想实验——木板一块块全换掉，它还是那条船（目的延续）；一块木板不换、拖进博物馆当展品，它反而不再是原来那条船（目的改变）。同一性跟着目的走，不跟着物质走。推论：**目的漂移是架构失效的第一病因**；目的变更 = 架构重审的强制触发器。〔治理侧接驳：此问句的判据端是**目的**，而目的的定义权在治理机构、不在架构师，架构与目的的一致也不会自己保持——这正是架构治理存在的理由（29 号 §3.1 前提一／前提二）；治理对象四层中与本节最贴的一层是「架构决策」（29 号 §4.1）。〕
...
**架构视角（Architecture Viewpoint）** 是**用于创建、解释和使用架构视图以框定一项或多项关注点的一组惯例**（*set of conventions for the creation, interpretation and use of an architecture view (3.7) to frame one or more concerns (3.10)*，42010:2022 §3.8 逐字）——它规定架构视图的构建、解读和使用所应遵循的约定，用来框定特定的关注点。它规定了：
- 框定哪些关注点（concerns）
...
**架构视图（Architecture View）** 是**组成架构描述的一部分的信息部件**（**information part**，42010:2022 §3.7「*information part (3.14) comprising portion of an architecture description (3.3)*」）——按某个架构视角的约定实际产出的架构表示：一张类图、一份部署拓扑、一个用例模型。
