...
>
> **术语夹注（2026-09-21；同日 v1.1 纠错）**：〔按：ISO 9000 的优先术语是 interested party，**stakeholder 是其许用术语**（2015 §3.2.3 索引标 admitted term；2026 §3.1.4 两词平列同行），GB/T 19000 译「**相关方**」；ISO/IEC/IEEE 15288（GB/T 22032）与 42010（GB/T 45630）的优先术语是 stakeholder，译「**利益相关方**」。**英文同词、中文分译两名**——实据在两处定义不同（9000 取「能影响／受影响／自认受影响」，15288 取「拥有权利、份额、主张或利益」），**理由在定义上，不在英文词上**。故本报告所述 ISO 9000:2026 §4.4.2「相关方」等一律读作 interested party，不改。另：§4.3「基本概念与原则」的 fundamental 系 ISO 9000 自身概念义，与 ISO/IEC/IEEE 42010 的 fundamental（本仓库定名「基本」）**同名不同义**，不受术语收口影响。〕

> **版本**：**v1.2（2026-09-24 依库内正本复核附 D）**——① **D1 已确证**（2026 侧「引导句在、列表缺」、2015 侧列表完整，均系两侧全文实测）；② **§7.1 表中「§5.4 原则措辞须全文复核」可升级**为「已在库内、可径核」（2026 §4.2.1–§4.2.8 与 2015 §2.3.1–§2.3.7 全段在库，位置迁移 2015 §2.3.x → 2026 §4.2.(x+1) 可逐条核对）；③ **本报告所引两版全文现均在一手位**（含中英对照版），附 A 的 138 条映射表已由独立程序化复核为 **0 错**；④ **附 D 仍不可结清的只剩 D3／D4／D6**（依赖 ISO/IEC Directives、TC 176 决议、ISO 10007／TS 10020 等**库外**来源）。详见文末补记。v1.1（2026-09-21 术语纠错）；v1.0（2026-09-13 初版）。
...
3. **文件状态提示**：两份全文 md 及其源 DOCX/PDF 已于 2026-09-12 连同 26 号报告一并入库（提交 `be0ac7a2`、`a9a6176f`）。26 号报告原有两处"本仓库不存 ISO 全文"的口径声明（§7.3 的"本仓库口径"段、头部 ★ 声明第 3 条），与该事实不符；**两处已于 2026-09-13 依用户指示清理**（26 号 v1.5——§7.3 那段移除，★ 第 3 条改为只约束该报告自身不转录原文）。本报告的比对结论不受此事影响。
...
| 领域标注（subject field） | 7 类 19 处 | 13 类 44 处 | 新增"财务与经济效益""组织变更管理""质量文化"等 8 类 |
...
| 3.2 organization | **3.1** organization | **−1** | "人"组撤销后前移一位；customer 移出至 3.9；metrological function 移出至 3.11 |
...
**结构性看点**：
- **`⟨financial and economic benefits⟩` 是新增的最大簇（8 条）**——全部对应 ISO 10014《质量管理体系—组织质量结果的实现—财务与经济效益实现指南》修订后的内容：metric、lagging/leading indicator、productivity、dashboard、economic/financial benefit、process approach。
...
>
> **第 5～8 条一并提示（v1.1 补注）**：这四条**只是许用术语／缩写并入标题行的著录变动，语义两版完全相同**——尤其**第 6 条**：`stakeholder` 在 **2015 §3.2.3 本就是 interested party 的许用术语**（正文以斜体单列一行，索引标 `stakeholder (admitted term for interested party)`），2026 只是把它并进标题行。**凡读作"2026 才把 stakeholder 与 interested party 视为同一概念"者，是误读**；本子项目一度据此写下「两词不是同一个英文词」的断言，已于 2026-09-21 全目录更正（见下条更正说明）。
>
...
>
> **一处本报告自身的更正（v1.1）**：§3.2 组级映射表原把「3.2 organization → 3.1 organization」的"移动"栏记为「号不变」，**系笔误**——组号由 **3.2 减到 3.1**（"人"组撤销后前移一位），与同表其余各行体例一致。已就地改正。
...
> **一处必须补全的读法（v1.1）**：上表只讲了注 3——**改变的是"主判据"的措辞方向，"无责任关系"这条老的证明方式并未消失**。2026 **注 4** 原文仍写着：
>
> > *"Internal audits are conducted for management review (3.11.2) and other internal purposes and can form the basis for an organization's declaration of conformity. Independence can be demonstrated by the freedom from responsibility for the activity (3.2.12) being audited."*
>
> 即注 3 给的是**人选的资格要求**（经甄选保证公正与客观），注 4 给的仍是**独立性的一种证明方式**（免于对所审活动的责任）。两者并存，不互相取消。**凡据"审核独立性＝与被审对象无责任关系"立论者，宜改按注 3 的正向口径重述，同时知道注 4 仍留此路**——不要写成"2026 取消了这条判据"。
...
| **R6 关联标准联动** | 38 条新增中的约 22 条；4 条删除 | 财务与经济效益簇、配置簇重构 |
...
| **ISO 10014**（财务与经济效益） | **8 条新增**：economic benefit、financial benefit、metric、lagging indicator、leading indicator、productivity、dashboard、process approach | §3.4.13／§3.4.14／§3.7.4~§3.7.6／§3.7.7／§3.8.2／§3.3.4 |
...
| # | 存疑项 | 性质 | 处置建议 |
|---|---|---|---|
| D1 | **characteristic §3.10.1 注 3 的 a)~f) 六类分类**（physical／sensory／behavioural／temporal／ergonomic／functional）在 2026 转换件中**只有引导句、无列表** | 疑为**转换丢失**（2015 该列表完整） | **✅ 已确证（2026-09-24 两侧全文在库复核）**：**2026 侧确为「引导句在、列表缺」**——§3.10.1 注 3 至 *There are various classes of characteristic, such as the following:* 即断，直接跳 §3.10.2；**2015 侧该列表完整**（a) physical／b) sensory／c) behavioural／d) temporal…）。**故「转换丢失」的判断成立**，仍**不得据本报告判定为「删除」** |
...
> **★ 2026-09-24 补记（本轮依库内标准正本复核）**：① **§7.1 表中「§5.4 原则措辞不可据以引用｜部分结清，须全文复核」可升级**——库内 2026 全文含 **§4.2.1–§4.2.8 全段**（§4.2.1 General ＋ 七项原则），2015 全文含 **§2.3.1–§2.3.7**；两版**位置迁移可逐条核对**（2015 §2.3.x → 2026 §4.2.(x+1)，序位不变），「须全文复核」的门槛降为「**已在库内，可径核**」。② **本报告所引 ISO 9000 条款号的核对状态整体升为一手**：两版全文（2026 英文版＋中英对照版、2015 英文版）均在 `00-系统工程相关研究报告/` 下，附 A 的 138 条映射表**已由第三方独立程序化复核为 0 错**（见 `90-治理登记/02-标准条款号核对表.md` §三）。③ **附 D 仍不可结清的只有 D3／D4／D6**（均属 ISO/IEC Directives、TC 176 决议、ISO 10007／TS 10020 等**库外**依赖）。
...
> **版本**：**v1.1 · 2026-09-21**（本报告自身的三处回修——① **§3.2 组级映射表笔误**：「3.2 organization → 3.1 organization」的"移动"栏原记「号不变」，应为 **−1**，已改正；② **§4.4 audit 独立性判据补全**：原只讲注 3，现补出 **2026 注 4** 仍保留 *"Independence can be demonstrated by the freedom from responsibility for the activity being audited"*——「无责任关系」并未被取消，改变的是注 3 主判据的措辞方向；③ **§3.5 改名表第 5~8 条补注**：这四条只是许用术语／缩写并入标题行的**著录变动**，语义两版相同，其中第 6 条 `stakeholder` 在 2015 §3.2.3 **本就是** interested party 的许用术语，勿读作"2026 才视为同一概念"。另**头部术语夹注按仓库《ISO 9000:2026 中英文对照版·译注 1》口径改写**——原文写「interested party 与 stakeholder 不是同一个英文词」失实，已改为「英文同词、中文分译两名，实据在两处定义不同」。三处回修与夹注改写**均不动实测数据与迁移结论**，附 A 138 条映射一字未改）
> **v1.0** · 2026-09-13（初版：两侧全文实测对照，建立全 138 条新旧条款号对照表，结清 26 号报告的 6 项待核，更正 3 处）
