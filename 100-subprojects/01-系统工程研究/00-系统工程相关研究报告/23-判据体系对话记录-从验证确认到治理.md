...
> 〔按：工作产品层的替代判定只是"**实现之前的可判定代理**"（24 号 §4.3）——design 的职责是 *sufficient to enable*，**合规的承担者是 implementation**，故该层的结论形态只能是**条件性合规**（若实现如承诺则合规），真正的合规必须由实现举证（24 号 §10.7）。即 technical review 不产出 validation 结论，只产出**条件性**结论。〕
