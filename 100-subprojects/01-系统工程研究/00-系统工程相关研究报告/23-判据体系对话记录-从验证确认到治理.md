...
> **文档版本**：v1.2（记录 2026-09-11；v1.1 加夹注；v1.2 第二轮收口夹注，未改原话）→ v1.3（2026-09-21 依 29 号对齐架构治理口径：只加夹注，未改原话）→ v1.4（2026-09-21 三职能句按 28 号统一作「担责」）→ **v1.5（2026-09-21 术语按 28 号统一：宗旨／担责；只改夹注，未改原话）** → **v1.6（2026-09-21 第四轮：依 27 号对齐 ISO 9000:2026 版本口径；只加夹注，未改原话）** → **v1.7（2026-09-21 第五轮：术语纠错；只改既有夹注，未改原话）** → **v1.8（2026-09-21 第六轮·依 24 号对齐：只加夹注，未改原话）** → **v1.9（2026-09-21 第七轮·15288 版本锚定：只加夹注，未改原话）** → **v2.0（2026-09-21 第九轮·15288:2023 过程号结清：只改夹注，未改原话）**
> **夹注约定**：本轮对齐以「〔按：…〕」形式就地加注，**原话一字不改**。**2026-09-21 第二轮收口**：governing → **管控原则**、life cycle → **生存周期**、stakeholder → **利益相关方**（fundamental 取「基本」）；指 stakeholder 的「相关方」同改，ISO 9000/9001 原文的「相关方」（interested party）保留原样。**2026-09-21 第三轮（依 29 号对齐架构治理口径）**：本记录所说的「治理」是 ISO 37000 的**组织治理**（判据阶梯第五级，对象＝组织）；**架构治理**（architecture governance）是同一职能在架构这一领域对象上的专门展开，其定义、目的、对象、输入与输出、责任主体与机制一律以 **29 号《架构治理研究报告——从标准依据到责任主体》**为准（治理本体在 28 号）。凡涉治理的**层级与对象**、**ISO/IEC 38500 的版本口径**与**「自指」**三处，就地加「〔按：…〕」夹注；**原话仍一字不改**。 **2026-09-21 第四轮（术语与判据措辞统一）**：治理三职能的第三成分按 **28 号 §2.3.3** 作「**担责**」（担责＝术语位与职能位、问责＝动词位、述责＝动作层；禁用履责／追责）；**判据族分工**——三条分界判据判「算不算治理」，与「跨出单管理层权限，就要治理介入」这条**介入门槛**（判该不该上治理）分开用，两者不可互换。**仍不加改原话、不重复加注。** **2026-09-21 第五轮（依 27 号《ISO 9000:2026 相对 2015 版差异研究报告》对齐 ISO 9000 版本口径）**：本记录所引 ISO 9000 条款号一律按 27 号附 A 迁移映射表**换算为 2026 号后阅读**——requirement §3.6.4→**§3.5.1**；objective §3.7.1→**§3.7.11**；policy §3.5.8→**§3.4.5**；vision §3.5.10→**§3.4.10**；mission §3.5.11→**§3.4.11**；strategy §3.5.12→**§3.4.12**；process §3.4.1→**§3.3.1**；procedure §3.4.5→**§3.3.2**；defect §3.6.10→**§3.5.14**；traceability §3.6.13→**§3.5.11**；verification §3.8.12→**§3.11.12**；validation §3.8.13→**§3.11.14**；audit §3.13.1→**§3.12.1**；audit criteria §3.13.7→**§3.12.14**；auditee §3.13.12→**§3.12.5**；inspection §3.11.7→**§3.11.11**；review §3.11.2 原号未变。**尤须注意**：原话把 2015 的 verification 记作 §3.8.5、validation 记作 §3.8.6 是**条款号错误**（§3.8.5 是 document、§3.8.6 是 documented information；2015 正号为 §3.8.12／§3.8.13）——引文无误、号错，详见第 2 轮夹注。另：`outcome` 在 2026 统一为 `result`（§3.7.11 注3 现作 "an intended result"）；audit 定义 2026 删去 `documented` 与 `objective` 两限定词，独立性判据改为"由经甄选以保证审核过程公正与客观的人实施"；**review／verification／validation 三词 2026 同归 §3.11「确定」组，「分组不同」的论据失效，改以定义的动作词为据**。**原话仍一字不改，夹注不重复加。** **2026-09-21 第五轮：术语纠错**——既有夹注写「ISO 9000／9001 原文出现的『相关方』是 interested party 的国标译文，与 stakeholder 不是同一个英文词」**失实**（stakeholder 是 ISO 9000 中 interested party 的**许用术语**，2015 §3.2.3／2026 §3.1.4 两版皆然），按仓库《ISO 9000:2026 中英文对照版·译注 1》口径就地改正该夹注；命名政策不变。**原话仍一字不改。** **2026-09-21 第六轮·依 24 号对齐**：用户令「以 24 号研究报告为依据，把 24 号之前的报告与之对齐」。本记录涉**架构侧接口**者按 **24 号《架构与设计边界研究报告——从定义辨析到架构视角与架构视图》**（v1.6）就地加「〔按：…〕」夹注 4 处——①**第 8 轮末**：本记录的「判据阶梯」与 24 号 §6.6 的「**判据栈**」是两回事（栈＝①目的必要性总闸 → ②可违反性 → ③可引用性／④可判定性两**代理** → 关注点闸门，环境为第三类硬条件；**① 才是判据来源**）；②**第 15 轮末**：`specified` 的门槛词是 **`stated`**（§3.5.1 注2），"可对照"是**可判定性代理**的投影（24 号 §6.1、§6.6、§10.2）；③**第 19 轮「判的性质」处**：「符合性」＝ISO 9000 的 conformity，与**架构合规**（对象是 architecture、须与 42010:2022 §4 Conformance 分开）不同层（24 号 §4.1、§11.5）；④**第 19 轮冲突 1 处**：工作产品层的替代判定只是**实现之前的可判定代理**，结论只能是**条件性合规**，**合规的承担者是 implementation**（24 号 §4.3、§10.7）。**原话仍一字不改，节号、表格行数、图结构一律不动。**
> **2026-09-21 第九轮·15288:2023 过程号结清**（v1.9 → v2.0）：ISO/IEC/IEEE 15288:2023 的**技术过程号已核实**（证据＝INCOSE SEH 5.0 中文版 2023-07 所印条款号：验证 §6.4.9 见 p168、确认 §6.4.11 见 p176，技术过程全段 p132–p186）。本记录**只改既有夹注 1 处**——第 2 轮末「2023 版号未核」的〔按〕改记**已核·同号**（verification＝§6.4.9、validation＝§6.4.11）；**第 1–25 轮原话一字未改**，其余夹注一处未动、亦不重复加注。**仍未核**：2015 版术语条款号在 2023 版的对应号（本地无 2023 版原文）。
> **概念依据**：架构、关注点、架构视角、架构视图四概念的定义、关系、界分与译名，一律以 30 号《架构描述核心概念研究报告》为准（依 GB/T 45630-2025／ISO/IEC/IEEE 42010:2022）。**边界与判据口径**：架构与设计的边界（architecture 是抽象物、可交付物是架构描述 AD、**合规的对象是 architecture**）、「基本」的判据栈、合规判定的三条门槛与可采性四条件，一律以 **24 号《架构与设计边界研究报告——从定义辨析到架构视角与架构视图》**为准（24 号 §4、§6、§10、§11）。
...
> 〔按：上列两条 ISO 9000 条款号有误，**引文无误、号错**——ISO 9000:2015 的 verification 实为 **§3.8.12**、validation 实为 **§3.8.13**（§3.8.5 是 document、§3.8.6 是 documented information）；2026 版对应 **§3.11.12／§3.11.14**（27 号 §7.1 结清）。本记录各轮的 §3.8.5／§3.8.6 均照此读。〕
>
> 〔按：15288 现行版为 **2023**（替代 2015 版）；上列条款号（§6.4.9.1／§6.4.11.1 与 NOTE 1）系 **2015 版序**；两个过程在 2023 版**同号**——**已核：verification＝§6.4.9、validation＝§6.4.11**（证据＝INCOSE SEH 5.0 中文版 2023-07 所印 15288:2023 条款号，p168／p176）。原话保留。〕
...
> 〔按：`life cycle` 的规范定名是**生存周期**（30 号 §7.4）；本文属原样记录，保留原话「生命周期」，下同。〕
...
> 〔按：本轮的版本对照成立，但 2015 侧条款号须订正——validation 是 **§3.8.13**、verification 是 **§3.8.12**（非 §3.8.5／§3.8.6）。**2026 版沿用了这一分岔**：validation §3.11.14 仍不带 "specified"，verification §3.11.12 仍带 "specified requirements"，故"标准自己承认确认的判据不在明示需求内"这条证据在现行版下照旧成立。2005 侧条款号本仓库无原文可核，未改。〕
...
> 〔按：**这条位置论据在 2026 版失效**——2026 把 verification／validation 两条术语从 §3.8 数据组迁入 §3.11「确定」组，三词同组（review §3.11.2、verification §3.11.12、validation §3.11.14），"评审不在 §3.8"不再构成与另两者的分野。**结论不变，论据下沉为定义的动作词**：review 建立在 **determination**（§3.11.1 确定）之上、定义中不出现 "objective evidence"；verification／validation 用 **confirmation** 且必须 *through the provision of objective evidence*。〕
...
> 〔按：本轮（含以下各轮）的「治理」是 **ISO 37000 的组织治理**——**对象是组织**，这是治理的**本体**层（本体与六类域治理实例见 28 号《治理概念研究报告》）。**架构治理**（architecture governance）是同一职能在**架构**这一领域对象上的专门展开，其定义、目的、对象、输入与输出、责任主体与机制以 **29 号《架构治理研究报告——从标准依据到责任主体》**为准：架构治理是治理机构对「架构」这一领域对象持续行使指导、监督、担责的职能——通过明确决策权、设定规则、检查符合性、处置例外、监视架构状态，使架构在持续演化中保持与宗旨的一致（四要素＝主体／对象／手段／目的，缺一即不成立，29 号 §2.1）。故「治理的对象是组织」这条**只对组织治理成立**：领域治理的对象是该**领域对象**，架构治理的对象是架构决策、决策遵从、架构演化、架构资产四层（29 号 §4.1），其裁量基准是**宗旨**（29 号 §3.1 前提一）。另记一条事实：架构治理在 ISO/IEC/IEEE 体系里**没有正式术语定义**（42010 范围排除过程与方法、42020 只作过程名、38500 不说「架构治理」一词），定义只能取 TOGAF、治理语义只能取 ISO/IEC 38500（29 号 §1.1）。〕
...
> 〔按：两处条款号迁移为 **§3.12.1 audit／§3.12.14 audit criteria**。**2026 的 audit 定义删去了 `documented` 与 `objective` 两限定词**（定义改为 *systematic and independent process for obtaining **evidence**…*），故引文应作 *systematic and independent process…*；**audit criteria 的定义直接写为 *"set of **requirements** used as a reference against which objective evidence is compared"***——policies 与 procedures 移入该条注2，"判据回落到需求家族"这条论据在 2026 版反而更硬。〕
...
> 〔按：条款号迁移为 **§3.7.11 objective**。另，2026 的注2 措辞有变——"战略层"移入**注1**（战略／战术／运行），注2 改列"组织整体或具体的项目、产品、**服务**、过程"；**结论不变**：目标不必然是组织层，故"客体所属组织的目标"这一收窄仍不成立。〕
...
> 〔按：①「治理按动词处理」这一读法与本项目工作定义同构——29 号 §2.1 的架构治理定义同样以动词串表述（对「架构」这一领域对象持续行使**指导、监督、担责**），故组织治理与架构治理在**职能**上同构，差别只在**对象**（组织／架构）。②上列的「动词三连 + 六项原则」是 **ISO/IEC 38500 2015 版**口径：**2024 版**已不再自定原则，改用 ISO 37000 的**十一项原则**，执行模型以**四活动**循环表述（利益相关方参与、评估、指导、监督）；「**四活动**」是 38500 治理执行模型的专名，与 ISO/IEC/IEEE 42020 第 6 章的**六项任务**不是同一物（29 号 §1.1、§1.4）。〕
...
> 〔按：本记录的「判据阶梯」说的是**判定活动各自对着什么尺子**；**24 号 §6.6** 的「**判据栈**」说的是**一条内容够不够格进架构层**的准入判据——① 目的必要性（fundamental，**总闸**，参照**目的**，承重测试）→ ② 可违反性 → ③ 可引用性 → ④ 可判定性 → **关注点闸门**（取舍与呈现）；**环境**（法规／物理条件／外部接口／资源）为**第三类硬条件**（24 号 §6.5）。两处不可混读：**③④ 是可机械检查的代理，① 才是判据来源**（24 号 §6.1、§6.6），接口在 ①——阶梯第五级的「目的」正是栈 ① 的参照。另：本行上文「一旦要可判定、可举证」中的**可判定**，在栈中属**代理**（④），不是判据来源。〕
...
> 〔按：条款号迁移为 **§3.12.5**；且该引文有误——ISO 9000 两版的 auditee 定义句均为 *organization being audited*（2026 §3.12.5），"as a whole or parts thereof" 系 **ISO 19011** 的 auditee 措辞。结论不变：审核的对象是**组织**（体系），不是单个客体。〕
...
> 〔按：该"演进趋势"在 2026 已成**现行条文**——**ISO 9000:2026 §3.12.14 的 audit criteria 定义即 *set of requirements…***，不必等 19011 草案推行。〕
...
> 〔按：**这条判定须收回**——该讲义的两处编号与 2015 实测**一致**：documented information 确为 **§3.8.6**、verification 确为 **§3.8.12**；当时据以判它"不可信"的 §3.8.5 verification／§3.8.6 validation 才是错的（§3.8.5 是 document）。2026 版对应 §3.8.14／§3.11.12（27 号 §7.1 结清）。〕

> 〔按：本轮的「自指」是对**组织治理**说的（判据由判定者自产：「目的」是治理的第一个产出）。这层自指在**架构域**同样成立且同构——架构治理的判据（**宗旨**）由治理机构定义与裁决，架构师只有质询权与守护责，不定义目的（29 号 §3.1 前提一）；但**责任主体是治理机构、不是架构师**，委员会掌裁决、协调人掌过程、架构师掌技术判断（29 号 §6.1、§6.2）。本表「对象 = 组织自身 → 自指」只述组织治理；架构治理作为领域治理，其对象四层与判据见 29 号 §4.1、§2.1，本记录不在架构域内继续下钻。〕
...
> 〔按：ISO 9000:2026 §3.5.1 注2 的门槛词是 **stated**；"写成可对照的形式"是**可判定性**这一**代理**在需求侧的通俗说法——判据栈中 ③可引用性／④可判定性皆系**代理**，判据来源是 **①目的必要性**（24 号 §6.1、§6.6）；24 号 §10.2 亦以"已明示"作合规凭据的门槛词。故上表末行读作"未成文／无法逐条判"，不作"不重要"解。〕
...
> 〔按：此处的「符合性」是 ISO 9000 的 conformity＝*fulfilment of a requirement*，与**架构合规**不是同一层——**合规的对象是 architecture**（15288:2023 §3.13 *compliant implementation **of the architecture***），**不是架构描述**；且须与 **42010:2022 §4 Conformance**（这份架构描述合不合 42010 的要求，描述质量问题）**分开**（24 号 §4.1、§11.5）；**合规 ≠ 正确**——合规属 verification 侧、满足需要／期望属 validation 侧（24 号 §4.5）。〕
...
（IEEE 1028-2008 各术语的**准确条款号待核**——已登记于 `90-治理登记/02-标准条款号核对表.md` §三，取证走该表路径②／④。）
...
> 〔按：工作产品层的替代判定只是"**实现之前的可判定代理**"（24 号 §4.3）——design 的职责是 *sufficiently complete to support*（15288:2023 §3.13 逐字；〔2026-09-24 按：原引 *sufficient to enable* 在正本全文零命中〕），**合规的承担者是 implementation**，故该层的结论形态只能是**条件性合规**（若实现如承诺则合规），真正的合规必须由实现举证（24 号 §10.7）。即 technical review 不产出 validation 结论，只产出**条件性**结论。〕

**冲突 2：inspection 同名，且意思正好相反**

| 标准 | inspection 指什么 | 看什么 |
|---|---|---|
| **ISO 9000:2015** | *conformity evaluation by **observation and judgement** accompanied as appropriate by measurement, testing or gauging*（2015 版 **§3.6.11**——由 27 号附 A 迁移映射表反查已核；2026 版对应 **§3.5.9**） | **看实物** |
...
> 〔按：条款号结清——2015 为 **§3.11.7**、2026 为 **§3.11.11**，两版定义同文：*determination of conformity to specified requirements*。上表所引 *conformity evaluation by **observation and judgement**…* 是 **ISO 9000:2005** 的 inspection 措辞，2015 起已改。故"ISO 看实物、IEEE 看文档"的旧依据（observation and judgement 对 visual examination）不再成立；**"同名异义"的结论不变，改由参照与对象承载**：ISO 9000 的检验以**规定需求**为参照（其结果可作 verification 的客观证据），IEEE 1028 的审查是对软件工作产品的**可视化检查**。〕
