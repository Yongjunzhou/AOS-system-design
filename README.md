...
**产品M（运营体系）** 涵盖市场、研发、生产、售后等业务，同时也包含**"设计运营体系本身"**这项业务。

运营体系中的不同业务活动可按信息处理模式分为13类：计划事务、工单事务、审批流程、合规事务、规范知识、指标看板、进展追踪、信息台账、xBOM（跨领域通用），以及设备IO、领域分析、领域建模、领域算法（领域特有）。

不同模式的活动需要不同的系统来实现，因此产品M必然表现为多个异构信息系统的组合。其中前9类跨领域通用的处理模式可以统一到同一个信息化平台中。后4类领域特有的处理模式各自独立。

**产品L（系统设计流水线）** 处理的是需求项、架构节点、追溯链接这类元信息，属于**规范知识类**处理模式，原生输出形态就是文档+AI，因此以文档形态存在即可满足需求。

**产品L在产品M中的位置：**

运营体系涵盖市场、研发、生产、售后等业务，同时也包含**"设计运营体系本身"**这项业务——产品L就是支撑这项业务的方法和工具集。产品L的 PA 构件（准则、指南、AI辅助文档、任务定义）被放置在 `50-aos-design/10-aos-system-design/` 目录下，作为团队M设计产品M时的操作规程。
...
产品M和产品L遵循同一套五层方法论，但 PA 产物形态不同——不是因为方法论不同，而是因为**产品类型和消费方式不同**：
...
> 产品形态是连续谱。产品L不是"不能"表现为信息系统（可以做成低代码设计平台），而是"文档 + AI"的形态对运营体系开发者而言刚好够用。
...
- **产品M — AOS 企业运营体系**：企业实际运行所依赖的 IT/AI 化运营系统。**团队M** 负责开发和优化产品M。

**团队L的构成**：流水线开发者（用户）+ AI 成员
