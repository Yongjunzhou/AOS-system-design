...
- **EOS 流水线**：服务于 EOS 全生命周期的流水线，由设计线（系统设计）、开发线（组件开发）、集成线（集成交付）、运维线（系统运维）四个子流水线组成。当前设计线已完成，其余待开发。构件位于 `20-pl4eos/10-pl4eos-subpl-sysdev/`。
...
每条流水线的工作空间按子线组织：`10-*-subpl-sysdev/`（系统设计子线）、`20-*-subpl-comdev/`（构件开发子线，待扩展）、`30-*-subpl-sysint/`（集成交付子线，待扩展）。此外，`30-eos/` 为 EOS 产品本体，按四条流水线的产出组织为五个子目录。
...
```
OR（原始需求）→ SR（相关方需求）→ BP（业务流程）→ SysReq（系统需求）→ PA（产品架构）
```
