...
**目的**：将规范化和分解后的相关方需求末级节点分配到业务架构承接层（5 级节点）的节点，确保每条相关方需求有唯一的业务承接点，业务架构完全承接相关方需求

**适用场景**：新增相关方需求的业务架构设计

**重要说明**：本准则处理相关方需求的分配。非功能需求不经过业务架构，直接在系统需求层面标记为系统约束（详见准则 1 第 4 节）
...
**BA承接层**：
- 业务架构的 **5 级节点**（组件层级）
- 直接承接相关方需求末级节点的 BA 节点层级
- 由业务组件和引擎组件组成
- 业务组件：由引擎组件以 UI 配置方式生成
- 引擎组件：业务平台提供的基础引擎

**关键特征**：
- ✅ 位于业务架构的 5 级（组件层级）
- ✅ 与相关方需求末级节点有 N:1 多对一映射关系
- ✅ 每条相关方需求末级节点只能分配给一个 BA 承接层节点（1:1 约束）
- ✅ 有明确的输入（相关方需求末级节点）和输出（业务能力）
- ✅ 可以被追踪和管理
- ✅ 代表用户视角下的业务流程和业务规则
...
查找现有业务组件定义
   ...
   └─ 没找到
      ↓
      根据初步设计的处理方式：
      ├─ 配置改进 → 通过UI配置改进现有业务组件定义或生成新业务组件定义
...
**引擎组件类型**：
- 流程引擎（Process Engine）- 支持流程定义、流程实例、流程监控
- 计划引擎（Plan Engine）- 支持计划定义、计划执行、计划追踪
- 工单引擎（Work Order Engine）- 支持工单定义、工单分配、工单跟踪
- 指标引擎（Metric Engine）- 支持指标定义、指标计算、指标展示
- 表单引擎（Form Engine）- 支持表单定义、表单渲染、数据收集
- 关系引擎（Relationship Engine）- 支持关系定义、关系维护、关系查询
- 其他引擎...
