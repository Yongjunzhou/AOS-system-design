# CLAUDE.md（50-program/05-投资工程研究 子项目）
...
本子项目是 AOS 主项目 `50-program/` 项目集中的一员，各子项目共同指向「**把不同专业的工作或事项工程化**」这一目标；本子项目负责该目标在 **投资**上的展开。**自包含在本目录内**（AOS 主 CLAUDE.md §九）：

- **处理本子项目**：工作目录 = 本目录（`50-program/05-投资工程研究/`），读本文件 + `10-claude memory/`。
