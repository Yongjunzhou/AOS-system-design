...
系统设计流水线（System Design Pipeline）是为 AOS 开发者打造的"设计工具"，帮助他们将原始需求逐层推导至产品架构。产品B 的范围严格限定在产品开发阶段1（系统设计）。
...
产品B 阶段1 的完整工作空间，详见 [10-pipeline-system-design/README.md](10-pipeline-system-design/README.md)。

| 子目录 | 性质 | 文档数 | 说明 |
|--------|------|--------|------|
| `01-system-design-standards/` | 通用（共享） | 8 | 5层结构、映射规则、各层通用规范、术语表 |
| `02-system-design-guidelines/` | 通用（共享） | 6 | 瀑布/敏捷/逆向工程/DevOps 四种指南 + 快速参考卡 |
| `03-system-design-ai-support/` | 产品B 构件 | 17 | AI 文档理解要求 + 四场景辅助文档（各含 README/checklist/example/prompts） |
| `10-system-product-data/` | 产品B 设计输出 | 10 | 标准产品数据（01-OR ~ 09-验证报告 + README） |

### 20-pipeline-component-dev/

产品B 阶段2 的工作空间。当前为空，待系统设计完成后扩展。

### 30-pipeline-integration-delivery/

产品B 阶段3 的工作空间。当前为空，待构件开发完成后扩展。

---

## 产品数据概览

产品数据位于 `10-pipeline-system-design/10-system-product-data/`，按五层结构组织：
