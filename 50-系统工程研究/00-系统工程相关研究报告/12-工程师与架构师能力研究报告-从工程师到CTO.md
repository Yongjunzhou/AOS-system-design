...
> **另一组轴线（治理 vs 管理，与 08 号 §九 一句话核心判据一致）**：本节是「管理 vs 技术领导」，不要与「治理 vs 管理」混淆——治理 = 治理机构对组织行使指导（定方向和规矩）、监督（查执行）、问责（担责）三职能，在管理层之上定组织方向与框架；管理 = 在治理定下的框架内执行、达成运行目标。架构师对目的的「守护责」（做符合性判断并报告偏差）属治理一侧，团队的自愿执行属管理一侧（框架内达成运行目标）；两组轴线正交，不可互相替代。
