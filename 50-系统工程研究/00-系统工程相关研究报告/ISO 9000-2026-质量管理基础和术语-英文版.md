...
5. **术语条目的多术语行**：3.1.4、3.1.9、3.1.10、3.4.9、3.7.15、3.8.3、3.9.9、3.9.11、3.9.12 九条，其优先术语与许用术语／缩写在源 docx 中**同处一行**（如 `interested party stakeholder`）；**仅 3.1.11 分了行**（`DRP-provider` ／ `dispute resolution process provider`）。本文件按 docx 原样照录，未作改动。**此处的行分合不可据以判断标准印刷版面**——源 docx 系重排版产物（逐字符 run、异常右缩进），无助于还原原版行款；在取得正版前按「存疑」记。（中文对照版为消歧另有一套排版，见该文件【对照体例说明】第 3 条。）
