- [培训教材+教材书项目](concept-book-training-project.md) — 2026-08-19：基于50-reserch八份笔记做两个共享同一架构的产出（分组研讨式培训教材 + 独立成书教材书，12章）；书=思想著作/标准当骨架/使命=消除误解/每章以流行误解为主轴；写作进度：ch1✅15.1k→ch3✅19.0k→ch2✅15.7k（2026-08-19修订：删4.3、增3.5工程师六能力清单）→ch4✅13.9k→**ch5需求的完整性✅17k**（三类需求：功能/环境适应性/可实现性+验证载体：原理样机/环境样机/产品+承制方主导；冰链"样机通过=产品成功→量产翻车八条"误解现场+恒信"原型/试点/产品"镜照；需求全不全=承制方责任/可实现性靠组织自觉积累资产）→ch6✅20.3k→ch7✅16.9k→ch8✅16.8k→**ch9架构决策✅17.6k（架构决策不可避免/不可逆决策提前投资；恒信"合同编号想改就改"把数据模型当字段误解现场+冰链"数据所有权被随手一改"镜照；三标志+三层判断框架+期权价值+六问记录；概念卡片×2）**→**ch10配置管理✅17.1k（配置≠参数文件/版本顺着流基线垂直切/没有基线变化就不可言说；冰链"把固件版本当基线、批次追溯查不清"误解现场+恒信"另存为v2"镜照；配置=实体被记录的形态/基线=被批准冻结的参照/版本=时间演进刻度；决策记录六问↔配置信息同构；概念卡片×3）**→**ch11治理✅16.2k（治理≠管理/治理是事件触发/委员会是过滤器；恒信"数据治理平台当清洗工具"术语膨胀误解现场+冰链"断链事故归咎技术问题"镜照；掌舵≠统治词源翻译史+决策系统vs执行系统+三条件+双触发+两种失效+过滤器+三活动循环；概念卡片×2）**；**ch12整合✅17.1k**（所有概念是同一系统的不同切片/顺次vs横切/四条接缝承接参照记录问责/双线合流走七站点时间线，概念跨域成立）**→附录✅**（13-附录.md：A术语表T-01~95英中对照编号（增补T-25工程师）+B概念档案集16张完整九格（工程/需求/设计开发/质量/规定需求/三类需求/验证载体/承制方主导/评审/验证/确认/架构/架构决策/配置/基线/治理）+C误解清单12章病药判据三栏+D标准索引含经典文献+E练习答案要点与自查对错速查）——全书12章+附录全部完成；训练教材按书重派生进行中（00-总览✅题卡设计原则含反直觉强度四维：意外/深度/难度/收获；单元一6题+单元二7题已锁入文件，ch2 三轮打磨=真悖论靶心+覆盖映射表+讲师版同步，Q3/Q5 已按"前提自证"修正——事故原因题面给全、换成恒信集成案例；待铺开3~12）；下一步全书逐章一致性通读
...
- [wft02-eng 待对齐 biz 结构](wft02-eng-align-to-biz-structure.md) — 2026-08-20：新对话任务——以 wft02-biz v8.9 + wft01-eng v8.8 为标杆调整优化 wft02-eng（v8.6 旧结构[概念混排/无转换总览/无自检清单/无反馈双轨]→知识层三章重构 + SKILL v1.0 同步）；关键决策点：语言层级加不加（wft02-biz 有、wft01-eng 无）、Step 框架对齐 v8.8（Step Start 四子节/Step End 三角色四区）；参照 wft01-eng 趟通样板
