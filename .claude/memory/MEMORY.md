...
- [wft01-nfr 深审落地（2026-08-25）](wft01-nfr-deep-review.md) — 知识层三章重构（§二概念含转换总览/§三原理判据含 3.1 转换原理/§四演进/§五操作 3.x→5.x）+ **三准则评审落地**（22 契约 v1.1 兑现：可复用性/可定义性/可验证性，Q3/Q3+ 与 ort03 候选待写→通过写 22 疑似、未通过建议废弃；明确不写具体需求归 wft04）+ OR 残留清扫（改名清扫漏网）+ C2 变更影响声明下游查询 + C3 目标表顺排 + C4 输入校验去重 + E1 待量化示例修正 + E2 补充材料文件名统一「补充原始需求材料」+ **D1 SKILL 数据路径 2→3 级（wft01~05 十 SKILL 全链同修）**；**v6.1 标题正名（人类定名）**「NFR 规范化需求 → bph-nfr · 角色非功能需求分类设计」→「**规范化非功能需求 → 非功能需求概要定义**」全链术语替换（14 文件，bph-nfr 前缀保留、Step 3 活动名「角色 NFR 分类设计」保留）；**挂账闭合（2026-08-25）**——wft04-nfr 知识层三章重构完成（v5.11，nfr 链全链对齐）+ OR 全链 grep 复核 EOS 链 0 残留（17 处清扫补遗清零，通用规范/元流水线保留 OR 为通用方法论术语）；版本 wft01-nfr v6.1/v1.10 + 91 v5.46 + wft04 v5.11/v1.9 + wft06 v3.7/v1.4 + 22 v1.5 + ort01 v0.62/v2.3 + ort03 v0.66/v2.4 + biz v3.32/v1.28 + eng v10.14/v1.23 + wft02~05 七 SKILL 各 +1
