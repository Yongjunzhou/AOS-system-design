...
- [94 号＝全链树权威（2026-09-16 第三次修订）](94-eos-sys-dev-dbs-tree-authority.md) — **v1.26**：业务流程拆**三份文档**（`02` 角色履职视角业务流程概要／`03` 产品开发视角业务流程概要／`04` 角色履职视角业务流程方案，**04 已恢复**）；承接回到 `04→05`／`06→07`；§2.2 补「R／P 只适用于'把业务信息化'这类产品」与「详细定义的落点是界面」；Rn／Pn 末三级＝业务文档⑤→任务节点⑥（枝梢）→用例场景⑦（叶）；**产品数据即资产、R 树与 P 树就是资产**（21／23／25 作废）
...
- [biz 链重编号（2026-09-16）](biz-chain-renumbering-2026-09-16.md) — 01a `stfr2rbph`／01b `rbph2pbph`／02 `rbph2rbpd`；R→P 是**同一个任务位的两半**；**P 侧不需要方案**（界面只属 R 侧）；`04` 恢复；biz02 正文**已重写**（第 1 版）；上轮 `git mv` 改名配错了对（旧 biz03 内容落在 biz02 路径）
