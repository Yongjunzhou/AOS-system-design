...
- [94 号＝全链树权威（2026-09-10）](94-eos-sys-dev-dbs-tree-authority.md) — 94-eos-sys-dev-dbs.md 树形/级语义唯一权威（**v1.13，全文两章**：一「产品数据及其形态」＝§1.1 EOS 的产品数据清单（**原始需求加工链四态**/01/02·04/05·06/07，六·七待定义）／**§1.2 三个前提（为什么可以条目化 / 可以树形化 / 可以 N:1 分配）**／§1.3 两个形态+叶的判据／**§1.4 承接＝「上游最深可用原子 → 下游枝干/枝梢，以语义定级」（"叶→枝梢"只是"上游完整态·下游概要"步上的显影；02↔04、05↔06 不是承接）**；二「各产品数据的分解结构」＝**§2.0 原始需求（入口加工链）**／**§2.1.1 相关方功能需求＝相关方角色→场景业务→需求条目（v1.13 改画：原为 A2 镜像树，01 条目实际止于场景级）**／§2.1.2 相关方非功能需求／**§2.2 业务流程：A2/Bn 末两级并一级（⑤ 输出文档＝枝梢 / ⑥ 活动节点·正文条目＝叶；"任务"是文档的展开项不占树位）**；A1 加【待裁】注（级数与"有无 WBS"同案）／§2.3 系统需求／§2.4 平台构成／§2.5 配置方案／§2.6 前后端组件方案；**94 只给结论+依据名称，完整论证在 95**）；叶=末级·枝梢=叶上一级·概要=根→枝梢·完整态=根→叶；**92 号已删除退役**；已裁 SR 末级（91 v5.54 枝梢⑤/叶⑥，§10.3/§10.4 残留待清）；**待裁 A1 树↔91 §3.1（与 WBS 并案）** + 07/§2.4 平台构成口径；外引联动 91 v5.57 / 93 v0.11 / 05 v3.9 / 01-pl4eos-spec v2.2；**v1.12+v1.13 未提交**
...
- [分类视角 vs 页面视角（2026-09-10）](page-view-vs-classification-view-correspondence.md) — 两视角正交（③类单归属 / 功能表单多归属）；功能表单↔A2/Bn **⑤输出文档（枝梢）下的任务步骤**（v1.13 校正，原记⑥任务节点）、**配置信息组↔功能表单的配置信息组区**；SR 树**枝梢＝⑤ 用例场景功能 / 叶＝⑥ IPO功能**（只在完整态）；功能表单**不属于** SR 树；91 已改 v5.54；未决 A1 是否有 ⑦ 用例场景类操作层（与 WBS 并案）
