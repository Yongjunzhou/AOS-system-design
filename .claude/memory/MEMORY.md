- [培训教材+教材书项目](concept-book-training-project.md) — 2026-08-19：基于50-reserch八份笔记做两个共享同一架构的产出（分组研讨式培训教材 + 独立成书教材书，11章）；书=思想著作/标准当骨架/使命=消除误解/每章以流行误解为主轴；写作进度：ch0✅15.1k→ch2✅19.0k→ch1✅15.7k→ch3✅13.9k→ch4✅20.3k→ch5✅16.9k→ch6✅16.8k→**ch7架构决策✅17.6k（架构决策不可避免/不可逆决策提前投资；恒信"合同编号想改就改"把数据模型当字段误解现场+冰链"数据所有权被随手一改"镜照；三标志+三层判断框架+期权价值+六问记录；概念卡片×2）**→**ch8配置管理✅17.1k（配置≠参数文件/版本顺着流基线垂直切/没有基线变化就不可言说；冰链"把固件版本当基线、批次追溯查不清"误解现场+恒信"另存为v2"镜照；配置=实体被记录的形态/基线=被批准冻结的参照/版本=时间演进刻度；决策记录六问↔配置信息同构；概念卡片×3）**→**ch9治理✅16.2k（治理≠管理/治理是事件触发/委员会是过滤器；恒信"数据治理平台当清洗工具"术语膨胀误解现场+冰链"断链事故归咎技术问题"镜照；掌舵≠统治词源翻译史+决策系统vs执行系统+三条件+双触发+两种失效+过滤器+三活动循环；概念卡片×2）**；下一步ch10整合（开工要点已写，新会话直接用）→附录+通读→训练教材重派生
- [wft01-biz 审视发现与运行模型定案](wft01-biz-review-ort02-benchmark.md) — 2026-08-19：可读性审视七轮全落地（v3.4~v3.10，知识层三章+链级术语统一）+ 原理与方法审视全落地（v3.11：PCA 计划清单强制要求/C-A 运行时口径/流程骨架同构/R4 原理/渐进闭环收敛/治理判据/规则地图补齐，SKILL v1.8）+ 任务节点业务确认定位（v3.12：捕获节点=供人类确认业务流转关系）
