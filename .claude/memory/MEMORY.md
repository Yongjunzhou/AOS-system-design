...
- [反馈：变更记录要简洁（2026-09-10）](feedback-changelog-concise.md) — 变更记录一行一条摘要，不写论证/不抄正文/不复述删改；论证归正文与研究报告（人类就 94 号"内容太多了"提出）
- [反馈：草稿不转 PDF（2026-09-10）](feedback-no-pdf-for-drafts.md) — 研究报告/草案类先只出 md，定稿后再 md2pdf；已有 PDF 的规范文档内容修改仍照常重生成
- [94 号＝全链树权威（2026-09-10）](94-eos-sys-dev-dbs-tree-authority.md) — 94-eos-sys-dev-dbs.md 树形/级语义唯一权威（**v1.15，全文两章**：一「产品数据及其形态」＝§1.1 EOS 的产品数据清单（**原始需求加工链四态**/01/02·04/05·06/07，六·七待定义）+ **业务树名矩阵（两视角族×产品索引：R 树＝角色履职视角↔SL／P 树＝产品开发视角↔PL；i＝0 平台本身、i≥1 各业务；原 A1/A2/Bn 名表作废、A1 归位 P₀）**／**§1.2 三个前提（为什么可以条目化 / 可以树形化 / 可以 N:1 分配）**／§1.3 两个形态+叶的判据／**§1.4 承接＝「上游最深可用原子 → 下游枝干/枝梢，以语义定级」**（"叶→枝梢"只是"上游完整态·下游概要"步上的显影；02↔04、05↔06 不是承接）；二「各产品数据的分解结构」＝**§2.0 原始需求（入口加工链）**／**§2.1.1 相关方功能需求＝相关方角色→场景业务→需求条目**／§2.1.2／**§2.2 业务流程＝§2.2.1 R_i／§2.2.2 P_i（末级＝⑤ 输出文档·枝梢／其下一级＝⑥ 活动节点·正文条目·叶；"任务"不占树位）／§2.2.3 P₀（树形待裁：应删 ④WBS 与 ⑦叶）**／§2.3 系统需求／§2.4 平台构成／§2.5 配置方案／§2.6 前后端组件方案；**94 只给结论+依据名称，完整论证在 95**）；叶=末级·枝梢=叶上一级·概要=根→枝梢·完整态=根→叶；**92 号已删除退役**；已裁 SR 末级（91 v5.54 枝梢⑤/叶⑥）；**待裁 P₀ 级数（判据已备）＋ R₀ 存在性 ＋ 全链 R/P 改名未走** + 07/§2.4 平台构成口径；外引联动 91 v5.57 / 93 v0.11 / 05 v3.9 / 01-pl4eos-spec v2.2；**v1.14 未提交**
