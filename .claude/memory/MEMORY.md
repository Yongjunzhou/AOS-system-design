- [培训教材+教材书项目](concept-book-training-project.md) — 2026-08-19：基于50-reserch八份笔记做两个共享同一架构的产出（分组研讨式培训教材 + 独立成书教材书，11章）；书=思想著作/标准当骨架/使命=消除误解/每章以流行误解为主轴；写作进度：ch0✅15.1k→ch2✅19.0k→ch1✅15.7k→ch3✅13.9k→**ch4设计开发落地✅18.9k（一棵树=枝干概要+叶子详细；三动作=分解/分配/分析；恒信主）**；下一步ch5评审(03)→ch6架构→ch7架构决策→ch8配置→ch9治理→ch10整合→附录+通读→训练教材重派生
...
- [wft02-biz 知识层重构与审视修复](wft02-biz-v80-knowledge-layer-restructure.md) — 2026-08-19：以 wft01-biz v3.11 为标杆知识层三章重构（v7.4→v8.0：§二 基本概念+转换总览/§三 判据/§四 演进/§五 操作/§七 自检）+ 原理与方法审视修复（v8.1：PCA 计划清单强制要求入方法层/C-A 运行时口径/变更类型补无变更/PL3 四类补咨询/覆盖自查并入自检清单/D-PCA 表述统一）；SKILL 锚点 #262→#332
