...
- [94 号＝全链树权威（2026-09-10）](94-eos-sys-dev-dbs-tree-authority.md) — 94-eos-sys-dev-dbs.md 树形/级语义唯一权威（**v1.16，两章**：一「产品数据及其形态」＝§1.1 清单表（**原始需求加工链四态**/01/02·04/05·06/07，六·七待定义）+ **业务树名矩阵**（`Rn／Pn`＝n≥1 各业务、`R0／P0`＝平台本身；R 树＝角色履职视角↔SL、P 树＝产品开发视角↔PL）／§1.2 三个前提（条目化/树形化/N:1）／§1.3 两个形态+叶的判据／§1.4 承接＝「上游最深可用原子→下游枝干/枝梢，以语义定级」；二「各产品数据的分解结构」＝§2.0 原始需求入口加工链／§2.1 相关方需求（§2.1.1 相关方角色→场景业务→需求条目）／**§2.2 业务流程四节（2.2.1 业务运行角色履职·Rn／2.2.2 产品开发·Pn（引擎平台之外的产品）／2.2.3 引擎平台开发·P0／2.2.4 业务配置角色履职·R0）**／§2.3 系统需求／§2.4 平台构成／§2.5·§2.6 待定义；**§一~§二已去噪**——不留跨文档关系句/来历说明/报告内跨节关系句/内联节号（保留：清单表「本文」列＝索引、依据指针 3 处），95 论证指针已删）；叶=末级·枝梢=叶上一级·概要=根→枝梢·完整态=根→叶；**两棵树**（Rn 与 Pn 独立、在输出文档处认亲互指）；**92 已退役**；**待裁 P0 级数（判据已备）＋P0 的 PL6 算不算叶**＋07/§2.4 口径；**待定：去噪规则推不推到 wft 链 16 份**；全链扫尾已做（wft 16+试点 3+数据文件 5+01-pl4eos-spec→v2.3），记号统一 Rn/Pn/R0/P0；**v1.15+v1.16+扫尾 未提交**
