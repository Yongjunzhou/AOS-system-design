...
- [反馈：代号白名单 X／Y／R树／P树（2026-09-16）](feedback-code-whitelist-2026-09-16.md) — SKILL 指南正文**只允许四个代号**，其余一律给具体指代（文档/资产写本名、规范首现给文题、任务按链上位置称呼、需求类型写全称、D／PCA→执行类／管理类）；层级序号 ①… 只留树图与级语义表；编号不当文档名；状态值/节点ID/字段名保留原值。同轮另立三条：引用不作主语谓语且同一源全文只挂一处 · 不写元评论 · 不写来历与待办。**已立为 93 §2.4「正文写法」**，首发实例 biz01 v0.21-pilot
- [反馈：草稿不转 PDF（2026-09-10）](feedback-no-pdf-for-drafts.md) — 研究报告/草案类先只出 md，定稿后再 md2pdf；已有 PDF 的规范文档内容修改仍照常重生成
- [94 号＝全链树权威（2026-09-15）](94-eos-sys-dev-dbs-tree-authority.md) — v1.19：**§2.1.1 重画**——① 业务输出文档（根，带主责角色／产品构件／E2E场景三属性）→ ② 任务节点 → ③ 系统用例场景级操作（叶，ID 前缀 `stfr`），角色与场景由树级降为属性；连带改 §1.4 承接表 01→02 行、§2.0.1 表格与散文、§2.0.2；v1.18 骨架：两章 + 树名矩阵 + 三个依据；§2.2 四节序 Rn／Pn／R0／P0（P0 收五级、R0 少「角色职责」）；全文去 PL／SL 层码与枝梢叶标注；变更记录整节删除；**§2.4 平台构成勿自行改**（人类退回）；含 14 条仍开着的分叉与待裁（新增 01 树两级谁写、`spr-`→`stfr-` 链级更名、F 不覆盖 NFR/FR-ENG）
...
- [人类方案六章布局改造 · 93规范（2026-09-04）](eos-design-landing-spec-93-refactor.md) — 以 20 号落地方法改造 EOS 设计线人类方案：新六章骨架（目的与目标/输入文档结构及规则/输出文档结构及规则/输出与输入转换规则/转换流程及检验规则/资产文档结构及规则）+ 三条原则（概念跟文档走/schema 全链单源/边界文档 A.ch3 生产侧=B.ch2 消费侧接口契约）；93-eos-skill-guide-spec.md 升 v0.4 + biz 三试点（eos-biz01/02/03）v0.9-pilot 成稿→批次1/2 审改已提交（试点 v0.11-pilot / 93 v0.5 / 91 v5.53·§5.3.1 判据之家）；批次3 02 打样待铺；**2026-09-15 本会话：biz01 两轮重构落地 v0.20-pilot**（v0.19 按 93 v0.35+94 v1.18——Step 归五步／头注七条判死／树权威改指 94；v0.20 随 94 v1.19 重画 §2.1.1——X 树改「① 业务输出文档／② 任务节点／③ 系统用例场景级操作」、角色与场景降为属性、承接落点改由 ① 属性定、单承接由树形保证）+ 全链去 PL·SL 层码 + 四处 93 §4.2「总表」断链修复 + biz02/03 同构待办 + ort03 未随新 01 树
