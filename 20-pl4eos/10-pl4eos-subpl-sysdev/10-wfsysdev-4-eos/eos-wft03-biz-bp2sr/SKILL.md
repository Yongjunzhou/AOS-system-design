...
一个场景业务（SL5）= 一个场景标签页。从 BP 提取 PL5 输出文档序列（wft02 Step 8 文档流转顺序），逐文档生成 TAB：
...
| 支撑引擎 | wft02 Step 5"支撑引擎" | → 功能表单类型判定 |
...
将 BP 的 PL6-A 活动节点序列（wft02 Step 6）与 24 资产定义对照：
...
| **缺失型** | BP 缺少必要 IPO、引擎支撑信息或变更声明 | 将 BP 标为 `需wft02修订` 候选 |
