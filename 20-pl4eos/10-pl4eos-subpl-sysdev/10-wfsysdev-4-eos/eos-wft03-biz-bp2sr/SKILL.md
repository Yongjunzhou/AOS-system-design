...
description: BP→业务配置需求/PA·页面组件开发。接收≥1个BP节点（可SR设计/待补充SR设计），
...
| 方向 | Skill | 交接内容 |
|------|-------|---------|
| 上游 | `wft02-biz` | 产出 BP 节点（`可以SR设计`/`待补充SR设计`），含 A2 角色场景 + Bn 输出产品 + PL6-B 条目 + 引擎支撑信息。通过 `## AI可以处理节点` 的"待 wft03-biz 处理"分节检测 |
...
将 BP 的 PL6-A 活动节点序列（wft02 Step 4 任务活动设计）与 24 资产定义对照：

| 判定 | 条件 | 动作 |
|------|------|------|
| **Q1 复用** | BP 节点序列与 24 定义一致（节点数量/类型/顺序均匹配），BP 动作均在 24 定义动作清单中存在 | 以 24 定义为主体，BP 仅覆盖处理角色 |
| **Q2 改进** | 24 定义缺少某些节点或动作 | 以 24 为基础，BP 额外节点/动作标注 `[推断]` 作为改进建议 |
| **Q3 新建** | 24 资产中不存在该流程定义 | 基于 BP 的 PL6-A 推断完整定义，全量 `[推断]` 作为新建建议 |
| **无法匹配** | BP 未标注任务类型、或结构性差异无法归入 Q2 | 退回 `需wft02修订` |

**5.3 输出流程类业务需求**

基于匹配结果，以 24 定义为主体、BP 的 PL6-A 做场景补充，定义三层需求：

| 层 | 主体（24 定义） | 场景补充（BP 的 PL6-A） |
|----|---------------|----------------------|
| **节点层** | 标准活动节点序列 + 各节点功能表单 + 默认处理角色 | BP 额外节点（Q2）+ BP 处理角色（覆盖默认） |
| **动作层** | 动作清单 + 状态集与状态变迁 | BP 额外动作 + 特有状态分支 |
| **规则层** | 流转规则 + 超时与异常规则 + 任务状态附着 | BP 特有异常流转 + 特有超时时限 |

每个活动节点对应一个窗口标签页（单据模式，ID 格式 `@srf-tab-{场景业务简称}-act-{序号}`）；act 标签页编排为该文档的**流程类页面序列**（在文档内部按节点顺序流转，不并入场景标签页文档级 TAB）。
...
每条引擎需求包含：需求 ID（`FR-ENG-REQ-{YYYYMMDD}-{序号}`）/ 来源（业务页面 ID + BP 节点 ID）/ 目标引擎 / 需求类型 / 缺口层级（构件级/布局组件级/CU级）/ 当前能力 / 需求能力 / 指标要求 / 业务场景 / 业务影响。
...
| 判定 | 条件 | 动作 |
|------|------|------|
| **Q1 复用** | BP 节点序列与 24 定义一致且动作均在 24 清单中 | 以 24 为主体，BP 仅覆盖处理角色 |
| **Q2 改进** | 24 定义缺少某些节点或动作 | 以 24 为基础，BP 额外节点/动作标注 `[推断]` |
| **Q3 新建** | 24 资产中不存在该流程定义 | 基于 BP 推断完整定义，全量 `[推断]` |
