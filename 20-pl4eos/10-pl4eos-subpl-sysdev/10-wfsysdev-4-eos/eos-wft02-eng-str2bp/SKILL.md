...
| STR-E 无已分配 CU 或承接结构不完整（缺配置信息组结构） | 输入不足 | STR-E→`需wft01修订`+缺失说明 → **退出** |
...
3. **承接已分配 CU**——读取 STR-E 的 CU 承接关系 + 配置信息组结构，校验结构完整，不重新收敛
...
| 承接缺陷（CU 承接关系/配置信息组结构无法在该 CU 下定义，§3.1） | 不静默改——退回 `需wft01修订`，由 wft01-eng 重新分配 |
| 配置能力缺口（入口/操作步骤/系统处理/运行期能力/指标缺失） | 生成补充 FR-ENG |
| 依赖缺口（跨引擎无对应模型） | 设计新模型记录（走 25 类型模板，`[推断]` 供流水线开发者审核决策） |

**2. 要素机械检查**——A2 上下文完整（承接自 STR-E）、A1 路径到位、CU 承接非空且含配置信息组结构、每个 CU 操作活动定义（配置操作/系统处理/运行期能力）+ 配置入口/FR-ENG 指标已定义、依赖已定义并校验、变更影响声明已写入。
...
|---------|------|
| 上游缺口（STR-E 场景边界缺失/能力链不闭合） | 退回 `需wft01修订` |
| 承接缺陷（承接关系/配置信息组结构无法定义） | 不静默改——退回 `需wft01修订` |
| 配置能力缺口（入口/操作步骤/系统处理/运行期能力/指标缺失） | 生成补充 FR-ENG |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-08-21 | v1.10 | 人类方案 v10.0 同步（eng 链「概要/详细」分工框架第②步——本 Skill 定位为**业务详细定义**，详设形态对齐 PDF v1.5.2 操作活动定义）：description/标题补「业务详细定义（操作活动定义）」；负责表不负责列——「配置页面/布局组件/构件设计」→「功能概要（功能表单+概要说明）→ wft03-eng；功能详细（功能表单组件及其业务）→ wft04-eng」、「定义配置信息组/配置要素/系统处理/运行期能力/FR-ENG 指标」→「操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式，基准=PDF v1.5.2）/配置入口/FR-ENG 指标」；下游衔接补「经 wft04-eng 功能详细→wft05-eng 产品概要」+「组织功能概要（功能表单）」；Step 3 Phase C 定义表改操作活动定义（配置操作步骤序列/系统处理[编辑时+生效后]/运行期能力分支显式化）+ 配置入口/系统处理/运行期能力三行；Step 4 缺口/要素机械检查同步；Step 5 写 25 改「操作活动定义」；Step End 可用性锚点/审核锚点同步；附录 A.2 定义表同步、A.5 出口闸门改「组织功能概要」。AI 执行规则语义不变 |
