...
description: STR-E→BP·A1配置单元详细定义。接收≥1个STR-E节点（可以详细定义/待补充详细定义），
             承接wft01-eng已分配的CU，展开配置信息组/配置能力/依赖/状态迁移详细定义，
...
| 承接已分配 CU——读取 STR-E 的 CU 承接关系，不重新收敛/不重新判定引擎归属 | 配置页面、布局组件、构件设计 → `wft03-eng` |
| 定义每个 CU 的配置信息组/配置入口/配置要素/系统处理/运行期能力/FR-ENG 指标 + 引擎状态迁移 | 前后端组件、接口、数据库设计 → `wft05-eng` |
...
| 下游 | `wft03-eng` | 消费 `可以SR设计` 的 BP，依据 CU 配置信息组/配置能力推断配置页面。缺陷时退回 `需wft02修订` |
...
3. **CU 详设与变更处理**——承接已分配 CU，定义配置信息组/配置入口/配置要素/系统处理/运行期能力 + 引擎状态迁移（详见 human spec §3.3.2）：

**详设变更处理**（详见 human spec §3.3.3）：

| 变更类型 | 触发 | 处理 |
|----------|------|------|
| 配置能力扩展 | 已有 CU 的配置信息组内容/配置要素/系统处理/运行期能力扩展 | 修订 CU 配置能力，叠加 `[调整]` + 变更原因。不改变 CU 承接关系 |
...
4. **定义 CU 配置能力**——承接 wft01 的配置信息组清单+概要说明，逐配置信息组展开内容：

| 定义项 | 回答的问题 | 设计粒度 |
|------|------------|---------|
| 配置信息组 | 该组配置字段共同提供什么可以运行的能力 | 承接 wft01 的配置信息组清单+概要说明，逐组展开配置操作/系统处理/运行期能力 |
...
| 配置能力缺口（入口/要素/处理/运行期能力/指标缺失） | 生成补充 FR-ENG |
| 依赖缺口（跨引擎无对应模型） | 设计新模型记录（走 25 类型模板，`[推断]` 供流水线开发者审核决策） |

**2. 要素机械检查**——A2 上下文完整（承接自 STR-E）、A1 路径到位、CU 承接非空且含配置信息组结构、每个 CU 配置信息组/配置能力已定义、依赖已定义并校验、变更影响声明已写入。
...
| 运行期可消费（出口闸门） | 足以让wft03-eng推断配置页面 | 不足则保持待确认方案 |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-08-21 | v1.9 | 人类方案 v9.9 同步（非主价值链 Step 框架链级统一——移除三角色分工/Step End 单区/输入校验 biz 格式/标注纪律框架）。AI 执行规则语义不变 |
