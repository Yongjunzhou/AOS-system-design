...
| 缺口补需——CU缺口+依赖缺口→补充FR-ENG或设计新 CU 类型（记录进 25） | — |
...
| 新增 CU | 无可复用CU，粒度可展开 | 新建CU：25 盘存有承接类型→实例化；盘存无承接→设计新 CU 类型（走 25 类型模板，`[推断]`，人机协同设计决策点） |
...
| CU 缺口（能力链段无对应 CU，**需求侧可锚定**：`STR-E隐含`/缺口推断） | 本 Skill 内收敛设计新增候选 CU，标注 `[推断]` |
| CU 缺口（**需求侧无锚**：链不隐含/无缺口证据，25 资产提示不构成身份锚） | 生成补充 FR-ENG 材料，待身份带回后再收敛设计 |
...
| `同类引擎推断` | 同类引擎已有相似配置能力（资产侧·模式参照，不构成需求锚） |
| `25模型线索` | 25资产中已有CU类型定义（资产侧·复用承接标记，不构成需求锚） |
| `引擎内置` | 引擎预定义能力，本轮未触发（资产侧·预定义占位） |

> 非 `STR-E明示` 来源均标注 `[推断]`。**需求侧来源**（`STR-E明示`/`STR-E隐含`/`构件缺口推断`/`布局组件缺口推断`）是缺口 CU 本 Skill 内新增的必要条件；**资产侧参照**（`同类引擎推断`/`25模型线索`/`引擎内置`）不构成需求证据，不得单独支撑缺口 CU 新增（人类方案 §3.1 缺口锚定边界）。
...
| 25关系清晰 | 已有/待确认/缺失的模型关系已标注 | 标注新类型设计记录（走 25 类型模板，`[推断]` 待决策） |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-08-20 | v1.5 | 人类方案 v9.5 同步——来源类型 A.3 标需求侧/资产侧两分（`25模型线索` 改复用承接标记、`同类引擎推断` 标模式参照、`引擎内置` 标预定义占位，均注明"不构成需求锚"）；Step 4 CU 缺口拆需求侧可锚定/无锚两况（无锚→补充 FR-ENG 材料）。AI 执行规则语义不变 |
