...
| 条件 | 判定 | 动作 |
|------|------|------|
| STR-NFR 状态=`需wft01修订` 且人类未提供改进方案 | 退回方案缺失（硬退出） | 输出 wft04 缺失说明，提示提供改进方案 → **退出** |
| OR 状态≠`待处理` | 状态不符（硬退出） | 输出不合格条目及当前状态 → **退出** |
| OR 类型为 FR-BIZ | 类型不匹配（硬退出） | 输出类型分布，提示走 `wft01-biz` → **退出** |
| OR 类型为 FR-ENG | 类型不匹配（硬退出） | 输出类型分布，提示走 `wft01-eng` → **退出** |
...
> **待量化线索的处理**：wft01-nfr 只记录 OR 中相关方已确认的目标值（数值、比较词和验证条件），**不将其转为系统量化指标**（系统侧指标设计在 wft04-nfr）。如 OR 说"页面查询要在2秒内响应"（目标值已在 ort02 澄清中由相关方确认），wft01 记录 `待量化线索：OR提到"2秒内"和"页面查询响应"`。
...
| 适用对象线索 | 可能约束的 STR-F、STR-E、业务域、平台能力或全局对象（页面类型/具体页面 ID 仅作适用范围线索备注、不进归组键——页面 NFR 绑定由 wft04-nfr 执行） |
...
| 适用对象有线索 | 至少能标注全局/业务域/STR-F/STR-E/平台能力中的一种线索（页面类型/具体页面 ID 仅作适用范围备注，不参与归组判定） | 标记适用范围缺口 |
