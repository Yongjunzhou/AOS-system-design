...
description: NFR OR→bph-nfr 角色非功能需求分类设计。接收正式NFR OR（≤10条），
             完成OR→bph-nfr角色NFR分类设计（语境提取→分类需求清单→22资产对齐→完备性检查），
             将22分类对齐结论写回22资产，形成可进入wft04-nfr的bph-nfr节点。
---

# eos-wft01-nfr · NFR OR → bph-nfr 角色非功能需求分类设计
...
| 检出分类/适用范围/验证语义三类缺口 → 生成补充 NFR OR 材料 | 生成 SysReq-NFR、页面 NFR、CU/pa-eng 组件约束 → `wft04-nfr` |
| 人类确认后写回 22 资产 + OR→`已分配` + bph-nfr→`可以NFR设计` | 修改 22 已确认分类维度的权威定义（本 Skill 只写引用和候选建议） |
...
3. **宿主匹配**——OR 的四元组（角色+语境+分类维度+适用对象）落入已有 bph-nfr 分类边界 → Phase B；否则 → 待聚合池 → Phase C
...
2. **边界匹配检查**——确认 OR 在 bph-nfr 边界内（详见 human spec §2.4.3）：
