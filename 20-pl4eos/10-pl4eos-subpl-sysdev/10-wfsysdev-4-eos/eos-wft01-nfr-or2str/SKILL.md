...
description: NFR 规范化需求→bph-nfr 角色非功能需求分类设计。接收正式NFR 规范化需求（≤10条），
             完成规范化需求→bph-nfr角色NFR分类设计（语境提取→分类需求清单→22资产对齐→完备性检查），
             将22分类对齐结论写回22资产，形成可进入wft04-nfr的bph-nfr节点。
---

# eos-wft01-nfr · NFR 规范化需求 → bph-nfr 角色非功能需求分类设计
...
| 负责 | 不负责（路由指向） |
|------|-------------------|
| 接收正式 NFR 规范化需求（≤10条），校验类型/状态/语境 | FR-BIZ 业务场景和输出文档设计 → `wft01-biz` |
...
| 检出分类/适用范围/验证语义三类缺口 → 生成补充 NFR 规范化需求 材料 | 生成 SysReq-NFR、页面 NFR、CU/pa-eng 组件约束 → `wft04-nfr` |
...
| 方向 | Skill | 交接内容 |
|------|-------|---------|
| 上游 | 规范化需求预处理（`ort03`） | 正式 NFR 规范化需求 写入 `01-*.md`，状态=`待处理`。通过 `## AI可以处理节点` 的"待 wft01-nfr 处理"分节检测 |
