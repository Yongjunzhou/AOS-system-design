...
1. **引擎需求**——FR-ENG（功能需求-引擎类）→ 由 `wft01-eng-spr2bpl` 处理
...
| 2026-08-24 | v3.26 | 输出术语正名（人类纠正）——业务概要定义→业务流程概要定义（bpl）；业务详细定义→业务流程详细定义（成对一致）；章节标题/锚点随迁。 |
...
| 2026-08-24 | v3.24 | 全篇术语清扫（人类定名，概念层）——规范化需求（spr）/业务流程概要定义（bpl）替换 OR/STR-F/原始需求/相关方需求/业务需求（含章节标题，锚点随迁）；资产名（01/02-*.md、节点 ID 前缀）待后续资产更名同步。 |
| 2026-08-24 | v3.23 | 文件名更名（人类定名 2026-08-24）——`eos-wft01-biz-or2str.md` → `eos-wft01-biz-spr2bpl.md`（本环节语义 = 规范化（Specified）需求→业务流程概要定义，`spr`=规范化（Specified）需求、`bpl`=BP high-level definition）；SKILL 指南ID 与全库引用同步（eng/nfr/wft02/wft03 命名不动，链级后续单议）。 |
