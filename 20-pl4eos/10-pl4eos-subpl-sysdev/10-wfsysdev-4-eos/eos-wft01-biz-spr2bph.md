...
| 2026-08-24 | v3.23 | 文件名更名（人类定名 2026-08-24）——`eos-wft01-biz-or2str.md` → `eos-wft01-biz-spr2bph.md`（本环节语义 = 规范化（Specified）需求→业务流程概要定义，`spr`=规范化（Specified）需求、`bph`=BP high-level definition）；SKILL 指南ID 与全库引用同步（eng/nfr/wft02/wft03 命名不动，链级后续单议）。 |
