...
description: FR-ENG OR→STR-E 业务概要定义。接收正式FR-ENG OR（≤10条），
             完成OR→STR-E业务概要定义（溯源→能力链→概要末级→配置信息组（概要说明，结构判定级）→CU承接→完备性检查），
...
| 识别概要末级节点（名称级 CU 候选）+ 分解为配置信息组（概要说明，结构判定级）+ 分配配置信息组到 CU 承接（含收敛判定/骨架判定）+ 构建能力链 + 完备性检查 | 功能概要（功能表单+概要说明）→ `wft03-eng`；功能详细（功能表单组件及其业务）→ `wft04-eng` |
| 装配 A2 上下文（支撑 CU 来源判定与追溯链前端锚点） | 前后端组件、接口和实现边界 → `wft05-eng` |
| 检出能力链缺口 → 生成补充 FR-ENG OR 材料 | 操作活动定义展开 → `wft02-eng`（业务详细定义）承接已分配 CU 详设 |
...
5. **分解概要末级为配置信息组（概要说明）**——识别概要末级节点（名称级 CU 候选），逐节点按"可以运行的能力"分解为配置信息组（概要说明，结构判定级），逐候选标注来源类型、粒度判断（详见 human spec §3.3.1）
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-08-21 | v1.11 | 人类方案 v9.5 同步（eng 链「概要/详细」分工框架第①步——本 Skill 定位为**业务概要定义**）：description/标题补「业务概要定义」；负责表不负责列——「CU 配置信息组内容/配置要素/校验保存规则/依赖/状态迁移」→「配置信息组的操作活动定义（配置操作/系统处理/运行期能力）」、「配置页面/操作活动/页面功能需求」→「功能概要（功能表单+概要说明）→ wft03-eng；功能详细（功能表单组件及其业务）→ wft04-eng」；「CU 详细定义…承接已分配 CU 详设」→「操作活动定义展开（业务详细定义）」；下游衔接补「业务详细定义（操作活动定义）」；Phase C 分解补「结构判定级」。AI 执行规则语义不变 |
