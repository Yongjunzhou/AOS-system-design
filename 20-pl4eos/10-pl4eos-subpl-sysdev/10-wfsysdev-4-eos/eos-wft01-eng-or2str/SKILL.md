...
| 人类确认后推进 STR-E→`可以分解分配` + OR→`已分配` | 引擎/CU 模型详细定义（→ `wft02-eng` 设计，25 记录）；本 Skill 仅名称级识别 + 对照盘存的骨架判定 |
...
4. **识别 CU 候选**——名称级列出可能触及的 CU，逐候选标注来源类型、粒度判断和骨架判定（复用/改进/新增，对照 25 盘存；详见 human spec §3.3.1~§3.3.2）
...
**3. 补充 FR-ENG 材料生成**——写入 `../../80-pl4eos-2-eosdata/00-origin-requirement-materials/10-raw-files/<YYYYMMDD>-<STR-E名称>补充原始需求材料.md`，登记到 OR 原料状态表。与 wft03-biz 缺口线索同一管道（人类方案 §5.5）：经确认 + ort00→ort03 转正式 FR-ENG OR，不豁免预处理链。**模型缺口（25 盘存无承接的配置面/引擎）不走补充 OR 回路**——由骨架判定（§3.3.1）作为设计决策处理，经人类审核决策后由 wft02-eng 设计详细定义。
...
2. 遍历 CU 候选，写回 25 资产——仅名称级引用与骨架判定（复用/改进/新增）记录，不写详细定义（详细定义由 wft02-eng 设计后记录）
3. 对应 OR 条目 → `已分配`
