...
description: FR-ENG OR→STR-E 平台能力场景业务设计。接收正式FR-ENG OR（≤10条），
             完成OR→STR-E平台能力场景设计（溯源→能力链→概要末级→详细需求条目→CU承接→完备性检查），
             写回23/25资产 CU 承接引用，形成可进入wft02-eng的STR-E节点。
---
...
| 按三元组（能力域+目标+验收结果）匹配已有 STR-E | CU 配置信息组内容、配置要素、校验保存规则、依赖、状态迁移 → `wft02-eng` |
| 识别概要末级节点（名称级 CU 候选）+ 分解为详细需求条目（按可以运行的能力分组）+ 分配条目到 CU 承接（含收敛判定/骨架判定）+ 构建能力链 + 完备性检查 | 配置页面、操作活动、页面功能需求 → `wft03-eng` |
| 装配 A2 上下文（支撑 CU 来源判定与追溯链前端锚点） | 前后端组件、接口和实现边界 → `wft05-eng` |
| 检出能力链缺口 → 生成补充 FR-ENG OR 材料 | CU 详细定义（配置信息组/配置能力/依赖/状态迁移）→ `wft02-eng` 承接已分配 CU 详设 |
...
| 下游 | `wft02-eng` | 消费 `可以详细定义`/`待补充详细定义` 的 STR-E，承接已分配 CU 做详细定义。缺陷时退回 `需wft01修订`+缺失说明 |
...
3. **分解分配重识别**——从本轮 OR 重新分解概要末级为详细需求条目（§3.3.1）、重分配 CU 承接（收敛判定，§3.3.2），再执行 CU 承接关系检测——比对已有基线，判定确认/无新增/引擎归属变更/补充/过时（详见 human spec §3.3.4）：
...
5. **分解概要末级为详细需求条目**——识别概要末级节点（名称级 CU 候选），逐节点按"可以运行的能力"分解为详细需求条目（配置信息组结构），逐候选标注来源类型、粒度判断（详见 human spec §3.3.1）
6. **分配条目到 CU 承接**——逐条目找 CU 承接（1:1），执行收敛判定（复用/新增/拆分/合并）+ 对照 25 盘存骨架判定（复用/改进/新增）+ 引擎归属判定（详见 human spec §3.3.2~§3.3.3）
...
**2. 分解分配完备检查**（前置门禁·追溯完整）——每条详细需求条目已分配 CU；每条 CU 承接可追溯 STR-E 与 OR；需求侧无锚的缺口 CU 已生成补充 FR-ENG 材料；每个 CU 承接的能力点集合可构成完整配置信息组结构（详见 human spec §3.4/§5.5）。

**3. 要素机械检查**——引擎名称/场景边界非空、A2 上下文已装配、能力诉求/运行期结果有标注、追溯链（OR→STR-E→概要末级→详细需求条目→CU 承接）完整、概要末级清单非空且逐条标注来源类型和粒度判断、详细需求条目已分解、CU 承接关系完整、变更影响声明已写入。

**4. 补充 FR-ENG 材料生成**——写入 `../../80-pl4eos-2-eosdata/00-origin-requirement-materials/10-raw-files/<YYYYMMDD>-<STR-E名称>补充原始需求材料.md`，登记到 OR 原料状态表。与 wft03-biz 缺口线索同一管道（人类方案 §5.5）：经确认 + ort00→ort03 转正式 FR-ENG OR，不豁免预处理链。**模型缺口（25 盘存无承接的配置面/引擎）不走补充 OR 回路**——由骨架判定（§3.3.2）作为设计决策处理，经人类审核决策后由 wft02-eng 承接详设。
...
2. 遍历 CU 承接关系，写回 25 资产——仅写 CU 承接关系与骨架判定（复用/改进/新增）记录，不写详细定义（配置信息组/配置能力由 wft02-eng 详设后记录）
3. 对应 OR 条目 → `已分配`
...
| 分配完备（前置门禁·追溯完整） | 每条详细需求条目已分配 CU；每条 CU 承接可追溯 STR-E 与 OR | 无锚条目走补充 FR-ENG 回路 |
| 配置信息组结构完整 | 每个 CU 承接的能力点集合可构成完整配置信息组结构 | 补充分解或标记 `[需裁决]` |
