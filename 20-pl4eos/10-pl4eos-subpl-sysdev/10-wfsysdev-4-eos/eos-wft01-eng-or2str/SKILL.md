...
description: FR-ENG OR→STR-E 平台能力场景业务设计。接收正式FR-ENG OR（≤10条），
             完成OR→STR-E平台能力场景设计（溯源→能力链→概要末级→配置信息组（概要说明）→CU承接→完备性检查），
...
| 识别概要末级节点（名称级 CU 候选）+ 分解为配置信息组（概要说明）+ 分配配置信息组到 CU 承接（含收敛判定/骨架判定）+ 构建能力链 + 完备性检查 | 配置页面、操作活动、页面功能需求 → `wft03-eng` |
...
3. **分解分配重识别**——从本轮 OR 重新分解概要末级为配置信息组（概要说明）（§3.3.1）、重分配 CU 承接（收敛判定，§3.3.2），再执行 CU 承接关系检测——比对已有基线，判定确认/无新增/引擎归属变更/补充/过时（详见 human spec §3.3.4）：
...
5. **分解概要末级为配置信息组（概要说明）**——识别概要末级节点（名称级 CU 候选），逐节点按"可以运行的能力"分解为配置信息组（概要说明），逐候选标注来源类型、粒度判断（详见 human spec §3.3.1）
...
**3. 要素机械检查**——引擎名称/场景边界非空、A2 上下文已装配、能力诉求/运行期结果有标注、追溯链（OR→STR-E→概要末级→配置信息组（概要说明）→CU 承接）完整、概要末级清单非空且逐条标注来源类型和粒度判断、配置信息组（概要说明）已分解、CU 承接关系完整、变更影响声明已写入。
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-08-21 | v1.9 | 人类方案 v9.3 同步（分解产物正名——「详细需求条目」→「配置信息组（概要说明）」，链级待办 #1）——负责表"分解为详细需求条目（按可以运行的能力分组）+ 分配条目到 CU 承接"改"分解为配置信息组（概要说明）+ 分配配置信息组到 CU 承接"；Phase C 步骤5/6、Phase B 分解分配重识别同步；Step 4 分解分配完备检查与要素机械检查改"配置信息组（概要说明）"；附录 A.7"分配完备"改"每个配置信息组（概要说明）已分配 CU"、"配置信息组结构完整"改"配置信息组清单完整"；frontmatter description 同步。AI 执行规则语义不变 |
