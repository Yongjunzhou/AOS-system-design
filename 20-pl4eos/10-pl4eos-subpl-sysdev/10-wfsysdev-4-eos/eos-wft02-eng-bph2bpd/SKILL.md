...
# eos-wft02-eng · 平台能力业务流程概要定义 → 平台能力业务流程详细定义 · A1 配置信息组详细定义
...
| 承接已分配 CU——读取 bph-eng 的 CU 承接关系，不重新收敛/不重新判定引擎归属 | 系统需求概要（功能表单+概要说明）→ `wft03-eng`；系统需求详细（功能表单组件及其业务）→ `wft04-eng` |
...
| 下游 | `wft03-eng` | 消费 `可以srh设计` 的 bpd-eng，依据 CU 操作活动定义组织系统需求概要（功能表单），经 wft04-eng 系统需求详细→wft05-eng 平台产品架构。缺陷时退回 `需wft02修订` |
