...
| 下游 | `wft03-eng` | 消费 `可以srh设计` 的 bpd-eng，依据 CU 操作活动定义组织系统需求概要（功能表单），经 wft04-eng 系统需求详细→wft05-eng 产品概要。缺陷时退回 `需wft02修订` |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-08-24 | v1.20 | 人类方案 v10.11 同步（wft02-eng 深审，人类逐项裁决 2026-08-24）——①标题全称正名：`平台能力业务流程概要定义 → 平台能力业务流程详细定义 · A1 配置信息组详细定义`；②Step 4/A.3 依赖缺口「设计新模型记录」→「标记 `[推断]` 依赖 + 生成补充 FR-ENG，不设计新引擎/CU 模型记录」（S1，对齐 v10.4 B11）；③A.4 治理依赖删「或设计新治理配置面（记录进 25）」→「缺失时生成补充 FR-ENG，由 wft01-eng 判定新增」（S2）；④Step 4/A.3 配置能力缺口补「配置操作步骤缺失→标记 `[需裁决]`」分流（S3）；⑤负责表补「保留配置页面推导线索」+ Phase C 补步骤5「输出页面推导线索」（S4）；⑥Step 2 操作条目补 FR-ENG 指标 + Step 5 补 01 登记 + Step End 资产落账补 01（S5）；⑦输入校验「STR 类型不是 bph-eng」→「bph-eng 不是 A1 类型」+ 删「需wft02修订未提供方案」行（对齐主文档 §5.1.1/§5.1.2，M3/M4）；⑧上游产出删「配置信息组候选」（T2）。AI 执行规则语义不变 |
