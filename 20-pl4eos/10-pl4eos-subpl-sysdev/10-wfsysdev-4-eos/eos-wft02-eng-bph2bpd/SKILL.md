...
> **设计依据**：[eos-wft02-eng-bph2bpd.md](../eos-wft02-eng-bph2bpd.md)（SKILL 指南——权威源）
...
**锁定写回**：输入校验通过后，将承接的 bph-eng 状态写为 `在详细定义`（锁定态，本 Skill 消费展开期间，对齐 SKILL 指南 §4.1 与 91 §A.6 锁定态=下游写入）；完成操作活动定义且人类确认后推进 `已详细定义`。
