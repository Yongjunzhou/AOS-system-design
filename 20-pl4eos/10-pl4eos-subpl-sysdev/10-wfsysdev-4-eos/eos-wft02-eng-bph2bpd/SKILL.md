...
description: bph-eng→bpd-eng·A1业务流程方案（操作活动定义）。接收≥1个bph-eng节点（可以详细定义/待补充详细定义），
             承接wft01-eng已分配的CU，展开配置信息组操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式）
             /依赖/状态迁移详细定义，写回23/25资产，形成可进入wft03-eng（系统需求概要）的bpd-eng节点。
---

# eos-wft02-eng · 平台能力业务流程概要 → 平台能力业务流程方案 · A1 配置信息组方案
...
| 负责 | 不负责（路由指向） |
|------|-------------------|
| 接收≥1个 bph-eng 节点（`可以详细定义`/`待补充详细定义`），校验状态/主责能力域/已分配 CU 结构 | bph-eng 概要定义、分解分配与 CU 承接设计（含收敛判定）、A2 装配 → `wft01-eng` |
| 按目标引擎+能力域匹配已有 bpd-eng——同域不同场景不强行合并 | A2/Bn 业务流程输出文档设计 → `wft02-biz` |
| 承接已分配 CU——读取 bph-eng 的 CU 承接关系，不重新收敛/不重新判定引擎归属 | 系统需求概要（功能表单+概要说明）→ `wft03-eng`；系统需求方案（功能表单组件及其业务）→ `wft04-eng` |
| 定义每个 CU 的配置信息组操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式，基准=PDF v1.5.2）/配置入口/FR-ENG 指标 + 引擎状态迁移 | 前后端组件、接口、数据库设计 → `wft05-eng` |
| 保留配置页面推导线索——记录配置角色、配置入口、活动顺序和运行期支撑需求，供 wft03-eng 组织系统需求概要（功能表单）——不形成页面设计结论 | — |
| 承接 bph-eng 的 A2 上下文（校验字段完整）+ A1 装配 | 将业务实例配置写入 25（25 是模型事实源） |
| 定义并校验四类依赖（引擎内/跨引擎/布局组件构件/治理） | — |
| 详设缺口补需——配置能力/依赖/状态迁移缺口→补充FR-ENG；承接缺陷→退回 wft01-eng；详设变更 `[调整]` | — |
| 人类确认后推进 bpd-eng→`可以srh设计` + bph-eng→`已详细定义` | — |
| 接收 wft03-eng 退回的 `需wft02修订` bpd-eng，按人类方案修订 | — |

### 上下游衔接

| 方向 | Skill | 交接内容 |
|------|-------|---------|
| 上游 | `wft01-eng` | 产出 bph-eng（主责能力域、能力链、配置信息组（概要说明）清单、CU 承接关系、依赖bph-eng）。通过 `## AI可以处理节点` 的"待 wft02-eng 处理"分节检测 |
| 下游 | `wft03-eng` | 消费 `可以srh设计` 的 bpd-eng，依据 CU 操作活动定义组织系统需求概要（功能表单），经 wft04-eng 系统需求方案→wft05-eng 平台产品架构。缺陷时退回 `需wft02修订` |
...
| 条件 | 判定 | 动作 |
|------|------|------|
| bph-eng 不是 A1 类型 | 类型不匹配 | 输出类型分布 + 路由指引 → **退出** |
| bph-eng 状态≠`可以详细定义`/`待补充详细定义` | 状态不符 | 输出状态分布 → **退出** |
| bph-eng 缺少场景边界/能力链/触发追溯 | 上游待修 | bph-eng→`需wft01修订`+缺失说明 → **退出** |
| bph-eng 无已分配 CU 或承接结构不完整（缺配置信息组（概要说明）清单） | 输入不足 | bph-eng→`需wft01修订`+缺失说明 → **退出** |
| 选中 bph-eng >5 个 | 软提示 | 输出超限提示 → 人类确认 |
| 无任何待处理 | — | 输出"无待处理对象"→ **退出** |

**锁定写回**：输入校验通过后，将承接的 bph-eng 状态写为 `在详细定义`（锁定态，本 Skill 消费展开期间，对齐人类方案 §4.1 与 91 §A.6 锁定态=下游写入）；完成操作活动定义且人类确认后推进 `已详细定义`。
...
3. 修改后检查是否仍满足 bph-eng 需求
4. 结构性变更（A1路径/CU清单/配置能力/依赖重组）→ 按需重入 Step 3
5. 全部条目获认可 → bpd-eng → `可以srh设计`
6. **元信息维护**：
...
| 维度 | wft02-eng 值 |
|------|-------------|
| **锚点需求** | bph-eng 节点——以 bph-eng 场景业务需求为锚 |
| **操作条目** | A1 产品路径 + 每个 CU（配置入口/操作活动定义/FR-ENG 指标）+ 每条依赖 + 每条缺口补需 |
| **推进目标** | 全部条目 `[同意]`+`[已处理]` → bpd-eng→`可以srh设计` |
| **清除条件** | 已有 `[同意]`+`[已处理]` 默认跳过。仅当本轮新输入满足任一条件时清除 `[已处理]`（保留 `[同意]`）：<br>1. 新输入为该条目的直接上游节点（如新 bph-eng 是 bpd-eng 节点的来源）<br>2. 新输入与条目同属一个架构层级且边界重叠（如同属 CU，新 CU 改变了已有 CU 的范围）<br>3. 新输入导致条目定义被修订（变更影响声明标注为修订型或结构型，§A.5.3 Step 1） |
| **级联触发条件** | A1路径/CU清单变更、依赖变更或引擎归属变更 → 触发需求级联检查 |
| **资产对齐级联** | CU 定义变更 → 更新 25 引用；A1 路径变更 → 更新 23 引用 |

---

### Step 3 · A1/CU bpd-eng 设计

#### Phase A — bph-eng 宿主匹配

逐 bph-eng 按目标引擎+能力域匹配已有 bpd-eng：

| 条件 | 路由 |
|------|------|
| 同一目标引擎 + 能力域一致或互补，落入已有 bpd-eng 的 A1 路径 | 归入该 bpd-eng → Phase B |
| 不满足 | 进入待聚合池 → Phase C |
| 同一 CU 被多个主引擎争用或 A1 路径不一致 | 标记 `[需裁决]` |
| CU 属于依赖的共享 bph-eng | 不进入本 bpd-eng（共享能力独立建模，消费方只记录依赖） |

待聚合池按**同一目标引擎 + 同一能力域**分组。

#### Phase B — 修订已有 bpd-eng

1. **要素提取**——从本轮 bph-eng 增量合并 A2 上下文和 CU 承接关系。已有上下文仅追加不重建
...
| 承接关系变更 | 新 bph-eng 的已分配 CU 与已有承接冲突 | 不静默改——退回 `需wft01修订`，由 wft01-eng 重新分配 |
...
2. **承接 A2 上下文**——读取 wft01-eng 装配的 A2 上下文（业务配置人员→bph-eng→配置活动序列→运行期能力可用），校验字段完整
3. **承接已分配 CU**——读取 bph-eng 的 CU 承接关系 + 配置信息组（概要说明）清单，校验清单完整，不重新收敛
...
5. **输出页面推导线索**——为每个 CU 记录配置角色、入口职责、配置对象、活动顺序和运行支撑需求，交由 wft03-eng 组织系统需求概要（功能表单）；不形成页面设计结论
6. **定义引擎状态迁移**——配置草稿/已校验/已发布/已装载/已停用/已回滚等状态及触发条件
7. **定义并校验依赖**——四类依赖（引擎内/跨引擎/布局组件构件/治理），写入 `04-*.md`
...
| 缺口类型 | 处理 |
|---------|------|
| 上游缺口（bph-eng 场景边界缺失/能力链不闭合） | 退回 `需wft01修订` |
...
**3. 补充 FR-ENG 材料生成**——写入 `../../../80-pl4eos-2-eosdata/00-origin-requirement-materials/10-raw-files/<YYYYMMDD>-<bpd-eng名称>补充原始需求材料.md`，登记到 材料状态表。与 wft03-biz 缺口线索同一管道（人类方案 §5.5）：经确认 + ort00→ort03 转FR-ENG 规范化的相关方需求节点，不豁免预处理链。

---

### Step 5 · 资产写回与落账

仅写回状态=`可以srh设计` 的 bpd-eng 节点。
...
4. `02` bph-eng → `已详细定义`（记录 `bph-eng 消费版本：v{N}`）
...
| 运行期可消费（出口闸门） | 足以让wft03-eng组织系统需求概要（功能表单） | 不足则保持待确认方案 |
...
| 日期 | 版本 | 说明 |
| 2026-08-28 | v1.23 | 层名更名（人类指示 2026-08-28）——业务流程概要定义→业务流程概要、业务流程详细定义→业务流程方案、系统需求概要定义→系统需求概要、系统需求详细定义→系统需求方案（nfr/eng 链同构，含短名/标题后缀）；历史变更记录保留旧词。 AI 执行规则语义不变 |
| 2026-08-25 | v1.22 | 链级机械修正（wft01-nfr 深审链级联动，人类裁决 2026-08-25）——数据文件相对路径修正 `../../80-pl4eos-2-eosdata/`→`../../../80-pl4eos-2-eosdata/`（自 skill 子目录上跳 3 级）。AI 执行规则语义不变 |
|------|------|------|
| 2026-08-27 | v1.23 | 链级术语清扫同步（人类指示 2026-08-27）——规范化需求→规范化的相关方需求。AI 执行规则语义不变 |
| 2026-08-25 | v1.21 | 人类方案 v10.12 同步（层名全对齐）——下游衔接「wft05-eng 产品概要」→「wft05-eng 平台产品架构」。AI 执行规则语义不变 |
...
| 2026-08-21 | v1.10 | 人类方案 v10.0 同步（eng 链「概要/详细」分工框架第②步——本 Skill 定位为**业务详细定义**，详设形态对齐 PDF v1.5.2 操作活动定义）：description/标题补「业务详细定义（操作活动定义）」；负责表不负责列——「配置页面/布局组件/构件设计」→「系统需求概要（功能表单+概要说明）→ wft03-eng；系统需求详细（功能表单组件及其业务）→ wft04-eng」、「定义配置信息组/配置要素/系统处理/运行期能力/FR-ENG 指标」→「操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式，基准=PDF v1.5.2）/配置入口/FR-ENG 指标」；下游衔接补「经 wft04-eng 系统需求详细→wft05-eng 产品概要」+「组织系统需求概要（功能表单）」；Step 3 Phase C 定义表改操作活动定义（配置操作步骤序列/系统处理[编辑时+生效后]/运行期能力分支显式化）+ 配置入口/系统处理/运行期能力三行；Step 4 缺口/要素机械检查同步；Step 5 写 25 改「操作活动定义」；Step End 可用性锚点/审核锚点同步；附录 A.2 定义表同步、A.5 出口闸门改「组织系统需求概要」。AI 执行规则语义不变。**v1.10 补充修正（2026-08-21 收尾联动核查）**：承接/校验/缺口判定上下文「配置信息组结构」→「配置信息组（概要说明）清单」5 处（输入校验/Phase C 承接/Step 4 承接缺陷/要素机械检查/附录 A.3），对齐 v9.3 正名口径与人类方案 v10.0（§1.1 负责1/负责7/§5.1.1/§7 自检清单同款 4 处） |
