...
description: bph-eng→bpd-eng·P0业务流程方案（操作活动定义）。接收≥1个bph-eng节点（可以详细定义/待补充详细定义），
             承接wft01-eng已分配的CU，展开配置信息组操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式）
             /依赖/状态迁移详细定义，写回23/25资产，形成可进入wft03-eng（系统需求概要）的bpd-eng节点。
---

# eos-wft02-eng · 平台能力业务流程概要 → 平台能力业务流程方案 · P0 配置信息组方案
...
| 负责 | 不负责（路由指向） |
|------|-------------------|
| 接收≥1个 bph-eng 节点（`可以详细定义`/`待补充详细定义`），校验状态/主责能力域/已分配 CU 结构 | bph-eng 概要定义、分解分配与 CU 承接设计（含收敛判定）、R 装配 → `wft01-eng` |
...
| 承接 bph-eng 的 R 上下文（校验字段完整）+ P0 装配 | 将业务实例配置写入 25（25 是模型事实源） |
...
| 条件 | 判定 | 动作 |
|------|------|------|
| bph-eng 不是 P0 类型 | 类型不匹配 | 输出类型分布 + 路由指引 → **退出** |
...
| 资产 | 用途 |
|------|------|
| `23-eos-output-architecture.md` 资产全貌层 | P0 产品路径锚点 |
| `24-eos-business-assets.md` 资产全貌层 | R／P/治理触发追溯线索 |
...
1. **要素提取**——从本轮 bph-eng 增量合并 R 上下文和 CU 承接关系。已有上下文仅追加不重建
...
2. **承接 R 上下文**——读取 wft01-eng 装配的 R 上下文（业务配置人员→bph-eng→配置活动序列→运行期能力可用），校验字段完整
...
**2. 要素机械检查**——R 上下文完整（承接自 bph-eng）、P0 路径到位、CU 承接非空且含配置信息组（概要说明）清单、每个 CU 操作活动定义（配置操作/系统处理/运行期能力）+ 配置入口/FR-ENG 指标已定义、依赖已定义并校验、变更影响声明已写入。
...
|------|------|------|
| 2026-09-10 | v1.23 | 人类方案同步（树简称改 P0／R／P，对齐 94 v1.14）；AI 执行规则语义不变。 |
