...
> 把职责视角已归拢的这批文档级E2E任务，按各自的产品归属重新归位，使每张文档都落到它该在的产品数据位、并按段内序位排出开发次序，再以序列文档的业务逻辑对WBS业务下的文档查缺补漏。
...
2. **组装 P 树 · 按节点意义定 WBS业务**：待文档级E2E任务落位之后整批一起定，段的边界取在文档相互关联、耦合度较高之处。
...
9. **覆盖检查与补充原始需求**：文档级E2E任务整段进入 Y、一个不少、同名同批；该构件级E2E业务应有的产品数据与本WBS业务已收的文档做名称级比对，WBS业务内的输入从源头贯通到交付；检出的缺口在 P 树上落占位节点，并补成产品数据类补充原始需求材料交回原始需求处理，占位待后续汇入的文档按同指合并兑现。
...
本任务交出的是概要，各级节点只判**结构**——树上有没有这个节点、它落在哪一级。内容写到名称与说明为止；文档级E2E任务另写到 P 坐标与输入。

**表3-2：P 树各级节点实现规则**

| 级 | 节点 | 实现规则 |
|----|------|---------|
| ① | 业务域 | **本任务产出**；在本树里作认亲检索与完备对账的域边界。**构建规则**：由属性直接确定——名取「所属业务域」属性值原文，挂在根下；不设扩展一档——域是企业运营体系的顶层结构，加减它不由一批文档驱动 |
| ② | 产品级E2E业务 | **本任务产出**；P 坐标的第二位。**构建规则**：由属性直接确定——名取「所属产品级E2E业务」属性值原文，挂在同一业务域之下 |
| ③ | 构件级E2E业务 | **本任务产出**；P 坐标的第三位。**构建规则**：由属性直接确定——名取「所属构件级E2E业务」属性值原文，挂在同一产品级E2E业务之下 |
| ④ | WBS业务 | **本任务产出**；本任务的新建／修订单位；本级的节点待文档级E2E任务落位之后才定。新增的文档级E2E任务按本级的节点意义定段——归得进已有的就归入；归不进的出**新建WBS业务方案**，并列出该节点应增补的文档级E2E任务、以**占位**落树；节点名取组内文档名称与用途的共性特征 |
| ⑤ | 文档级E2E任务 | **承袭**，取自 X 的同名节点；**占位节点例外**——由本任务产出。**构建规则**：与「所属构件级E2E业务」之下的所有占位逐项做语义匹配——同指的，替换该占位；不同指的，新建节点。**写到结构判定为止**——只标 P 坐标与输入 |
...
| 输入 | 本WBS业务内文档的序与输入：前序文档的哪部分正文成为后序文档的输入 |
...
4. 覆盖检查的结果已经写进节点——**文档级E2E任务整段进入本文件、一个不少、同名同批**；**该构件级E2E业务应有的产品数据 ⊆ 本WBS业务已收的文档**，且WBS业务内的输入从源头贯通到交付。缺口已经在树上占位，也已经走补充材料回路，不是隐藏遗漏。
