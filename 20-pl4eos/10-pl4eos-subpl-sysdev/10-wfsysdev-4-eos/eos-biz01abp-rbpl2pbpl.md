...
> 把职责视角已归拢的这批文档级E2E任务，按各自的产品归属重新归位，使每张文档都落到它该在的产品数据位、并按段内序位排出开发次序；再对这批产品数据做覆盖检查——应有的产品数据齐不齐、WBS业务内文档的输入接不接得上——缺什么就补什么。
...
1. **组装 P 树 · 由属性生成干枝**：业务域、产品级E2E业务、构件级E2E业务这三级由属性直接确定，本批一次定完。产品级E2E业务与构件级E2E业务各有三种处置：引用已有的节点、扩展已有节点的边界，或新建一个节点；业务域只有两种——不在时新建，在则直接引用。
2. **组装 P 树 · 按节点意义定 WBS业务**：本任务待文档级E2E任务都落位之后整批一起定，段的边界取在文档相互关联、耦合度较高之处。
...
> **文档级E2E任务与 X 同指，实体归 X**：同一张文档在 R 树里是文档开发任务，在本树里是产品数据位。它的**实体由职责视角业务流程概要持有**，本文件带同名视图，不重写实体；**认亲**就是逐项给 X 的文档级E2E任务定出它在 P 树上的归属，两棵树靠这层互指联通。
...
| 级 | 节点 | 实现规则 |
|----|------|---------|
| ① | 业务域 | **本任务产出**；在本树里作认亲检索与完备对账的域边界。**构建规则**：由属性直接确定——名取「所属业务域」属性值原文，挂在根下。这一级的档位只有引用与新建两档——域由企业运营体系的顶层结构定，加减它不由一批文档驱动 |
...
| ④ | WBS业务 | **本任务产出**；P 坐标的第三项，也是本任务的新建与修订单位。**构建规则**：本任务待文档级E2E任务落位之后才定这一级的节点——新增的文档级E2E任务按本级的节点意义定段，仍与某个已有WBS业务同属一段业务的就归入它或扩展它的边界，与任何已有WBS业务都不属同一段业务的，本任务另出一份**新建WBS业务方案**，并列出这一级应增补的文档级E2E任务、以**占位节点**落在 P 树上。节点名取自组内文档的名称与用途的共同特征 |
| ⑤ | 文档级E2E任务 | **承袭**，取自 X 的同名节点；**占位节点例外**，由本任务产出。**构建规则**：本项挂在「所属构件级E2E业务」之下，与它名下的所有占位逐项做语义匹配——同指的，替换该占位；不同指的，新建节点。**写到结构判定为止**——名称与说明之外，只标 P 坐标、在WBS业务内的序位、输入与内联稳定标签 |
...
一次转换依次走四步。干枝三级逐级拿**同一父节点下**的已有节点作比对，文档级E2E任务按属性落位，WBS业务待它落位之后整批定；判定分三档：**已有、直接引用**，**需扩展**，**无、给新建建议**。这三档与一致性三类的判据出处是 [91 号规范 §5.3.1](91-eos-biz-eng-spec.md)。
...
X 的文档级E2E任务整段搬入 Y，落位只判一件事：挂在哪个构件级E2E业务之下，由「所属构件级E2E业务」这个属性定。这一步定的是复用已有的一张、新增一张，还是兑现一个占位。

**表4-4：文档级E2E任务的落位**

| 级 | 判定 | 判据 | 处置 |
|----|------|------|------|
| ⑤ 文档级E2E任务 | 目标构件级E2E业务下有没有这同一张文档，有没有同指的占位 | 内联稳定标签对得上——两树上同指一张文档；仅有同指占位时按占位兑现处置 | 复用／新增／兑现占位 |

**文档级E2E任务不合并**——R 树上定下的这张文档是权威，它在 P 树上只对同一张：已在的复用，不在的新增；两张不同的文档级E2E任务不并成一张。目标构件级E2E业务下已有这同一张文档，就复用它；没有这一张文档，就新增一张，X 的这一级整段搬入。

**占位兑现**——本批文档先与目标构件级E2E业务之下的**所有占位**逐项比对；与某个占位同指的，占位消解，这张文档落到该位。**不与任何占位同指**的新文档自占新位，各占位不动——占位是一份具体文档的主张，别的文档进来说明不了它、也消不掉它。

**名称不另起**：文档级E2E任务取 X 的同名节点，不重命名、不重排、不增减；它的实体在职责视角业务流程概要里，本文件只带同名视图。占位是唯一的例外——占位有名与坐标，没有方案。

#### 4.2.3 第三步 · 按节点意义定 WBS业务

WBS业务不来自属性，待文档级E2E任务都落到构件之下以后整批定——段的边界要看得见整批文档才判得准。判据是本级节点的意义：段内的产品数据相互关联、耦合度较高，与其中最后一份同时被完成。

**表4-3：WBS业务由节点意义定段**

| 结果 | 判定 | 处置 |
|------|------|------|
| 并入已有WBS业务 | 本批与某个已有WBS业务内的文档相互关联、耦合度较高 | 归入该WBS业务，排入WBS业务内序位 |
| 扩展WBS业务边界 | 本批超出所有已有WBS业务的当前边界，但与某个WBS业务内的文档仍属同一段业务 | 扩展该WBS业务边界，重排WBS业务内序位 |
| 新建WBS业务 | 本批与任何已有WBS业务都不属同一段业务 | 新建WBS业务，排定WBS业务内序位 |
...
**边界匹配检查**——判本批各文档是否仍落在它所在的那个 WBS业务的边界内，即是否仍与WBS业务内的文档相互关联、耦合度较高：

**表4-6：边界匹配检查**

| 结果 | 判定 | 处置 |
|------|------|------|
| 边界内 | 与WBS业务内的文档相互关联、耦合度较高 | 进入WBS业务内文档关系检测 |
| 边界扩展 | 超出当前WBS业务边界，但与WBS业务内的文档仍属同一段业务 | 更新WBS业务边界 + 重排WBS业务内序位；粒度不一致时标 `[推断]` 供人类裁决 |
| 边界外 | 与WBS业务内文档不同链 | 不入本WBS业务——退回归组判定另择WBS业务；无法判定的标 `[需裁决]` 交人类 |

**WBS业务内文档关系检测**——本批某张文档判为复用之后，逐文档与该 WBS业务内的文档比对，以文档身份与它在本WBS业务内的位次为准：
...
| 位次变更 | 文档已在，本次输入改变它在本WBS业务内的位次 | 更新WBS业务内序位与输入，不新增文档 |
| 新增 | 文档不在WBS业务内 | 新增该文档节点，标定 P 坐标与输入 |
...
对本批，依次走四项：**由属性生成干枝、生成文档级E2E任务节点、按节点意义定 WBS业务、标定它的 P 坐标**。干枝是本批共同的干枝——本批同属一个构件级E2E业务，干枝的落位一次定出。**WBS 归组待文档级E2E任务落位之后整批一起判**：段的边界要看得见整批文档（含存量的）才判得准，逐张判正是误新建的来路。P 坐标仍然逐文档，认亲本体唯一。该构件级E2E业务在 Y 上已有节点的，四项都按并入处理。
