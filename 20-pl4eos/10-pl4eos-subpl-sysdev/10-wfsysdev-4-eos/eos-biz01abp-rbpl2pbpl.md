...
|------|------|
| 名称 | 该包的名称 |
| 说明 | 一句话说清本包收哪些文档级E2E任务、按什么序 |
| 数据依赖 | 包内文档的序与依赖：前序文档的哪部分正文成为后序文档的输入 |
| 认亲 | 本包内每项文档级E2E任务的 P 坐标——所属 产品级E2E业务 × 构件级E2E业务 × 本包，加层级判定档位 |
...
| ② 产品级E2E业务 | 「所属产品级E2E业务」 | 这个类型在不在该域下 | 同域已有的产品级E2E业务 | 已有则直接引用，需扩展则改其边界，都没有则新建 |
| ③ 构件级E2E业务 | 「所属构件级E2E业务」 | 这个构件在不在该类型下 | 同一产品级E2E业务下的已有构件级E2E业务 | 已有则直接引用，需扩展则改其边界，都没有则新建 |
...
WBS业务不由属性给，由归组判据定。把这项文档级E2E任务归进一个包，判据是该文档与包内已有文档之间的**数据依赖**——前序文档的正文数据成为后序文档的输入。
...
X 的文档级E2E任务、任务节点、用例场景整段搬入 Y，判的是它挂到哪儿——挂到哪个构件级E2E业务下由「所属构件级E2E业务」定，归进哪个包由数据依赖定；这一步定的是复用已有的一张，还是新增一张。

**表4-4：末三级的落位**

| 级 | 判定 | 判据 | 处置 |
|----|------|------|------|
| ⑤ 文档级E2E任务 | 该包内有没有这同一张文档 | 内联稳定标签对得上——两树上同指一张文档 | 复用／新增 |
| ⑥ 任务节点 | 该文档级E2E任务下有没有这同一个任务节点 | 内联稳定标签对得上——两树上同指一个任务节点 | 复用／新增 |
| ⑦ 用例场景 | 该任务节点下有没有这同一个用例场景 | 内联稳定标签对得上——两树上同指一个用例场景 | 复用／新增 |

**文档级E2E任务不合并**——R 树上定下的这张文档是权威，它在 P 树上只对同一张：已在的复用，不在的新增；两张不同的文档级E2E任务不并成一张。该包内已有这同一张文档，就复用它，把它的节点与本次带来的逐一对上：判该文档级E2E任务下的**全部**任务节点，以及各任务节点之下的**全部**用例场景，各定复用还是新增。该包内没有这一张文档，就新增一张，X 的三级整段搬入。
...
**边界匹配检查**——判本次这项文档级E2E任务是否仍落在这个 WBS业务的边界内，即是否仍与包内文档数据依赖闭合：
...
**包内文档关系检测**——本项文档级E2E任务判为复用后，逐文档与该包内的文档比对，以文档身份与数据依赖为准：
...
**读取输入**——本次这一项文档级E2E任务及其下的任务节点与用例场景，文档级E2E任务上的所属业务域、所属产品级E2E业务、所属构件级E2E业务三个属性，以及同一产品级E2E业务／同域的候选存量方案节点。文档承载什么、怎么开发，是上游的事；本任务判的是它作为产品数据落在哪个构件、哪个包、哪个序位。X 是本任务唯一的外部输入，P 树即资产、就在本任务的产出里，不另读资产文档。加载读取协议是处理节点优先、逐节点加载、按需扩大，不默认加载全文。
...
- **推进目标** = `可以srb设计`
- **清除条件** = 已标定 P 坐标的文档级E2E任务默认跳过。本轮新输入的这项文档级E2E任务满足下列任一条件时，清除该文档的 P 坐标、使其回到待处理：本次新文档是它在包内的直接前序或后序；本次新文档与它同属一个包且数据依赖重叠；本次新文档的加入使它的包内序位被修订
...
- **生成**：P 树组装完成，逐级定位与直接引用／扩展／新建判定正确；WBS业务的归入判定正确、包边界取在依赖闭合处；修订转换的边界匹配／关系检测／变更影响声明已执行；文档级E2E任务、任务节点、用例场景已按属性归位；认亲逐文档标定且本体唯一；树收敛，判据是覆盖完整 + 兄弟互斥。
- **Y 侧要素**：每项文档级E2E任务齐 P 坐标、包内序位、数据依赖与内联稳定标签；任务节点与用例场景到名称级并带内联稳定标签；包节点齐数据依赖、认亲、P 树一致性结论；要素非空或合理空置。
