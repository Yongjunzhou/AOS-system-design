...
> **本文件是什么**：本文件是 EOS 设计线业务路径首个任务位的 SKILL 指南；本任务占这一位里的**产品视角**这一半。它说明本任务这一次转换：输入文档 X 是职责视角业务流程概要，输出文档 Y 是产品视角业务流程概要，转换规则是组装规则在本需求-方案对上的实例化。
>
...
- **任务位**：本阶段第一个对里的环节一。两个并列任务分担、合占这一位。
...
本任务在已有的产品视角业务流程概要上做两件事：把 X 已就位的文档级E2E任务按它们自带的产品归属归位，再排出这些文档的开发次序。目标有七项：

1. **组装 P 树 · 由属性生成干枝**：业务域、产品级E2E业务、构件级E2E业务，逐文档判定它落在这一级的哪个节点。每一级有三种处置：引用已有的节点、扩展已有节点的边界、或新建一个节点。
2. **组装 P 树 · 由数据依赖归入 WBS业务**：整批一起归入，WBS业务的边界取在数据依赖闭合之处；再做 P 树一致性三类检查，产出 **P 树一致性结论**。
3. **标定文档级E2E任务的 P 坐标**：X 的每项文档级E2E任务逐一标定 P 坐标，即所属 产品级E2E业务 × 构件级E2E业务 × WBS业务；**本体唯一**——一张文档可被多个场景引用，只占一个产品数据位。
4. **归位与 Y 侧视图**：X 已经给出的文档级E2E任务、任务节点、用例场景按对位进入本文件，并带上同名视图。
5. **属性齐备保证**：每项文档级E2E任务的三个 P 侧属性都齐备且唯一，并且所属构件级E2E业务挂在它的产品级E2E业务之下。归位对不对，全由这些属性决定。
6. **覆盖检查与补充原始需求**：末三级整段进入 Y、一个不少同名同级；该构件级E2E业务应有的产品数据与本WBS业务已收的文档做名称级比对，WBS业务内数据依赖从源头贯通到交付；检出的缺口在 P 树上落占位节点，并补成产品数据类补充原始需求材料交回原始需求处理，占位待后续汇入的文档按同指合并兑现。
7. **写回与推进**：要素齐备，覆盖结论写进节点；人类确认之后推进状态，并受理上游的退回与变更。
...
> **本任务的移交承诺与职责视角业务流程任务的移交承诺互为对侧**：两个并列任务分担、合占一个任务位，本任务读它移交过来的 X——末三级整段搬入，干枝各长各的。本任务的受理前提分作两处：可承接的对象写在第二章的输入文档结构里，门槛与检验在第五章的入口检验。本侧止于概要，不进入下一个需求-方案对；**P 坐标是这份承诺的交接内容**，它随末三级的同名视图进入下一个需求-方案对，由那一对取用。
