...
| 负责 | 不负责（路由指向） |
|------|-------------------|
| 接收≥1个 bpd-biz 节点（`可以srh设计`/`待补充srh设计`），校验状态/类型/数量 | P0 平台引擎产品 srh-biz → `wft03-eng-bpd2srh` |
...
| 方向 | Skill | 交接内容 |
|------|-------|---------|
| 上游 | `wft02-biz` | 产出**业务流程方案**bpd-biz 节点（`可以srh设计`/`待补充srh设计`），含 R 角色场景（含表单使用顺序）+ P 输出产品（消费 wft01 P 组装结论）+ PL6-B 条目 + 引擎支撑信息。通过 `## AI可以处理节点` 的"待 wft03-biz 处理"分节检测 |
...
**业务覆盖**：
1. R 线：每个 SL5 场景业务均有 srh-biz 承接
2. R 线：每个被编排的功能表单/主子表单均有需求定义承接
3. P 线：每个 PL5 输出文档均有生成、维护或查询类 srh-biz 承接
4. P 线：每个 PL5 输出文档的实体定义/关系/状态事件/业务规则均已被对应 srh-biz 承接
...
|------|------|------|
| 2026-09-10 | v1.26 | 人类方案同步（树简称改 P0／R／P，对齐 94 v1.14）；AI 执行规则语义不变。 |
