...
description: bpd-biz→系统需求概要定义·页面组件开发。接收≥1个bpd-biz节点（可以srh设计/待补充srh设计），
...
| 方向 | Skill | 交接内容 |
|------|-------|---------|
| 上游 | `wft02-biz` | 产出**业务流程详细定义**bpd-biz 节点（`可以srh设计`/`待补充srh设计`），含 A2 角色场景（含表单使用顺序）+ Bn 输出产品（消费 wft01 Bn 组装结论）+ PL6-B 条目 + 引擎支撑信息。通过 `## AI可以处理节点` 的"待 wft03-biz 处理"分节检测 |
...
仅针对有任务类型（计划/流程/工单）的 PL5 输出文档。消费 wft02 Step 4（任务活动设计）产出的 PL6-A 活动节点需求定义，编排为窗口标签页序列（对齐 91 §7.3 Task 5）。流程定义的复用与新建由 wft02 在业务流程详细定义中完成，本 Step 只消费其结果、不读取 24 资产中的流程定义（26 号资产已取消）。
