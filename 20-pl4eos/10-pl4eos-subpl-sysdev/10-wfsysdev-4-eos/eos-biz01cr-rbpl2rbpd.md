...
| 推理依据与补全说明 | 本节点哪些由下级生成、哪些照搬 X，依据是什么 |
| 完备性检查结果 | 入口检验与自检的结论 |
...
一次转换按节点层级自下而上走五步：定义功能操作项需求，落在用例场景；定义表单标签页需求，落在任务节点；定义窗口标签页需求，落在文档级E2E任务；定义末级菜单与岗位标签页需求，落在岗位级E2E业务；定义非末级菜单需求，落在相关方角色与业务域。自下而上，是因为上级的各项需求都由**下级生成**——菜单的可见性由下级取并，岗位标签页的编排、文档序列与任务间触发关系都成于下级；上级先写则无据可依。用例场景、任务节点、文档级E2E任务三级反过来，它们的定义由分析各级的需求说明得出。职责级E2E业务没有界面落点，不设步。第三章的表按级升序逐级列要素，本章按级降序逐级走过程，两序不同各当其用。
...
一次转换起于叶。上游交来的用例场景带着它在 X 上的原文与**需求说明**，本步逐用例场景分析它，写出它所在页上的可点操作。

1. 取 X 的叶节点原文作它的名，不另拟名。
2. 写出该操作项的形态、可用条件与执行结果——使用**功能操作项写实规则**。
3. 定义平台能力需求——使用**引擎化规则**；本叶只是复用已有操作项的，这一样不写。
...
叶落定之后，本步逐任务节点分析它的需求说明，写出它在页面上的形态，并把同一文档的若干页排成序列。
...
**表单标签页写实规则**——一个任务节点对应一张表单标签页，不并页、不拆页。X 这一级只到名称与说明，处理角色、可执行动作与状态变迁由本任务按**需求说明分析规则**从本级的需求说明里取证，取不到的归补足并标 `[推断]`。**处理角色 ⊇ 本节点之下各用例场景的可点角色；可执行动作即本节点之下各功能操作项之并**——叶先定，本步认得它们。
...
6. 逐文档定义平台能力需求——使用**引擎化规则**。

本步定义这批文档级E2E任务的窗口标签页需求，并定义平台能力需求。
...
1. 照搬 X 的场景职责边界陈述，本任务不另定。
2. 定义本业务的访问权限需求——使用**访问权限需求规则**。
3. 写出末级菜单的菜单名与可见性——使用**末级菜单规则**。
4. 排出这批文档级E2E任务的文档序列，并据它编排岗位标签页的窗口标签页——使用**文档序列规则**与**场景装配规则**。
5. 定义任务之间的触发关系——使用**任务间触发关系规则**。
6. 定义平台能力需求——使用**引擎化规则**。
...
菜单的叶子落定之后，本步向上生成两级：先逐相关方角色，再逐业务域。
