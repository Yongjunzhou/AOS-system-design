...
**详细定义的落点是界面**——场景标签页、窗口标签页、表单标签页、功能操作项四级都挂在 R 树上，所以定义只做职责视角，写出的是对这四级的要求；产品视角那一份止于概要，没有定义形态。
...
本任务在已有的职责视角业务流程概要上，把 X 已就位的各级节点深化到对信息系统界面与数据的具体要求：逐级补上这一级在界面与数据上的要求，再对深化的结果做覆盖检查、缺什么就补什么。目标有七项：

1. **写实场景装配**：逐岗位级E2E业务写足对场景标签页的要求——初始态单窗、双击下钻的分屏双窗、持续下钻的层级上限、面包屑导航；场景职责边界承接自上游，本任务不再重定。
2. **写实业务文档实现方式**：逐文档写足对窗口标签页的要求——封面映射、正文实体结构、支撑引擎**校验权威值**、数据权限与显示模式；并点出支撑这张文档的平台能力，即它的引擎、配置单元与配置信息组。
3. **写实表单标签页序列**：逐任务节点写足对表单标签页的要求——本页应有的**表单字段**、处理角色、可执行动作、状态变迁与页面间流转规则；同一文档的若干表单标签页按任务顺序构成序列。
4. **写实叶定义**：逐用例场景取 X 的叶节点原文，写足对功能操作项的要求——形态、触发条件与执行结果；一个用例场景一个功能操作项，不多不少。
5. **任务关系引擎化**：文档之间的流转顺序排成输出文档序列；任务之间的触发关系写成对引擎触发规则的要求，含触发条件与启动参数。
...
Y 与 X 是同一条产品数据的两个形态——X 是它的概要，Y 是它的定义。**Y 复述 X 的整棵树**：各级节点一律承袭 X 的同名节点，编号与内联稳定标签沿用 X 的；本任务不重定域、不重定角色、不重定职责清单，也不新增或改动末三级节点。本任务对岗位级E2E业务、文档级E2E任务、任务节点、用例场景四级各补一项详细定义，业务域、相关方角色、职责级E2E业务三级只读。**写的是对信息系统界面与数据的要求**——平台的功能引擎按配置生成页面组件，页面本身不由本任务交出；这四级各对应一个页面层级，本任务写足这一级在那一层上的要求。逐级的实现规则见表3-2，各级要素见表3-3~表3-9。
...
| ④ | 岗位级E2E业务 | **承袭**，与 X 同名同级；写足**对场景标签页的要求**——单窗／双窗／下钻层级／面包屑；场景职责边界照搬 X |
| ⑤ | 文档级E2E任务 | **承袭**，与 X 同名同级；写足**对窗口标签页的要求**——封面映射、正文实体结构、支撑引擎校验权威值、数据权限与显示模式；点出支撑它的引擎、配置单元与配置信息组 |
| ⑥ | 任务节点 | **承袭**，与 X 同名同级；写足**对表单标签页的要求**——表单字段、处理角色、可执行动作、状态变迁与页面间流转规则 |
| ⑦ | 用例场景 | **承袭**，与 X 同名同级；**叶的详细定义本任务产出**——写足**对功能操作项的要求**，即这一个操作项的形态是功能按钮、右键操作项，还是快捷键 |

- **收敛自查**：父节点语义覆盖子节点语义之并，即覆盖完整；同层兄弟不重叠，即兄弟互斥。

### 3.3 Y 树各级节点的要素

写入 Y 的各级节点都带块头的三个字段：块ID、节点类型 bpd-biz、当前状态。此外，各级节点还须带齐下列各表所列的要素。各表里的「说明」，写的是这一个节点自身是什么、做什么，与该级的级语义不同；级语义见表3-1。封面映射、正文实体结构、支撑引擎校验权威值、数据权限、窗口标签页显示模式、配置单元及配置信息组六项合称**业务文档实现方式**，每张文档级E2E任务一段。
...
> **配置单元与配置信息组只引用不设计**：二者的能力边界、配置入口、配置要素与运行期输出由引擎路径设计，本任务只点出兑现这张文档的是哪一个、哪几个，引其名或记缺口。
...
2. 写出该场景工作区形态上的要求——使用**场景装配规则**。

本步写出该岗位级E2E业务对场景标签页的要求。
...
**支撑能力引用规则**——本张文档的支撑引擎按**支撑引擎权威值规则**落定之后，沿平台引擎的能力清单查它由哪个配置单元、哪些配置信息组供应：平台已有的引其名，没有的记 `[引擎缺口]`，走引擎类补充原始需求。**只引用既有能力，不设计配置单元与配置信息组**——后者由引擎路径设计（判据引 91 号规范 §3.3）。
...
**任务关系引擎化规则**——文档之间的流转顺序排成输出文档序列；任务之间的触发关系写成对引擎触发规则的要求，含触发条件与启动参数。可任意类型互相触发。
...
| 2 | 逐级写实 | 场景装配 → 业务文档实现方式 → 表单标签页序列 → 叶定义，四级的要求逐级写足 | 无（AI 产出，之后对话反馈） | 统一 | 场景装配 + 业务文档实现方式 + 表单标签页序列 + 叶定义 | — |
...
1. 写实场景装配——使用**场景装配规则**。
2. 写实业务文档实现方式——使用**封面映射规则**、**正文实体结构规则**、**数据权限规则**、**支撑引擎权威值规则**、**窗口标签页显示模式规则**与**支撑能力引用规则**。
3. 写实表单标签页序列——使用**表单字段规则**、**表单标签页写实规则**、**表单标签页序列规则**与**任务关系引擎化规则**。
...
**引擎能力对照**——两半各管一头。**接住的那一半**：逐张文档级E2E任务的支撑引擎落定之后，沿平台引擎的能力清单点出兑现它的配置单元与配置信息组，引其名，见 Step 2 的**支撑能力引用规则**；这一半使本任务的产出既是对下游的要求，也是对平台能力的台账。**接不住的那一半**：以用例场景逐一对照平台引擎的能力清单，接不住的操作记 `[引擎缺口]`。它指向的是引擎能力、不是业务数据，故不挂在任何一棵树上。判据引 91 号规范 §4.4 的扩展规则，引用数据即平台引擎的能力清单。
...
- **写实**
  ...
  - 四级各按第四章的规则给出：场景装配按**场景装配规则**；业务文档实现方式按**封面映射规则**、**正文实体结构规则**、**数据权限规则**、**支撑引擎权威值规则**、**窗口标签页显示模式规则**与**支撑能力引用规则**；表单标签页序列按**表单字段规则**、**表单标签页写实规则**、**表单标签页序列规则**与**任务关系引擎化规则**；叶定义按**功能操作项写实规则**。
...
- **Y 侧要素**
  ...
  - 每个任务节点带齐表单字段、处理角色、可执行动作、状态变迁与页面间流转规则。
