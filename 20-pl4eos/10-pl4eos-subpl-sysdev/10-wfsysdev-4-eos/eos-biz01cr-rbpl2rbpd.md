...
本任务在已有的职责视角业务流程概要上，把 X 已就位的各级节点深化到需求定义：逐级写出这一级在界面与数据上的需求，再对深化的结果做覆盖检查、缺什么就补什么。目标有八项：

1. **定义菜单分组需求**：逐业务域定义一级菜单分组需求、逐相关方角色定义二级菜单分组需求——分组名与它下挂的分组；职责级E2E业务没有界面落点，本任务不深化。
2. **定义一站式业务标签页需求**：逐岗位级E2E业务定义一站式业务标签页需求——初始态单窗、双击下钻的分屏双窗、持续下钻的层级上限、面包屑导航；场景职责边界承接自上游，本任务不再重定；并排出该岗位级E2E业务下这批文档级E2E任务的**文档序列**。
3. **定义窗口标签页需求**：逐文档定义窗口标签页需求——封面映射、正文实体结构、支撑引擎**校验权威值**、数据权限与显示模式；并点出支撑这张文档的平台能力，即它的引擎、配置单元与配置信息组。
4. **定义表单标签页需求**：逐任务节点定义表单标签页需求——本页应有的**表单字段**、处理角色、可执行动作、状态变迁与页面间流转规则；同一文档的若干表单标签页按任务顺序构成序列；并写出任务之间的**触发关系**——触发条件与启动参数。
5. **定义功能操作项需求**：逐用例场景取 X 的叶节点原文，定义功能操作项需求——形态、触发条件与执行结果；一个用例场景一个功能操作项，不多不少。
...
Y 与 X 是同一条产品数据的两个形态——X 是它的概要，Y 是它的定义。**Y 复述 X 的整棵树**：各级节点一律承袭 X 的同名节点，编号与内联稳定标签沿用 X 的；本任务不重定域、不重定角色、不重定职责清单，也不新增或改动末三级节点。本任务**逐级补详细定义**：业务域、相关方角色两级补菜单分组需求，岗位级E2E业务、文档级E2E任务、任务节点、用例场景四级补一站式业务标签页、窗口标签页、表单标签页、功能操作项需求；**职责级E2E业务没有界面落点，不补定义**。**写的是各级节点在信息系统界面与数据上的需求**——平台的功能引擎按配置生成页面组件，页面本身不由本任务交出。逐级的实现规则见表3-2，各级要素见表3-3~表3-9。

**表3-2：R 树各级节点实现规则**

| 级 | 节点 | 实现规则 |
|----|------|---------|
| ① | 业务域 | **承袭**，与 X 同名同级；**定义一级菜单分组需求**——分组名与它下挂的分组；并点出兑现它的配置单元与配置信息组 |
| ② | 相关方角色 | **承袭**，与 X 同名同级；**定义二级菜单分组需求**——分组名与它下挂的岗位级E2E业务；并点出兑现它的配置单元与配置信息组 |
| ③ | 职责级E2E业务 | **只读**——该级没有界面落点，本任务不写定义；它随职责视角业务流程概要演进 |
| ④ | 岗位级E2E业务 | **承袭**，与 X 同名同级；**定义一站式业务标签页需求**——单窗／双窗／下钻层级／面包屑；场景职责边界照搬 X；并点出兑现它的配置单元与配置信息组 |
...
2. 各表所列要素齐备——菜单分组、一站式业务标签页、窗口标签页、表单标签页、功能操作项五级已写实到位；
...
一次转换依次走五步：定义菜单分组需求，落在业务域与相关方角色这两级；定义一站式业务标签页需求，落在岗位级E2E业务这一级；定义窗口标签页需求，落在文档级E2E任务这一级；定义表单标签页需求，落在任务节点这一级；定义功能操作项需求，落在用例场景这一级。
...
上游交来的业务域与相关方角色带着它们的名称与说明，本步在这两级上各补菜单分组需求。

1. 逐业务域定义一级菜单分组需求——使用**菜单分组规则**。
2. 逐相关方角色定义二级菜单分组需求——使用**菜单分组规则**。
3. 逐节点点出兑现它的配置单元与配置信息组——使用**引擎化规则**。

本步定义业务域与相关方角色的菜单分组需求。
...
### 4.3 第二步 · 定义一站式业务标签页需求

#### 转换过程

菜单分组落定之后，本步给岗位级E2E业务这一级补一站式业务标签页需求；一条岗位级E2E业务对应一页一站式业务标签页。

1. 取该节点的场景职责边界与类型，照搬 X，本任务不重定。
2. 排出这批文档级E2E任务的文档序列——使用**文档序列规则**。
3. 定义该场景工作区形态上的需求——使用**场景装配规则**。
4. 点出兑现它的配置单元与配置信息组——使用**引擎化规则**。

本步定义该岗位级E2E业务的一站式业务标签页需求，以及这批文档级E2E任务的文档序列。
...
上一步把一站式业务标签页落定之后，本步逐张文档级E2E任务补它的窗口标签页需求。
...
| 2 | 逐级定义需求 | 菜单分组 → 一站式业务标签页 → 窗口标签页 → 表单标签页 → 功能操作项，五级需求逐级定义 | 无（AI 产出，之后对话反馈） | 统一 | 五级需求定义 | — |
...
1. 定义菜单分组需求——使用**菜单分组规则**与**引擎化规则**。
2. 定义一站式业务标签页需求——使用**文档序列规则**、**场景装配规则**与**引擎化规则**。
...
- **操作条目** = 一站式业务标签页需求 + 窗口标签页需求 + 表单标签页需求 + 功能操作项需求
- **推进目标** = `可以srb设计`
...
- **载入与校验**
  ...
  - 用例场景的内联稳定标签齐备且唯一。
- **逐级定义需求**
  - 五级需求定义完成——菜单分组、一站式业务标签页、窗口标签页、表单标签页、功能操作项。
  - 五级各按第四章的规则给出：菜单分组按**菜单分组规则**与**引擎化规则**；一站式业务标签页按**文档序列规则**、**场景装配规则**与**引擎化规则**；窗口标签页按**封面映射规则**、**正文实体结构规则**、**数据权限规则**、**支撑引擎权威值规则**、**窗口标签页显示模式规则**与**引擎化规则**；表单标签页按**表单字段规则**、**表单标签页写实规则**、**表单标签页序列规则**、**任务间触发关系规则**与**引擎化规则**；功能操作项按**功能操作项写实规则**与**引擎化规则**。
