...
> **同一个节点可对应多条上游需求**：承接记录把业务功能需求条目逐条引到节点上，多条汇聚成一个节点、跨批追加不覆盖。
...
Y 是 R 树的定义形态，一份文档，根到叶。**R 树（各业务域 · 业务运行职责视角）**回答「某角色在哪个场景、按什么序、处理哪些文档级E2E任务，在页面上怎么操作完成闭环」；在概要里只到说明为止的各级节点，到这里都带上详细定义。同一批文档级E2E任务在 **P 树**里另有一面，由产品视角业务流程概要持有；**两树在文档级E2E任务这一级上认亲互指，同名同批**。
...
逐级加上的，是各级节点在信息系统界面与数据上的需求。平台的功能引擎按配置生成页面组件，页面本身不由本任务交出。逐级的实现规则见表3-2，各级要素见表3-3~表3-9。
...
| ④ | 岗位级E2E业务 | **承袭**，与 X 同名同级；**作末级菜单**，定其菜单名与可见性；**定义岗位标签页需求**——其名称、可见性、构成窗口标签页秩序，并定其访问权限需求；场景职责边界照搬 X；并点出兑现它的配置单元与配置信息组 |
| ⑤ | 文档级E2E任务 | **承袭**，与 X 同名同级；**定义窗口标签页需求**——业务文档实现方式（封面映射、正文实体结构、支撑引擎校验权威值、数据权限、窗口标签页显示模式）；点出支撑它的引擎、配置单元与配置信息组 |
| ⑥ | 任务节点 | **承袭**，与 X 同名同级；**定义表单标签页需求**——表单字段、处理角色、可执行动作、状态变迁、页面间流转规则与其构成序列、支持信息引用；并点出兑现它的配置单元与配置信息组 |
| ⑦ | 用例场景 | **承袭**，与 X 同名同级；**叶的详细定义本任务产出**——**定义功能操作项需求**：形态取功能按钮、右键操作项或快捷键并记其出现位置，可用条件与执行结果按该场景的业务语义写实；并点出兑现它的配置单元与配置信息组 |

- **收敛自查**：父节点语义覆盖子节点语义之并，即覆盖完整；同层兄弟不重叠，即兄弟互斥。

### 3.3 Y 树各级节点的要素

写入 Y 的各级节点都带块头的三个字段（块ID、节点类型 bpd-biz、当前状态）与下列各表所列的要素。各表的「说明」写该节点自身是什么、做什么，与表2-1 的级语义不同。
...
| 功能操作项 | 本场景对应的可点操作——形态（功能按钮／右键操作项／快捷键）与出现位置、**本操作项的可用条件**、执行结果 |
| 配置单元及配置信息组 | 兑现功能操作项的平台能力——由哪个配置单元、哪些配置信息组供应，引其名；平台没有的记 `[引擎缺口]` |
| 内联稳定标签 | X 与 Y 上同一个节点的对上键——节点在本形态与概要形态之间靠它对上，下游凭它逐级追溯 |

> **配置单元与配置信息组逐级都有，且只引用不设计**：它是兑现本级需求所需的平台能力，其能力边界、配置入口、配置要素与运行期输出由引擎路径设计（判据引 91 号规范 §3.3）。本任务只点出它是哪一个、哪几个——平台已有的引其名；平台没有的记 `[引擎缺口]` 并给**建议名**，名字作候选交引擎路径设计。
...
一次转换按节点层级自下而上走五步：定义功能操作项需求，落在用例场景；定义表单标签页需求，落在任务节点；定义窗口标签页需求，落在文档级E2E任务；定义末级菜单与岗位标签页需求，落在岗位级E2E业务；定义非末级菜单需求，落在相关方角色与业务域。自下而上，是因为上级的各项需求都由下级合成——菜单的可见性由下级取并，岗位标签页的编排、文档序列与任务间触发关系都成于下级；上级先写则无据可依。职责级E2E业务没有界面落点，不设步。第三章的表按级升序逐级列要素，本章按级降序逐级走过程，两序不同各当其用。

### 4.1 X 与 Y 的对应

X 与 Y 是同一条产品数据的两个形态，也是同一棵 R 树：级位逐级对上，都是七级；两侧每一对指向同一个节点，编号相同、名称相同。X 只写到说明，Y 写到详细定义，**逐级的深度不同**。

本任务不另做承接，也不另做分配——承接在 X 那一处已随它的节点完成，记在 X 的承接记录里；本任务做的是在那之上逐级加深。承接规则与基数引 [94 号文档 §1.2~§1.4](94-eos-sys-dev-dbs.md)，本任务不重述。
...
一次转换起于叶。上游交来的用例场景带着它在 X 上的原文，本步逐用例场景写出它所在页上的可点操作。

1. 取 X 的叶节点原文作它的名，不另拟名。
2. 写出该操作项的形态、可用条件与执行结果——使用**功能操作项写实规则**。
...
**功能操作项写实规则**——一个用例场景对应一个功能操作项，不多不少。形态取功能按钮、右键操作项或快捷键；可用条件与执行结果按该场景的业务语义写实，**可点它的角色属可用条件之一，也在这里定**。叶的名称、次序与内联稳定标签由 X 给出，照搬不改；本任务不新增叶节点，发现应有而未被列出的叶，标 `[需确认]` 交人类，不就地新增。

**引擎化规则**——逐级给出兑现本级需求所需的平台能力，沿**引擎 → 配置单元 → 配置信息组**三级落下去：先定兑现它的引擎，再定该引擎之下由哪个配置单元供应，最后定该配置单元里由**哪些配置信息组**兑现这一项需求，引其名；平台没有的记 `[引擎缺口]` 并给**建议名**，缺口写到配置信息组这一级。

### 4.3 第二步 · 定义表单标签页需求

#### 转换过程

叶落定之后，本步逐任务节点写出它在页面上的形态，并把同一文档的若干页排成序列。

1. 逐任务节点写这一页应有的表单字段——使用**表单字段规则**。
2. 逐任务节点写处理角色、可执行动作与状态变迁——使用**表单标签页写实规则**。
3. 定义同一文档下各表单标签页之间的流转规则——使用**表单标签页序列规则**。
4. 逐任务节点写出本步作业所依据的支持信息——使用**支持信息引用规则**。
...
**表单字段规则**——与文档级E2E任务的正文实体结构**同源于 X 的正文实体类型**：那一级展开整体，本步只取这一页上应有的字段，名称级；本步要填而正文里没有的字段一并列出，**该文档的正文实体结构在下一步把这些字段并进去**；判不出的按业务语义写实并标 `[推断]`。

**表单标签页写实规则**——一个任务节点对应一张表单标签页，不并页、不拆页。X 这一级只到名称与说明，处理角色、可执行动作与状态变迁由本任务补，判不出的按业务语义写实并标 `[推断]`。**处理角色 ⊇ 本节点之下各用例场景的可点角色；可执行动作即本节点之下各功能操作项之并**——叶先定，本步认得它们。
...
页落定之后，本步逐张文档级E2E任务写出它的窗口标签页需求。

1. 逐文档定封面映射——使用**封面映射规则**。
2. 逐文档写正文实体结构——使用**正文实体结构规则**。
3. 逐文档定数据权限——使用**数据权限规则**。
4. 逐文档校验 X 给的支撑引擎候选，把权威值落定——使用**支撑引擎权威值规则**。
5. 逐文档定窗口标签页的显示模式——使用**窗口标签页显示模式规则**。
6. 逐文档点出兑现它的配置单元与配置信息组——使用**引擎化规则**。

本步定义这批文档级E2E任务的窗口标签页需求，并点出支撑它们的配置单元与配置信息组。
...
**支撑引擎权威值规则**——X 给的支撑引擎候选一律是 `[推断]`；本任务在定义的上下文里校验，确认／修正／推翻，落定为本任务给出的校验权威值，仍存疑的标 `[推断]`。X 未给候选的，按文档类型推出，同样标 `[推断]`。落定值下游据此分配指标。
...
1. 定义本业务的访问权限需求——使用**访问权限需求规则**。
2. 写出末级菜单的菜单名与可见性——使用**末级菜单规则**。
3. 排出这批文档级E2E任务的文档序列，并据它编排岗位标签页的窗口标签页——使用**文档序列规则**与**场景装配规则**。
...
**任务间触发关系规则**——**文档之间与同一文档内**任务之间的启动关系：**触发条件**（上一任务走到什么状态）与**启动参数**（指派给谁、期限、初值、附件）；可任意类型互相触发。

### 4.6 第五步 · 定义非末级菜单需求

#### 转换过程

菜单的叶子落定之后，本步向上合成两级：先逐相关方角色，再逐业务域。
...
| 2 | 逐级定义需求 | 自下而上五级：功能操作项 → 表单标签页 → 窗口标签页 → 末级菜单与岗位标签页 → 非末级菜单，逐级定义 | 无（AI 产出，之后对话反馈） | 统一 | 五级需求定义 | — |
...
**处理对象**——入口检出待本任务处理的节点，取两处。**X 这一处**：处于 `可以srb设计` 或 `待补充srb设计` 的节点——**可落在任意一级**，落在哪一级就做那一级的档，其下级是另一次运行的事；状态为 `待补充srb设计` 的，即 X 汇入的版本大于本任务消费的版本，按上游的变更影响声明分型读增量部分。**Y 这一处**：状态为 `需biz01cr修订` 的节点，即下游检出缺陷退回的，本任务捡起它、按修订转换处理。**职责级E2E业务没有界面落点，本任务不认领它**——它随职责视角业务流程概要演进，状态由那一处维护。
...
| 用例场景与所属任务节点不同指一个业务对象 | 软限制 | 该节点挂 `[需确认]`，其余照常 |
| 承接的业务功能需求条目与叶对不上——某条承接在叶定义里找不到落点 | 软限制 | 该节点挂 `[需确认]`，其余照常 |
...
**状态推进**——本任务写入 `待确认方案`／`可以srb设计`／`待补充srb设计` 三个状态；`在srb设计`／`已srb设计` 与 `需biz01cr修订` 由下游写入，后者是下游检出缺陷、把节点退回本任务时的值。Start 按状态路由，Step 5 在写回时推进。
...
**逐级定义需求**——针对选中的节点，自下而上逐级，依次走第四章的五步：

1. 定义功能操作项需求——使用**功能操作项写实规则**与**引擎化规则**。
2. 定义表单标签页需求——使用**表单字段规则**、**表单标签页写实规则**、**表单标签页序列规则**、**支持信息引用规则**与**引擎化规则**。
...
五级走完，有新增或改进时 → 自下而上调树至收敛，判据是覆盖完整 + 兄弟互斥；无法调整收敛的标 `[需裁决]` 随方案一并交 Step 3，不自定架构。X 自身的缺陷不在本步补，按受理门槛处置。
...
**缺口成材料**——**引擎能力对照**检出缺口即生成**引擎类补充原始需求材料**候选，交原始需求处理补回，**本任务不脑补节点，也不代替引擎做设计**。候选去留由人类裁决。**本任务只出「接不住」这一件事**——业务类的缺口不出材料，标 `[需确认]` 交人类裁决。
...
- **逐级定义需求**
  ...
  - 五级各按第四章的规则给出，自下而上：功能操作项按**功能操作项写实规则**与**引擎化规则**；表单标签页按**表单字段规则**、**表单标签页写实规则**、**表单标签页序列规则**、**支持信息引用规则**与**引擎化规则**；窗口标签页按**封面映射规则**、**正文实体结构规则**、**数据权限规则**、**支撑引擎权威值规则**、**窗口标签页显示模式规则**与**引擎化规则**；末级菜单与岗位标签页按**访问权限需求规则**、**末级菜单规则**、**文档序列规则**、**场景装配规则**、**任务间触发关系规则**与**引擎化规则**；非末级菜单按**非末级菜单规则**与**引擎化规则**。
...
- **关口**
  ...
  - 检验结论已写入完备性检查结果，引擎类材料已写入补充材料引用——本任务不出业务类材料，业务类的不确定已挂 `[需确认]`。
- **收尾**
  - 三区摘要已输出，下一步已给出。
