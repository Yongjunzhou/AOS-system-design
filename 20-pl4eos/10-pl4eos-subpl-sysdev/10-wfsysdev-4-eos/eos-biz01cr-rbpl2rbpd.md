...
> **本文件是什么**：本文件是 EOS 设计线业务路径第二个工位的 SKILL 指南。它说明本任务这一次转换：输入文档 X 是职责视角业务流程概要，输出文档 Y 是职责视角业务流程定义，转换规则是深化规则在本需求-方案对上的实例化。
>
...
> 把职责视角已归拢的这批业务，在说明级之上深化到界面与数据，使业务在页面上照着跑得下来。
...
这棵 R 树有三种深浅不同的形态——**框架**最浅、**概要**居中、**定义**最深。三个形态都是方案的不同详细程度，都不占需求-方案对的一端。上游交出的是概要，本任务交出的是定义，也就是在概要之上，再给各级节点补详细定义。

**详细定义的落点是界面**——场景标签页、窗口标签页、表单标签页、功能操作项四级都挂在 R 树上，所以定义只做职责视角；产品视角那一份止于概要，没有定义形态。
...
本任务在已有的职责视角业务流程概要上，把 X 已就位的各级节点写实到界面与数据：逐级补上它的界面形态与数据定义，再对写实的结果做覆盖检查、缺什么就补什么。目标有七项：

1. **写实场景装配**：逐岗位级E2E业务定它这一级的工作区形态——初始态单窗、双击下钻的分屏双窗、持续下钻的层级上限、面包屑导航；场景职责边界承接自上游，本任务不再重定。
2. **写实业务文档实现方式**：逐文档定封面映射、正文实体结构、支撑引擎**校验权威值**、数据权限，并定它窗口标签页的显示模式。
3. **写实表单标签页序列**：逐任务节点定处理角色、可执行动作、状态变迁与页面间流转规则；同一文档的若干表单标签页按任务顺序构成序列。
4. **写实叶定义**：逐用例场景取 X 的叶节点原文，定它的形态、触发条件与执行结果——一个用例场景一个功能操作项，不多不少。
5. **任务关系引擎化**：文档之间的流转顺序排成输出文档序列；任务之间的触发关系落成引擎触发规则。
6. **覆盖检查与补充原始需求**：X 的每个叶都对上一个详细定义；**节点累计承接的**全部需求语义不超出其叶定义之并；用例场景逐一对照平台引擎，接不住的操作记 `[引擎缺口]`，补成引擎类补充原始需求材料交回原始需求处理。
7. **写回与推进**：要素齐备、覆盖结论写进节点；人类确认之后写回并推进状态，受理下游的退回与上游的变更。
...
| 同级 | 产品视角业务流程任务 | 与本任务同占一个工位；它交出的那份止于概要，本任务不取它的产出 |
| 产品数据 | 职责视角业务流程概要（X）／职责视角业务流程定义（Y） | 本任务读 X、写 Y |

> **下游承接的是叶**：本任务交出的定义里，用例场景这一级带界面形态与数据定义，是下游逐级对上的承接源；X 的概要形态不供下游承接。
...
> **各级只到说明**：X 从根到叶，各级节点都写到说明为止——本级是什么、做什么，一句话说清；界面形态与数据定义一概不写，那是本任务的活。
...
Y 与 X 是同一条产品数据的两个形态——X 是它的概要，Y 是它的定义。**Y 复述 X 的整棵树**：各级节点一律承袭 X 的同名节点，编号与内联稳定标签沿用 X 的；本任务不重定域、不重定角色、不重定职责清单，也不新增或改动末三级节点。本任务对岗位级E2E业务、文档级E2E任务、任务节点、用例场景四级各补一项详细定义，业务域、相关方角色、职责级E2E业务三级只读。逐级的实现规则见表3-2，各级要素见表3-3~表3-9。
...
| ① | 业务域 | **只读**——本任务不重定域 |
| ② | 相关方角色 | **只读**——本任务不重定角色 |
| ③ | 职责级E2E业务 | **只读**——该级随职责视角业务流程概要演进 |
| ④ | 岗位级E2E业务 | **承袭**，与 X 同名同级；写实为场景标签页——单窗／双窗／下钻层级／面包屑；场景职责边界照搬 X |
| ⑤ | 文档级E2E任务 | **承袭**，与 X 同名同级；写实为窗口标签页——补封面映射、正文实体结构、支撑引擎校验权威值、数据权限与显示模式 |
| ⑥ | 任务节点 | **承袭**，与 X 同名同级；写实为表单标签页——补处理角色、可执行动作、状态变迁与页面间流转规则 |
...
写入 Y 的各级节点都带块头的三个字段：块ID、节点类型 bpd-biz、当前状态。此外，各级节点还须带齐下列各表所列的要素。各表里的「说明」，写的是这一个节点自身是什么、做什么，与该级的级语义不同；级语义见表3-1。封面映射、正文实体结构、支撑引擎校验权威值、数据权限、窗口标签页显示模式五项合称**业务文档实现方式**，每张文档级E2E任务一段。
...
| 名称 | 业务语言名称 |
| 功能操作项 | 本场景对应的可点操作——形态（功能按钮／右键操作项／快捷键）、触发条件、执行结果 |
| 内联稳定标签 | 下游逐级对上的追溯键 |
...
一次转换依次走四步：写实场景装配，落在岗位级E2E业务这一级；写实业务文档实现方式，落在文档级E2E任务这一级；写实表单标签页序列，落在任务节点这一级；写实叶定义，落在用例场景这一级。
...
> **两端粗细不同**：本任务补的是各级节点的**界面形态与数据定义**——节点本身由 X 给出，本任务给它们写实。

### 4.2 第一步 · 写实场景装配

#### 转换过程

上游交来的岗位级E2E业务带着它的场景职责边界与类型，本步在这一级上加场景标签页；一条岗位级E2E业务对应一页场景标签页。

1. 取该节点的场景职责边界与类型，照搬 X，本任务不重定。
2. 定该场景的工作区形态，按窗口标签页的编排写出它在页面上的样子——使用**场景装配规则**。

本步交出该岗位级E2E业务的场景标签页。

#### 转换规则

**场景装配规则**——场景标签页由它名下的窗口标签页编排而成，场景内的窗口标签页序列即该岗位级E2E业务处理这批文档级E2E任务的文档序列。场景职责边界取自 X 的场景职责边界陈述与类型，照搬不改。工作区形态取四档，出自 91 号规范 §1.2 的交互模式：初始态单窗；双击窗口标签页下钻时进分屏双窗；持续下钻按层级上限终止；下钻路径以面包屑导航呈现。

### 4.3 第二步 · 写实业务文档实现方式

#### 转换过程

上一步把场景标签页落定之后，本步逐张文档级E2E任务补它的实现方式与展示载体。
...
1. 逐任务节点写处理角色、可执行动作与状态变迁——使用**表单标签页写实规则**。
2. 编排同一文档下各表单标签页之间的流转规则——使用**表单标签页序列规则**。
3. 把任务之间的触发关系落成引擎触发规则——使用**任务关系引擎化规则**。

本步交出这批文档级E2E任务的表单标签页序列与引擎触发规则。

#### 转换规则

**表单标签页写实规则**——一个任务节点对应一张表单标签页，不并页、不拆页。X 只写到业务语言名称级，处理角色、可执行动作与状态变迁由本任务补到架构语言一级，判不出的按业务语义写实并标 `[推断]`。

**表单标签页序列规则**——同一文档的若干表单标签页按任务节点的顺序构成序列，页与页之间的流转规则由本任务编排。

**任务关系引擎化规则**——文档之间的流转顺序排成输出文档序列；任务之间的触发关系落成引擎触发规则，含触发条件与启动参数。可任意类型互相触发。
...
2. 定它的形态、触发条件与执行结果——使用**功能操作项写实规则**。

本步交出这批用例场景的功能操作项，内联稳定标签随叶携带。
...
| 2 | 逐级写实 | 场景装配 → 业务文档实现方式 → 表单标签页序列 → 叶定义，四级的界面形态与数据定义逐级落定 | 无（AI 产出，之后对话反馈） | 统一 | 场景标签页 + 窗口标签页方案 + 表单标签页序列 + 功能操作项 | — |
...
**状态推进**——本任务写入 `待确认方案`／`可以srb设计`／`待补充srb设计`／`需biz01cr修订` 四个状态；`在srb设计`／`已srb设计` 由下游写入。Start 按状态路由，Step 5 在写回时推进。
...
**退回上游**：X 有缺陷、且缺陷的处置权不在本任务 → 标 `需biz01修订`，交回职责视角业务流程任务，本轮不再受理。**退回节点状态值** = `需biz01cr修订`，由下游写入，本任务按状态路由捡起。
...
**逐级写实**——针对一条岗位级E2E业务，自外向内四级写实，依次走第四章的四步：

1. 写实场景装配——使用**场景装配规则**。
2. 写实业务文档实现方式——使用**封面映射规则**、**正文实体结构规则**、**数据权限规则**、**支撑引擎权威值规则**与**窗口标签页显示模式规则**。
3. 写实表单标签页序列——使用**表单标签页写实规则**、**表单标签页序列规则**与**任务关系引擎化规则**。
4. 写实叶定义——使用**功能操作项写实规则**。

**本侧特有两处**：一是窗口标签页的显示模式按数据源类型定，指标与看板两档还要看它挂在哪张清单文档上；二是用例场景这一级取 X 的叶节点原文，不另拟名，本任务不新增叶节点。

四级走完，有新增或改进时 → 自下而上调树至收敛，判据是覆盖完整 + 兄弟互斥；无法调整收敛的标 `[需裁决]` 随方案一并交 Step 3，不自定架构。X 自身的缺陷不在本步补，按受理门槛的退回处置。
...
**覆盖检查**——**节点累计承接的**全部需求语义 ⊆ 其叶定义之并。同一节点的需求分多批到来，故检查范围累计——每批检新增部分；本批改动了已有干枝的，另按影响面复查其下已写实的部分。落到 EOS 实例上就是**界面闭合检查**——每个用例场景落在至少一个功能操作项上、每个任务节点落在至少一张表单标签页上、每张文档级E2E任务落在至少一个窗口标签页上、每条岗位级E2E业务落在至少一页场景标签页上。

**引擎能力对照**——以用例场景逐一对照平台引擎的能力清单，接不住的操作记 `[引擎缺口]`；它指向的是引擎能力、不是业务数据，故不挂在任何一棵树上。判据引 91 号规范 §4.4 的扩展规则，引用数据即平台引擎的能力清单。
...
- **载入与校验**
  ...
  - 用例场景的内联稳定标签齐备且唯一。
- **写实**
  - 四级写实完成——场景装配、业务文档实现方式、表单标签页序列、叶定义。
  - 四级各按第四章的规则给出：场景装配按**场景装配规则**；业务文档实现方式按**封面映射规则**、**正文实体结构规则**、**数据权限规则**、**支撑引擎权威值规则**与**窗口标签页显示模式规则**；表单标签页序列按**表单标签页写实规则**、**表单标签页序列规则**与**任务关系引擎化规则**；叶定义按**功能操作项写实规则**。
...
- **Y 侧要素**
  - 每条岗位级E2E业务带齐场景装配。
  - 每张文档级E2E任务带齐封面映射、正文实体结构、支撑引擎校验权威值、数据权限、窗口标签页显示模式与内联稳定标签。
  - 每个任务节点带齐处理角色、可执行动作、状态变迁与页面间流转规则。
  - 每个用例场景带齐功能操作项与内联稳定标签。
  - 要素非空或合理空置。
- **确认与写回**
  - 要素机械检查已执行。
  - 方案已列全、逐条确认。
