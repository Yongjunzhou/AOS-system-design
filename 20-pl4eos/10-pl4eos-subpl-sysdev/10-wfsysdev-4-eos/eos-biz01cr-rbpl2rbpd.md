...
逐级加上的，是各级节点在信息系统界面与数据上的需求。平台的功能引擎按配置生成页面组件，页面本身不由本任务交出。逐级的需求定义实现规则见表3-2，各级要素见表3-3~表3-9。

**表3-2：R 树各级节点需求定义实现规则**

| 级 | 节点 | 实现规则 |
|----|------|---------|
| ① | 业务域 | **定义一级菜单需求**——菜单名、下挂的二级菜单、可见性；并点出兑现它的配置单元与配置信息组 |
| ② | 相关方角色 | **定义二级菜单需求**——菜单名、下挂的末级菜单、可见性；并点出兑现它的配置单元与配置信息组 |
| ③ | 职责级E2E业务 | **只读**——该级没有界面落点，本任务不写定义；它随职责视角业务流程概要演进 |
| ④ | 岗位级E2E业务 | **作末级菜单**，定义其菜单名与可见性；**定义岗位标签页需求**——其名称、可见性、构成窗口标签页秩序，并定义其访问权限需求；场景职责边界照搬 X；并点出兑现它的配置单元与配置信息组 |
| ⑤ | 文档级E2E任务 | **分析本级的需求说明**，**定义窗口标签页需求**——业务文档实现方式（封面映射、正文实体结构、支撑引擎校验权威值、数据权限、窗口标签页显示模式）；点出支撑它的引擎、配置单元与配置信息组 |
| ⑥ | 任务节点 | **分析本级的需求说明**，**定义表单标签页需求**——表单字段、处理角色、可执行动作、状态变迁、页面间流转规则与其构成序列、支持信息引用；并点出兑现它的配置单元与配置信息组 |
| ⑦ | 用例场景 | **分析本级的需求说明**，**定义功能操作项需求**：形态取功能按钮、右键操作项或快捷键并记其出现位置，可用条件与执行结果按该场景的业务语义写实；并点出兑现它的配置单元与配置信息组 |

- **需求来源分两段**：文档级E2E任务及其下两级**分析本级的需求说明**、逐级定义；其上各级的需求**由下级生成**，只到名称、可见性与所有者，不分析需求说明。
...
本任务不另做承接，也不另做分配——相关方需求在 X 那一处已随它的节点整合上树，成为**文档级E2E任务及其下两级**的需求说明；本任务做的是分析这些需求说明，自下而上写出定义——其上各级的需求由下级生成，只到名称、可见性与所有者。承接规则与基数引 [94 号文档 §1.2~§1.4](94-eos-sys-dev-dbs.md)，本任务不重述。
