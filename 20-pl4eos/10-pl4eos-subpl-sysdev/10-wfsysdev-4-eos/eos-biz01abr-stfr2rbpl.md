...
这一步只判一件事：这条 E2E职责的业务对象闭环走完没有。走不完即缺了应有的业务文档或任务节点——这是业务链类的缺口，记 `[业务链缺口]`，生成**补充原始需求材料**回原始需求处理，由它加工成业务功能需求条目、归业务路径。人类裁决「需要」→ 落地；「不要」→ 废弃。**本任务不脑补节点。**
...
执行两件——**长干枝**：用这张业务文档根上的三个属性在 R 树上落出业务域、相关方角色、E2E职责，再由 E2E职责聚出角色职责；**末三级落位**：把业务文档连同它的任务节点与用例场景整段搬到 E2E职责之下。随后排该 E2E职责的文档序列，为业务文档补业务文档实现方式；有新增或改进 → 自下而上调树至收敛，判据是覆盖完整 + 兄弟互斥。
