...
**承接记录**——这条对应关系要回写到 X 上：每张业务文档记下它被 Y 的哪个节点承接、以什么方式承接、凭什么可以承接。

**承接节点**是它落进去的那个 E2E职责节点——Y 的节点即一个 E2E职责，业务文档是它下面的一级。节点写 Y 的块ID，其后 `@` 接文档本名，如 `bpd-biz-GOV-001@职责视角业务流程概要`；块ID 不含所属文档，职责视角业务流程定义用的是同一套 ID，不接文档本名就定不到是哪一份。

**承接方式**取复用、改进、新增三档——**复用**是该 E2E职责下已有承接这份业务的文档，本次条目的语义被现有节点覆盖；**改进**是已有那张文档，但本次改变了它的正文特征，或扩展了该 E2E职责的边界；**新增**是该 E2E职责下原先没有承接这份业务的文档。

**可承接理由**把所据判据说成业务语言一句：这份业务落在该 E2E职责的触发到自然终点之内、属同一个业务对象闭环，且它就是要输出的那份产品数据。
...
| 3 | E2E职责检查并补充需求 | 覆盖检查取两面；承接记录补覆盖面；缺口生成文档序列类补充原始需求材料候选 | 无（AI 自动） | 统一 | 覆盖结论 + 缺口候选 | 相关方需求 |
...
**也写的是 X**——本次每张业务文档的承接记录回写进相关方需求：承接节点、承接方式、可承接理由三项写在该业务文档名下的条目上，同一张业务文档的条目写同一个承接节点与承接方式。
...
**承接记录补覆盖面**——两面核完，把结论写进 X 上那张业务文档的承接记录：末级整段进入、文档序列兑现。核不过的，记录上标出缺口，不写「已承接」。
...
**写回**——判「需要」的缺口按**文档序列类补充原始需求材料**成文，材料交原始需求处理补回；覆盖检查的结论写入该 E2E职责节点的完备性检查结果，材料写入补充材料引用。本步改动让某张业务文档换了承接节点、换了承接方式，或认了、撤了缺口标记的，X 上的承接记录随之改。写回动作按 [91 号规范 §A.3.3](91-eos-biz-eng-spec.md) 执行。
...
- **确认与写回**：要素机械检查已执行；R 树方案与 E2E职责聚类方案已晒全、逐条确认；确认结果已按表5-3 写回，已被下游消费过的节点已按上游变更分流落定状态与版本；每张业务文档的承接记录已回写 X 各条目——承接节点、承接方式、可承接理由三项齐；`## AI最近变更` 与 `## AI可以处理节点` 分节已维护、版本号已递增。
- **检查**：覆盖检查已执行、两面都取；承接记录的覆盖面已补；缺口以闭合检查产出文档序列类补充原始需求材料候选，去留有人类裁决，**不脑补节点**。
