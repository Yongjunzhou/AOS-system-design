...
**源节点**是本条所属的业务文档名下的条目——一次运行处理的就是一张业务文档及其下级，故一轮只产出一条记录，写在该业务文档名下的全部条目上。

**关系类型**是承接。

**目标节点**是承接它的那张业务文档，即 Y 上业务文档级的一级节点，写它的内联稳定标签。

**目标文档**是职责视角业务流程概要——业务文档级的标签与 E2E职责节点的块ID，在职责视角业务流程定义里是同一套，不写目标文档就定不到是哪一份。

**状态**随覆盖检查落。

**说明**记可以承接的理由一句话：这份业务落在该 E2E职责的触发到自然终点之内、属同一个业务对象闭环，且它就是要输出的那份产品数据。理由的完整成文落进承接它的那个 E2E职责节点的推理依据与补全说明。
...
**也写的是 X**——本次这张业务文档的承接记录回写进相关方需求：源节点、关系类型、目标节点、目标文档、状态、说明六项写在该业务文档名下的条目上，同一张业务文档的条目写同一套值。
...
**承接记录复核**——两面核完，把结论落进 X 上这张业务文档的承接记录的状态：核过的写已承接；核不过的写缺口，不认它已承接。
...
**写回**——判「需要」的缺口按**文档序列类补充原始需求材料**成文，材料交原始需求处理补回；覆盖检查的结论写入该 E2E职责节点的完备性检查结果，材料写入补充材料引用。本步改动让某张业务文档的承接记录换了目标节点或换了状态的，记录随之改。写回动作按 [91 号规范 §A.3.3](91-eos-biz-eng-spec.md) 执行。
...
- **Y 侧要素**：每张业务文档齐封面模式、正文实体类型与内联稳定标签；任务节点与用例场景到名称级并带内联稳定标签；加支撑引擎候选 `[推断]`、任务关系；要素非空或合理空置。
