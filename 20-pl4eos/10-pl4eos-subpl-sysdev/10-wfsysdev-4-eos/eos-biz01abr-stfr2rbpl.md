...
> **本文件是什么**：EOS 设计线业务路径首个任务位里**职责视角那一半**的 SKILL 指南。它说明本任务这一步的 X→Y：输入文档 X 是相关方需求，输出文档 Y 是职责视角业务流程概要，转换规则是组装规则在本需求-方案对上的实例化。
>
...
> 把新进的相关方功能需求分析清楚，按属性在 R 树上逐级定位：相关方角色与 E2E职责判**合并／新建**，业务文档、任务节点、用例场景判**新增／改进**；判定结果增补进已有的职责视角业务流程概要。

### 1.2 在设计链上的位置

设计开发 = 由需求到方案的逐对生成。业务路径的第一对，需求端是**相关方需求**、方案端是**业务流程**，分两个阶段：阶段一产出**业务流程概要**，阶段二把概要补成**业务流程定义**。阶段一由两个并列任务分担、合占一个任务位：**职责视角业务流程概要**由本任务产出，**产品开发视角业务流程概要**由产品开发视角业务流程任务产出。本任务的产出是：

```
相关方需求 → 职责视角业务流程概要：  X = 相关方需求，不分形态，最深一级＝用例场景
        │  第一对的叶由上游直接给出；这一步既长干枝、也逐级判定落位，跳过框架直接产出概要
        ▼
       Y = 职责视角业务流程概要（02），即 R 树——根→叶，各级到说明
```
...
**输入语义属组装型**：X 的末级节点整段成为 Y 的末级节点——业务文档、任务节点、用例场景三级同名同批。本任务不做映射、不做分配：按属性在 R 树上逐级定位，判**合并／新建**与**新增／改进**后落位。
...
| ② | 任务节点 | 该业务文档开发所对应的任务，原则上，一个节点对应一个表单标签页 | **预处理链产出** | **到 Y 侧就是枝梢节点**：归拢到所属业务文档之下，标 `[待复核]`；首次归位时其下节点全新，已归位的文档再来新需求时判**新增**还是**改进**。这两级的规范化归预处理链 |
| ③ | 用例场景 | 任务节点上包括的用例场景操作，原则上对应着标签页上的一个功能按钮、右键操作项，或一个快捷键所提供的功能 | **预处理链产出** | **到 Y 侧就是叶节点**：归拢到所属任务节点之下，标 `[待复核]`；首次归位时其下叶全新，已归位的任务节点再来新需求时判**新增**还是**改进** |
...
> 方法读法：**业务域到任务节点的各级都是干枝**，即非叶节点；其中**任务节点 = 枝梢**，即框架的最深一级；**用例场景 = 叶**，即概要的末级。末三级由 X 直接给出，本任务不另造它们——业务文档判它是新增文档节点还是已在，已在的其下任务节点与用例场景与树上已有的做语义与业务逻辑分析，判**新增**还是**改进**。**承接锚点落在叶**——X 的用例场景条目汇聚到叶上，枝梢不承担跨需求-方案对承接。**本对两侧的叶是同一批**，故**不涉及映射与分配**。业务流程的文档转译到功能表单，走的是**页面视角**、不是承接，见 91 号规范 §7.3。
...
- **覆盖检查**，这是本需求-方案对的关口：**节点累计承接的**全部需求语义 ⊆ 其叶节点即用例场景之并——叶由上游直接给出，这一面即 X 的用例场景**整段进入 Y**、一个不少、同名同级；业务链这一面则核其兑现，即 ⊆ 其业务链所能实现的范围。同一张业务文档的需求分多批到来，故检查范围累计——每批检新增部分；本批改动了已有干枝，另按影响面复查其下已归位部分。EOS 实例化 = **端到端闭合检查**——执行类从需求提出贯通到需求满足；管理类按计划→监控→分析→纠正核对，须含计划清单文档 + 承载监控与分析环节的业务文档，这两环节的运行时推进结果不作需求缺口。缺口 → 生成补充原始需求材料 → 经预处理链补回，**不脑补节点**。
