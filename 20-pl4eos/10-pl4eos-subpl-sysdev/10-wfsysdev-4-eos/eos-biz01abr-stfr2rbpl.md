...
本任务把**框架与概要合做**，两级一手做完，叶的详细定义留给下一个任务位；交出的**概要**是根到叶、各级只到说明，框架只是概要内部的一层截面。人类在轮内介入两处：先在 Step Start 从候选清单中选定本次处理的对象，再在 Step 3 的对话里讨论并确认方案、裁决覆盖检查检出的缺口候选。确认在本轮内完成，方案不靠事后在文档上标注回流；没讨论完的留在 `待确认方案`，由下一轮接着讨论。
...
一次运行起于 Step Start（为任务选择并校验需求），走 Step 1~3 的设计动作，收于 Step End（入库确认后产品数据）。**方案的确认在本轮内完成**——讨论与确认都在这一轮里做完，不靠事后在文档上标注回流；没讨论完的方案留在 `待确认方案`，下一轮接着讨论。**三步并非每轮都走**——只有新输入汇入或有下游退回的节点待修订才走 Step 1，Step 2 随 Step 1 走；Step 3 与 Step End 每轮都走。**本任务的检验共三处**——入口的受理门槛（Step Start）、覆盖检查（Step 2）、写回前的要素机械检查（Step End），其中覆盖检查在 Step 3 按改动的影响面复跑；Step End 收尾另按自检清单逐项核对。
...
| 1 | 基于需求生成 R 树方案 | 读生成材料；长干枝、聚角色职责、末三级落位、排文档序列与补实现方式 | 无（AI 自动） | 统一 | R 树方案 + 聚类方案 | — |
...
**选定本次处理对象**——X 的物理文件以角色分片直接挂着条目，本步先按条目正文语义把它们归拢到所属业务文档，标 `[待复核]`，再给出**候选清单**交人类选定：每张待处理的业务文档一行，列出它名下的 `待处理` 条目数。人类选定其中之一，该文档下全部处于 `待处理` 的业务功能需求条目一次读完、一并组装，**不跨文档成批**。业务文档与任务节点两级的规范化与写入归原始需求处理。
...
**状态推进与退回**——本任务写入 `待确认方案`／`可以srb设计`／`待补充srb设计`／`需biz01修订` 四个状态，`在srb设计` 与 `已srb设计` 由下游写入；Start 按状态路由，Step End 把本轮结果写回：
...
**两种退回，方向相反**：X 有缺陷、且缺陷的处置权不在本任务（切分与建树归原始需求处理）→ 标 `需退回重切`，交回原始需求处理，本轮不再受理；Y 被下游挑出缺陷 → 标 `需biz01修订`，Start 过退回门禁后交 Step 1 走修订转换。

### 5.2 Step 1 · 基于需求生成 R 树方案

有新输入汇入，或有下游退回的节点待修订时，走本步；两者都没有时本步不走。退回的节点先于新输入处理，走并入已有 Y 的路子。

**读生成材料**——读本次这一张业务文档下全部条目的正文与业务文档上的三个 R 侧属性，以及同一业务文档／同域的候选存量方案节点。**判的是归属，不是内容**——不同相关方、不同时期提的需求各说各事，都落在同一张业务文档上即成立。所属业务域、所属主责角色、所属E2E职责三个属性给出这批条目在 R 树上该去的位置，另两个属性归 P 树、不在本步。加载按 [91 号规范 §A.3.3](91-eos-biz-eng-spec.md) 读取协议——处理节点优先、逐节点加载、按需扩大，不默认加载全文。
...
2. **聚角色职责**——E2E职责有新增或合并的，给出 E2E职责聚类方案，留待 Step 3 的对话确认；没有新增与合并，角色职责不动。
...
本轮有未确认的方案时走本步——Step 1 刚生成的方案，或状态停在 `待确认方案` 的遗留节点，两者处置一样。

**晒未确认清单**——把本轮新生成的方案（含 E2E职责聚类方案）、状态为 `待确认方案` 的遗留节点，以及 Step 2 检出的缺口候选，一并列出交人类讨论。这份清单就是本步的界面。

**逐条讨论与确认**——人类的反馈分四类处置：显式修改指令 → 执行修改并复跑覆盖检查；质疑 → 解释理由、不改；整体确认 → 该节点的全部条目追加 `[已处理]`、反馈要点写入 `## AI最近变更`、方案转为已确认；模糊意见 → 追问到可执行。确认的单位是节点——一份方案一条结论，可以逐份给，也可以一次全认。
...
- **生成**：R 树组装完成，逐级定位与合并／新建判定正确——E2E职责以端到端为判据，任务节点与用例场景以名称与业务含义为判据；修订转换的边界匹配／关系检测／变更影响声明已执行；业务文档、任务节点、用例场景已按属性归位；角色职责的重聚类方案已随方案交 Step 3 确认；树收敛，判据是覆盖完整 + 兄弟互斥。
- **Y 侧要素**：每张业务文档齐封面模式与正文实体类型；用例场景到名称级并带内联稳定标签；加支撑引擎候选 `[推断]`、任务关系；要素非空或合理空置。
- **检查**：覆盖检查已执行，检的是节点累计承接的全部需求语义；缺口以闭合检查产出文档序列类补充原始需求材料候选，去留有人类裁决，**不脑补节点**；Step 3 对每处改动的复跑已执行。
