...
```
相关方功能需求
└── 业务文档 ──────── ① 根　业务运行结束后应输出的产品数据文档，带五个属性：所属业务域 × 所属主责角色 × 所属E2E职责 × 所属产品类型 × 所属产品构件
...
| 级 | 节点 | 本任务如何处置 |
|----|------|--------------|
| ① | 业务文档 | **到 Y 侧就是业务文档**：归位；归拢的依据是它自带的属性——「所属业务域」定 R 树的域，「所属主责角色」定 R 树的角色，「所属E2E职责」定它落在 R 树的哪位 E2E职责。「所属产品类型」与「所属产品构件」不由本任务消费，供同一任务位的产品开发视角业务流程概要所用 |
| ② | 任务节点 | **到 Y 侧就是枝梢节点**：归拢到所属业务文档之下，标 `[待复核]`；首次归位时其下级全新，已归位的文档再来新需求时逐节点判**复用、改进还是新增** |
| ③ | 用例场景 | **到 Y 侧就是叶节点**：归拢到所属任务节点之下，标 `[待复核]`；首次归位时该节点全新，已归位的任务节点再来新需求时逐节点判**复用、改进还是新增** |
...
| ④ | E2E职责 | **本任务产出**，取自 X 的「所属E2E职责」；与 R 树上同一业务域、相关方角色下的已有 E2E职责节点作语义与业务逻辑分析，或合并或新建（**端到端为判据**）；本任务的新建／修订单位 + 人类确认挂点：归拢条目、职责边界、收敛自查 |
| ⑤ | 业务文档 | **本任务产出**，取自 X 的「业务文档」；与 R 树上同一业务域、相关方角色、E2E职责下的已有文档节点作语义与业务逻辑分析，或合并或新建，并整理该业务文档下的任务节点，AI 给出方案，人类反馈意见或确认 |
| ⑥ | 任务节点 | **本任务产出**，取自 X 的「任务节点」；与 R 树上该业务文档下的已有任务节点作语义与业务逻辑分析，逐节点定**复用 → 改进 → 新增**，并整理该任务节点下的用例场景，AI 给出方案，人类反馈意见或确认 |
| ⑦ | 用例场景 | **本任务产出**，取自 X 的「用例场景」；与 R 树上该任务节点下的已有用例场景作语义与业务逻辑分析，逐节点定**复用 → 改进 → 新增**，AI 给出方案，人类反馈意见或确认 |
...
> **树权威**：R 树的拓扑与各级语义以 94 号文档 §2.2.1 为准；Y 的树画像以职责视角业务流程概要的数据文件为准。

### 3.2 节点类型与结构要点

- **E2E职责**：一个端到端的职责业务，即主责角色的一个业务对象闭环。**哪些业务文档归入同一个 E2E职责，由端到端定**：**执行类**从接收需求到需求被确认得到满足；**管理类**按计划→监控→分析→纠正走完一轮，即 PCA。
- **业务文档**：业务运行结束后应输出的产品数据文档，对应本文档的开发任务；一般对应一个流程、一个工单、一个计划，或一个看板业务。**它的业务文档实现方式** = 封面模式、正文实体类型、支撑引擎候选。
- **轻量/正式业务文档**：判定看它有没有**独立于正文 CRUD 的生命周期**——需要审批流转、版本追溯、派发验收或独立状态变迁的，是**正式文档**，有封面实体；都不需要的，是**轻量文档**，无封面、正文即文档。
- **任务类型**：E2E职责中**第一个带任务类型的文档，其任务类型必然是计划**——该 E2E职责的业务由这份计划启动（策划 → 发布）。**文档本体不受此限**：它仍是开发/记录类文档，正文是产品数据或管理数据，不是计划清单。

### 3.3 Y 侧规则

- **形态边界**：本任务交出概要，各级只写到**结构判定**为止——判它在树上存在与否、落在哪一级，不写实现内容：
  - **业务文档**：是否存在，以及它的**业务文档实现方式**——封面模式、正文实体类型、支撑引擎**候选**；
  - **任务节点**：任务类型与节点名称；
  - **用例场景**：名称。

  更深的一层——**封面映射、正文实体结构、支撑引擎权威值、数据权限**，任务节点的**处理角色与状态变迁**，以及四级页面的**写实**——由下一个任务位补，不在本任务。
- **E2E职责构建规则**，用于新建：识别触发 → 追踪自然终点，不得在终点之后扩展 → 闭合验证 → 边界四标准，即完整性、一致性、紧凑性、粒度匹配，产出场景职责边界陈述。
- **文档序列构建规则**：执行类按输入-输出传递链排列；管理类按计划→监控→分析循环排列，须含计划清单文档 + 承载监控与分析环节的业务文档；需求明示的任务触发关系照录。
...
- **正文要素行**，依次为：
  ...
  - **承接的条目引用**——X 的用例场景条目引用，N:1 汇聚，**跨批追加不覆盖**，作覆盖与追溯锚点；治理预置节点无独立需求时按治理覆盖口径；
  - **文档序列**——该 E2E职责下这批业务文档的处理序；需求明示的任务触发关系；
...
> **接口契约，与两个下游任务的受理前提互为对侧**：与**产品开发视角业务流程任务**的移交在同一需求-方案对内——末三级整段搬入，干枝各长各的；与**职责视角业务流程定义任务**的移交跨概要与定义两个形态——说明级之上补详细定义。受理前提分作两处：可承接对象在各任务第二章的输入文档结构，门槛与检验在各任务第五章的入口检验。要素缺失、冲突 → 退回 `需biz01修订` + 缺失说明。核对方法 = 93 号规范 §4.2 的逐条映射。
