...
6. **覆盖检查与补充原始需求**：逐 E2E职责核对它的业务对象闭环是否走完——执行类从接收需求到需求被确认得到满足，管理类按计划→监控→分析→纠正；走不完即缺了应有的业务文档或任务节点，记 `[业务链缺口]`，生成补充原始需求材料。
...
| ④ | E2E职责 | **本任务产出**；本任务的新建／修订单位 + 人类确认挂点：归拢条目、职责边界、收敛自查。**用于新建的构建规则**：识别触发 → 追踪自然终点，不得在终点之后扩展 → 闭合验证 → 边界四标准，即完整性、一致性、紧凑性、粒度匹配，产出场景职责边界陈述 |
| ⑤ | 业务文档 | **本任务产出**，取自 X 的「业务文档」，AI 给出方案，人类反馈意见或确认。**写到结构判定为止**——是否存在，以及它的**业务文档实现方式**：封面模式、正文实体类型、支撑引擎**候选**；封面映射、正文实体结构、支撑引擎权威值、数据权限由下一个任务位补。**文档序列构建规则**：执行类按输入-输出传递链排列，管理类按计划→监控→分析循环排列，须含计划清单文档 + 承载监控与分析环节的业务文档；需求明示的任务触发关系照录 |
| ⑥ | 任务节点 | **本任务产出**，取自 X 的「任务节点」，AI 给出方案，人类反馈意见或确认。**写到任务类型与节点名称**；处理角色与状态变迁由下一个任务位补 |
| ⑦ | 用例场景 | **本任务产出**，取自 X 的「用例场景」，AI 给出方案，人类反馈意见或确认。**写到名称**；四级页面的写实由下一个任务位补 |
...
E2E职责分两支。**合并**——新进这条闭环与同一相关方角色下已有的某个 E2E职责是同一个，并入该节点；**新建**——都不是，则新建，并按构建规则把边界成文：识别触发 → 追踪自然终点（不得在终点之后扩展）→ 闭合验证 → 边界四标准，即完整性、一致性、紧凑性、粒度匹配 → 产出**场景职责边界陈述**。
...
| 级 | 判定 | 判据 | 处置 |
|----|------|------|------|
| ⑤ 业务文档 | 并入该 E2E职责下已有的一张，还是新建一张 | 语义与业务逻辑分析——是不是同一份业务输出的产品数据 | 并入的进下一行；新建的把 X 的三级整段搬入 |
...
Step Start 与 Step End 两端由 [91 附录A](91-eos-biz-eng-spec.md) 统一承载，下表一并列出以便看全序列。受理判定按入口检验表特化；列定义引 00-doc-conventions §十三。
...
**状态推进与退回**——这台机器由 Start 按它路由，由 Step 5 按它推进：
...
**两种退回，方向相反**：X 有缺陷、且缺陷的处置权不在本任务（切分与建树归原始需求处理）→ 标 `需退回重切`，交回原始需求处理；Y 被下游挑出缺陷 → 标 `需biz01修订`，本任务读出下游的缺失说明后走修订转换。

### 5.2 Step 1 · 设计材料加载

受理通过后即走本步：读本次这张业务文档下处于 `待处理` 的业务功能需求条目，以及同一业务文档／同域的候选存量方案节点。

**判的是归属，不是内容**——不同相关方、不同时期提的需求各说各事，都落在这一张文档上即成立。按树定位、按属性归拢：所属业务域、所属主责角色、所属E2E职责三个属性给出这批条目在 R 树上该去的位置，另两个属性归 P 树、不在本步。

加载按 91 号规范 §A.3.3 读取协议——处理节点优先、逐节点加载、按需扩大，不默认加载全文。

X 的现行物理文件以角色分片直接挂条目，尚未含业务文档与任务节点两级——按条目正文语义归拢，标 `[待复核]`；这两级的规范化与写归原始需求处理。
...
有新输入，即有待 `待处理` 的条目汇入时，走本步。对本次这批条目所在的业务文档，依次走四件：

1. **长干枝**——用这张业务文档根上的三个属性在 R 树上落出业务域、相关方角色、E2E职责；每级拿同一父节点下的已有节点作比对，判在不在、并入还是新建。
...
四件走完，有新增或改进 → 自下而上调树至收敛，判据是覆盖完整 + 兄弟互斥。X 侧缺陷不在本步补，退回重切。

### 5.5 Step 4 · 覆盖检查与补充原始需求

组装收尾后即走本步，跑本需求-方案对的关口。**节点累计承接的**全部需求语义 ⊆ 其叶节点即用例场景之并——叶由上游直接给出，这一面即 X 的用例场景**整段进入 Y**、一个不少、同名同级；业务链这一面则核其兑现，即 ⊆ 其业务链所能实现的范围。同一张业务文档的需求会在多次运行里反复到来，故检查范围累计——每次只核本次新增部分；本次改动了已有干枝，另按影响面复查其下已归位部分。

EOS 实例化 = **端到端闭合检查**——执行类从接收需求到需求被确认得到满足；管理类按计划→监控→分析→纠正核对，须含计划清单文档 + 承载监控与分析环节的业务文档，这两环节的运行时推进结果不作需求缺口。

检出缺口即缺了应有的业务文档或任务节点，记 `[业务链缺口]`，生成**补充原始需求材料**，候选去留交**人类裁决**——「需要」→ 落地，「不要」→ 废弃。材料交原始需求处理补回，**不脑补节点**。
...
关口收尾后即走本步：把 R 树写回职责视角业务流程概要；写回动作、状态推进与基线提交按 91 号规范 §A.3.3 与 §A.6 执行，节点状态推进为 `可以srb设计`。

写回前做**要素机械检查**：逐文档要素非空或合理空置 + 标注齐备，含 `[推断]`／`[待下游推断]`。
...
- **关口**：覆盖检查已执行，检的是节点累计承接的全部需求语义；缺口以闭合检查产出补充原始需求材料交原始需求处理，候选去留有人类裁决，**不脑补节点**。
