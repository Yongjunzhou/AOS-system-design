...
| 2 | 基于对话确认 R 树方案 | 晒未确认清单、讨论修改；要素机械检查后写回 | **有**——讨论与确认 | 对话 | 确认结论 + 未确认清单 | 职责视角业务流程概要 |
| 3 | E2E职责检查并补充需求 | 覆盖检查取两面；缺口生成文档序列类补充原始需求材料候选 | 无（AI 自动） | 统一 | 覆盖结论 + 缺口候选 | — |
...
**写回**——判「需要」的缺口按**文档序列类补充原始需求材料**成文，材料交原始需求处理补回；覆盖检查的结论写入该 E2E职责节点的完备性检查结果，材料写入补充材料引用。写回动作按 [91 号规范 §A.3.3](91-eos-biz-eng-spec.md) 执行。
...
- **确认与写回**：要素机械检查已执行；R 树方案与 E2E职责聚类方案已晒全、逐条确认；确认结果已按表5-3 写回，已被下游消费过的节点已按上游变更分流落定状态与版本；`## AI最近变更` 与 `## AI可以处理节点` 分节已维护、版本号已递增。
- **检查**：覆盖检查已执行、两面都取；缺口以闭合检查产出文档序列类补充原始需求材料候选，去留有人类裁决，**不脑补节点**。
