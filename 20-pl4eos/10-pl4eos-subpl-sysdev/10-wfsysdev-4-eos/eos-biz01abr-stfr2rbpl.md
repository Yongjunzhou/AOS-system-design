...
- **E2E职责**：一个端到端的职责业务，即主责角色的一个业务对象闭环。**哪些业务文档归入同一个 E2E职责，由端到端定**：**执行类**从接收需求到需求被确认得到满足；**管理类**按计划→监控→分析→纠正走完一轮，即 PCA，须含计划清单文档 + 承载监控与分析环节的业务文档。
...
- **形态边界**：只到结构判定——文档是否存在、封面模式、正文实体类型、支撑引擎候选、任务类型与任务节点，以及用例场景的名称。叶的详细定义、封面映射、正文实体结构、数据权限、引擎权威值、处理角色与状态变迁，以及四级页面的写实，由下一个任务位补。**Y 侧不重写 X 给的末三级条目**，只在其上长干枝、补方案内容。
- **E2E职责构建规则**，用于新建：识别触发 → 追踪自然终点，不得在终点之后扩展 → 闭合验证 → 边界四标准，即完整性、一致性、紧凑性、粒度匹配，产出场景职责边界陈述。
- **文档序列构建规则**：执行类按输入-输出传递链排列；管理类按计划→监控→分析循环排列；需求明示的任务触发关系照录。
