...
| 下游，同一任务位 | 产品开发视角业务流程任务 | 在同一批业务文档上组装 P 树；消费本任务传出的节点，状态为 `可以srb设计` |
| 下游，下一任务位 | 职责视角业务流程定义任务 | 取本任务传出的概要，在说明级之上补各节点的详细定义 |
...
一次运行起于 Step Start（为任务选择并校验需求），走 Step 1~4 的设计动作，收于 Step End（给出本轮任务成果总结）。**两次确认都在轮内完成，且确认即写回**——Step 2 确认 R 树方案，Step 4 确认补充需求，两处都是一确认就把产品数据写回。**四步并非每轮都走**——只有新输入汇入或有待本任务处理的存量节点才走 Step 1，Step 3 随 Step 1 走；Step 2、Step 4 与 Step End 每轮都走。**本任务的检验共三处**——入口的受理门槛（Step Start）、覆盖检查（Step 3）、写回前的要素机械检查（Step 2），其中覆盖检查在 Step 4 按改动的影响面复跑；Step End 收尾另按自检清单逐项核对。

**表5-1：Step 布局总览**

| Step | 名称 | 职责 | 人类参与 | 执行模式 | 产出 | 写资产 |
|------|------|------|---------|---------|------|--------|
| Start | 为任务选择并校验需求 | 受理门槛校验、给出候选清单、捡起存量节点、按状态路由 | **有**——从候选清单中选定 | 对话 | 受理结论 + 候选清单 | 相关方需求 |
...
Step Start 的固定机制——变更感知、入口的检测与优先序、退回门禁——按 [91 号规范 附录A](91-eos-biz-eng-spec.md) 执行；本任务在此写它这一侧的特化：选定处理对象、受理门槛、状态推进。
...
**捡起存量节点**——待本任务处理的存量节点另由入口检出，与本次选定的需求一并作为处理对象。其中状态为 `待确认方案` 的即上一轮没讨论完留下的方案，可以散在本任务产出的任意位置，与本轮新生成的方案同路处置。
...
有新输入汇入，或入口检出待本任务处理的存量节点时，走本步；两者都没有时本步不走。存量节点先于新输入处理，走并入已有 Y 的路子。
...
本步每轮都走。拿到这里讨论确认的有两样——Step 1 刚生成的方案，以及状态停在 `待确认方案` 的遗留节点，两者处置一样。
...
- **受理**：受理门槛已校验通过；候选清单已给出、人类已选定其中一张业务文档；入口检出的存量节点已捡起；无待处理对象已退出。
