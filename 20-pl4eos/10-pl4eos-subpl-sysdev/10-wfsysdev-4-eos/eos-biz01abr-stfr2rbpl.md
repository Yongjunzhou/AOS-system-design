...
在已有的职责视角业务流程概要上，把新进的相关方功能需求自上而下组装到位：业务域、相关方角色、E2E职责、业务文档先各归其位，任务节点与用例场景再整理到位。文档排出处理的序，每张文档初步定下业务实现方式。再逐 E2E职责核对业务对象闭环，没走完的补成原始需求材料，交回需求预处理链。
...
**输入语义属组装型**：X 的末级节点整段成为 Y 的末级节点——业务文档、任务节点、用例场景三级同名同批。本任务不做映射、不做分配：按属性在 R 树上逐级定位，判**合并／新建**与**复用、改进还是新增**后落位。
...
| ② | 任务节点 | 该业务文档开发所对应的任务，原则上，一个节点对应一个表单标签页 | **预处理链产出** | **到 Y 侧就是枝梢节点**：归拢到所属业务文档之下，标 `[待复核]`；首次归位时其下节点全新，已归位的文档再来新需求时逐节点判**复用、改进还是新增**。这两级的规范化归预处理链 |
| ③ | 用例场景 | 任务节点上包括的用例场景操作，原则上对应着标签页上的一个功能按钮、右键操作项，或一个快捷键所提供的功能 | **预处理链产出** | **到 Y 侧就是叶节点**：归拢到所属任务节点之下，标 `[待复核]`；首次归位时其下叶全新，已归位的任务节点再来新需求时逐节点判**复用、改进还是新增** |
