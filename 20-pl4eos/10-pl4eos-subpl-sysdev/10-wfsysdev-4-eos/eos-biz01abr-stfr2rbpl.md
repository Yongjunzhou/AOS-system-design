...
本任务与产品视角业务流程任务一起，把**相关方需求**变成**业务流程**；本任务负责其中的**职责视角**那一份。

- **阶段**：业务系统设计，业务设计链的第二阶段。
- **需求-方案对**：本阶段第一个——「相关方需求 → 业务流程」。
- **环节**：一。框架与概要在本处合做，环节一与环节二并作一环。
- **工位**：本阶段第一个对里的环节一。两个并列任务分担、合占这一位。
- **本任务**：占在这一位上的一条任务——产出 Y，即**职责视角业务流程概要**。它的输入 X 是**相关方需求**，只有一份、不分框架与概要，末级即用例场景；同一位上的另一条任务是产品视角业务流程任务，产出**产品视角业务流程概要**。

两个任务跑的是同一批文档级E2E任务，各长一棵树的**干枝**——文档级E2E任务之上那几级：本任务长 R 树的干枝，产品视角业务流程任务长 P 树的干枝。两棵树最下的三级，也就是**末三级**——文档级E2E任务、任务节点、用例场景——是同一批。

这棵树最下的三级——文档级E2E任务、任务节点、用例场景——已经由上游给出：相关方需求里给的就是这三级，本任务不产出它们。本任务往上做两级，**框架**与**概要**，一次做完；交出的是概要，从根到叶、各级只到说明。框架不是单独的一份产出，它只是概要粗看的那一层。**叶**——也就是用例场景那一级——的详细定义不在本任务，留给下一个工位，由职责视角业务流程定义任务补上；产品视角那一份也止于概要，没有定义形态。所以框架与概要都不占需求-方案对的一端。
...
本轮没讨论完的方案留在 `待确认方案`，由下一轮接着讨论。

### 1.3 主要目标

在已有的职责视角业务流程概要上，把新进的相关方功能需求自上而下组装到位：业务域、相关方角色、岗位级E2E业务、文档级E2E任务先各归其位，任务节点与用例场景再整理到位。再排出文档序列，初步定下每张文档的实现方式。再逐 岗位级E2E业务核对业务对象闭环，闭环没走完的补成原始需求材料，交回原始需求处理。
...
4. **整理 岗位级E2E业务的文档序列**：排出该 岗位级E2E业务处理这批文档级E2E任务的顺序，并加需求明示的任务触发关系；均以业务语言表达。
...
8. **并入已有 Y 时的三项复查**：本 岗位级E2E业务在 Y 上已有节点时，四步都按并入处理，另做边界匹配检查、文档序列关系检测、变更影响声明与影响面复查三项。
9. **覆盖检查与补充原始需求**：逐 岗位级E2E业务核对它的业务对象闭环是否闭合——执行类从接收需求到需求被确认得到满足，管理类按计划→监控→分析→纠正。闭环不完整即缺了应有的文档级E2E任务或任务节点，记 `[文档序列缺口]`，生成文档序列类补充原始需求材料。
10. **写回与推进**：要素齐备，覆盖结论写进节点；人类确认之后推进状态，并受理下游的退回与上游的变更。
...
Y 是一份文档，也就是业务流程的 **R 树**。它从根到叶，各级节点只写到说明。**R 树（各业务域 · 业务运行职责视角）**回答的问题是：某角色在哪个场景、按什么顺序、处理哪些文档级E2E任务，完成端到端的职责闭环。同一批文档级E2E任务在 **P 树**里还另有一面，由产品视角业务流程任务产出。两棵树通过**认亲**互相指向，也就是逐文档标定 P 坐标，末三级同名同批。
...
本任务交出概要，各级节点判的是**结构判定**——在树上存在与否、落在哪一级。内容写到名称与说明为止，文档级E2E任务另写到**文档级E2E任务实现方式**的三项候选。
...
**说明**记录可以承接的理由，写成一句话：这份业务落在该 岗位级E2E业务的触发到自然终点之内、属同一个业务对象闭环，且它就是要输出的那份产品数据。理由的完整成文落进承接它的那个 岗位级E2E业务节点的推理依据与补全说明。
...
> **有三处确认都挂在 岗位级E2E业务上**——归拢条目经候选清单由人类选定，职责边界与收敛自查随方案进对话确认。
...
对本次这一张文档级E2E任务，依次走四个步骤：**长干枝、生成职责级E2E业务、生成文档级E2E任务与任务节点及用例场景、确定文档序列与文档级E2E任务实现方式**。该 岗位级E2E业务在 Y 上已有节点的，这四个步骤都按并入处理。
