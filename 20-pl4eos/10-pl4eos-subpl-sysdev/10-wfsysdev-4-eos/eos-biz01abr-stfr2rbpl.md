...
本任务把**框架与概要合做**，两级一手做完，叶的详细定义留给下一个任务位；交出的**概要**是根到叶、各级只到说明，框架只是概要内部的一层截面。人类在轮内介入两处：先在 Step Start 从候选清单中选定本次处理的对象，再在 Step 3 的对话里讨论并确认方案、裁决覆盖检查检出的缺口候选。确认在本轮内完成；没讨论完的留在 `待确认方案`，由下一轮接着讨论。
...
一次运行起于 Step Start（为任务选择并校验需求），走 Step 1~3 的设计动作，收于 Step End（入库确认后产品数据）。**方案的确认在本轮内完成**——讨论与确认都在 Step 3 的对话里做完。**三步并非每轮都走**——只有新输入汇入或有下游退回的节点待修订才走 Step 1，Step 2 随 Step 1 走；Step 3 与 Step End 每轮都走。**本任务的检验共三处**——入口的受理门槛（Step Start）、覆盖检查（Step 2）、写回前的要素机械检查（Step End），其中覆盖检查在 Step 3 按改动的影响面复跑；Step End 收尾另按自检清单逐项核对。

**表5-1：Step 布局总览**

| Step | 名称 | 职责 | 人类参与 | 执行模式 | 产出 | 写资产 |
|------|------|------|---------|---------|------|--------|
| Start | 为任务选择并校验需求 | 受理门槛校验、给出候选清单、捡起未确认的方案、退回路由 | **有**——从候选清单中选定 | 对话 | 受理结论 + 未确认清单 | — |
| 1 | 基于需求生成 R 树方案 | 读生成材料；长干枝、生成角色职责、生成业务文档与任务节点及用例场景、确定文档序列与业务文档实现方式 | 无（AI 自动） | 统一 | R 树方案 + 聚类方案 | — |
| 2 | E2E职责检查并补充需求 | 覆盖检查取两面；缺口生成文档序列类补充原始需求材料候选 | 无（AI 自动） | 统一 | 覆盖结论 + 缺口候选 | — |
| 3 | 基于对话讨论确认方案 | 晒未确认清单与缺口候选；执行修改、复跑覆盖检查；逐条确认 | **有**——讨论与确认、缺口裁决 | 对话 | 确认结论 + 未确认清单 | — |
| End | 入库确认后产品数据 | 要素机械检查、写回产品数据、推进状态、提交基线、结果摘要 | **有**——查看结果摘要 | 对话 | 状态推进 + 基线 + 行动清单 | 职责视角业务流程概要 |
...
**状态推进与退回**——本任务写入 `待确认方案`／`可以srb设计`／`待补充srb设计` 三个状态，并在 X 侧写 `需退回重切`；`在srb设计`／`已srb设计`／`需biz01修订` 由下游写入。Start 按状态路由，Step End 把本轮结果写回：
...
四件走完，有新增或改进 → 自下而上调树至收敛，判据是覆盖完整 + 兄弟互斥；调不动的标 `[需裁决]` 随方案一并交 Step 3，不自定架构。X 自身的缺陷不在本步补，按受理门槛的退回处置。

**产出去向**——生成的 R 树方案与 E2E职责聚类方案都不作状态写入，随 Step 2 的检查结论一并交 Step 3 讨论。

**失败处置**——条目与业务文档的五个属性不自洽、或节点 ID 定位不到的，该条目挂 `[需确认]` 交 Step 3 的对话讨论，其余条目照常；这张业务文档的条目整体归拢不到位的，按受理门槛的退回处置退出。

### 5.3 Step 2 · E2E职责检查并补充需求

Step 1 走完即走本步，执行本需求-方案对的关口——覆盖检查。**检查取两面**：
...
检出缺口即缺了应有的业务文档或任务节点，记 `[文档序列缺口]`，生成**文档序列类补充原始需求材料**候选，去留在 Step 3 的对话里交人类裁决。材料交原始需求处理补回，**不脑补节点**。
...
**产出去向**——覆盖结论与缺口候选交 Step 3，随方案一并摆给人类。本步无人类参与点。

**失败处置**——检查所依据的文档序列读不到时，该节点标 `[需确认]` 交 Step 3 讨论，不静默通过。

### 5.4 Step 3 · 基于对话讨论确认方案

本轮有未确认的方案时走本步——Step 1 刚生成的方案（含由 `需biz01修订` 的节点修订后转来的），或状态停在 `待确认方案` 的遗留节点，两者处置一样。

**晒未确认清单**——把本轮新生成的方案（含 E2E职责聚类方案）、状态为 `待确认方案` 的遗留节点，以及 Step 2 检出的缺口候选，一并列出交人类讨论。这份清单就是本步的界面。

**逐条讨论与确认**——本任务这一侧的落点是：修改落在方案上即复跑覆盖检查；人类确认后，对该节点的全部条目追加 `[已处理]`、反馈要点写入 `## AI最近变更`，方案转为已确认。确认的单位是节点——一份方案一条结论，可以逐份给，也可以一次全认。

**复跑覆盖检查**——本步每执行一处修改即复跑 Step 2 的覆盖检查，范围限被改动的节点；改动触及文档序列、边界，或使 E2E职责聚类发生变更的，按 [91 号规范 §A.5.3](91-eos-biz-eng-spec.md) 的级联处理标 `[需重设计]` 回 Step 1 重走。同一处反复打回超过一次的停手，标 `[需重设计]` 留待下一轮。

**裁决缺口候选**——逐条判「需要」即落地、判「不要」即废弃；结构性改动触及缺口面的，材料留待下一轮重判。
...
- **级联触发条件** = 改动使 E2E职责的边界或干枝发生拆分、合并 → 重验受影响节点是否仍满足其承接的需求；改动超出本步可处理的范围 → 受影响条目标 `[需重设计]`

**产出去向**——确认结论，即哪些节点转为已确认、哪些留在 `待确认方案`，与缺口裁决结果一并交 Step End 入库。**人类参与点在本节**——方案与聚类方案的讨论、确认与缺口裁决都在这段对话里完成，Step 1 与 Step 2 不停。

### 5.5 Step End · 入库确认后产品数据

写回前做**要素机械检查**：逐文档要素非空或合理空置 + 标注齐备，含 `[推断]`／`[待下游推断]`。检查不过的就地补齐；补不上的说明它是结构性缺陷，回 Step 1 重排，不带不合的要素落账。

**写回**——按 Step 3 的讨论结果落账，分四种情形：

**表5-3：入库的四种情形**

| 对象 | 讨论结果 | 入库动作 |
|------|---------|---------|
| 本轮新生成的方案 | 已确认 | 写入，状态写 `可以srb设计` |
| 本轮新生成的方案 | 未确认 | 写入，状态写 `待确认方案`，留作下一轮的处理对象 |
| 遗留的待确认节点 | 已确认 | 推状态到 `可以srb设计`，方案本体按讨论结果改或不改 |
| 遗留的待确认节点 | 仍未确认 | 不动 |

**写的是 Y**——职责视角业务流程概要：R 树即资产，写在 Y 里，不另立一册。

落账时一并维护 AI 最近变更与可处理节点两个分节，并提交基线；写回动作、状态推进与基线提交按 [91 号规范 §A.3.3 与 §A.6](91-eos-biz-eng-spec.md) 执行。
...
- **生成**：R 树组装完成，逐级定位与合并／新建判定正确；修订转换的边界匹配／关系检测／变更影响声明已执行；业务文档、任务节点、用例场景已按属性归位；角色职责的 E2E职责聚类方案已随方案交 Step 3 确认；树收敛，判据是覆盖完整 + 兄弟互斥。
- **Y 侧要素**：每张业务文档齐封面模式与正文实体类型；任务节点与用例场景到名称级并带内联稳定标签；加支撑引擎候选 `[推断]`、任务关系；要素非空或合理空置。
- **检查**：覆盖检查已执行；缺口以闭合检查产出文档序列类补充原始需求材料候选，去留有人类裁决，**不脑补节点**；Step 3 对每处改动的复跑已执行。
- **确认**：未确认清单已晒全——本轮新生成的与遗留的一并讨论；每份方案的状态按讨论结果落定，未确认的留在 `待确认方案`。
- **写回**：R 树写进职责视角业务流程概要，节点状态按四种情形落定；AI 最近变更/可处理节点分节已维护、基线已提交。
