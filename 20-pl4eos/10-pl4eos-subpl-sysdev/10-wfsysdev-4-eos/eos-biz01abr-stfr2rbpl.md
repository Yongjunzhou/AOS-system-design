...
读 X 时取树中处于 `待处理` 的业务功能需求条目，按业务文档聚集、同一张业务文档下的条目优先成批；按树定位、按属性归拢。**判的是归属，不是内容**：不同相关方、不同时期提的需求各说各事，都落在这一张文档上即成立。加载遵循 91 号规范 §A.3.3 读取协议——处理节点优先、逐节点加载、按需扩大，不默认加载全文。X 的现行物理文件以角色分片直接挂条目，尚未含业务文档与任务节点两级——按条目正文语义归拢，标 `[待复核]`；这两级的规范化与写归原始需求处理。
