...
| X 的树 | Y 的树 | 关系 |
|--------|--------|------|
| ① 业务文档（根，带五个属性） | ⑤ 业务文档 | 整段搬入 |
| ② 任务节点 | ⑥ 任务节点 | 整段搬入 |
| ③ 用例场景 | ⑦ 用例场景 | 整段搬入 |
| — | ① 业务域 | 由属性「所属业务域」落地 |
| — | ② 相关方角色 | 由属性「所属主责角色」落地 |
| — | ③ 角色职责 | 由 ④ 聚类 |
| — | ④ E2E职责 | 由属性「所属E2E职责」落地 |

> **整段搬入的意思**：X 的 ① ② ③ 进 Y 之后就是 ⑤ ⑥ ⑦——**同一批节点，不是两批节点互相对应**。所以没有映射可做，它们不需要"对上谁"，只需要定"挂在哪条干枝下"。本任务对它们不改名、不重排、不增减。
>
> **Y 的 ①②③④ 在 X 里没有对应**——X 只给出业务文档身上的属性值，这几级的节点是由属性值在 Y 这一侧落出来的。本任务只用其中三个属性——所属业务域、所属主责角色、所属E2E职责；另两个即所属产品类型、所属产品构件，归 P 树，由产品开发视角业务流程任务消费。
...
X 的根是那张业务文档，带五个属性；本任务只取三个——「所属业务域」「所属主责角色」「所属E2E职责」，分别用来落 Y 的 ① 业务域、② 相关方角色、④ E2E职责。另两个即「所属产品类型」「所属产品构件」，归 P 树，由产品开发视角业务流程任务消费。

#### 4.2.2 第二步 · 长干枝

三个属性值在 Y 上逐级落位，每级拿**同一父节点下**的已有节点作比对：

**表4-2：干枝由属性落地**

| 级 | 取根上哪个属性 | 判定 | 比对范围 | 处置 |
|----|--------------|------|--------|------|
| ① 业务域 | 「所属业务域」 | 这个域在不在 | — | 不在时新建，在则不动；R／P 两棵树各有一份节点，同值、不共享；本任务以此作定位与对账域 |
| ② 相关方角色 | 「所属主责角色」 | 这个角色在不在该域下 | 同域**同类角色** | 不存在时与同类角色合并，或新建；本任务以此作分片与复用宿主范围 |
| ④ E2E职责 | 「所属E2E职责」 | 这个业务对象闭环在不在该角色下 | 同域·同角色下的已有 ④ | 合并或新建，**以端到端为判据** |

次序是有依赖的：② 要在"同域"内比同类角色，① 不先落定就无从比；④ 要在"同域·同角色"内比已有闭环，② 不先落定也无从比。所以只能自上而下逐级走。

④ 分两支。**合并**——新进这条闭环与同角色下已有的某个 ④ 是同一个，并入该节点；**新建**——都不是，则新建，并按构建规则把边界成文：识别触发 → 追踪自然终点（不得在终点之后扩展）→ 闭合验证 → 边界四标准，即完整性、一致性、紧凑性、粒度匹配 → 产出**场景职责边界陈述**。

④ 的**类型**由需求动作词判：偏向执行的判执行类，偏向计划、监控、分析、纠正的判管理类；存疑标 `[推断]`。

> **④ 是本任务的新建与修订单位**，也是人类确认的挂点——归拢条目、职责边界、收敛自查三项确认都挂在 ④ 上。

> **X 侧缺陷不就地改**：同一张业务文档下的条目指向不同业务对象，是 X 的缺陷——标 `需退回重切` 回原始需求处理。

#### 4.2.3 第三步 · 聚出 ③ 角色职责

Y 的 ③ 在 X 里没有对应——它由 ② 之下的 ④ 集合聚成。本次动过 ④，即有新增或合并时，给出 **E2E职责聚类方案**，即这些 ④ 归成哪几个角色职责，交人类**反馈修订建议**或确认，确认后 ③ 落定；本次没动 ④，③ 不动。

它在树上位于 ② 与 ④ 之间，却要等 ④ 落定才聚得出——所以它排在长干枝之后。

#### 4.2.4 第四步 · 把末三级挂到第五级

X 的 ① ② ③ 整段搬入 Y 的第五级，判的是它挂到哪儿。

**表4-3：末三级的落位**

| 级 | 判定 | 判据 | 处置 |
|----|------|------|------|
| ⑤ 业务文档 | 挂到哪个 ④ 下 | 属性「所属E2E职责」 | 随长干枝落定 |
| ⑤ 业务文档 | 并入该 ④ 下已有的一张，还是新建一张 | 语义与业务逻辑分析——是不是同一份业务输出的产品数据 | 并入的进下一行；新建的把 X 的三级整段搬入 |
| ⑥ 任务节点 | 与该业务文档下已有任务节点的关系 | 语义与业务逻辑分析 | 复用 → 改进 → 新增 |
| ⑦ 用例场景 | 与该任务节点下已有用例场景的关系 | 语义与业务逻辑分析 | 复用 → 改进 → 新增 |

挂在哪个 ④ 下由「所属E2E职责」定，上一节已落定，这一步不再判。要判的是并入还是新建：该 ④ 下已有承接这份业务的文档，就并入那一张，把它的节点与本次带来的逐一对上——⑥ 判该业务文档下的**全部**任务节点，⑦ 判其中经合并或变更的任务节点之下的**全部**用例场景，各定复用、改进还是新增；该 ④ 下没有承接它的文档，就新建一张，X 的三级整段搬入。

新建一张时不必逐个判：那张文档之下还没有已有节点可对，逐节点判定只会得出新增，与整段搬入同结果。

**任务类型**与**用例场景的名称**在这一步写出：任务类型读 X 的任务节点——明示则照判，只给出管理模式（审批、派发等）则按模式判类型，两者皆无则按业务语义推断，**推断只及类型，不及节点**，X 未给的任务节点按缺口规则处置；用例场景取 X 的条目原文，不另拟名。

> **判到哪里为止**：判的是结构判定——节点在树上存在与否、落在哪一级。内容写到名称级：⑥ 到任务类型与节点名称，⑦ 到名称；三级都带内联稳定标签，作下游逐级对上的追溯键。封面映射、正文实体结构、支撑引擎权威值、数据权限、处理角色与状态变迁，以及窗口标签页、表单标签页、功能按钮的界面细节，都由下一个任务位补。

#### 4.2.5 第五步 · 整这条 E2E职责

这条 ④ 有了业务文档之后，把它整成一条完整的枝。

**文档序列**——把这张业务文档排进该 ④ 的文档序列。
...
这一步只判一件事：这条 ④ 的业务对象闭环走完没有。走不完即缺了应有的业务文档或任务节点——这是业务链类的缺口，记 `[业务链缺口]`，生成**补充原始需求材料**回原始需求处理，由它加工成业务功能需求条目、归业务路径。人类裁决「需要」→ 落地；「不要」→ 废弃。**本任务不脑补节点。**

它锚在业务链上，是这张业务文档序列自身的缺口，不挂在别处。
...
④ 在 Y 上已有节点时，第二到第五步走并入的路子，依次做三件事。
...
| 结果 | 判定 | 处置 |
|------|------|------|
| 边界内 | 全部落在现有触发到自然终点的范围内 | 进入业务链关系检测 |
| 边界扩展 | 超出当前自然终点，但仍属同一个业务对象闭环 | 更新场景职责边界陈述 + 重追踪终点；粒度不一致时标 `[推断]` 供人类裁决 |
| 退回上游 | 混合多个业务对象，不属同一个业务对象闭环 | 标 `需退回重切` 回原始需求处理，本次跳过 |

**业务链关系检测**——逐文档与该 E2E职责的业务链比对，以文档名称与职责为准：

**表4-5：业务链关系检测**

| 关系 | 判定 | 处置 |
|------|------|------|
| 确认／无新增 | 文档已在业务链上，本次条目的语义已被现有节点覆盖 | 追加承接条目引用，节点定义不动 |
| 引擎判定变更 | 文档已在，本次条目改变它的正文特征，支撑引擎候选随之改变 | 更新支撑引擎候选，不新增文档 |
| 补充 | 文档不在业务链上 | 新增文档节点，并整理其下的任务节点与用例场景 |
| 过时 | 业务链上的已有文档本次无条目覆盖，且边界扩展后不再落入闭环范围 | 标 `[需裁决]` 交人类——移除节点是架构决策，本任务不自行删 |

**变更影响声明与影响面复查**——本次有修订时，按 91 号规范 §A.7.2 判变更类型、按 §A.7.3 的格式写入节点末尾；判为修订型或结构型的，须复查被改动干枝之下的业务文档是否失配——失配的批量重归位或回退。
...
执行两件——**长干枝**：用这张业务文档根上的三个属性在 R 树上落出 ① ② ④，再由 ④ 聚出 ③；**末三级落位**：把业务文档连同它的任务节点与用例场景整段搬到第五级。随后排该 E2E职责的文档序列，为业务文档补业务文档实现方式；有新增或改进 → 自下而上调树至收敛，判据是覆盖完整 + 兄弟互斥。

#### 5.1.4 Step 4 · 覆盖检查与补充原始需求

执行关口检查，其中**端到端闭合**是本需求-方案对的关口——核对这条 E2E职责的业务对象闭环是否走完，须含计划清单文档与承载监控、分析环节的业务文档。检出缺口即缺了应有的业务文档或任务节点，记 `[业务链缺口]`。
...
| 这张业务文档的五个属性缺标或矛盾 | 软限制 | 本次其余条目照常，该文档的条目挂 `[需确认]` |
| 这张业务文档下的条目不同指一个业务对象 | 退回上游 | 输出退回记录 → 标 `需退回重切` → 退出 |
| 条目类型含引擎能力功能需求 / 非功能需求 | 路由错误 | 输出类型分布 + 路由指引 → 退出 |
| 这张业务文档下的条目 > 10 条 | 软限制 | 输出超限提示 → 人类确认 |
| 无待处理条目 + 无待反馈 + 无退回 | 无待处理对象 | 输出提示 → 退出 |

读 X 时取这张业务文档下处于 `待处理` 的业务功能需求条目；按树定位、按属性归拢。**判的是归属，不是内容**：不同相关方、不同时期提的需求各说各事，都落在这一张文档上即成立。加载遵循 91 号规范 §A.3.3 读取协议——处理节点优先、逐节点加载、按需扩大，不默认加载全文。X 的现行物理文件以角色分片直接挂条目，尚未含业务文档与任务节点两级——按条目正文语义归拢，标 `[待复核]`；这两级的规范化与写归原始需求处理。

X 的切分与建树权在原始需求处理：业务对象闭环的切分、业务文档的五个属性、任务节点与条目的落位，都由它制定并执行，**本任务不重切 X、不重定属性**。挂起的条目不算退回，下一轮重试。

**关口，对 Y**

- **覆盖检查**，这是本需求-方案对的关口：**节点累计承接的**全部需求语义 ⊆ 其叶节点即用例场景之并——叶由上游直接给出，这一面即 X 的用例场景**整段进入 Y**、一个不少、同名同级；业务链这一面则核其兑现，即 ⊆ 其业务链所能实现的范围。同一张业务文档的需求会在多次运行里反复到来，故检查范围累计——每次只核本次新增部分；本次改动了已有干枝，另按影响面复查其下已归位部分。EOS 实例化 = **端到端闭合检查**——执行类从接收需求到需求被确认得到满足；管理类按计划→监控→分析→纠正核对，须含计划清单文档 + 承载监控与分析环节的业务文档，这两环节的运行时推进结果不作需求缺口。缺口 → 生成补充原始需求材料 → 经原始需求处理补回，**不脑补节点**。
