...
> **本文件是什么**：本文件是 EOS 设计线业务路径首个任务位的 SKILL 指南；本任务占这一位里的**职责视角**这一半。它说明本任务这一次转换：输入文档 X 是相关方需求，输出文档 Y 是职责视角业务流程概要，转换规则是组装规则在本需求-方案对上的实例化。
>
...
本任务把**框架**与**概要**合做，两个级别一次做完；交出的概要从根覆盖到叶、各级只到说明。框架不是单独的一份产出，它只是概要粗看的那一层。**叶**——也就是用例场景那一级——的详细定义留给下一个任务位，由职责视角业务流程定义任务补上。
...
> **交出的概要由两个下游取用**：同一任务位的**产品视角业务流程任务**按对位取同一批文档级E2E任务，下一个任务位在说明级之上补详细定义。两棵树的文档级E2E任务、任务节点、用例场景这三级是同一批，实体由职责视角业务流程概要持有。
