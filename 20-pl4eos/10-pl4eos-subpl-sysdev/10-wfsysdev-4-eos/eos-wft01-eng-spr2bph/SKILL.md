...
description: FR-ENG 规范化的相关方需求节点→bph-eng 业务流程概要。接收正式FR-ENG 规范化的相关方需求节点（≤10条），
             完成规范化的相关方需求→bph-eng业务流程概要（溯源→能力链→名称级识别→配置信息组（概要说明，业务流程概要末级节点；结构判定级）→CU承接→完备性检查），
             写回23/25资产 CU 承接引用，形成可进入wft02-eng（业务流程方案）的bph-eng节点。
---

# eos-wft01-eng · 平台能力规范化的相关方需求 → 平台能力业务流程概要
...
| 负责 | 不负责（路由指向） |
|------|-------------------|
| 接收FR-ENG 规范化的相关方需求节点（≤10条），校验类型/状态/主责能力域 | FR-BIZ 业务场景和输出文档设计 → `wft01-biz` |
...
| 名称级识别配置信息组候选 + 分解为配置信息组（概要说明，业务流程概要末级节点；结构判定级）+ 分配配置信息组到 CU 承接（含收敛判定/骨架判定）+ 构建能力链 + 完备性检查 | 系统需求概要（功能表单+概要说明）→ `wft03-eng`；系统需求方案（功能表单组件及其业务）→ `wft04-eng` |
| 装配 R 上下文（支撑 CU 来源判定与追溯链前端锚点） | 前后端组件、接口和实现边界 → `wft05-eng` |
| 检出能力链缺口 → 生成补充 FR-ENG 材料 | 操作活动定义展开 → `wft02-eng`（业务流程方案）承接已分配 CU 详设 |
| 人类确认后推进 bph-eng→`可以详细定义` + 规范化的相关方需求→`已分配` | — |
...
| 方向 | Skill | 交接内容 |
|------|-------|---------|
| 上游 | 规范化的相关方需求预处理（`ort03`） | FR-ENG 规范化的相关方需求节点 写入 `01-*.md`，状态=`待处理`。通过 `## AI可以处理节点` 的"待 wft01-eng 处理"分节检测。wft03-biz 产生的 FR-ENG 候选线索须先经人类确认+完整通过 ort00→ort03 才能进入 |
| 下游 | `wft02-eng` | 消费 `可以详细定义`/`待补充详细定义` 的 bph-eng，承接已分配 CU 做业务流程方案（操作活动定义）。缺陷时退回 `需wft01修订`+缺失说明 |
...
| 规范化的相关方需求 状态≠`待处理` | 不可处理 | 输出当前状态 → **退出** |
| 规范化的相关方需求 类型为 FR-BIZ | 路由错误 | 输出类型分布 + 路由指引（→ `wft01-biz`）→ **退出** |
| 规范化的相关方需求 类型为 NFR | 路由错误 | 输出类型分布 + 路由指引（→ `wft01-nfr`）→ **退出** |
| 规范化的相关方需求 为业务配置实例 | 不适用 | 标记并提示 → **退出** |
| 规范化的相关方需求 是 wft03-biz 线索但未转规范化的相关方需求节点 | 未正式化 | 提示先确认并写入 `01-*.md` → **退出** |
| 规范化的相关方需求 无法判定唯一主责能力域或独立验收结果 | 归属不明 | 标记 `需澄清` → **退出** |
| 规范化的相关方需求 >10 条 | 软提示 | 输出超限提示 → 人类确认 |
| 无任何待处理 | — | 输出"无待处理对象"→ **退出** |
...
| 资产 | 用途 |
|------|------|
| `23-eos-output-architecture.md` 资产全貌层 | P0 产品/引擎归属锚点 |
...
#### Phase A — 规范化的相关方需求 溯源与路由

对每条新 规范化的相关方需求：
1. 提取触发来源、能力诉求、运行期结果、主责能力域（详见 human spec §3.2.1）
2. **路径归属判定**——规范化的相关方需求 实际为业务文档/流程 → 退回 `wft01-biz`；规范化的相关方需求 为性能/安全等约束 → 退回 nfr 路径；规范化的相关方需求 仅为配置数据 → 标记不进入
3. **宿主匹配**——规范化的相关方需求 的三元组（主责能力域+配置/治理目标+独立验收结果）落入已有 bph-eng → Phase B；否则 → 待聚合池 → Phase C

待聚合池按**主责能力域（引擎）**分组。

#### Phase B — 修订已有 bph-eng

1. **要素提取**——从本轮 规范化的相关方需求 提取触发来源、能力诉求、运行期结果、主责能力域
2. **场景匹配检查**——确认 规范化的相关方需求 与 bph-eng 的三元组一致（详见 human spec §3.2.4）：
...
3. **分解分配重识别**——从本轮 规范化的相关方需求 重新名称级识别并分解配置信息组（概要说明）（§3.3.1）、重分配 CU 承接（收敛判定，§3.3.2），再执行 CU 承接关系检测——比对已有基线，判定确认/无新增/引擎归属变更/补充/过时（详见 human spec §3.3.4）：

| 关系 | 判定 | 处理 |
|------|------|------|
| 确认 | CU匹配+引擎相同+来源非规范化的相关方需求明示 | 来源升级 |
| 无新增 | CU匹配+全部已规范化的相关方需求明示 | 追加引用+增量合并 |
...
1. **确定主责能力域**——从 规范化的相关方需求 语义提取，判定为引擎专属或平台共享
2. **定义能力场景范围**——配置/治理目标 → 系统处理要求 → 运行期制品/构件/功能 → 可用性确认（终点=规范化的相关方需求 显式覆盖、不得在终点后扩展，详见 human spec §3.2.3）
3. **构建能力链**——按四段排列（配置意图→系统处理→运行期产出→可用确认）
4. **装配 R 上下文**——从 规范化的相关方需求 触发来源与 23/24 资产装配业务运行对象/配置角色/触发事件/状态影响（详见 human spec §3.2.2）
5. **分解为配置信息组（概要说明）**——名称级识别配置信息组候选，逐候选按"可以运行的能力"分解为配置信息组（概要说明，业务流程概要末级节点；结构判定级），逐候选标注来源类型、粒度判断（详见 human spec §3.3.1）
6. **分配配置信息组到 CU 承接**——逐配置信息组找 CU 承接（1:1），执行收敛判定（复用/新增/拆分/合并）+ 对照 25 盘存骨架判定（复用/改进/新增）+ 引擎归属判定（详见 human spec §3.3.2~§3.3.3）
7. **标注 规范化的相关方需求 类型**——逐条 规范化的相关方需求 判定五种 FR-ENG 规范化的相关方需求节点 类型之一（配置生成类/构件扩展类/布局组件扩展类/横切治理类/运行支撑类），不用于划分 bph-eng 边界
...
**1. 能力链闭合检查**——四段：配置/治理意图 → 系统处理要求 → 运行期制品/构件/功能 → 可用性确认。配置/治理意图段缺失→`补充规范化的相关方需求`；系统处理要求段缺失→`补充规范化的相关方需求`；运行期制品/构件/功能段缺失→`补充规范化的相关方需求`；可用性确认段缺失（判据不明确）→`澄清项`。

**2. 分解分配完备检查**（前置门禁·追溯完整）——每个配置信息组（概要说明）已分配 CU；每条 CU 承接可追溯 bph-eng 与 规范化的相关方需求；需求侧无锚的缺口 CU 已生成补充 FR-ENG 材料；每个 CU 承接的配置信息组（概要说明）清单可覆盖其全部能力点（详见 human spec §3.4/§5.5）。

**3. 要素机械检查**——引擎名称/场景边界非空、R 上下文已装配、能力诉求/运行期结果有标注、追溯链（规范化的相关方需求→bph-eng→配置信息组（概要说明）→CU 承接）完整、配置信息组候选清单非空且逐条标注来源类型和粒度判断、配置信息组（概要说明）已分解、CU 承接关系完整、变更影响声明已写入。

**4. 补充 FR-ENG 材料生成**——写入 `../../../80-pl4eos-2-eosdata/00-origin-requirement-materials/10-raw-files/<YYYYMMDD>-<bph-eng名称>补充原始需求材料.md`，登记到 材料状态表。与 wft03-biz 缺口线索同一管道（人类方案 §5.5）：经确认 + ort00→ort03 转FR-ENG 规范化的相关方需求节点，不豁免预处理链。**模型缺口（25 盘存无承接的配置面/引擎）不走补充 规范化的相关方需求 回路**——由骨架判定（§3.3.2）作为设计决策处理，经人类审核决策后由 wft02-eng 业务流程方案。
...
2. 遍历 CU 承接关系，写回 25 资产——仅写 CU 承接关系与骨架判定（复用/改进/新增）记录，不写操作活动定义（配置信息组/配置能力由 wft02-eng 业务流程方案后记录）
3. 对应 规范化的相关方需求条目 → `已分配`
...
### A.1 bph-eng 归组三元组 + FR-ENG 规范化的相关方需求节点 五种类型

**归组锚点**：`主责能力域 + 配置/治理目标 + 可独立验收结果`。主责类型分引擎专属和平台共享。

**规范化的相关方需求 类型**（不用于划分 bph-eng 边界，仅作 规范化的相关方需求 级特征标注）：
...
| 来源类型 | 性质 | 含义 |
|---------|------|------|
| `规范化的相关方需求明示` | 需求侧 | 规范化的相关方需求 文本直接说出该 CU |
| `规范化的相关方需求隐含` | 需求侧 | 不推出该 CU 则能力链存在逻辑缺口 |
...
> 非 `规范化的相关方需求明示` 来源均标注 `[推断]`。资产侧参照（`同类引擎推断`/`模型线索推断`）不构成需求证据，不得单独作为缺口 CU 的承接依据（人类方案 §3.1 缺口锚定边界）。
...
| 主责不匹配 | 规范化的相关方需求 归属其他能力域 | 退回待聚合池 |
...
| 分配完备（前置门禁·追溯完整） | 每个配置信息组（概要说明）已分配 CU；每条 CU 承接可追溯 bph-eng 与 规范化的相关方需求 | 无锚配置信息组走补充 FR-ENG 回路 |
...
| 2026-08-28 | v1.25 | 层名更名（人类指示 2026-08-28）——业务流程概要定义→业务流程概要、业务流程详细定义→业务流程方案、系统需求概要定义→系统需求概要、系统需求详细定义→系统需求方案（nfr/eng 链同构，含短名/标题后缀）；历史变更记录保留旧词。 AI 执行规则语义不变 |
...
| 2026-08-24 | v1.19 | 人类方案 v10.4 同步——标题正名（对齐 wft01-biz v3.26 输出术语正名）：「FR-ENG 规范化需求节点 → 业务概要定义（bph-eng）· 平台能力场景业务设计」→「平台能力规范化需求 → 平台能力业务流程概要定义」。AI 执行规则语义不变 |
| 2026-08-24 | v1.18 | 材料状态表联动（wft03 单独轮 B4）——「规范化需求 原料状态表」→「材料状态表」（2 处）。AI 执行规则语义不变 |
...
| 2026-08-21 | v1.11 | 人类方案 v9.5 同步（eng 链「概要/详细」分工框架第①步——本 Skill 定位为**业务概要定义**）：description/标题补「业务概要定义」；负责表不负责列——
| 2026-08-21 | v1.9 | 人类方案 v9.3 同步——分解产物正名「详细需求条目」→「配置信息组（概要说明）」（Step 2 特化参数/Phase C 分解/Step 4 完备检查/A.7 随迁）。AI 执行规则语义不变 |「CU 配置信息组内容/配置要素/校验保存规则/依赖/状态迁移」→「配置信息组的操作活动定义（配置操作/系统处理/运行期能力）」、「配置页面/操作活动/页面功能需求」→「系统需求概要（功能表单+概要说明）→ wft03-eng；系统需求详细（功能表单组件及其业务）→ wft04-eng」；「CU 详细定义…承接已分配 CU 详设」→「操作活动定义展开（业务详细定义）」；下游衔接补「业务详细定义（操作活动定义）」；Phase C 分解补「结构判定级」。AI 执行规则语义不变 |
...
| 2026-08-20 | v1.7 | 人类方案 v9.1 同步——来源类型资产侧定位修正（A.3 表加性质列：需求侧身份锚 vs 资产侧参照；资产侧参照不得单独作为缺口 CU 承接依据）。AI 执行规则语义不变 |
| 2026-08-20 | v1.6 | 人类方案 v9.0 同步——新增/改进引擎、CU 落地为设计内容：不负责表补"名称级识别 + 骨架判定"；Phase C 识别 CU 候选补骨架判定（复用/改进/新增，对照 25 盘存）；Step 4 补充材料补"模型缺口不走补充 规范化需求 回路"；Step 5 写 25 改"名称级引用 + 骨架判定记录"、新引擎候选改"设计判定落账"；附录 A.4 补骨架判定说明（人机协同设计决策点，25 是记录层）。AI 执行规则语义不变 |
...
| 2026-08-19 | v1.3 | 人类方案原理深度分析落定（v8.6）同步——Step 4 补充 FR-ENG 材料生成补管道句（与 wft03-biz 缺口线索同一管道：经确认 + ort00→ort03 转FR-ENG 规范化需求节点，不豁免预处理链）。AI 执行规则语义不变 |
| 2026-08-19 | v1.2 | 人类方案原理与方法审视修复（v8.5）同步——Step 4 能力链闭合检查三段→四段（配置/治理意图→系统处理要求→运行期制品/构件/功能→可用性确认，对齐 §2.2.1 四段）；Step 5 补 git 提交基线（顺序：AI最近变更 → git add+commit → 文件头，链级不变式，对齐人类方案 §5.6）；Phase C 定义能力场景范围补「终点=规范化需求 显式覆盖、不得在终点后扩展」。AI 执行规则语义不变 |
