...
description: FR-ENG 规范化的相关方需求节点→bph-eng 业务流程概要定义。接收正式FR-ENG 规范化的相关方需求节点（≤10条），
             完成规范化的相关方需求→bph-eng业务流程概要定义（溯源→能力链→名称级识别→配置信息组（概要说明，业务流程概要定义末级节点；结构判定级）→CU承接→完备性检查），
             写回23/25资产 CU 承接引用，形成可进入wft02-eng（业务流程详细定义）的bph-eng节点。
---

# eos-wft01-eng · 平台能力规范化的相关方需求 → 平台能力业务流程概要定义
...
| 名称级识别配置信息组候选 + 分解为配置信息组（概要说明，业务流程概要定义末级节点；结构判定级）+ 分配配置信息组到 CU 承接（含收敛判定/骨架判定）+ 构建能力链 + 完备性检查 | 系统需求概要（功能表单+概要说明）→ `wft03-eng`；系统需求详细（功能表单组件及其业务）→ `wft04-eng` |
| 装配 A2 上下文（支撑 CU 来源判定与追溯链前端锚点） | 前后端组件、接口和实现边界 → `wft05-eng` |
| 检出能力链缺口 → 生成补充 FR-ENG 材料 | 操作活动定义展开 → `wft02-eng`（业务流程详细定义）承接已分配 CU 详设 |
...
| 下游 | `wft02-eng` | 消费 `可以详细定义`/`待补充详细定义` 的 bph-eng，承接已分配 CU 做业务流程详细定义（操作活动定义）。缺陷时退回 `需wft01修订`+缺失说明 |
...
5. **分解为配置信息组（概要说明）**——名称级识别配置信息组候选，逐候选按"可以运行的能力"分解为配置信息组（概要说明，业务流程概要定义末级节点；结构判定级），逐候选标注来源类型、粒度判断（详见 human spec §3.3.1）
...
**4. 补充 FR-ENG 材料生成**——写入 `../../../80-pl4eos-2-eosdata/00-origin-requirement-materials/10-raw-files/<YYYYMMDD>-<bph-eng名称>补充原始需求材料.md`，登记到 材料状态表。与 wft03-biz 缺口线索同一管道（人类方案 §5.5）：经确认 + ort00→ort03 转FR-ENG 规范化的相关方需求节点，不豁免预处理链。**模型缺口（25 盘存无承接的配置面/引擎）不走补充 规范化的相关方需求 回路**——由骨架判定（§3.3.2）作为设计决策处理，经人类审核决策后由 wft02-eng 业务流程详细定义。
...
2. 遍历 CU 承接关系，写回 25 资产——仅写 CU 承接关系与骨架判定（复用/改进/新增）记录，不写操作活动定义（配置信息组/配置能力由 wft02-eng 业务流程详细定义后记录）
