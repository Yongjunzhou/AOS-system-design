...
description: FR-ENG 规范化需求节点→bph-eng 业务流程概要定义。接收正式FR-ENG 规范化需求节点（≤10条），
             完成规范化需求→bph-eng业务流程概要定义（溯源→能力链→名称级识别→配置信息组（概要说明，业务流程概要定义末级节点；结构判定级）→CU承接→完备性检查），
             写回23/25资产 CU 承接引用，形成可进入wft02-eng（业务流程详细定义）的bph-eng节点。
---

# eos-wft01-eng · 平台能力规范化需求 → 平台能力业务流程概要定义
...
| 负责 | 不负责（路由指向） |
|------|-------------------|
| 接收FR-ENG 规范化需求节点（≤10条），校验类型/状态/主责能力域 | FR-BIZ 业务场景和输出文档设计 → `wft01-biz` |
...
| 人类确认后推进 bph-eng→`可以详细定义` + 规范化需求→`已分配` | — |
...
| 方向 | Skill | 交接内容 |
|------|-------|---------|
| 上游 | 规范化需求预处理（`ort03`） | FR-ENG 规范化需求节点 写入 `01-*.md`，状态=`待处理`。通过 `## AI可以处理节点` 的"待 wft01-eng 处理"分节检测。wft03-biz 产生的 FR-ENG 候选线索须先经人类确认+完整通过 ort00→ort03 才能进入 |
...
| 规范化需求 状态≠`待处理` | 不可处理 | 输出当前状态 → **退出** |
| 规范化需求 类型为 FR-BIZ | 路由错误 | 输出类型分布 + 路由指引（→ `wft01-biz`）→ **退出** |
| 规范化需求 类型为 NFR | 路由错误 | 输出类型分布 + 路由指引（→ `wft01-nfr`）→ **退出** |
| 规范化需求 为业务配置实例 | 不适用 | 标记并提示 → **退出** |
| 规范化需求 是 wft03-biz 线索但未转规范化需求节点 | 未正式化 | 提示先确认并写入 `01-*.md` → **退出** |
| 规范化需求 无法判定唯一主责能力域或独立验收结果 | 归属不明 | 标记 `需澄清` → **退出** |
| 规范化需求 >10 条 | 软提示 | 输出超限提示 → 人类确认 |
| 无任何待处理 | — | 输出"无待处理对象"→ **退出** |
...
2. **路径归属判定**——规范化需求 实际为业务文档/流程 → 退回 `wft01-biz`；规范化需求 为性能/安全等约束 → 退回 nfr 路径；规范化需求 仅为配置数据 → 标记不进入
3. **宿主匹配**——规范化需求 的三元组（主责能力域+配置/治理目标+独立验收结果）落入已有 bph-eng → Phase B；否则 → 待聚合池 → Phase C

待聚合池按**主责能力域（引擎）**分组。

#### Phase B — 修订已有 bph-eng

1. **要素提取**——从本轮 规范化需求 提取触发来源、能力诉求、运行期结果、主责能力域
2. **场景匹配检查**——确认 规范化需求 与 bph-eng 的三元组一致（详见 human spec §3.2.4）：
...
3. **分解分配重识别**——从本轮 规范化需求 重新名称级识别并分解配置信息组（概要说明）（§3.3.1）、重分配 CU 承接（收敛判定，§3.3.2），再执行 CU 承接关系检测——比对已有基线，判定确认/无新增/引擎归属变更/补充/过时（详见 human spec §3.3.4）：

| 关系 | 判定 | 处理 |
|------|------|------|
| 确认 | CU匹配+引擎相同+来源非规范化需求明示 | 来源升级 |
| 无新增 | CU匹配+全部已规范化需求明示 | 追加引用+增量合并 |
...
1. **确定主责能力域**——从 规范化需求 语义提取，判定为引擎专属或平台共享
2. **定义能力场景范围**——配置/治理目标 → 系统处理要求 → 运行期制品/构件/功能 → 可用性确认（终点=规范化需求 显式覆盖、不得在终点后扩展，详见 human spec §3.2.3）
3. **构建能力链**——按四段排列（配置意图→系统处理→运行期产出→可用确认）
4. **装配 A2 上下文**——从 规范化需求 触发来源与 23/24 资产装配业务运行对象/配置角色/触发事件/状态影响（详见 human spec §3.2.2）
...
7. **标注 规范化需求 类型**——逐条 规范化需求 判定五种 FR-ENG 规范化需求节点 类型之一（配置生成类/构件扩展类/布局组件扩展类/横切治理类/运行支撑类），不用于划分 bph-eng 边界
...
**1. 能力链闭合检查**——四段：配置/治理意图 → 系统处理要求 → 运行期制品/构件/功能 → 可用性确认。配置/治理意图段缺失→`补充规范化需求`；系统处理要求段缺失→`补充规范化需求`；运行期制品/构件/功能段缺失→`补充规范化需求`；可用性确认段缺失（判据不明确）→`澄清项`。

**2. 分解分配完备检查**（前置门禁·追溯完整）——每个配置信息组（概要说明）已分配 CU；每条 CU 承接可追溯 bph-eng 与 规范化需求；需求侧无锚的缺口 CU 已生成补充 FR-ENG 材料；每个 CU 承接的配置信息组（概要说明）清单可覆盖其全部能力点（详见 human spec §3.4/§5.5）。
...
### A.1 bph-eng 归组三元组 + FR-ENG 规范化需求节点 五种类型

**归组锚点**：`主责能力域 + 配置/治理目标 + 可独立验收结果`。主责类型分引擎专属和平台共享。

**规范化需求 类型**（不用于划分 bph-eng 边界，仅作 规范化需求 级特征标注）：
...
| 来源类型 | 性质 | 含义 |
|---------|------|------|
| `规范化需求明示` | 需求侧 | 规范化需求 文本直接说出该 CU |
| `规范化需求隐含` | 需求侧 | 不推出该 CU 则能力链存在逻辑缺口 |
...
> 非 `规范化需求明示` 来源均标注 `[推断]`。资产侧参照（`同类引擎推断`/`模型线索推断`）不构成需求证据，不得单独作为缺口 CU 的承接依据（人类方案 §3.1 缺口锚定边界）。
...
| 分配完备（前置门禁·追溯完整） | 每个配置信息组（概要说明）已分配 CU；每条 CU 承接可追溯 bph-eng 与 规范化需求 | 无锚配置信息组走补充 FR-ENG 回路 |
