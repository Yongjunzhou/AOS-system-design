...
> **设计依据**：[eos-wft01-eng-spr2bph.md](../eos-wft01-eng-spr2bph.md)（SKILL 指南——权威源）
...
| 接收 wft02-eng 退回的 `需wft01修订` bph-eng，按 SKILL 指南修订 | — |
...
**4. 补充 FR-ENG 材料生成**——写入 `../../../80-pl4eos-2-eosdata/00-origin-requirement-materials/10-raw-files/<YYYYMMDD>-<bph-eng名称>补充原始需求材料.md`，登记到 材料状态表。与 wft03-biz 缺口线索同一管道（SKILL 指南 §5.5）：经确认 + ort00→ort03 转FR-ENG 规范化的相关方需求节点，不豁免预处理链。**模型缺口（25 盘存无承接的配置面/引擎）不走补充 规范化的相关方需求 回路**——由骨架判定（§3.3.2）作为设计决策处理，经人类审核决策后由 wft02-eng 业务流程方案。
