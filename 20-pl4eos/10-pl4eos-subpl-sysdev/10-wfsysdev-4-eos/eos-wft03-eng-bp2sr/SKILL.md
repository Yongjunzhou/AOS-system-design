...
| 负责 | 不负责（路由指向） |
|------|-------------------|
| 接收≥1个 A1 BP 节点（`可以SR设计`/`待补充SR设计`），校验 CU 配置能力（入口/要素/处理/运行期能力）与 FR-ENG 指标完整 | CU 配置能力定义 → `wft02-eng`；CU 拆分/合并（收敛判定）→ `wft01-eng` |
...
| CU 配置能力定义缺少任一项（入口/要素/处理/运行期能力/指标） | 上游待修 | BP→`需wft02修订`+缺失说明 → **退出** |
