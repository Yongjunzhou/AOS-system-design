---
name: eos-wft03-biz-bpd2srb
description: bpd-biz→系统需求概要·页面组件开发。接收≥1个bpd-biz节点（可以srb设计/待补充srb设计），
...
> **设计依据**：[eos-wft03-biz-bpd2srb.md](../eos-wft03-biz-bpd2srb.md)（SKILL 指南——权威源）
...
| 负责 | 不负责（路由指向） |
|------|-------------------|
| 接收≥1个 bpd-biz 节点（`可以srb设计`/`待补充srb设计`），校验状态/类型/数量 | P0 平台引擎产品 srb-biz → `wft03-eng-bpd2srb` |
| 按来源 bpd-biz、需求对象和主职责匹配已有 srb-biz，判定 SR-Q1/Q2/Q3 | NFR → `wft04-nfr-str2sr` |
...
| 方向 | Skill | 交接内容 |
|------|-------|---------|
| 上游 | `wft02-biz` | 产出**业务流程概要**bpd-biz 节点（`可以srb设计`/`待补充srb设计`），含 R 角色场景（含表单使用顺序）+ P 输出产品（消费 wft01 P 组装结论）+ PL6-B 条目 + 引擎支撑信息。通过 `## AI可以处理节点` 的"待 wft03-biz 处理"分节检测 |
...
| bpd-biz 状态≠`可以srb设计`/`待补充srb设计` | 状态不符 | 输出状态分布 → **退出** |
| bpd-biz 含 P0 平台引擎产品 | 类型不匹配 | 提示分路径（→ `wft03-eng`）→ **退出** |
| bpd-biz 状态=`需wft02修订` | 上游待修 | 输出缺失说明 → **退出** |
| 选中 bpd-biz >5 个 | 超建议阈值（软提示） | 输出建议分批 → 人类确认 |

**退回优先**：存在 `需wft03修订` srb-biz → 检查人类是否提供改进方案。已提供 → 读取缺失说明，按 SKILL 指南修订。未提供 → 输出缺失说明 → **退出**。若缺失根因在 bpd-biz（PL6-A/PL6-B 定义或支撑引擎信息缺失），将 bpd-biz 标为 `需wft02修订`。
...
**2. 读取 AI 可以处理节点**：读取 `04-*.md` 的 `## AI可以处理节点`（待 wft03-biz 处理，bpd-biz）与 `05-*.md`/`06-*.md` 的 `## AI可以处理节点`（待反馈/待下游退回，srb-biz）中本 Skill 对应分节。有节点 → 逐节点 `grep -n '@{节点ID}'` 定位行号 → Read 节点块。
...
| 特化维度 | wft03-biz 值 |
|---------|-------------|
| **锚点需求** | 当前 srb-biz 所承接的全部 bpd-biz 方案——人类反馈以 bpd-biz 角色的场景职责及输出文档需求为最终锚点 |
...
| bpd-biz 消费状态 | `04-*.md` | bpd-biz 节点状态→`已srb设计`（bpd-biz 消费版本由 srb-biz 节点「来源 bpd-biz」记录，04 不重复写） |
...
| 资产写回记录 | `04-*.md` | 日期/来源 bpd-biz/目标 srb-biz/动作/消费版本/下游状态 |

**AI可以处理节点维护**：在 `04-*.md` 的 `## AI可以处理节点` 中将已处理的 bpd-biz 节点移出"待 wft03-biz 处理"分节；在 `05/06-*.md` 的 `## AI可以处理节点` 中将产生 `[修改]`/`[需裁决]` 的 srb-biz 条目加入"待反馈处理"分节。
...
6. 追溯：每条 srb-biz 均记录 bpd-biz 节点 ID、bpd-biz 对象 ID、bpd-biz 消费版本
...
| `待补充srb设计` | 按 bpd-biz 变更影响声明增量补全 srb-biz |
| `在srb设计` | 写入活动说明，避免并发重入 |
| `已srb设计` | 处理完成后写入 |

### A.8 bpd-biz→srb-biz 变更类型判定
