...
| 04 → 05 | §2.2.1 / §2.2.2 的叶（A2/Bn 两棵业务树末级同义；A1 同此理，其叶级见 §2.2.3【待裁】） | §2.3.1 的**枝梢**（⑤ 用例场景功能） |
...
>
> **枝梢与叶的边界（A2/Bn）**：**业务输出文档（⑤）＝枝梢**（PL5，概要的末级），**其下一级＝叶**——PL6-A 活动节点、PL6-B 正文条目两支并列。**「任务」不占树位**：一个输出文档有 0 或 1 个任务，任务的主要节点与条目一样是**枝梢的分解源判据展开项**（名称级预告，完整态在 04 定义成叶）。
...
> 【待裁】**A1 的级数与 91 的 PL 编号差一级，与本树"有无 WBS 层"是同一个待裁项**——94 本树：③ 构件 / ④ WBS(L4) / ⑤ 配置单元(L5) / ⑥ 配置信息组(L6) / ⑦ 叶(L7)；91 §3.1/§10.4/§10.6：A1 **无 WBS**、CU = PL4、配置信息组 = PL5 **末级**、**无 PL6**。两说不能同时成立，且**不能拆开裁**（删掉"叶"这一级会让 ⑥ 的 L 标号跟着位移），故本树本轮保持不动，整体并入该待裁项一次裁定。
