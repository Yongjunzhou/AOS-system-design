...
| 承接 | 上游叶 | 下游落点 |
|------|--------|---------|
| 01 → 02 | 相关方功能需求的叶（③ 系统用例场景级操作） | 角色履职树的 ④ **场景业务（枝干）**——落点取叶的 ① 业务输出文档所带的「所属E2E场景业务」属性 |
...
| 规范化的相关方需求 | `01-eos-specified-requirements.md` | `eos-ort03-norm` | **长满 01 树**（① 业务输出文档 → ② 任务节点 → ③ 条目）+ 条目落位 |
...
**切分产出的「业务对象闭环」在相关方功能需求树里是 ① 业务输出文档的「所属E2E场景业务」属性**——切分定下这条坐标，规范化再把 ① 业务输出文档 → ② 任务节点 → ③ 条目补满。

> 「业务对象闭环」＝入口加工链的切分单位＝角色履职树的 `场景业务` 级，也＝相关方功能需求树 ① 上的属性值——同一个对象，链上称闭环、R 树里占级、01 树里作属性。
...
```
相关方功能需求
└── 业务输出文档 ───── ① 根　所属主责角色 × 所属产品构件 × 所属E2E场景业务（三个坐标作为本级的属性）
    └── 任务节点 ───── ② 该业务输出文档开发所对应的任务
        └── 系统用例场景级操作 ─ ③ 任务节点中的一个系统用例场景级操作（需求条目 stfr-XXX）
```

> **一级节点即根**：业务输出文档，其**所属主责角色／所属产品构件／所属E2E场景业务**以**属性**随节点携带、不各占一级——角色与场景是本份数据的**索引／分片轴**，不是归集级。它到方案侧成为 Rn／Pn 树 ⑤ 的**文档开发任务**，见 §2.2。
>
> **条目是叶**：叶 ＝ ③ 一个系统用例场景级操作（stfr-XXX）；② 任务节点与 ① 业务输出文档是条目的**归集级**。
...
>
> **枝梢与叶的边界（Rn／Pn 两棵业务树）**：**业务输出文档（⑤）＝枝梢**（概要的最深一级）。同一级上它又是**文档开发任务**——节点名取物件（业务输出文档），这一层的角色名取它在该层的身份（文档开发任务，即"要开发哪份文档"这件事）。文档本体到完整态才成形，但它在概要级就以名称级存在。
>
> 其下依次是 ⑥ **活动节点**、⑦ **系统用例场景级操作**——⑦ 挂在活动节点之下，**⑦ 才是叶**。
>
> **文档正文不占树位**——它由活动节点承载，不另设一级。
