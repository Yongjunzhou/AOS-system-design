...
| 角色履职视角业务流程方案 | `04-eos-business-detailed.md` | 业务 | §2.2 |
...
> **两处例外**：`01` 相关方需求不分形态；**配置方案 / 前后端组件方案** 的名称不含形态标记，其形态归属【待裁】（注意：名称中的"方案"不表示完整态）。**业务流程**这一对的概要态分两个视角各一份——`02` 角色履职视角业务流程概要、`03` 产品开发视角业务流程概要；完整态只有一份 `04`，且只做角色履职视角，见 §2.2。
...
- **两形态之间不是承接**：同一份产品数据的概要态与完整态之间（05↔06）是形态轴的两态，不在此列。本链上的承接只有两处：04→05、06→07。
...
> **"叶 → 枝梢"只是其中一例的显影**：在"上游已到完整态、下游只到概要"的步上（04→05），落点恰是枝梢、于是错开一级。这只是两侧"**各自最深可用原子**"对齐的一般结果在特定完成度差下的样子——**不推出**"上游第 N 级对应下游第 N−1 / N−2 级"这类数量关系，也不表示落点必在枝梢。
...
**这条链不是形态轴。** 概要/完整态说的是**同一棵树长到多深**；本链说的是**同一份材料加工到多细**。两轴的落点也不同：切分与澄清两态树形相同（澄清只清语义），原始与文本化两态**不产树**（一个尚未进 EOS 结构、一个只换载体）——五个加工形态里只有两处动结构：切分（长出业务对象闭环）与规范化（长满 01 树）。
...
```
相关方功能需求
└── 业务文档 ───── ① 根　所属主责角色 × 所属产品构件 × 所属E2E职责（三个坐标作为本级的属性）
    └── 任务节点 ───── ② 该业务文档开发所对应的任务
        └── 用例场景 ─ ③ 任务节点中的一个用例场景（需求条目 stfr-XXX）
```

> **一级节点即根**：业务文档，其**所属主责角色／所属产品构件／所属E2E职责**以**属性**随节点携带、不各占一级——角色与场景是本份数据的**索引／分片轴**，不是归集级。它到方案侧对应 Rn／Pn 树 ⑤ 的**文档开发任务**，见 §2.2。
>
...
>
> **详细定义的落点是界面**：菜单、窗口标签页、表单标签页、功能操作项四级都挂在 R 树上。故完整态（`04`）只做角色履职视角；产品开发视角止于概要（`03`），其详细定义不在本树的完整态里。
>
...
>
> **枝梢与叶**：**⑥ 任务节点＝枝梢**（概要的最深一级），**⑦ 用例场景＝叶**（完整态的末级）。概要到 ⑥ 为止，完整态到 ⑦ 为止，两个形态同在一份文档里。
>
...
└── 业务域 ──────── ① 流程与IT（平台自身域——单域，不分业务域）
    └── 相关方角色 ─── ② 业务配置角色：业务配置人员 / 平台治理角色（业务架构管理员、数据治理员等）
        └── E2E职责 ─ ③ 一个业务对象闭环（bph-eng 的能力场景 / 平台资产治理业务）
            └── 业务文档 ─ ④ 业务输出的产品数据，对应本文档开发任务
                └── 任务节点 ─ ⑤ 文档开发任务的多个步骤，枝梢节点
                    └── 用例场景 ─ ⑥ 是叶
```
