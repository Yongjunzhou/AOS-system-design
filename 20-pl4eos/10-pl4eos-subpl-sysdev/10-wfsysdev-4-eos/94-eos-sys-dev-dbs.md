...
> **三处例外**：`01` 相关方需求不分形态；**业务流程**这一对的两个形态**合并在一份文档里**——`04` 业务流程方案包含概要，`02` 业务流程概要退役；**配置方案 / 前后端组件方案** 的名称不含形态标记，其形态归属【待裁】（注意：名称中的"方案"不表示完整态）。
...
本链上真正用到这条分配的只有 `04 → 05` 与 `06 → 07`；`01 → 业务流程` 这一步两侧的叶是同一批节点，不涉及映射，见 §1.4。
...
- **01 → 业务流程 也不是承接**：这一步两侧的 ⑤ ⑥ ⑦ 是同一批节点（`01` 树的 ① ② ③），整段搬入、无映射；本对的活是组装 ④ 及其以上的干枝，见 §2.2。
...
| 承接 | 上游叶 | 下游落点 |
|------|--------|---------|
| 04 → 05 | Rn／Pn 两棵业务树的叶（末级同义；P0 同此理） | 系统功能需求树的**枝梢**（⑤ 用例场景功能） |
...
>
> **本份数据的 ① ② ③ 与 Rn／Pn 树的 ⑤ ⑥ ⑦ 是同一批节点**——本份产品数据是按文档平铺的一棵，R／P 树是按角色／场景／产品归拢的一棵，见 §2.2。
>
...
>
> **⑤ ⑥ ⑦ 由相关方需求直接给出**：这三级的节点就是 `01` 树的 ① ② ③——⑤ 业务文档、⑥ 任务节点、⑦ 用例场景。**这一步不做映射、不做分配**，三级的节点整段搬入；本任务只长 ④ 及其以上的干枝。
>
