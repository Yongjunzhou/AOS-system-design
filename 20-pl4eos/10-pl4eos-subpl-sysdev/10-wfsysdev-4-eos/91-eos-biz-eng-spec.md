...
## 五、业务树两族：R（职责视角）× P（产品开发视角）

EOS 平台的业务配置和运行围绕**两类产品**展开，每类产品有两棵**视角树**——**R 树**（职责视角，roles & responsibilities；沿 SL 展开）与 **P 树**（产品开发视角，product or service；沿 PL 展开）。下标索引**所指产品**：`0` ＝ 平台本身，`i ≥ 1` ＝ 基于平台引擎构建的各个业务。简称、树间对应与级语义以 [94-eos-sys-dev-dbs.md](94-eos-sys-dev-dbs.md) §1.1 为唯一权威，本章不复述其口径。
...
**判据之家（2026-09-04 定）**：面向 23 的资产对齐判据族统一定义于此——**23B 对齐 Q1/Q2/Q3**（Q1=已有、直接引用；Q2=需扩展；Q3=无、新建建议）、**PL4 WBS 归组**（数据依赖链 + 完成耦合硬判据）、**23 一致性三类**（归属冲突 / 误新建 / 误引用）、**缺口推断**（按构件产品数据清单做名称级比对，不依赖任务类型/引擎）。**引用数据**（P 树全貌 / PL 路径 / WBS / 产品数据清单）在 23 资产（`80-pl4eos-2-eosdata/23-eos-output-architecture.md`），不载入判据正文。各 wft/试点 SKILL 指南只写本任务的**判定动作**，判据定义引本节、引用数据读 23，不重述判据正文（93 §四.1 判据归属分级）。
...
|-------|------|------|---------|---------|
| **wft01-biz** | FR-BIZ 规范化的相关方需求条目 | 业务流程框架——bpl-biz 场景业务（枝）+ PL5 输出文档序列（框架末梢）+ P 组装（WBS 归组/23 一致性/缺口推断） | 文档（PL5，结构判定级框架说明） | 向业务方证明"我听懂了你的业务" |
...
| **产出物** | 业务流程框架（bpl-biz）——场景业务 + 输出文档序列 + P 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务流程概要（BP）——R 角色场景（功能表单/主子表单 + 表单使用顺序）+ P 输出产品（输出文档详细设计 + PL6-A 节点需序）+ P 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统需求框架（业务配置需求暨配置业务系统 pa-eng）——⑤ 用例场景功能（枝梢；功能表单为其业务侧载体，窗口标签页为页面组件承载）+ 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
| **供谁消费** | 业务方确认 → wft02-biz | wft03-biz | 构件开发流水线 + eng 路径（引擎缺口线索） |

> **框架/方案分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「框架/方案」重述——wft01 业务流程框架（末级=文档，框架说明为**结构判定级**，字段级详设由 wft02-biz 承接）；wft02 业务流程概要（详细条目=**活动 + 正文条目 + 表单使用顺序**）；wft03 系统需求框架（**枝梢＝⑤ 用例场景功能**，叶＝**⑥ IPO功能**只在完整态；功能表单是业务侧 R／P 树 ⑤ 输出文档（枝梢）下任务步骤的载体，窗口标签页为其页面组件承载形式——2026-08-22 深审统一，2026-09-10 按 94 树权威校正末级口径）。两棵树读法——**R 树**（SL3 角色类型→SL4 角色职责→SL5 场景业务）+ **P 树**（PL3 构件类型→PL4 WBS→PL5 输出文档）是**两棵独立的树**，在**输出文档（PL5）**处对位互连、经认亲互指，**不是一棵树的共享节点**；两条链自文档（PL5）起并行展开 → 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bpl-biz、BP→bpd-biz、SR-F→srb-biz；追溯/23 不迁。命名「业务流程概要」源于 bpd-biz IPO 承担方案角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = P 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 规范化的相关方需求 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
|-------|---------|---------|---------|--------|
| wft01-biz | 文档（PL5，框架末梢） | PL6-A 活动节点 / PL6-B 正文条目初步判定（结构判定级） | 基于 规范化的相关方需求 推断**业务流程框架**——场景业务（枝）+ 输出文档序列 + **P 组装**（WBS 归组/23 一致性/按构件产品数据清单完整性缺口推断），结构性缺口补充 规范化的相关方需求 回路——"向业务方证明我听懂了" | §七 |
