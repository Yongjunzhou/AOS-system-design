...
>
> **版本**：v4.0 | **修订**：2026-07-10 | **说明**：引擎体系收敛——取消 @engine-document/@engine-ledger（文档不是引擎概念，是实体组装模式：封面实体+正文实体 1:1+引用+处理配置，正文即实体，由各引擎按需转化为功能表单）；取消 PL6（活动节点/管理子项降维为 wft03-biz 处理配置需求）；取消 D/PCA 分类（文档类型由 document_type 承载，不设硬分类字段）；指标引擎双模式展示（列表+图示 2×3 网格）；引擎体系 13→11。
...
DocumentCover 表一行                         无封面行
  ...
  ├── 创建人/部门/日期                         CRUD 就是它的全部生命周期
  ├── 处理配置（可选的流程/工单/计划）
  └── 1:1 → 正文实体（1 行 = 1 文档内容条目实例）

例子：                                      例子：
  采购申请单/检验记录/不合格品报告               会议纪要/技术规范条目/
...
| **R3** | 封面实体存在时，DocumentCover 行与正文行严格 1:1 + `derived_from` 追溯链 |
| **R4** | 场景业务中第一个有任务类型的输出文档，其任务类型必然是计划（唯一自启动任务类型）。此文档本体是开发/记录类文档，正文为产品数据或管理数据，非"计划清单" |

**wft01-biz 的判定**：推断输出文档时需判定——这个文档有独立生命周期吗？（需要审批/派发/版本追溯？）→ 是则走正式模式，否则走轻量模式。

**任务关系规则**：

| 规则 | 说明 |
|------|------|
| **R5** | 任务间三角互触发——计划完成可触发流程和工单，流程节点可触发工单和计划，工单完成可触发流程和计划 |
| **R6** | 计划是唯一自启动任务类型——计划有内置"策划→发布"生命周期，发布完成即是触发器；流程和工单靠上游任务完成触发启动 |

**PCA 场景的 P/C/A**：

P（策划）产出多份输出文档，每份对应一类清单。**这些文档的任务类型都是计划**（自启动，策划→发布），因为"做策划"这个动作本身不需要再被谁计划——它由角色职责或上一轮 A→P 闭环驱动。

其中**计划清单**是 P 阶段的"总调度表"——正文包含策划所有清单的条目：第 1 条为"策划本次计划清单及其余清单"，指向文档自身的任务节点，不递归；后续条目分别为"制定指标清单"、"识别风险清单"、"设计看板清单"、"梳理问题清单"等，每条对应一个输出文档的策划任务。

P 阶段发布完成后，正文中的计划条目触发后续执行——流程/工单任务启动。C（监控）和 A（改进）是任务链运行时推进的结果——被触发的流程/工单执行→指标引擎持续度量→看板呈现→发现偏差→生成风险/问题工单→改进完成后回到下一轮 P。不存在"策划书"中间层或"管理子项"概念。
...
| **PL**（输出产品视角） | PL0→PL5 | 平台 → 业务域 → 产品类型 → 构件 → WBS → 输出文档 |
| **EL**（引擎视角） | EL1→EL3 | 菜单引擎 → 功能引擎 → 实体引擎 |

> 其余视角层级的完整定义见 `23-eos-output-architecture.md` 和 `24-eos-business-assets.md`。

**PL5 输出文档**的组成见 §1.5——封面实体 + 正文实体（1:1）+ 支持信息引用 + 处理配置。PL5 之下不再设 PL6 产品架构层级。文档的处理配置（封面被流程/工单/计划引擎驱动的节点序列）和正文转化（正文实体被各引擎转化为功能表单）属于配置需求规格和引擎运行层面，不是产品架构节点。


> **"输出文档"的指代**：本文档及 biz 路径各 Skill 中提及的"输出文档"，均指代文档的**内容本身**而非其容器或文件名。例如"相关方需求文档"指需求条目本身，"采购履约看板"指该看板的定义与布局本身。看板上的每个指标为该看板的正文内容条目，由指标引擎转化为指标功能表单。文档的承载表单（如 @engine-kanban）仅为组织容器，不影响每个条目作为独立 PL5 输出文档的地位。
...
| 3 | @engine-workorder | 工单引擎 | 功能级（产品数据类） | 配置工单类型、派发规则、处理流程、验收标准和归档策略——亦支撑风险与问题管理 |
| 4 | @engine-project | 项目引擎 | 功能级（业务管理类） | 基于物理表配置项目对象、阶段、里程碑、成员和项目状态 |
| 5 | @engine-wbs | WBS 引擎 | 功能级（业务管理类） | 基于物理表配置 WBS 层级、分解规则、节点属性和进度汇总方式 |
| 6 | @engine-task | 计划引擎 | 功能级（业务管理类） | 基于物理表配置计划任务、时间约束、依赖关系和执行状态 |
| 7 | @engine-kanban | 看板引擎 | 功能级（业务管理类） | 配置指标维度、度量口径、聚合规则、展示方式和看板视图布局，为物理表生成指标型+看板型功能表单 |
| 8 | @engine-indicator | 指标引擎 | 功能级（业务管理类） | 定义指标的名称/口径/目标/维度/图示，生成指标结果功能表单（列表模式/图示模式 2×3网格） |
| 9 | @engine-progress | 进展引擎 | 功能级（业务管理类） | 基于物理表配置进展填报、进度口径、状态更新和进展记录 |
| 10 | @engine-scheduler | 定时任务引擎 | 功能级（业务管理类） | 基于物理表配置定时触发规则、任务事件和执行策略 |
| 11 | @engine-menu | 菜单引擎 | 菜单级 | 配置菜单树节点和场景编排 |

> EL 三层中，「功能引擎」是层级（EL2）而非独立引擎资产。上表中 9 个具体功能引擎均归属功能级。菜单引擎（EL1）和实体引擎（EL3）同时是层级名称和该层级唯一的引擎资产。
...
- **功能引擎**：将物理表转换为功能表单并按主子关系（1+N）组装为主子表单，配置行/列级数据访问权限。承载四方面能力：① 表单能力（结构/字段/布局/校验/行为/发布）；② 功能能力（各具体引擎为表单增加的功能）；③ 组装能力（主子表单组装、子表单显示规则匹配、双击下钻）；④ 权限能力（数据行/列对角色/人员的可见、可编辑或不可访问约束）。
  - **产品数据类**（流程/工单）：支撑产品数据文档的开发与管理——流程引擎管理审批流转，工单引擎管理派发/处理/验收/关闭（亦支撑风险与问题管理）。封面-正文 1:1 绑定由功能引擎的组装能力统一完成，不设独立文档引擎。
...
#### 内置 CAX
- **正式文档**：封面实体和正文实体由实体引擎生成物理表，封面由功能引擎的表单能力转化为封面表单（单据头样式），正文由对应引擎（流程/工单/指标/看板/项目/WBS/计划/进展等）转化为正文功能表单；封面表单 + 正文表单由功能引擎的组装能力按 1:1 主子关系组装为主子表单
- **轻量文档**：正文实体由实体引擎生成物理表，正文由对应引擎转化为功能表单，不产生封面表单和主子组装——正文即文档
- 若该文档与其他文档存在层级引用关系（如正文行下钻到另一个文档），按 1+N 扩展组装
...
A1 是**平台引擎产品**，归属流程与IT域。构件为引擎（`@engine-entity`、`@engine-flow` 等 11 个），末级节点为 CU（配置单元，详见 [25 号资产 §2](../../80-pl4eos-2-eosdata/25-eos-engine-models.md#§2-引擎定义)）。
...
3. 判定任务类型与捕获/推断主要节点（**业务语言**，仅正式文档，供人类确认）：
   - 任务类型三选一：**计划**（自启动，策划→发布）、**流程**（审批流转）、**工单**（派发/处理/验收）
...
4. 推断任务关系（**业务语言**，仅正式文档）——文档之间的任务触发关系：
   - 文档间流转顺序（输出文档A→输出文档B的输入输出链）
   - 计划完成触发流程/工单、流程节点触发工单/计划、工单完成触发流程/计划（三角互触发模型）
5. 基于E2E补充业务需求——闭合检查（端到端闭环），检出缺口→生成补充OR材料
6. 推断输出文档的正文转化引擎——按文档类型匹配引擎（流程/工单/指标/看板/项目/WBS/计划/进展等），全量标注`[推断]`
7. 推断输出文档所属Bn

> 以上为 wft01-biz 完成的设计任务的**逻辑罗列**，非严格执行顺序。任务类型与节点（3）和任务关系（4）是"听懂业务"的证据——业务方需要确认"你理解的任务流转关系是否正确"。节点的业务语言描述留在 wft01-biz；引擎语言转化由 wft02-biz 完成。

### 6.2 业务架构开发（wft02-biz）

**wft02-biz** 将 STR-F 的场景业务展开为 BA 方案——A2 角色场景视角 + Bn 输出产品视角。职责：生成 A2 产品业务架构，推断生成 Bn 产品业务架构；消费 wft01-biz 的任务类型、节点和任务关系（业务语言）→转化为引擎语言；推断输出文档的封面字段/正文结构/正文转化引擎/支持信息引用关系；基于 Bn 构件的 WBS 补充业务需求。D 和 PCA 处理逻辑完全同构。
...
6. 任务节点引擎化（仅正式文档）——消费 wft01-biz 的任务类型与主要节点（业务语言），转化为引擎 CU 候选映射（引擎语言）：
   ...
   - 全部节点标注 `[推断]`
7. 任务关系引擎化（仅正式文档）——消费 wft01-biz 的任务关系（业务语言），转化为引擎可执行的触发规则：
   - 文档间流转顺序 → 输出文档序列编排
   - 三角互触发（计划→流程/工单、流程→工单/计划、工单→流程/计划）→ 各引擎的任务间触发规则配置
   - 含触发条件（计划完成/流程到达某节点/工单验收通过）和触发后的任务启动参数

> **业务语言 → 引擎语言的转化**：wft01-biz 用业务方听得懂的话描述节点和关系，wft02-biz 将其转化为引擎能执行的 CU 规格。任务关系的核心是三角互触发——计划完成可触发流程和工单，流程节点可触发工单和计划，工单完成可触发流程和计划。
...
**STR-E = 一个引擎的配置业务**：每个引擎（`@engine-flow`、`@engine-kanban`、`@engine-indicator` 等）对应一个 STR-E，即该角色对该引擎的端到端配置业务——从打开配置页面、编辑配置要素、系统校验保存、发布装载、到运行期构件/功能可用的完整闭环。当前 11 个引擎 → 11 个 STR-E。
...
| Skill | 末级节点 | 职责定位 | 所属章 |
|-------|---------|---------|--------|
| wft01-biz | 场景业务 | 基于 OR 推断定义场景业务节点和输出文档（含任务模式与主要节点，业务语言），基于端到端补充业务需求——"向业务方证明我听懂了" | §六 |
| wft02-biz | 输出文档 | 生成 A2+Bn 产品业务架构；将 wft01 的业务节点转化为引擎 CU 候选（引擎语言）——"设计方案" | §六 |
| wft03-biz | 配置需求方案 | 消费 wft02 的 CU 候选→生成菜单树/场景业务/内容/任务节点/引用各层具体 CU 配置需求——"出配置" | §六 |
