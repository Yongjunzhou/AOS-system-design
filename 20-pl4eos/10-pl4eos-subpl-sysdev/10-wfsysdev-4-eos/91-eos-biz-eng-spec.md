...
| **wft03-biz** | bpd-biz 节点 | 系统概要定义——功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 | 功能表单（窗口标签页为页面组件承载） | 产出可交付构件开发的配置业务系统 pa-eng，并向 eng 分配指标 |
...
| **产出物** | 业务流程概要定义（bph-biz）——场景业务 + 输出文档序列 + Bn 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务流程详细定义（BP）——A2 角色场景（功能表单/主子表单 + 表单使用顺序）+ Bn 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统概要定义（业务配置需求暨配置业务系统 pa-eng）——功能表单（窗口标签页为页面组件承载）+ 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
| **供谁消费** | 业务方确认 → wft02-biz | wft03-biz | 构件开发流水线 + eng 路径（引擎缺口线索） |

> **概要/详细分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要定义/详细定义」重述——s01 wft01 业务流程概要定义（末级=文档，概要说明为**结构判定级**，字段级详设留 s02）；s02 wft02 业务流程详细定义（详细条目=**活动 + 正文条目 + 表单使用顺序**）；s03 wft03 系统概要定义（末级=**功能表单**，窗口标签页为其页面组件承载形式，2026-08-22 深审统一）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bph-biz、BP→bpd-biz、SR-F→srh-biz；追溯/23 不迁。命名「业务流程详细定义」源于 bpd-biz IPO 承担详细角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 规范化的相关方需求 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
| wft03-biz | 功能表单（窗口标签页为页面组件承载） | 引擎能力使用需求 | 消费 wft02 业务流程详细定义 → 生成**系统概要定义**（s03）——页面组件；分配 FR-BIZ 指标到引擎/CU；输出 FR-BIZ 指标约束包和 FR-ENG 候选线索——"出配置需求暨配置业务系统pa-eng" | §七 |
