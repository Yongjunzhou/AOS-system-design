...
| 规则 | 说明 |
|------|------|
| **R7** | 就绪队列只列出节点级条目。子条目状态通过父节点 `**子条目状态**` 汇总，不单独列出 |
...
| 规则 | 说明 |
|------|------|
| **R9** | AI 不得遍历索引表以外的区域搜索节点。索引表是发现节点存在的唯一入口 |
| **R10** | 索引表不存储行范围。节点行号通过 `grep -n '@{节点ID}'` 实时定位 |
...
| 规则 | 说明 |
|------|------|
| **R11** | `**HEAD @上次 AI 运行**` 由 AI 自动更新为当前提交 hash。人类无需手动修改 |
| **R12** | 若 HEAD 字段不是最近的 AI 提交，AI 在 Step Start 中自动修正 |
| **R13** | 非标反馈经 AI 判定后标记 `[需确认]` |
...
| 规则 | 说明 |
|------|------|
| **R14** | 写回必须在本 Skill 同一运行步骤内完成全部更新（索引表 + 就绪队列 + 子条目状态 + 文件头字段） |
| **R15** | 节点 ID 是定位节点的唯一标识。不使用行号，不依赖文件的行序号 |
| **R16** | 每次写回后必须提交。未提交的写回无法被下次 git diff 检测到 |
