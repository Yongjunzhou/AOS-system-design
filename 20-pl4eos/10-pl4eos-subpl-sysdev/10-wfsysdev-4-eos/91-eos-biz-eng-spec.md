...
| 规则 | 说明 |
|------|------|
| **R1** | 文件第二段（标题行后第一个非空行）必须是 `索引行数：{N}` |
| **R2** | N 为 `## 业务域索引` 表格的总行数（含表头） |

**AI 读取协议**：

```
1. Read(file, offset=2, limit=1) → 获取索引行数 N     （加载：1 行）
2. Read(file, limit=N)           → 加载业务域索引表     （加载：N 行）
3. 按目标产品在业务域索引中查找域入口锚点
4. 进入域正文块后按域产品索引的行范围加载产品段
```
...
> 本约定适用 §十一 所列的全部产品数据与资产文档（01~07、21~28）。08（追溯矩阵）和 09（验证报告）保留原格式，不适用本约定。

**一、读取协议**

Skill 的 Step 1（设计材料加载）加载产品数据文件时，遵循以下流程：

```
1. Read(file, offset=2, limit=1)          → 获取索引行数 N
2. Read(file, limit=N)                    → 加载业务域索引
3. 按目标业务域在业务域索引中查找产品级入口锚点
4. Read(file, offset=域起始行, limit=行范围) → 加载域产品索引
5. 按目标产品在域产品索引中查找行范围
6. Read(file, offset=产品起始行, limit=行范围) → 加载产品正文块
```
...
AI 写入或修改产品数据文件中的节点后，必须同步更新文件头部的行索引：
