...
>
> **版本**：v5.36 | **修订**：2026-08-22 | **说明**：biz 链深审第三轮修复（人类逐项裁决 2026-08-22）——①**消费版本定位置规则**（§A.6.5，B2/B5）：消费版本记录于消费者节点「来源」字段、生产者不重复存、下游可见性由资产写回记录承担、下游已消费列表按来源字段定向查询；补两跳粒度注（STR-F→BP 靠状态路由、BP→SR-F 靠版本比对）；②**§1.4 引擎偏好声明重写**（C1/C2/C3，D 产品数据类优先 / PCA 业务管理类优先 / @engine-task 两场景共用 / 权威校验以任务类型↔引擎映射为准）；③**§1.4/§2.7/§7.1「完全同构」→「流程骨架同构」**（C4，对齐三 Skill 措辞）。
...
> 
> 主责能力域可以是引擎，也可以是平台共享能力域。一个能力域可包含多个 STR-E；多个引擎可消费同一共享 STR-E，但不得重复定义共享 CU。CU 是 STR-E 的细分节点，配置信息组是 A1 产品树的末级节点（PL5，§3.1 A1 PL 树）。
...
|-------|------|------|---------|---------|
| **wft01-biz** | FR-BIZ OR 条目 | 业务概要定义——STR-F 场景业务（枝）+ PL5 输出文档序列（概要末梢）+ Bn 组装（WBS 归组/23 一致性/缺口推断） | 文档（PL5，结构判定级概要说明） | 向业务方证明"我听懂了你的业务" |
| **wft02-biz** | STR-F 节点 | 业务详细定义——A2（角色→场景业务→表单使用顺序）+ Bn（输出文档详细设计 + PL6-A 活动节点需求 + PL6-B 条目需求） | 输出文档（PL5），细分 PL6-A / PL6-B + 表单使用顺序 | 设计正确的方案——业务语言→架构语言 |
| **wft03-biz** | BP 节点 | 系统概要定义——功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 | 功能表单（窗口标签页为页面组件承载） | 产出可交付构件开发的配置业务系统 PA，并向 eng 分配指标 |
...
| **产出物** | 业务概要定义（STR-F）——场景业务 + 输出文档序列 + Bn 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务详细定义（BP）——A2 角色场景（功能表单/主子表单 + 表单使用顺序）+ Bn 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统概要定义（业务配置需求暨配置业务系统 PA）——功能表单（窗口标签页为页面组件承载）+ 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
| **供谁消费** | 业务方确认 → wft02-biz | wft03-biz | 构件开发流水线 + eng 路径（引擎缺口线索） |

> **概要/详细分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要定义/详细定义」重述——s01 wft01 业务概要定义（末级=文档，概要说明为**结构判定级**，字段级详设留 s02）；s02 wft02 业务详细定义（详细条目=**活动 + 正文条目 + 表单使用顺序**）；s03 wft03 系统概要定义（末级=**功能表单**，窗口标签页为其页面组件承载形式，2026-08-22 深审统一）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名保留，不迁移；追溯/23 不迁。命名「业务详细定义」源于 BP IPO 承担详细角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 OR 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
> **26 号文档资产已取消（2026-08-22 深审决策）**：业务文档定义承载于 04/05/06 方案文档，承接锚点在 25 §1.7 业务文档承接。
>
> 相关方需求架构文档 `02-*.md` 在 wft01-biz 产出 STR-F 后形成。D 和 PCA 场景的处理逻辑流程骨架同构。
>
> **与 eng 路径的对称性**（详见 §八）：biz 路径的输出文档对应 eng 路径的配置信息组——都是各自产品的 PL5 末级节点；eng 的 CU（PL4）对应 biz 的 WBS（PL4）。wft03-biz 的配置需求方案 ←→ wft03-eng 的功能概要、wft04-eng 的功能详细，同属 SR-F 层。
...
```
输入                         产出
FR-BIZ OR（待处理）  ──→  业务概要定义 STR-F 场景业务方案（可以分解分配）
...
```
输入                               产出
STR-F 节点（含 wft01 Bn 组装结论）   业务详细定义 BP 方案（可以SR设计）
...
| 3 | 引擎候选校验 | Bn | 在 BP 上下文校验 wft01 引擎候选建议（确认/修正/推翻）——引擎权威在 BP 校验值 |
...
```
FR-ENG OR ──→ wft01-eng ──→ STR-E ──→ wft02-eng ──→ BP ──→ wft03-eng ──→ SR-F(概要) ──→ wft04-eng ──→ SR-F(详细) ──→ wft05-eng ──→ PA
              业务概要定义          业务详细定义            功能概要定义              功能详细定义               产品概要定义
              （场景+配置信息组概要+承接）（详设：操作活动定义） （功能表单+概要说明）     （组件业务+PA收敛项）      （前后端/平台服务组件）
```
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明"我听懂了你要的平台能力" | FR-ENG OR 条目 → STR-E 业务概要方案 | 配置信息组（概要说明，PL5 末级，业务概要定义末级节点；STR-E 架构末级=场景业务 SL5）+ CU 承接 + 依赖 STR-E（共享依赖） | 业务配置人员确认 → wft02-eng |
| **wft02-eng** | 详细定义已分配的 CU——操作活动定义/依赖/状态迁移 | STR-E 节点 → A1 BP 业务详细方案 | 配置信息组（操作活动定义：配置操作/系统处理/运行期能力；CU 为 PL4 选配单元） | wft03-eng |
| **wft03-eng** | 把操作活动定义组织为功能概要（功能表单+概要说明） | BP 节点（A1）→ SR-F 功能概要需求 | 功能表单（页面职责/页面类别/操作活动清单承接 02/页面编排概要） | wft04-eng |
| **wft04-eng** | 展开功能表单上各组件及其业务（功能详细） | 功能概要 SR-F → SR-F 功能详细需求 | 组件业务（布局组件/构件 + 组件业务 + PA 层收敛项） | wft05-eng |
| **wft05-eng** | 产出可交付开发的三类组件方案 | 功能详细 SR-F + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 → PA 组件需求定义 | PA 组件（前端/后端/平台服务） | 追溯验证 / 构件开发流水线 |
...
>
> **与 biz 路径的对应关系**（详见 §七）：eng 路径的**配置信息组**对应 biz 路径的输出文档（同为 PL5 末级节点）；CU 对应 biz 的 WBS（同为 PL4）；wft03-eng 的功能表单是平台软件 SR-F 概要，wft04-eng 的组件业务是平台软件 SR-F 详细，wft03-biz 的功能表单和窗口标签页兼具业务配置需求与配置业务系统 PA 身份。二者同写入 05，但节点类型、使用角色、终态和下游不同。
...
| 2 | 名称级识别并分解配置信息组（概要说明）+ 分配配置信息组到 CU 承接 | 对称于 STR-F 下辖的输出文档——场景业务由哪些 CU 组成；按"可以运行的能力"名称级识别并分解为配置信息组（概要说明，**结构判定级**——配置什么/运行期提供什么，不展开操作活动定义——那是 wft02-eng 业务详细定义），逐组分配 CU 承接（含收敛判定复用/新增/拆分/合并 + 对照 25 盘存的骨架判定） |
| 3 | 基于能力可交付性补充平台需求 | 逐 STR-E 检查配置/治理目标能否形成可独立验收的端到端闭环；检出实质缺口则形成补充 FR-ENG 材料 |
| 4 | 建立追溯链 | FR-ENG OR → STR-E主责能力域 → 配置信息组（概要说明） → CU承接/依赖STR-E；共享能力记录提供能力、消费方和复用范围 |

### 8.2 业务流程开发（wft02-eng）

**详细定义 wft01-eng 已分配的 CU——业务详细定义（02 层，操作活动定义步骤化三段式：配置操作/系统处理/运行期能力），承接 STR-E 的 A2 上下文与 A1 装配，展开操作活动定义/依赖/状态迁移。**

```
输入                                           产出
STR-E 节点                                    A1 配置单元 BP 业务详细方案（可以SR设计）
...
**将 CU 操作活动定义组织为功能概要 SR-F：功能表单+概要说明（结构判定级——页面职责/页面类别/使用角色/承载关系/操作活动清单/页面编排概要），只组织页面，操作活动清单承接 02 标注入口。**

```
输入                                           产出
BP 节点（A1）                                平台功能概要 SR-F（可以详细定义）
（可以SR设计 / 待补充SR设计）  ──→            末级节点：功能表单
...
**将功能概要 SR-F 展开为功能详细 SR-F：判定区域内布局组件/构件、定义组件业务（功能级不写实现）、标注 PA 层收敛项。组件业务是 wft05-eng 推断前后端组件的依据。**

```
输入                                           产出
功能概要 SR-F（05-*.md）                    平台功能详细 SR-F（已详细定义）
（可以详细定义 / 待补充详细定义）  ──→        末级节点：功能表单
                                              细分：组件业务（布局组件/构件 + 组件业务 + PA 层收敛项）
```
...
| 3 | 标注 PA 层收敛项 | 组件边界/接口/数据/运行/NFR 的待判断问题在组件业务条目标注 `PA层收敛-*`（自原 wft03 §2.3.3 下移），供 wft05-eng 设计组件 |

### 8.5 产品概要开发（wft05-eng）

**将功能详细 SR-F（组件业务）、FR-BIZ 指标约束和 NFR 约束展开为 PA 三类组件方案，构件开发流水线可据此进入开发。**（组件业务是推断前后端组件的依据。）

```
输入                                              产出
功能详细 SR-F + FR-BIZ指标约束包 + NFR约束包     PA 组件需求定义（待追溯验证）
（已详细定义）  ──→                              末级节点：PA 组件
                                                  细分：前端组件 / 后端组件 / 平台服务组件
```
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 基于功能详细 SR 及约束推断平台架构 | 判断组件属于引擎专属、布局组件扩展、公共构件、公共平台服务或运行支撑模块；同时消费 FR-BIZ 指标约束包和 wft04-nfr 的 wft05-eng NFR 约束包，形成必要的平台架构决策 |
...
| 6 | 闭合验证与状态推进 | 检查组件业务覆盖、活动覆盖、PA 收敛项、NFR 承接、依赖闭合和 SRD 完整性；正式 PA 推进到 `待追溯验证` |
...
| 6 | 装配 wft05-eng NFR 约束包 | 按约束范围组织为 wft05-eng NFR 约束包：全局约束 / 业务域约束 / SR-F 约束 / 页面级约束 / CU 约束 / PA 组件约束 / 共享约束。约束包只引用分层约束表行 ID 和页面 NFR 条目，不复制正文；分解子指标作为独立约束行装配（标注分解运算+父指标引用）。消费状态/已消费版本由 wft05-eng 消费后回写。NFR 约束仅流向 eng 路径——非功能质量由引擎层保障，biz 路径不消费 |
...
> wft 编号对应瀑布式流程步骤号，非分支编号。每个分支仅在有对应设计活动的步骤出现——如 biz 不涉及 NFR 量化（wft04），eng 从 BP→SR 概要（wft03）→ SR 详细（wft04）→ PA（wft05）。
...
第二轮：eng/nfr 汇合与收尾
  eng 线：wft04-eng 的功能详细 SR-F 基线就绪
  nfr 线：wft04-nfr 完成目标绑定并确认首版 wft05-eng NFR 约束包
  汇合条件满足 → wft05-eng 同时消费功能详细 SR-F、FR-BIZ 指标约束包和 wft05-eng NFR 约束包 → 平台 PA 组件需求定义
```
...
| 依赖关系 | 说明 | 策略 |
|---------|------|------|
| eng 线 wft05-eng ← nfr 线 wft04-nfr | wft05-eng 消费 NFR 约束包 | eng 前四步与 nfr 可并行；首版约束包须在相关 STR-E/CU/功能详细 SR-F 可定位后完成目标绑定并确认，wft05-eng 在两侧输入同时就绪后启动 |
...
| wft03-biz | 功能表单（窗口标签页为页面组件承载） | 引擎能力使用需求 | 消费 wft02 业务详细定义 → 生成**系统概要定义**（s03）——页面组件；分配 FR-BIZ 指标到引擎/CU；输出 FR-BIZ 指标约束包和 FR-ENG 候选线索——"出配置需求暨配置业务系统PA" | §七 |

### 10.4 eng 路径 Skill

| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 配置信息组（概要说明，PL5，业务概要定义末级节点） | 主责能力域 / 配置信息组（概要说明） / CU 承接 / 依赖 STR-E（STR-E 架构末级=场景业务） | 基于 FR-ENG OR 推断可独立配置、治理和验收的平台能力场景 + 名称级识别并分解配置信息组（概要说明） + 分配配置信息组到 CU 承接——"向平台方证明我听懂了"（业务概要定义，01 层） | §八 |
| wft02-eng | 配置信息组（操作活动定义） | CU（PL4 选配单元）/ 操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式） | 承接已分配 CU，展开操作活动定义/依赖/状态迁移详细定义——"详细定义配置单元"（业务详细定义，02 层） | §八 |
| wft03-eng | 功能表单 | 页面类别 / 操作活动清单（承接 02）/ 页面编排概要（区域/TAB/弹窗入口） | 承接操作活动定义，组织功能表单+概要说明——"出功能概要 SR-F"（功能概要定义，03 层） | §八 |
| wft04-eng | 功能表单（组件业务） | 布局组件/构件 / 组件业务 / PA 层收敛项 | 承接功能概要，展开功能表单上各组件及其业务——"出功能详细 SR-F"（功能详细定义，04 层） | §八 |
| wft05-eng | PA 组件 | 前端组件 / 后端组件 / 平台服务组件 | 消费功能详细 SR-F、FR-BIZ 指标约束包和 NFR 约束包，从组件业务推断三类组件及组件架构——"出组件需求"（产品概要定义，05 层） | §八 |
...
|------|----------------------|----------------------|----------------------------|
| 末级节点 | 文档（PL5，结构判定级概要说明） | 输出文档（PL5，细分 PL6-A/PL6-B + 表单使用顺序） | 功能表单（窗口标签页为页面组件承载；业务配置需求 + 配置业务系统 PA 双重身份） |
...
| 维度 | wft01-eng（OR→STR-E） | wft02-eng（STR-E→BP） | wft03-eng（BP→功能概要 SR-F） | wft04-eng（功能概要→功能详细 SR-F） | wft05-eng（功能详细→PA） |
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（概要说明，PL5，业务概要定义末级节点） | 配置信息组（操作活动定义） | 功能表单（概要说明） | 功能表单（组件业务） | PA 组件（前端/后端/平台服务） |
| 输入对象 | FR-ENG OR 条目（≤10） | STR-E 节点（≤5） | BP 节点（A1，≤5） | 功能概要 SR-F（≤5） | 功能详细 SR-F + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
| 输入合法状态 | `待处理` | `可以详细定义` / `待补充详细定义` | `可以SR设计` / `待补充SR设计` | `可以详细定义` / `待补充详细定义` | `已详细定义` |
| 产出状态 | `可以详细定义` | `可以SR设计` | `可以详细定义` | `已详细定义` | `待追溯验证` |
...
bash scripts/read-node.sh <文件> <节点ID>
  → 按节点ID自动识别格式（OR-001 / STR-F-XXX / @node-xxx / @engine-xxx / ROLE-XXX）
...
> **版本标记与消费追溯**：节点初次生成时版本 = `v1`；每次因上游新输入触发修订，版本号递增。下游 Skill 首次消费时记录 `消费版本：v{N}`，重入时比对版本——若当前版本 > 已消费版本 → 触发变更感知，读取变更影响声明（§A.7），按变更类型处理。**消费版本记录于消费者节点的「来源」字段**（如 SR-F 的「来源 BP」、BP 的「来源 STR-F」），生产者节点不重复存消费版本——生产者侧下游可见性由资产写回记录（含消费版本列）承担，变更影响声明的「下游已消费节点」按消费者「来源」字段定向查询。**两跳粒度注**：STR-F→BP 跳处理粒度是整个 STR-F（文档序列级），重入靠节点状态路由 + 变更影响声明；BP→SR-F 跳处理粒度是节点级（逐 SR-F 判断基于哪个 BP 版本），重入靠版本比对。两跳机制不同源于粒度不同，非漏实现。
