...
**不适用**：事务类文档（采购申请单的物料行——运行时交易数据）、内容类文档（会议纪要的议题条目——内容实例）。轻量文档无封面，正文即是全部内容，不展开 PL6-B。轻量文档同样可配置任务类型（如会议纪要配置流程进行审批流转），任务状态由对应引擎在正文实体上直接管理。
...
| **R7** | 清单类文档（指标清单/看板清单/计划清单等）的正文条目展开为 PL6-B——每个条目需满足三个条件：独立名称 + 独立定义要素 + 独立生命周期。事务类文档（采购申请单物料行）和内容类文档（会议纪要议题）不展开 PL6-B |
...
PCA 场景**必须包含一个输出文档为计划清单**（清单类文档，展开 PL6-B 正文条目）。其正文条目中：
...
C（监控）和 A（改进）是任务链运行时推进的结果：P 阶段触发的计划/流程/工单执行 → 指标引擎持续度量 → 看板呈现 → 发现偏差 → 生成风险/问题工单 → A 阶段改进完成。改进完成后可触发下一轮 P 的计划清单第 1 条（新一轮策划），也可由策划主管按周期手动启动。
