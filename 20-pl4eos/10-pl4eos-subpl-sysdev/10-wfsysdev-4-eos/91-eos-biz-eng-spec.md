...
> 本章定义 EOS 平台系统设计的 Skill 体系——三路径（biz/eng/nfr）自闭环模型及各 Skill 的职责定位和链路关系。每个 Skill 的内部操作流程和约定见 §十。
...
Skill 的入口条件、文档章节布局、操作流程等详细规范见 §十。
...
> 本章定义每个 Skill 文档的统一布局模板、入口条件规范，以及所有 Skill 共用的交互协议、生命周期模型、变更传播机制和格式约定。各 Skill 文档引用本章对应节次，并补充本 Skill 的特化规则。
...
| **Step 2 · 方案反馈处理** | 引用 91 规范 §10.3 + 本 Skill 的特化规则 + 确认状态格式示例 | 通用协议不重复，仅写本 Skill 的特化部分 |
| **Step 3~N** | 各 Skill 特化设计步骤 | 按需增减 Step 数量 |
| **Step End · 运行结束输出** | 三区结构（模板见 §10.6） | 人类下一步操作提示，不写入资产 |

**第四章 · AI 标记约定**

四类增量标记（`[新增]`/`[变更]`/`[确认]`/`[需裁决]`）的定义及使用规则见 §10.6。各 Skill 的第四章引用 §10.6，并补充本 Skill 的增量清单格式示例（含本层级的完整增量清单），供人类参照。

**第五章 · 变更记录**

三列表：日期 \| 版本 \| 说明。当前版本写完整说明，历史版本可折叠（"详见 Git 历史"）。

### 10.2 Step Start 入口条件
...
各 Skill 在满足四条通用规则的前提下，按自身输入类型特化以下内容：输入对象类型、输入合法状态值、退回节点状态值、特有校验项。各 Skill 的特化参数定义在其 Skill 文档的第三章 Step Start 中，此处不重复列出。
...
4. **级联处理**：`[修改]` / `[驳回]` 触发级联时，分两级执行：
   ...
   - **资产对齐级联**（需求级联检查通过后执行）：方案变更 → 更新受影响资产的引用和对齐结论。资产映射由各 Skill 在第三章 Step 2 中定义
...
反馈处理的特化参数（锚点需求、操作条目、推进目标、级联触发条件、资产对齐级联）由各 Skill 在第三章 Step 2 中定义，此处不重复列出。
...
各 Skill 的通用态到具体状态名的映射，由各 Skill 在第二章（生命周期）中定义，此处不重复列出。

> **版本标记与消费追溯**：节点初次生成时版本 = `v1`；每次因上游新输入触发修订，版本号递增。下游 Skill 首次消费时记录 `消费版本：v{N}`，重入时比对版本——若当前版本 > 已消费版本 → 触发变更感知，读取变更影响声明（§10.5），按变更类型处理。
...
以下约定为所有 Skill 的统一格式规范，各 Skill 的第四章（AI 标记约定）和第三章 Step End 引用本节，并补充本 Skill 的格式示例。
