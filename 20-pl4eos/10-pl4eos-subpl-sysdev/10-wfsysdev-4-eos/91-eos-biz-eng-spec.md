...
**A1 引擎产品的 PL 树**：A1 平台引擎产品（§5.1）同样沿产出视角（PL）展开——平台 → 业务域（流程与IT）→ 产品类型（A1 引擎产品）→ 构件（引擎）→ **CU（配置单元，PL4）→ 配置信息组（PL5，末级节点）**。A1 树无 WBS 层——Bn 树的 PL4=WBS 保留（§5.3），仅 A1 树以 CU 直接占据 PL4 位置。配置信息组是 A1 产品树的**末级节点**（可独立交付/验收的能力单元），对应 Bn 树的 PL5 输出文档——两者同为各产品树的 PL5 末级；CU 是可独立**选配**的配置单元（PL4，配置入口/页面边界），由若干配置信息组构成（选配粒度=CU、能力粒度=配置信息组，两层面正交不冲突）。A1 树到配置信息组为止，无 PL6——操作活动定义（配置操作/系统处理/运行期能力）是配置信息组的内容，非架构节点。
...
**配置单元定义口径**：每个 CU 用以下要素定义。**CU（PL4，可独立选配的配置单元）由配置信息组（PL5，A1 产品树末级节点）构成**（每组 = **某组配置字段，这组字段共同提供一个可以运行的能力**，可独立交付/验收），**配置操作、系统处理、运行期能力均以配置信息组为单位说明**；配置入口等留在 CU 级。叙事链为"配置人员操作 → 系统处理 → 业务人员操作"。
...
**三条规则**：① 组的分界按**可以运行的能力**——配置信息组=某组配置字段，这组字段共同提供一个可以运行的能力（运行期产出/驱动一项能力，含业务人员可操作能力与系统侧运行行为两类）；单字段即可提供一项能力时单字段成组，能力不可独立时对应合并；② 运行期能力非必填——某组只提供系统处理、不提供业务互动能力时，显式标注"以系统处理为主"，不硬凑运行期能力；③ 生效后响应一律编号式——"该组配置信息保存后，系统应①…②…③…"，逐条列系统动作。④ **配置信息组是 A1 产品树的末级节点（PL5，可独立交付/验收的能力单元），不是 CU 的随意字段分组**——分组的判定以"可独立交付的能力"为边界（§3.1 A1 PL 树）。
...
| 产品 | 归属域 | 构件 | 末级节点 |
|------|--------|------|---------|
| **A1** | 流程与IT | 引擎 | 配置信息组（CU 为 PL4 可独立选配的配置单元） |
| **A2** | 各业务域 | 角色 | 场景业务 |
...
A1 是**平台引擎产品**，归属流程与IT域。构件为引擎（`@engine-entity`、`@engine-flow` 等 12 个），末级节点为配置信息组（CU 为 PL4 可独立选配的配置单元，由配置信息组构成；详见 [25 号资产 §2](../../80-pl4eos-2-eosdata/25-eos-engine-models.md#2-引擎定义)）。

每个 CU 定义一个可独立配置、校验、发布的引擎元数据对象，由若干配置信息组（PL5，A1 末级节点）构成——配置人员在 CU 配置页面上编辑各组配置要素，提交后引擎校验并保存，运行时按 CU 定义生成运行期制品（物理表/功能表单/菜单入口+构件）。CU 的完整定义见 §3.3 和 §4.3。

对应设计路径为 **eng 路径**：wft01-eng（业务流程概要）→ wft02-eng（业务流程方案）→ wft03-eng（系统需求概要）→ wft04-eng（系统需求方案）→ wft05-eng（平台产品架构/三类组件）。

> **bph-eng 的双重身份**：wft01-eng 产出的 bph-eng 具有双重身份——
> - **A2 视角**：它是流程与IT域中**业务配置人员**的 PL5 场景业务。
> - **A1 视角**：它对应某个主责能力域中的可独立配置、治理和验收的能力场景。
> 
> 主责能力域可以是引擎，也可以是平台共享能力域。一个能力域可包含多个 bph-eng；多个引擎可消费同一共享 bph-eng，但不得重复定义共享 CU。CU 是 bph-eng 的细分节点，配置信息组是 A1 产品树的末级节点（PL5，§3.1 A1 PL 树）。

### 5.2 A2产品

A2 是**各业务域的角色场景**，构件为角色（如采购员、库存管理员），末级节点为场景业务。沿角色场景视角（SL）展开：SL3 角色类型 → SL4 角色职责 → SL5 场景业务 → SL6 功能表单/主子表单。

每个 A2 产品回答：某角色在哪个菜单入口、按什么顺序使用哪些功能表单/主子表单、完成端到端的职责闭环。场景业务（SL5）与 bph-biz 末级节点 1:1 映射。

对应设计路径为 **biz 路径的前半段**：wft01-biz（业务流程概要，场景业务为枝、文档为末级）→ wft02-biz（业务流程方案，组装 A2 产品业务流程 + 表单使用顺序）。

> **流程与IT域的 A2 产品**：流程与IT域同样有 A2 产品——**业务配置人员**是该域的核心角色类型。业务配置人员的场景业务（SL5）即 bph-eng，其归组锚点为`主责能力域 + 配置/治理目标 + 可独立验收结果`，不是引擎数量。bph-eng 与业务域 bph-biz 在架构层级上同构，区别在于场景产出物是**可用的平台能力**。
...
对应设计路径为 **biz 路径的后半段**：wft02-biz 展开 Bn 输出产品详细设计（消费 wft01 Bn 组装）→ wft03-biz 生成各层配置需求。

#### 5.3.1 资产对齐判据（Bn 组装）之家

**判据之家（2026-09-04 定）**：面向 23 的资产对齐判据族统一定义于此——**23B 对齐 Q1/Q2/Q3**（Q1=已有、直接引用；Q2=需扩展；Q3=无、新建建议）、**PL4 WBS 归组**（数据依赖链 + 完成耦合硬判据）、**23 一致性三类**（归属冲突 / 误新建 / 误引用）、**缺口推断**（按构件产品数据清单做名称级比对，不依赖任务类型/引擎）。**引用数据**（Bn 全貌 / PL 路径 / WBS / 产品数据清单）在 23 资产（`80-pl4eos-2-eosdata/23-eos-output-architecture.md`），不载入判据正文。各 wft/试点人类方案只写本任务的**判定动作**，判据定义引本节、引用数据读 23，不重述判据正文（93 §四.1 判据归属分级）。
...
> 平台资产治理是 EOS 平台的**内置场景业务**（A2 产品）：平台自身的管理对象（菜单、架构资产、数据标准、权限）就是平台的业务数据，平台提供对应的引擎和功能表单直接管理这些对象，治理人员使用平台能力治理平台。bph-biz 已预置在 `02-*.md` 中。
>
...
|-------|------|------|---------|---------|
| **wft01-biz** | FR-BIZ 规范化的相关方需求条目 | 业务流程概要——bph-biz 场景业务（枝）+ PL5 输出文档序列（概要末梢）+ Bn 组装（WBS 归组/23 一致性/缺口推断） | 文档（PL5，结构判定级概要说明） | 向业务方证明"我听懂了你的业务" |
| **wft02-biz** | bph-biz 节点 | 业务流程方案——A2（角色→场景业务→表单使用顺序）+ Bn（输出文档详细设计 + PL6-A 活动节点需求 + PL6-B 条目需求） | 输出文档（PL5），细分 PL6-A / PL6-B + 表单使用顺序 | 设计正确的方案——业务语言→架构语言 |
...
| **产出物** | 业务流程概要（bph-biz）——场景业务 + 输出文档序列 + Bn 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务流程方案（BP）——A2 角色场景（功能表单/主子表单 + 表单使用顺序）+ Bn 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统需求概要（业务配置需求暨配置业务系统 pa-eng）——⑤ 用例场景功能（枝梢；功能表单为其业务侧载体，窗口标签页为页面组件承载）+ 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
| **供谁消费** | 业务方确认 → wft02-biz | wft03-biz | 构件开发流水线 + eng 路径（引擎缺口线索） |

> **概要/方案分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要/方案」重述——wft01 业务流程概要（末级=文档，概要说明为**结构判定级**，字段级详设由 wft02-biz 承接）；wft02 业务流程方案（详细条目=**活动 + 正文条目 + 表单使用顺序**）；wft03 系统需求概要（**枝梢＝⑤ 用例场景功能**，叶＝**⑥ IPO功能**只在完整态；功能表单是业务侧 A2/Bn ⑥ 任务节点步骤的载体，窗口标签页为其页面组件承载形式——2026-08-22 深审统一，2026-09-10 按 94 树权威校正末级口径）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bph-biz、BP→bpd-biz、SR-F→srh-biz；追溯/23 不迁。命名「业务流程方案」源于 bpd-biz IPO 承担方案角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 规范化的相关方需求 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
> **SR 树的末级与功能表单的关系（2026-09-10 定，按 94 树权威校正）**：srh（系统需求）树的**枝梢＝⑤ 用例场景功能、叶＝⑥ IPO功能**（后者只在完整态——系统接收某组字段的值→处理→给出响应）。**功能表单不属于 SR 树**：它是「**用户操作页面视角**」的载体，对应业务侧 A2/Bn 树 **⑤ 业务输出文档（枝梢）下的任务步骤**（该文档的**分解源判据展开项**，见 [94 §2.2](94-eos-sys-dev-dbs.md)），跨视角聚合系统侧的 ④ 系统用例功能 / ⑤ 用例场景功能。两个视角正交——**分类视角**（树的 ③ 类）下一个系统用例功能只归一 类；**页面视角**下同一系统用例功能可按需出现在多张功能表单上。树形/级语义单源见 [94](94-eos-sys-dev-dbs.md)。
>
> **两归纳枝分工说明（2026-08-22 深审确认）**：A2 归纳枝（SL3→SL4）在 wft02、Bn 归纳枝（PL3→PL4 WBS）在 wft01——不对称是分工决定的，非遗漏：PL4 WBS 归组与 23B 对齐/23 一致性/缺口推断同属 wft01 Bn 组装（结构性绑定），SL4 归纳服务 A2 场景装配与表单使用顺序（wft02 Step 3），且 A2 侧无 23 资产产品数据清单类锚点可做缺口推断。
>
> **26 号文档资产已取消（2026-08-22 深审决策）**：业务文档定义承载于 04/05/06 方案文档，承接锚点在 25 §1.7 业务文档承接。
>
...
| 7 | Bn 组装 | 23B Q1/Q2/Q3 对齐判定 + PL4 WBS 归组（数据依赖链硬判据）+ 23 一致性检查（归属冲突/误新建/误引用）+ 按构件产品数据清单完整性缺口推断（结构性缺口 → 补充 规范化的相关方需求 回路） |
...
> 将 bph-biz 场景业务展开为完整的 bpd-biz 方案——A2（角色怎么工作）+ Bn（角色产出什么）。核心使命：**设计正确的方案**，完成业务语言 → 架构语言的转化。

```
输入                               产出
bph-biz 节点（含 wft01 Bn 组装结论）   业务流程方案 bpd-biz 方案（可以srh设计）
...
| # | 任务 | 视角 | 说明 |
|---|------|------|------|
| 1 | 装配 A2 产品业务流程 | A2 | 角色 → 场景业务 → 功能表单/主子表单 + 表单使用顺序 |
| 2 | 消费 Bn 组装结果 | Bn | 承接 wft01 Bn 组装结论（PL 路径 + WBS 归组 + 23 一致性，不重复归组/一致性检查），逐输出文档展开详细设计 |
| 3 | 引擎候选校验 | Bn | 在 bpd-biz 上下文校验 wft01 引擎候选建议（确认/修正/推翻）——引擎权威在 bpd-biz 校验值 |
| 4 | 定义输出文档方案 | Bn | 正式文档：封面字段 + 正文实体结构 + 支撑引擎 + 支持信息引用；轻量文档：正文实体结构 + 支撑引擎 + 支持信息引用 |
| 5 | 定义 PL6-A 活动节点需求 | Bn | 有任务类型时，消费 wft01 的业务节点（业务语言）→ 逐节点定义处理角色、可执行动作、状态变迁、系统自动节点/异常路径/超时处理，全量标注 `[推断]`。窗口标签页序列编排由 wft03 负责 |
| 6 | 判定并定义 PL6-B 正文条目需求 | Bn | 先判定正文是否为集合容器，再对其中的条目类型执行三独立判定。轻量文档的原子正文行不得重复作为 PL6-B；满足条件的条目逐类定义名称、定义要素和生命周期，并继承支撑引擎 |
| 7 | 任务关系引擎化 | — | 文档间流转顺序 → 输出文档序列编排；任务间触发规则 → 引擎任务间触发规则配置（含触发条件和启动参数） |
| 8 | 基于 Bn 补充业务需求 | Bn | 详设层设计性缺口（PL6-B/指标/活动节点级）→ 生成补充 规范化的相关方需求 材料；结构性缺口已由 wft01 Bn 组装处理 |

---

### 7.3 业务配置需求与配置产品架构开发（wft03-biz）

> 将 bpd-biz 方案展开为 srh-biz 系统需求（**枝梢＝⑤ 用例场景功能**，叶＝⑥ IPO功能），并形成引擎能力使用需求与 FR-BIZ 指标分配。该产出兼具两重身份——从 EOS 平台视角是业务配置需求，从配置业务系统视角是产品架构节点（**功能表单**为其在业务侧 A2/Bn ⑥ 任务节点步骤上的载体形式）；biz 路径在 wft03-biz 结束，不再设置 wft05-biz，也不在 07 重复定义。
...
| **wft02-eng** | 详细定义已分配的 CU——操作活动定义/依赖/状态迁移 | bph-eng 节点 → A1 bpd-eng 业务流程方案 | 配置信息组（操作活动定义：配置操作/系统处理/运行期能力；CU 为 PL4 选配单元） | wft03-eng |
| **wft03-eng** | 把操作活动定义组织为系统需求概要（⑤ 用例场景功能 + 功能表单 + 概要说明） | bpd-eng 节点（A1）→ srh-eng 系统需求概要需求 | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（页面职责/页面类别/操作活动清单承接 02/页面编排概要） | wft04-eng |
...
>
> **bph-eng 的双重身份**（详见 §5.1）：在 A2 视角下，bph-eng 是业务配置人员的场景业务（SL5/PL5 末级节点）；在 A1 视角下，它对应唯一主责能力域下的独立能力场景。主责类型分为`引擎专属`和`平台共享`，CU 是 bph-eng 的细分节点。
>
...
**详细定义 wft01-eng 已分配的 CU——业务流程方案（02 层，操作活动定义步骤化三段式：配置操作/系统处理/运行期能力），承接 bph-eng 的 A2 上下文与 A1 装配，展开操作活动定义/依赖/状态迁移。**

```
输入                                           产出
bph-eng 节点                                    A1 配置单元 bpd-eng 业务流程方案（可以srh设计）
...
| # | 任务 | 视角 | 说明 |
|---|------|------|------|
| 1 | 承接 bph-eng 的 A2 上下文 | A2 | 读取 wft01-eng 装配的 A2 信息（业务运行对象、配置/支撑角色、触发事件、业务状态影响）并校验字段完整；只服务 eng 链 CU 推导，不替代 wft02-biz 的 A2/Bn 输出文档 |
| 2 | 承接已分配 CU 并展开 A1 装配 | A1 | 读取 bph-eng 的 CU 承接关系（wft01-eng 已判定收敛/骨架/引擎归属）→ 按主责能力域→能力场景→CU 层级展开详设；共享能力独立建模，消费方只记录依赖 |
| 3 | 定义 CU 操作活动定义 | A1 | 承接 wft01 的配置信息组清单+结构判定级概要说明，逐组展开**操作活动定义**（配置操作=步骤序列每步含对象+约束+联动 / 系统处理=编辑时处理+生效后响应，统一保存机制与载件标注 / 运行期能力=操作处理分支显式化；基准=PDF v1.5.2），定义配置入口与引擎状态迁移；详设层变更（配置能力扩展）以 `[调整]` 标注；承接缺陷退回 `需wft01修订` |
| 4 | 基于 A1 补充引擎需求 | — | 检出详设缺口（操作活动/依赖/状态迁移） → 生成补充 FR-ENG 材料（详设层缺口，登记 材料状态表 A） |

> **A2/A1 双视角**：Task 1 对应 §5.2 所述流程与IT域的 A2 产品，但在 eng 路径中只做"上下文承接"——A2 信息由 wft01-eng 装配，本 Skill 读取校验后说明 CU 服务于哪个业务运行对象、配置/支撑角色和触发场景；Task 2~3 对应 §5.1 所述 A1 产品——描述引擎"怎么被配置"（引擎→CU 层级→配置信息组/操作活动定义）。功能表单本身由 wft03-eng 组织概要、wft04-eng 展开组件业务。
>
...
```
输入                                           产出
bpd-eng 节点（A1）                                平台系统需求概要 srh-eng（可以详细定义）
...
|-------|---------|---------|---------|--------|
| wft01-biz | 文档（PL5，概要末梢） | PL6-A 活动节点 / PL6-B 正文条目初步判定（结构判定级） | 基于 规范化的相关方需求 推断**业务流程概要**——场景业务（枝）+ 输出文档序列 + **Bn 组装**（WBS 归组/23 一致性/按构件产品数据清单完整性缺口推断），结构性缺口补充 规范化的相关方需求 回路——"向业务方证明我听懂了" | §七 |
| wft02-biz | 输出文档（PL5） | PL6-A 活动节点 / PL6-B 正文条目 / 表单使用顺序 | 消费 wft01 Bn 组装，生成 A2+Bn 双视角**业务流程方案**——引擎校验（权威）+ 文档方案 + 活动设计 + 权限 + 任务关系（架构语言）——"设计方案" | §七 |
...
| 资产写回 | 23（Bn 组装结论：Q 判定 + WBS 归组） | 23/24/27/28（26 已取消，文档定义承载于 04/05/06，锚点 25 §1.7） | 04/05/06 |
| 补充材料 | 补充 规范化的相关方需求 材料（闭合缺口 + Bn 组装结构性缺口） | FR-BIZ 规范化的相关方需求 材料（详设层设计性缺口） | FR-ENG 候选线索（引擎缺口） |
...
| 输入对象 | FR-ENG 规范化的相关方需求节点 条目（≤10） | bph-eng 节点（≤5） | bpd-eng 节点（A1，≤5） | 系统需求概要 srh-eng（≤5） | 系统需求方案 srd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
...
| 2026-09-04 | v5.53 | §5.3 补「5.3.1 资产对齐判据（Bn 组装）之家」（2026-09-04 判据之家定案，对齐 biz 三试点/93 §四.1）——面向 23 的对齐判据族统一定义于此（23B 对齐 Q1~Q3 / WBS 归组 / 23 一致性三类 / 缺口推断名称级比对），引用数据在 23 不载判据；补 **Q 族命名索引**（对齐 Q1~Q3 ↔ 能力覆盖 Q1~Q4 ↔ 22 分类 Q1~Q3+ 三族分家，防同名指错）。 |
...
| 2026-08-22 | v5.31 | eng 链架构升级（人类裁决 2026-08-22）——A1 树升级为 4 层（产品→引擎→CU→配置信息组）：配置信息组=A1 产品树末级节点（PL5，可独立交付/验收的能力单元）、CU=PL4（可独立选配的配置单元，由配置信息组构成）、A1 树无 WBS（Bn 树 PL4=WBS 保留）；§3.1 补 A1 PL 树、§3.3 配置信息组定位升级、§5.1 A1 末级=CU→配置信息组、§8/§10.6 末级节点同步（配置信息组对应 biz 文档、CU 对应 WBS）。 |
...
| 2026-08-21 | v5.26 | biz 链「概要/详细」分工框架叙述完整度补齐（wft01-biz v3.17 + wft02-biz v9.3 + wft03-biz v9.7 对偶实施，人类确认符合性核查后 2026-08-21）——§7.2 分工框架 blockquote 补「命名业务详细定义不破通用规范」兼容句（BP IPO 承担详细角色，通用规范 §1.3 特例）；一棵树读法（A2/Bn 归纳枝→场景业务→文档→活动/正文条目/表单使用顺序→功能表单）下沉到三个 Skill 概念章；wft01「场景业务（归纳枝）」措辞修正为「汇聚点」。 |
| 2026-08-21 | v5.25 | biz 链「概要/详细」分工框架定案同步（wft01-biz v3.16 + wft02-biz v9.2 对偶实施，人类确认 2026-08-21）——§七 三个 Skill 用「业务概要定义/业务详细定义/系统概要定义」重述分工：wft01 新增 Bn 组装（WBS 归组 + 23 一致性 + 按构件产品数据清单完整性缺口推断，结构性缺口 规范化需求 回路）、wft02 收缩纯详细层（迁出 WBS 归组/23 一致性/缺口推断）、wft03 末级=功能表单；STR-F/BP/SR-F 资产名保留；层级表联动审查同步——§10.3/§10.6 参数表（末级节点/资产写回/补充材料）、§7.1 任务 7 改 Bn 组装 + 末级节点=文档、§7.2 末级节点 block 与任务清单（任务 2 消费 Bn 组装 / 3 引擎候选校验 / 8 设计性缺口）、§5.2/§5.3 设计路径（wft02 详设承接 wft01 Bn 组装）、附录 A.2.3 标题命名示例。 |
