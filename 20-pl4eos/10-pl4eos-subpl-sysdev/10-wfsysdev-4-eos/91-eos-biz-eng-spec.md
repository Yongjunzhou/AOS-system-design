...
>
> **版本**：v4.5 | **修订**：2026-07-11 | **说明**：§九 新增 NFR 路径独立章节（原 §九→§十）——对齐 biz（§七）/eng（§八）结构，补充 wft01-nfr 和 wft04-nfr 的定位/输入/产出/任务清单完整定义。§十（原 §九）三路径体系编号同步更新。§9.5 eng 参数表补全资产写回行与空白字段；§7.1 Task 3 描述拆分；§7.3 关键澄清提升为正文。
...
> biz 路径将非结构化的业务原始需求（FR-BIZ OR）逐级转化为可交付开发的配置需求方案。三个 Skill 形成设计接力链：

```
FR-BIZ OR ──→ wft01-biz ──→ STR-F ──→ wft02-biz ──→ BA ──→ wft03-biz ──→ 配置需求方案
              场景业务                 输出文档                  各层配置需求
              业务语言                 引擎语言                  引擎 CU 配置
              "我听懂了"               "方案是对的吗"              "可以开始配了"
```
...
|-------|------|------|---------|---------|
| wft01-biz | FR-BIZ OR 条目 | STR-F 场景业务方案 | 场景业务 | 向业务方证明"我听懂了你的业务" |
| wft02-biz | STR-F 节点 | BA 业务架构方案（A2+Bn） | 输出文档 + PL6 | 设计正确的方案——业务语言→引擎语言 |
| wft03-biz | BA 节点 | 各层配置需求方案 | 配置需求方案 | 产出可交付引擎配置的规格 |

> 相关方需求架构文档 `02-*.md` 在 wft01-biz 产出 STR-F 后形成。D 和 PCA 场景的处理逻辑完全同构。

### 7.1 原始需求处理（wft01-biz）

**定位**：biz 路径的第一个 Skill，从零散但规范化的 OR 到结构化的场景业务方案。核心使命是**向业务方证明"我听懂了你的业务"**——产出供人类确认理解，而非最终设计方案。

**输入**：FR-BIZ OR 条目（状态 `待处理`）
**产出**：STR-F 场景业务方案（状态推进至 `可以分解分配`）
**末级节点**：场景业务（SL5 / PL5）
...
| 3 | 判定任务类型与子节点 | **业务语言**，供人类确认。判定任务类型（计划/流程/工单）→展开 PL6-A 活动节点，捕获 OR 明示节点。管理类（PCA）场景业务需判定 PL6-B 正文条目。首个有任务类型的文档必然为计划（R4）。OR 描述的三种处理情形见下文 |
| 4 | 推断任务间关系 | **业务语言**。文档间流转顺序 + OR 明示的任务触发关系 |
| 5 | 基于 E2E 补充业务需求 | 闭合检查（端到端闭环：执行类 D 从需求提出到需求满足，管理类 PCA 计划→监控→分析→纠正），检出缺口→生成补充 OR 材料 |
| 6 | 推断正文转化引擎 | 按文档类型匹配引擎（流程/工单/指标/看板/项目/WBS/计划/进展等），全量标注 `[推断]` |
| 7 | 推断输出文档所属 Bn | 判定文档归属的 Bn 产品构件类型 |
...
> 其中 R4 约束：场景业务中第一个有任务类型的输出文档，其任务类型必然是计划。

> 以上为逻辑罗列，非严格执行顺序。任务 3 判定输出文档（PL5）的 PL6 子节点类型——有任务类型则展开 PL6-A 活动节点，正文条目满足三独立则展开 PL6-B 正文条目。部分文档仅有 PL6-A，部分文档兼有 PL6-A + PL6-B。wft01 在业务语言层捕获/推断 PL6-A 活动节点，供人类确认"你理解的任务流转关系是否正确"。

### 7.2 业务架构开发（wft02-biz）

**定位**：biz 路径的第二个 Skill，将 STR-F 的场景业务展开为完整的 BA 方案——A2（角色怎么工作）+ Bn（角色产出什么）。核心使命是**设计正确的方案**。

**输入**：STR-F 节点（状态 `可以分解分配` / `待补充分解分配`）
**产出**：BA 业务架构方案（状态推进至 `可以SR设计`）
**末级节点**：输出文档（PL5），细分节点：PL6-A 活动节点 / PL6-B 正文条目

**关键转化——业务语言→引擎语言**：wft01-biz 用业务方听得懂的话描述活动节点和关系，wft02-biz 将其转化为引擎能执行的 CU 规格：

| wft01 输出（业务语言） | wft02 转化（引擎语言） |
|----------------------|----------------------|
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 装配 A2 产品业务架构 | 角色场景视角——角色→场景业务→功能表单/主子表单 |
| 2 | 装配 Bn 产品业务架构 | 输出产品视角——构件类型→WBS→输出文档 |
| 3 | 推断 Bn 构件输出文档 | 逐构件类型推断其所辖的输出文档集合 |
| 4 | 基于 Bn 补充业务需求 | 构件 WBS 分解中发现的文档缺口→生成补充 OR 材料 |
| 5 | 定义输出文档方案 | 正式文档：封面字段（DocumentCover 表映射）+ 正文实体结构 + 正文转化引擎 + 支持信息引用；轻量文档：正文实体结构 + 正文转化引擎 + 支持信息引用 |
| 6 | 展开 PL6-A 活动节点 | 有任务类型时，消费 wft01 的业务节点→转化为引擎 CU 候选（类型→引擎映射、节点→CU 配置、补全系统节点/异常路径/超时处理），全量标注 `[推断]` |
| 7 | 判定并展开 PL6-B 正文条目 | 正文条目满足三独立（独立名称+独立定义要素+独立生命周期）→逐条目展开；事务类（物料行）和内容类（议题条目）不满足三独立，不展开 |
| 8 | 任务关系引擎化 | 文档间流转顺序→输出文档序列编排；任务间触发规则→引擎任务间触发规则配置（含触发条件和启动参数） |

### 7.3 系统需求开发（wft03-biz）

**定位**：biz 路径的第三个 Skill，将 BA 方案展开为各层配置需求——描述在引擎上**如何配置**以实现方案。核心使命是**产出可交付引擎配置的规格**。

**输入**：BA 节点（状态 `可以SR设计` / `待补充SR设计`）
**产出**：各层配置需求方案（状态推进至 `待构件需求定义`）
**末级节点**：配置需求方案

wft03-biz 的产出是各层**配置需求**——描述在现有或新增引擎上如何配置以实现 BA 方案，而非直接生成 SR-F 功能表单定义。引擎开发完成后按配置需求进行配置实现。
...
| 2 | 生成场景业务方案及配置需求——主子表单序列编排、显示顺序与依赖关系 | @engine-menu CU |
| 3 | 生成输出文档内容配置需求——封面字段 CU 配置 + 正文实体字段 CU 配置 + 正文转化引擎 CU 配置 | @engine-entity CU / 各功能引擎 CU |
| 4 | 生成任务节点配置需求——消费 wft02 的任务节点 CU 候选，生成具体 CU 配置 | @engine-flow / @engine-task / @engine-workorder CU |
| 5 | 生成任务关系配置需求——消费 wft02 的任务关系，生成触发规则的引擎 CU 配置 | 各引擎的任务间触发规则 CU |
| 6 | 生成支持信息引用配置——文档间多对多引用的 CU 配置及引用角色标签 | @engine-entity CU |
| 7 | 基于配置需求补充引擎需求——发现引擎能力缺口→生成 FR-ENG 线索 | 转入 eng 路径（§八） |

---

## 八、平台能力之系统设计

> eng 路径将引擎能力原始需求（FR-ENG OR）逐级转化为可交付开发的前后端组件方案。与 biz 路径并行但设计对象不同——biz 设计业务场景，eng 设计支撑业务场景的引擎能力。

```
FR-ENG OR ──→ wft01-eng ──→ STR-E ──→ wft02-eng ──→ BA ──→ wft03-eng ──→ SR-F ──→ wft05-eng ──→ PA
              引擎场景业务            配置单元                 操作页面                  前后端组件
              名称级 CU              详细设计                  构件清单                  代码实现单元
```

| Skill | 输入 | 产出 | 末级节点 | 细分节点 |
|-------|------|------|---------|---------|
| wft01-eng | FR-ENG OR 条目 | STR-E 引擎场景业务方案 | 场景业务 | CU 候选（名称级） |
| wft02-eng | STR-E 节点 | A1 配置单元 BA 详细设计 | 配置单元（CU） | 配置页面 + 业务页面 |
| wft03-eng | BA 节点（A1） | 引擎功能 SR-F 需求 | 操作页面 | 构件 |
| wft05-eng | SR-F 构件清单 | PA 前后端组件方案 | 前后端组件 | 前端组件 + 后端组件 |

> **与 biz 路径的对称性**（详见 §七）：eng 路径的 CU 对应 biz 路径的输出文档——都是各自产品的 PL5 末级节点。wft03-eng 的操作页面 ←→ wft03-biz 的配置需求方案，同属 SR-F 层。

### 8.1 原始需求处理（wft01-eng）

**定位**：eng 路径的第一个 Skill，将 FR-ENG OR 转化为 STR-E 引擎场景业务方案。输出的 STR-E 节点与 biz 路径的 STR-F 在架构层级上完全同构。

**角色**：业务配置人员（流程与IT域）。该角色是 eng 路径的唯一角色——所有引擎的配置均由业务配置人员执行，STR-E 架构树上无需角色层做分组维度。

**STR-E 的双重身份**（详见 §5.1）：

| 视角 | 身份 | 说明 |
|------|------|------|
| A2 视角 | 流程与IT域的场景业务（SL5/PL5 末级节点） | 业务配置人员对该引擎的端到端配置业务——从打开配置页到运行期能力可用 |
| A1 视角 | 引擎是 PL3 构件节点 | CU 是 STR-E 的细分节点 |

当前 12 个引擎 → 12 个 STR-E，与输出文档（STR-F 下辖）的层级关系完全对称。

**输入**：FR-ENG OR 条目（状态 `待处理`）
**产出**：STR-E 引擎场景业务方案（状态推进至 `可以分解分配`）
**末级节点**：场景业务（SL5 / PL5），细分节点：CU 候选（名称级）
...
**定位**：eng 路径的第二个 Skill，将 STR-E 中名称级 CU 候选展开为 A1 配置单元 BA 详细设计——为每个 CU 定义其配置页面和业务页面的设计方案。

**输入**：STR-E 节点（状态 `可以分解分配` / `待补充分解分配`）
**产出**：A1 配置单元 BA 详细设计（状态推进至 `可以SR设计`）
**末级节点**：配置单元（CU），定义见 §3.3 和 §4.3

**CU 的两类页面**：

| 页面类型 | 实现方式 | 说明 |
|---------|---------|------|
| 配置页面 | 硬代码实现 | 配置人员编辑 CU 的配置要素，提交后引擎校验并保存 |
| 业务页面 | 引擎按 CU 定义渲染 | 在操作页面组件上渲染构件能力（见 §4.3） |

**任务清单**：

| # | 任务 | 说明 |
|---|------|------|
| 1 | 装配 A1 产品业务架构 | 接收 STR-E → 按引擎→CU 层级展开 |
| 2 | 展开 CU 详细设计 | 配置要素 / 系统处理说明 / 运行期产出 / 引擎状态迁移 |
| 3 | 为每个 CU 设计操作页面 | 配置页面 + 业务页面的设计方案 |
| 4 | 基于 A1 补充引擎需求 | 检出 CU 缺口 → 生成 FR-ENG 线索或补充 OR 材料 |

### 8.3 系统需求开发（wft03-eng）

**定位**：eng 路径的第三个 Skill，将 BA 中的操作页面展开为引擎功能 SR-F 需求——为每个操作页面识别其所需构件。对应 biz 路径的 wft03-biz，同属 SR-F 层。

**输入**：BA 节点（A1，状态 `可以SR设计` / `待补充SR设计`）
**产出**：引擎功能 SR-F 需求（操作页面 + 构件清单）
**末级节点**：操作页面，细分节点：**构件**（安装在操作页面组件上的可复用功能单元，详见 §4.2）

**任务清单**：

| # | 任务 | 说明 |
|---|------|------|
| 1 | 为每个操作页面推断所需构件清单 | 逐操作页面识别所需的展示类/交互类/数据类/反馈类构件 |
| 2 | 生成功能架构（SR-F） | 基于操作页面 + 构件组合形成功能架构 |
| 3 | 为构件推断交互行为 | 触发方式 / 操作对象 / 结果反馈 |

### 8.4 产品架构开发（wft05-eng）

**定位**：eng 路径的第四个 Skill，将 SR-F 中的构件展开为 PA 前后端组件方案——构件是功能设计概念，前后端组件是实现设计概念。从此 Skill 起，构件进入开发阶段。
...
| 组件类型 | 说明 |
|---------|------|
| 前端组件 | UI 层的代码实现单元——组件树 / 路由 / 状态管理 |
| 后端组件 | 服务层的代码实现单元——API 设计 / 服务划分 / 数据访问 |

**任务清单**：

| # | 任务 | 说明 |
|---|------|------|
| 1 | 逐构件判定前端组件需求 | 构件需要哪些前端组件来实现其 UI 呈现和交互 |
| 2 | 逐构件判定后端组件需求 | 构件需要哪些后端组件来实现其数据处理和业务逻辑 |
| 3 | 推断平台架构（NFR 约束） | 部署架构 / 通信架构 / 数据架构 / 安全架构 |
| 4 | 推断前端组件架构 | 组件树 / 路由设计 / 状态管理方案 |
| 5 | 推断后端组件架构 | API 设计 / 服务划分 / 数据访问方案 |

---

## 九、非功能需求之系统设计

> nfr 路径将非功能原始需求（NFR OR）逐级转化为可验证的量化指标和可消费的约束包。与 biz/eng 路径并行但设计对象不同——biz 设计业务场景，eng 设计平台引擎能力，nfr 设计横切两者的非功能质量约束。两个 Skill 形成设计接力链：
...
| Skill | 输入 | 产出 | 末级节点 | 核心命题 |
|-------|------|------|---------|---------|
| wft01-nfr | NFR OR 条目 | STR-NFR 角色非功能需求分类方案 | 角色非功能需求分类 | 向 NFR 相关方证明"我听懂了你的质量诉求" |
| wft04-nfr | STR-NFR 节点 | SysReq-NFR 架构 + 量化约束 + 操作页面 NFR | 非功能需求分类 | 产出可验证的指标和可消费的约束包 |

> 相关方需求架构文档 `02-*.md` 在 wft01-nfr 产出 STR-NFR 后形成。SysReq-NFR 写入 `05-*.md`，06 详细定义在 wft04-nfr 落账时写入 `06-*.md`。wft04-nfr 对 wft03-biz 的"消费"不是前置输入而是回补/重入输入——wft03-biz 已产出的 SR-F 在收到 NFR 约束包后增量补记验收标准或页面约束。

### 9.1 原始需求处理（wft01-nfr）

**定位**：nfr 路径的第一个 Skill，从零散的 NFR OR 到结构化的角色非功能需求分类方案。核心使命是**向 NFR 相关方证明"我听懂了你的质量诉求"**——识别角色语境、判定 22 分类路径、形成分类需求清单，供 wft04-nfr 展开量化设计。

**输入**：NFR OR 条目（状态 `待处理`）
**产出**：STR-NFR 角色非功能需求分类方案（状态推进至 `可以NFR设计`）
**末级节点**：角色非功能需求分类
...
### 9.2 系统需求开发（wft04-nfr）

**定位**：nfr 路径的第二个 Skill，将 STR-NFR 的定性分类需求转化为系统侧 SysReq-NFR 架构——量化指标 + 分层约束表 + 操作页面 NFR。核心使命是**产出可验证的指标和可消费的约束包**，供 biz/eng 路径和追溯验证消费。

**输入**：STR-NFR 节点（状态 `可以NFR设计` / `待补充NFR设计`）
**产出**：SysReq-NFR 架构 + 量化指标 + 分层约束表 + 操作页面 NFR（STR-NFR 推进至 `已NFR设计`，SysReq-NFR 推进至 `已验证冻结`）
**末级节点**：非功能需求分类
...
| 6 | 装配下游约束包 | 按消费目标组织：wft03-biz NFR 约束包（全局/业务域/SR-F/操作页面约束）+ wft05-eng NFR 约束包（CU/PA组件/共享约束）。约束包只引用分层约束表行 ID 和操作页面 NFR 条目，不复制正文 |
...
OR 按类型分流至三条路径，每条路径内的 Skill 形成串行接力链——上游 Skill 产出节点，下游 Skill 消费节点后展开为下一层节点。每个 Skill 自闭环完成设计→资产对齐→写回，Step 布局参考模板如下：
...
Skill 的入口条件、文档章节布局、操作流程等详细规范见附录A。
...
|------|----------------------|----------------------|---------------------|---------------------|
| 末级节点 | 场景业务（STR-E） | 配置单元（CU） | 操作页面 | 前后端组件 |
| 输入对象 | FR-ENG OR 条目（≤10） | STR-E 节点（≤5） | BA 节点（A1，≤5） | SR-F 构件清单 |
...
> **本附录定位**：定义所有 Skill 的统一框架。每个 Skill 有两面——**静态面**（作为一份文档，有固定的五章结构）和**动态面**（作为运行单元，遵循入口→处理→反馈→生命周期→传播的行为协议）。各 Skill 文档引用本附录对应节次，并补充本 Skill 的特化规则。
>
> **阅读指引**：写 Skill 文档 → §A.2（文档结构）；理解 Skill 如何运行 → §A.3~§A.6（运行时协议）；查格式规范 → §A.7（格式约定）；快速查阅 → §A.8（快速参考）。

### A.1 全景概览

Skill 的静态结构与动态行为之间的关系：

```mermaid
graph TB
  subgraph 静态面["静态面：Skill 作为文档（§A.2）"]
    direction TB
    CH1[第一章·职责定位<br/>负责什么/不负责什么]
    CH2[第二章·领域概念<br/>操作对象是什么]
    CH3[第三章·操作流程<br/>怎么运行]
    CH4[第四章·AI标记约定<br/>引用§A.7]
    CH5[第五章·变更记录<br/>版本历史]
    CH1 --> CH2 --> CH3 --> CH4 --> CH5
  end

  subgraph 动态面["动态面：Skill 作为运行单元（§A.3~§A.6）"]
    direction LR
    ENTER[入口决策<br/>§A.3]
    FEEDBACK[反馈处理<br/>§A.4]
    LIFECYCLE[生命周期<br/>§A.5]
    PROPAGATE[变更传播<br/>§A.6]
...
> **核心原则**：静态面定义"是什么"，动态面定义"怎么运行"，格式约定（§A.7）统一表达方式。阅读时可按需跳转——写 Skill 文档只需 §A.2 + §A.7，理解 Skill 运行逻辑则读 §A.3~§A.6。

---

### A.2 Skill 文档的五章结构（静态面）
...
| 四 | AI 标记约定 | 引用 §A.7 + 本 Skill 示例 | 格式规范，不含操作逻辑 |
...
| **Step Start** | 输入校验→退回优先→反馈处理→新输入路由（见 §A.3） | 校验条件用表格（校验条件 \| 判定 \| 动作） |
| **Step 1 · 设计材料加载** | 加载清单（资产文件 + 用途）+ 按需扩大规则 | 引用文档编写约定的读取/写回约定 |
| **Step 2 · 方案反馈处理** | 引用 91 规范 §A.4 + 本 Skill 特化规则 + 确认状态格式示例 | 通用协议不重复，仅写本 Skill 特化部分 |
| **Step 3~N** | 各 Skill 特化设计步骤 | 按需增减 Step 数量 |
| **Step End · 运行结束输出** | 三区结构（模板见 §A.7.2） | 人类下一步操作提示，不写入资产 |

#### A.2.5 第四章 · AI 标记约定

四类增量标记（`[新增]`/`[变更]`/`[确认]`/`[需裁决]`）的定义及使用规则见 §A.7.1。各 Skill 的第四章引用 §A.7.1，并补充**本 Skill 的增量清单格式示例**（含本层级的完整增量清单），供人类参照。
...
```mermaid
graph TD
    START([Skill 被调用]) --> CHECK1{① 有新输入？<br/>状态符合消费条件}
    CHECK1 -->|是| FLAG1[标记：有新输入]
    CHECK1 -->|否| CHECK2{② 有待处理反馈？<br/>含人类标注但无 已处理}
    CHECK2 -->|是| FLAG2[标记：有待处理反馈]
    CHECK2 -->|否| CHECK3{③ 有下游退回节点？<br/>状态为 需wftXX修订}
    CHECK3 -->|是| FLAG3[标记：有下游退回]
    CHECK3 -->|否| EXIT[输出"无待处理对象"→ 退出]
    ...
    PRIORITY[按优先级排序] --> ORDER{退回优先 → 反馈处理 → 新输入路由}
    ORDER --> PROCESS[进入处理流程]
```
...
| 步骤 | 操作 | 详细说明 |
|------|------|---------|
| **1. 扫描** | 遍历确认状态字段 | 筛选有人类标注（`[同意]`/`[修改]`/`[驳回]`）但无 `[已处理]` 的条目。已有 `[同意]`+`[已处理]` 的默认跳过——除非本轮新输入修订触及其定义/归属，此时先清除 `[已处理]`（保留 `[同意]`），使其回到待处理状态 |
| **2. 收集整体确认** | 处理「整体确认」指令 | 人类说「整体确认」→ 对无条目级标注的剩余条目写入 `[同意]` + `[已处理]` |
| **3. 逐条执行** | 按标注类型处理 | 见下方"分类型处理"表 |
| **4. 级联处理** | 处理变更的连锁影响 | 见下方"级联处理"表 |
| **5. 判定状态** | 决定方案是否可推进 | 全部条目含 `[同意]`+`[已处理]` → 推进至可消费状态；任一条目含 `[修改]`/`[驳回]` 且无 `[需重设计]` → 保持 `待确认方案`；存在 `[需重设计]` 条目 → 保持 `待确认方案`，Step End 时提示该条目需进入设计步骤重做 |
...
graph TD
    ...
    待确认方案 -->|人类确认| 可推进态
    可推进态 -->|下游Skill锁定消费| 锁定态
    锁定态 -->|下游消费完成| 已完成态

    锁定态 -->|下游退回| 待修订态
    已完成态 -->|下游退回| 待修订态
    待修订态 -->|本Skill修订后人类确认| 待确认方案

    可推进态 -.->|上游新输入,版本递增| 可推进态
    锁定态 -.->|上游新输入,版本递增| 锁定态
    已完成态 -.->|上游新输入,需下游补全| 待补充态
    待补充态 -.->|下游补全后| 可推进态
```
...
2. 本 Skill 重入时：读取下游缺失说明 → 按人类改进方案修订 → 人类确认 → 推进回可推进态

> **版本标记与消费追溯**：节点初次生成时版本 = `v1`；每次因上游新输入触发修订，版本号递增。下游 Skill 首次消费时记录 `消费版本：v{N}`，重入时比对版本——若当前版本 > 已消费版本 → 触发变更感知，读取变更影响声明（§A.6），按变更类型处理。
...
| 5 | 第三章 Step 2：引用 §A.4 并声明本 Skill 特化参数 | §A.4 |
| 6 | 第三章 Step End：使用三区模板 | §A.7.2 |
| 7 | 第四章：引用 §A.7.1 + 本 Skill 增量清单示例 | §A.7.1 |
| 8 | 生命周期：引用 §A.5 并声明状态名映射 | §A.5 |
| 9 | 变更传播：引用 §A.6 并补充特有影响范围判定表 | §A.6 |

#### A.8.2 运行时协议速查

| 阶段 | 关键问题 | 答案位置 |
|------|---------|---------|
| 入口 | Skill 什么时候被触发？ | §A.3.2 规则一（三种入口） |
| 入口 | 多个条件同时满足怎么办？ | §A.3.4 规则三（退回 > 反馈 > 新输入） |
| 入口 | 退回节点怎么处理？ | §A.3.3 规则二（先检查改进方案） |
| 处理 | 人类 `[修改]` 后 AI 做什么？ | §A.4.3 步骤 3（理解→执行→检查→标记） |
| 处理 | `[驳回]` 后怎么处理需求缺口？ | §A.4.3 步骤 3（检查缺口→记录→移除→标记） |
| 处理 | 修改触发了级联影响怎么办？ | §A.4.3 步骤 4（需求级联优先→资产对齐级联） |
| 状态 | 节点有哪些状态？ | §A.5.2（五态 + 待补充态） |
| 状态 | 上游变更后节点状态怎么变？ | §A.5.4（按原状态三分流） |
| 传播 | 怎么判断变更类型？ | §A.6.1（决策流程图）+ §A.6.2（四类型判定标准） |
| 传播 | 变更影响怎么写？ | §A.6.3（声明模板） |
| 格式 | AI 怎么标记本轮产出？ | §A.7.1（四类增量标记） |
| 格式 | Step End 输出什么？ | §A.7.2（三区模板） |

---

> 各 Skill 的完整参数（末级节点/输入对象/状态/下游/资产写回/补充材料）定义在其 Skill 文档的第一章和第三章 Step Start 中。全量速查见 §10.5。
