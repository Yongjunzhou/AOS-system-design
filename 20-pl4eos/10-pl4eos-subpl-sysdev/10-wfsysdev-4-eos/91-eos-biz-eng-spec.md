...
> **22 号资产**：即 NFR 分类资产，定义非功能需求的一级类目与分类维度、具体需求模板，是 wft01-nfr 判定分类路径和 wft04-nfr 量化设计、架构组织、资产落账的权威参照。nfr 路径两个 Skill 对 22 的写回见 §10.5 参数表。
>
...
| 6 | 装配下游约束包 | 按约束范围组织为 wft05-eng NFR 约束包：全局约束 / 业务域约束 / SR-F 约束 / 操作页面约束 / CU 约束 / PA 组件约束 / 共享约束。约束包只引用分层约束表行 ID 和操作页面 NFR 条目，不复制正文。NFR 约束仅流向 eng 路径——非功能质量由引擎层保障，biz 路径不消费 |
...
| 你的目标 | 阅读路径 | 预估时间 |
|---------|---------|---------|
| **写一个新的 Skill 文档** | 第一部分（§A.2 五章结构 → §A.3 格式约定）→ 用 §A.8.1 检查清单自检 | ~15 分钟 |
| **理解 Skill 如何运行** | 第二部分（§A.4 入口 → §A.5 反馈 → §A.6 生命周期 → §A.7 变更传播），按顺序阅读 | ~20 分钟 |
...
> **核心原则**：静态面定义"是什么"，动态面定义"怎么运行"，格式约定统一表达方式。写文档时读第一部分，理解运行时读第二部分，需要速查跳 §A.8。

---

# 第一部分：编写 Skill 文档

> **适合谁**：正在编写或维护 Skill 文档的人。其他读者可跳至第二部分。
