...
>
> **版本**：v5.16 | **修订**：2026-08-15 | **说明**：§A.5 预处理链特化（试点 ort01）细化——反馈载体限定文本化文档、以 Step 4/5/6 替代 Step End、Step 5 承担三区摘要输出
...
**配置单元定义口径**：每个 CU 用以下要素定义。**CU 先细分为配置信息组**（每组 = **某组配置字段，这组字段能够提供一个相对独立的业务能力**），**配置操作、系统处理、运行期能力均以配置信息组为单位说明**；配置入口等留在 CU 级。叙事链为"配置人员操作 → 系统处理 → 业务人员操作"。
...
| **wft02-eng** | 设计可配置的 CU——名称级 → 配置单元 BP | STR-E 节点 → A1 BP 配置单元方案 | 配置单元 CU（配置入口、配置要素、系统处理、运行期能力） | wft03-eng |
...
| # | 任务 | 视角 | 说明 |
|---|------|------|------|
| 1 | 装配 A2 产品业务流程上下文 | A2 | 从业务链或 STR-E 中摘取与 CU 推导有关的 A2 信息：业务运行对象、配置/支撑角色、触发事件、业务状态影响；只服务 eng 链 CU 推导，不替代 wft02-biz 的 A2/Bn 输出文档 |
| 2 | 装配 A1 产品业务流程 | A1 | 接收 STR-E → 按主责能力域→能力场景→CU 层级展开；共享能力独立建模，消费方只记录依赖 |
...
| wft02-eng | 配置单元（CU） | 配置入口 / 配置要素 / 系统处理 / 运行期能力 | 装配 A2 产品业务流程上下文 + A1 引擎→CU 层级 + CU 配置能力定义 + 依赖关系——"设计配置单元 BP" | §八 |
...
> 
> **反馈通道**：自然语言对话讨论。人类不直接在方案文档中标注——`确认状态` 字段由 AI 在产出方案时初始化（空），在人类确认后追加 `[已处理]` 作为处理记录，由 AI 维护，人类不编辑。
>
> **预处理链特化（试点：eos-ort01-chunk）**：OR 预处理链的 `eos-ort01-chunk` 不走本节对话反馈模式，而走「**文档编辑 + git**」反馈——人类直接编辑**文本化文档**（方案块/反馈区）表达反馈（切分文件是双方一致后的结果，不是反馈载体）；AI 每次运行开始 `git diff <基线>` 检出变更、判读意图、记录到状态文档表 A `内容摘要`，处理基于记录执行；AI 每轮 `scripts/ai-commit.sh` 提交基线（author=EOS-AI + `[AI]` 前缀 + `HEAD @上次AI运行` 记录）。**交互只记录不处理**——对话反馈在交互结束（「汇总」）统一记录到表 A。本步以 **Step 4 提交基线 / Step 5 反馈交互（含三区摘要输出）/ Step 6 变更汇总** 替代链上通用的「Step End」。详见 eos-ort01-chunk 的 Step 1 变更检测、Step 4~6。其余预处理链 Skill（ort00/02/03）待推广后同步本节。
...
| **确认状态字段** | AI 产出方案时为每个条目初始化空的确认状态字段。人类不操作该字段——AI 在修改完成后追加 `[已处理]` 作为处理标记。具体字段名、写入位置和条目粒度由各 Skill 在第三章 Step 2（方案反馈处理）的特化规则中定义并提供格式示例 |
