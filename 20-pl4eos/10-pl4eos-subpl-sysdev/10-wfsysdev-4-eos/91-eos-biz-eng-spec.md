...
**CU 设计权规则**：CU 是引擎提供的配置能力单元，其能力边界、配置入口、配置要素、系统处理、运行期输出和 FR-ENG 性能指标仅由 eng 路径设计。**wft01-eng 设计 CU 承接（分解概要末级为配置信息组（概要说明） + 分配配置信息组到 CU 承接，含收敛判定）**；**wft02-eng 详细定义 CU（承接已分配 CU，展开配置信息组/配置能力/依赖/状态迁移）**；wft03-eng 将 CU 展开为配置页面和操作活动；wft05-eng 将其展开为软件组件。biz 路径不得新建、修改或详细设计 CU。
...
FR-ENG OR ──→ wft01-eng ──→ STR-E ──→ wft02-eng ──→ BP ──→ wft03-eng ──→ SR-F ──→ wft05-eng ──→ PA
              ...
              （概要末级+配置信息组概要+承接）（详设：配置信息组）    （页面架构+活动）    （前端/后端/平台服务）
```
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明”我听懂了你要的平台能力” | FR-ENG OR 条目 → STR-E 平台能力场景业务方案 | 场景业务（主责能力域 + 概要末级节点 + 配置信息组（概要说明） + CU 承接 + 共享依赖） | 业务配置人员确认 → wft02-eng |
...
| 2 | 分解概要末级为配置信息组（概要说明）+ 分配配置信息组到 CU 承接 | 对称于 STR-F 下辖的输出文档——场景业务由哪些 CU 组成；把概要末级节点按"可以运行的能力"分解为配置信息组（概要说明，不展开三段式详设——那是 wft02-eng 详设），逐组分配 CU 承接（含收敛判定复用/新增/拆分/合并 + 对照 25 盘存的骨架判定） |
...
| 3 | 定义 CU 配置信息组与配置能力 | A1 | 承接 wft01 的配置信息组清单+概要说明，逐组展开配置操作/系统处理/运行期能力，定义配置入口与引擎状态迁移；详设层变更（配置能力扩展）以 `[调整]` 标注；承接缺陷退回 `需wft01修订` |
...
| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 场景业务（STR-E） | 主责能力域 / 概要末级节点 / 配置信息组（概要说明） / CU 承接 / 依赖 STR-E | 基于 FR-ENG OR 推断可独立配置、治理和验收的平台能力场景 + 分解概要末级为配置信息组（概要说明） + 分配配置信息组到 CU 承接——“向平台方证明我听懂了” | §八 |
