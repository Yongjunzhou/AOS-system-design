...
**定位**：biz 路径的第一个 Skill，从零散的 OR 到结构化的场景业务方案。核心使命是**向业务方证明"我听懂了你的业务"**——产出供人类确认理解，而非最终设计方案。
...
| 3 | 判定任务类型与捕获/推断主要节点 | **业务语言**，供人类确认。任务类型三选一：计划（策划→发布）/ 流程（审批流转）/ 工单（派发/处理/验收）。OR 描述的三种处理情形见下文 |
| 4 | 推断任务关系 | **业务语言**。文档间流转顺序 + 任务间触发关系（任意触发模型：任何任务类型均可触发任何任务类型） |
| 5 | 基于 E2E 补充业务需求 | 闭合检查（端到端闭环），检出缺口→生成补充 OR 材料 |
| 6 | 推断正文转化引擎 | 按文档类型匹配引擎（流程/工单/指标/看板/项目/WBS/计划/进展/台账等），全量标注 `[推断]` |
...
|------------|---------|
| OR 明确描述活动节点 | 原样捕获 |
| OR 只提任务模式（如"需要审批"） | 判定类型，节点标 `[待wft02推断]` |
| OR 缺失活动节点 | 推断类型，节点标 `[待wft02推断]` |

> 其中 R4 约束：场景业务中第一个有任务类型的输出文档，其任务类型必然是计划。

> 以上为逻辑罗列，非严格执行顺序。任务 3 是 PL6-A 活动节点的起点——wft01 在业务语言层捕获/推断主要活动节点，供人类确认"你理解的任务流转关系是否正确"。
