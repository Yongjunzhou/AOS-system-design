...
>
> **版本**：v3.18 | **修订**：2026-07-09 | **说明**：eng 路径 Skill 分工链精化——§七渐进链补全"构件"层（场景业务→CU→操作页面→构件→前端+后端）；§7.2 wft02-eng 重写（CU IPO 详细设计 + 配置页+业务页，两类页面下放至此）；§7.3 wft03-eng 重写（操作页面→构件，对称 biz 的功能表单→活动节点）；§7.4 wft05-eng 重写（构件→前后端组件）；§8.3 增补细分节点列。v3.17 的 STR-E 双重身份/一引擎一STR-E/单一角色等基础定义保留。与 wft01-eng v5.0 同步。
...
| **PL**（输出产品视角） | PL0→PL6 | 平台 → 业务域 → 产品类型 → 构件 → WBS → 输出文档 → 活动节点（流程/工单类）/ 指标（看板类） |
| **EL**（引擎视角） | EL1→EL3 | 菜单引擎 → 功能引擎 → 实体引擎 |

> 其余视角层级的完整定义见 `23-eos-output-architecture.md` 和 `24-eos-business-assets.md`。看板与指标的定义见 `28-eos-dashboard-indicator-assets.md`。

**PL6** 为 PL5 输出文档的内部构成，按文档构件类型分为两种：

- **活动节点**（流程类/工单类）：每个活动节点为该文档生命周期中的一个处理步骤，包含处理动作（谁在这一步做什么）和文档状态（该步骤执行后的文档所处状态）。活动节点序列即输出文档的状态变迁路径（如草稿→已提交→审批中→已通过/已驳回）。活动节点的完整定义见 wft02-biz §2.5.1。
- **指标**（看板类）：每个指标为该看板上的一个度量项（PL6），包含指标名称、度量口径、度量目标（引用对应的 PL5 输出文档的功能表单）、聚合维度、显示图示（柱状图/折线图/饼图/仪表盘/数值卡/表格等）和刷新频率。指标序列即该看板的完整度量方案。指标的完整定义见 wft02-biz §2.5.2。

非流程/非工单/非看板类 PL5 文档无 PL6。
...
- **功能引擎**：将物理表转换为功能表单并按主子关系（1+N）组装为主子表单，配置行/列级数据访问权限。承载四方面能力：① 表单能力（结构/字段/布局/校验/行为/发布）；② 功能能力（各具体引擎为表单增加的功能）；③ 组装能力（主子表单组装、子表单显示规则匹配、双击下钻）；④ 权限能力（数据行/列对角色/人员的可见、可编辑或不可访问约束）。
  ...
  - **业务管理类**（项目/WBS/计划/看板/进展/定时任务）：支撑过程管理数据的生成与管理
...
- **功能引擎 CU**（如 @engine-kanban CU）→ 在指标页面上增加指标定义、聚合计算和看板展示能力
...
> **与 biz 路径的对称性**：wft03-eng 的操作页面 ←→ wft03-biz 的功能表单；wft03-eng 的构件 ←→ wft03-biz 的活动节点。两端都是在 SR-F 层将 BA 方案展开为系统功能需求。
...
| Skill | 末级节点 | 所属章 |
|-------|---------|--------|
| wft01-biz | 场景业务 | §六 业务应用系统设计 |
| wft02-biz | 输出文档 | §六 业务应用系统设计 |
| wft03-biz | 功能表单 | §六 业务应用系统设计 |
...
| 操作条目 | 场景职责边界 + PL5 输出文档 | A2/Bn 方案 + 输出文档设计 + 权限 + 对齐结论 | SR-F 功能表单条目 |
| 推进目标 | `可以分解分配` | `可以SR设计` | `待构件需求定义` |
| 需求级联触发 | 场景职责边界变更 / 文档拆分合并 / 文档引擎变更 | 场景职责边界变更 / 输出文档拆分合并 / 文档职责归属变更 | A2 场景流变更 / Bn 产品数据或实体关系变更 / 功能表单引擎或 CU 变更 |
| 资产对齐级联 | → 更新 23B 的 Q 判定及引用 | → 更新 23A/23B/24/26/27/28 引用 | → 更新 05/06 横切权限/菜单 |
...
| 通用态 | wft01-biz（STR-F） | wft02-biz（BA） | wft03-biz（SR-F） |
|--------|-------------------|-----------------|-------------------|
...
| 维度 | wft01-biz（OR→STR-F） | wft02-biz（STR-F→BA） | wft03-biz（BA→SR-F） |
|------|----------------------|----------------------|---------------------|
| 末级节点 | 场景业务 | 输出文档 | 功能表单 |
...
| 资产写回 | 23（Bn 对齐引用） | 23/24/26/27/28 | 04/05/06（BA/SR-F 状态推进） |
