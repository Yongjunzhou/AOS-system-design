...
对应设计路径为 **eng 路径**：wft01-eng（业务流程概要）→ wft02-eng（业务流程详细）→ wft03-eng（系统需求概要）→ wft04-eng（系统需求详细）→ wft05-eng（平台产品架构/三类组件）。
...
对应设计路径为 **biz 路径的前半段**：wft01-biz（业务流程概要定义，场景业务为枝、文档为末级）→ wft02-biz（业务流程详细定义，组装 A2 产品业务流程 + 表单使用顺序）。
...
|-------|------|------|---------|---------|
| **wft01-biz** | FR-BIZ 规范化的相关方需求条目 | 业务流程概要定义——bph-biz 场景业务（枝）+ PL5 输出文档序列（概要末梢）+ Bn 组装（WBS 归组/23 一致性/缺口推断） | 文档（PL5，结构判定级概要说明） | 向业务方证明"我听懂了你的业务" |
| **wft02-biz** | bph-biz 节点 | 业务流程详细定义——A2（角色→场景业务→表单使用顺序）+ Bn（输出文档详细设计 + PL6-A 活动节点需求 + PL6-B 条目需求） | 输出文档（PL5），细分 PL6-A / PL6-B + 表单使用顺序 | 设计正确的方案——业务语言→架构语言 |
| **wft03-biz** | bpd-biz 节点 | 系统需求概要定义——功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 | 功能表单（窗口标签页为页面组件承载） | 产出可交付构件开发的配置业务系统 pa-eng，并向 eng 分配指标 |
...
| **产出物** | 业务流程概要定义（bph-biz）——场景业务 + 输出文档序列 + Bn 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务流程详细定义（BP）——A2 角色场景（功能表单/主子表单 + 表单使用顺序）+ Bn 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统需求概要定义（业务配置需求暨配置业务系统 pa-eng）——功能表单（窗口标签页为页面组件承载）+ 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
| **供谁消费** | 业务方确认 → wft02-biz | wft03-biz | 构件开发流水线 + eng 路径（引擎缺口线索） |

> **概要/详细分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要定义/详细定义」重述——wft01 业务流程概要定义（末级=文档，概要说明为**结构判定级**，字段级详设由 wft02-biz 承接）；wft02 业务流程详细定义（详细条目=**活动 + 正文条目 + 表单使用顺序**）；wft03 系统需求概要定义（末级=**功能表单**，窗口标签页为其页面组件承载形式，2026-08-22 深审统一）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bph-biz、BP→bpd-biz、SR-F→srh-biz；追溯/23 不迁。命名「业务流程详细定义」源于 bpd-biz IPO 承担详细角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 规范化的相关方需求 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
>
> **与 eng 路径的对称性**（详见 §八）：biz 路径的输出文档对应 eng 路径的配置信息组——都是各自产品的 PL5 末级节点；eng 的 CU（PL4）对应 biz 的 WBS（PL4）。wft03-biz 的配置需求方案 ←→ wft03-eng 的系统需求概要、wft04-eng 的系统需求详细，同属系统需求概要层（srh-biz/srh-eng/srd-eng）。
...
```
输入                         产出
FR-BIZ 规范化的相关方需求（待处理）  ──→  业务流程概要定义 bph-biz 场景业务方案（可以分解分配）
...
```
输入                               产出
bph-biz 节点（含 wft01 Bn 组装结论）   业务流程详细定义 bpd-biz 方案（可以srh设计）
...
FR-ENG 规范化的相关方需求节点 ──→ wft01-eng ──→ bph-eng ──→ wft02-eng ──→ bpd-eng ──→ wft03-eng ──→ srh-eng ──→ wft04-eng ──→ srd-eng ──→ wft05-eng ──→ pa-eng
              业务流程概要定义          业务流程详细定义            系统需求概要定义              系统需求详细定义               平台产品架构
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明"我听懂了你要的平台能力" | FR-ENG 规范化的相关方需求节点 条目 → bph-eng 业务流程概要方案 | 配置信息组（概要说明，PL5 末级，业务流程概要定义末级节点；bph-eng 架构末级=场景业务 SL5）+ CU 承接 + 依赖 bph-eng（共享依赖） | 业务配置人员确认 → wft02-eng |
| **wft02-eng** | 详细定义已分配的 CU——操作活动定义/依赖/状态迁移 | bph-eng 节点 → A1 bpd-eng 业务流程详细方案 | 配置信息组（操作活动定义：配置操作/系统处理/运行期能力；CU 为 PL4 选配单元） | wft03-eng |
| **wft03-eng** | 把操作活动定义组织为系统需求概要（功能表单+概要说明） | bpd-eng 节点（A1）→ srh-eng 系统需求概要需求 | 功能表单（页面职责/页面类别/操作活动清单承接 02/页面编排概要） | wft04-eng |
| **wft04-eng** | 展开功能表单上各组件及其业务（系统需求详细） | 系统需求概要 srh-eng → srd-eng 系统需求详细需求 | 组件业务（布局组件/构件 + 组件业务 + pa-eng 层收敛项） | wft05-eng |
| **wft05-eng** | 产出可交付开发的三类组件方案 | 系统需求详细 srd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 → pa-eng 组件需求定义 | pa-eng 组件（前端/后端/平台服务） | 追溯验证 / 构件开发流水线 |
...
**将零散的 FR-ENG 规范化的相关方需求节点 组织为结构化的平台能力场景业务方案（业务流程概要定义，01 层——概要说明为结构判定级，操作活动定义留 02 展开），供业务配置人员确认理解。**
...
| 2 | 名称级识别并分解配置信息组（概要说明）+ 分配配置信息组到 CU 承接 | 对称于 bph-biz 下辖的输出文档——场景业务由哪些 CU 组成；按"可以运行的能力"名称级识别并分解为配置信息组（概要说明，**结构判定级**——配置什么/运行期提供什么，不展开操作活动定义——那是 wft02-eng 业务流程详细定义），逐组分配 CU 承接（含收敛判定复用/新增/拆分/合并 + 对照 25 盘存的骨架判定） |
...
**详细定义 wft01-eng 已分配的 CU——业务流程详细定义（02 层，操作活动定义步骤化三段式：配置操作/系统处理/运行期能力），承接 bph-eng 的 A2 上下文与 A1 装配，展开操作活动定义/依赖/状态迁移。**

```
输入                                           产出
bph-eng 节点                                    A1 配置单元 bpd-eng 业务流程详细方案（可以srh设计）
...
**将系统需求概要 srh-eng 展开为系统需求详细 srd-eng：判定区域内布局组件/构件、定义组件业务（功能级不写实现）、标注 pa-eng 层收敛项。组件业务是 wft05-eng 推断前后端组件的依据。**

```
输入                                           产出
系统需求概要 srh-eng（05-*.md）                    平台系统需求详细 srd-eng（已详细定义）
...
**将系统需求详细 srd-eng（组件业务）、FR-BIZ 指标约束和 NFR 约束展开为 pa-eng 三类组件方案，构件开发流水线可据此进入开发。**（组件业务是推断前后端组件的依据。）

```
输入                                              产出
系统需求详细 srd-eng + FR-BIZ指标约束包 + NFR约束包     pa-eng 组件需求定义（待追溯验证）
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 基于系统需求详细 srd-eng 及约束推断平台架构 | 判断组件属于引擎专属、布局组件扩展、公共构件、公共平台服务或运行支撑模块；同时消费 FR-BIZ 指标约束包和 wft04-nfr 的 wft05-eng NFR 约束包，形成必要的平台架构决策 |
...
|-------|---------|------------|-----------------|---------|
| **wft01-nfr** | 向 NFR 相关方证明"我听懂了你的质量诉求" | 规范化非功能需求条目 → bph-nfr 非功能需求概要定义方案 | 非功能需求概要定义 | NFR 相关方确认 → wft04-nfr |
| **wft04-nfr** | 产出可验证的指标和可消费的约束包 | bph-nfr 节点 → SysReq-NFR 架构 + 量化约束 + 页面 NFR | 非功能需求详细定义 | wft05-eng / 追溯验证 |
...
**定位**：nfr 路径的第一个 Skill，从零散但规范化的 规范化非功能需求 到结构化的非功能需求概要定义方案。核心使命是**向 NFR 相关方证明"我听懂了你的质量诉求"**——识别角色语境、判定 22 分类路径、形成分类需求清单，供 wft04-nfr 展开量化设计。

```
输入                                   产出
规范化非功能需求条目（待处理）  ──→  bph-nfr 非功能需求概要定义方案（可以NFR设计）
                                末级节点：非功能需求概要定义
```
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 推断非功能需求概要定义节点 | 识别 规范化的相关方需求 背后的用户角色、业务域/平台能力语境、NFR 一级类目与分类维度，形成 bph-nfr 末级节点 |
...
| 7 | 闭环检查与资产落账 | 七项闭环检查（量化完整/宿主唯一/分层正确/页面可消费/eng一致/22可落账/06可验证）→22 具体需求写回→06 系统需求详细定义→05/02 状态推进→bph-nfr 消费版本记录 |
...
原始材料 → ort00文本化 → ort01切分 → ort02澄清
          ...
          (bpd-biz 双视角)          (业务流程详细定义)     (SysReq-NFR 量化)
                │                   │                   │
                ▼                   ▼                   │
...
第二轮：eng/nfr 汇合与收尾
  eng 线：wft04-eng 的系统需求详细 srd-eng 基线就绪
  nfr 线：wft04-nfr 完成目标绑定并确认首版 wft05-eng NFR 约束包
  汇合条件满足 → wft05-eng 同时消费系统需求详细 srd-eng、FR-BIZ 指标约束包和 wft05-eng NFR 约束包 → 平台 pa-eng 组件需求定义
```
...
| 依赖关系 | 说明 | 策略 |
|---------|------|------|
| eng 线 wft05-eng ← nfr 线 wft04-nfr | wft05-eng 消费 NFR 约束包 | eng 前四步与 nfr 可并行；首版约束包须在相关 bph-eng/CU/系统需求详细 srd-eng 可定位后完成目标绑定并确认，wft05-eng 在两侧输入同时就绪后启动 |
...
|-------|---------|---------|---------|--------|
| wft01-biz | 文档（PL5，概要末梢） | PL6-A 活动节点 / PL6-B 正文条目初步判定（结构判定级） | 基于 规范化的相关方需求 推断**业务流程概要定义**——场景业务（枝）+ 输出文档序列 + **Bn 组装**（WBS 归组/23 一致性/按构件产品数据清单完整性缺口推断），结构性缺口补充 规范化的相关方需求 回路——"向业务方证明我听懂了" | §七 |
| wft02-biz | 输出文档（PL5） | PL6-A 活动节点 / PL6-B 正文条目 / 表单使用顺序 | 消费 wft01 Bn 组装，生成 A2+Bn 双视角**业务流程详细定义**——引擎校验（权威）+ 文档方案 + 活动设计 + 权限 + 任务关系（架构语言）——"设计方案" | §七 |
| wft03-biz | 功能表单（窗口标签页为页面组件承载） | 引擎能力使用需求 | 消费 wft02 业务流程详细定义 → 生成**系统需求概要定义**——页面组件；分配 FR-BIZ 指标到引擎/CU；输出 FR-BIZ 指标约束包和 FR-ENG 候选线索——"出配置需求暨配置业务系统pa-eng" | §七 |

### 10.4 eng 路径 Skill

| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 配置信息组（概要说明，PL5，业务流程概要定义末级节点） | 主责能力域 / 配置信息组（概要说明） / CU 承接 / 依赖 bph-eng（bph-eng 架构末级=场景业务） | 基于 FR-ENG 规范化的相关方需求节点 推断可独立配置、治理和验收的平台能力场景 + 名称级识别并分解配置信息组（概要说明） + 分配配置信息组到 CU 承接——"向平台方证明我听懂了"（业务流程概要定义，01 层） | §八 |
| wft02-eng | 配置信息组（操作活动定义） | CU（PL4 选配单元）/ 操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式） | 承接已分配 CU，展开操作活动定义/依赖/状态迁移详细定义——"详细定义配置单元"（业务流程详细定义，02 层） | §八 |
| wft03-eng | 功能表单 | 页面类别 / 操作活动清单（承接 02）/ 页面编排概要（区域/TAB/弹窗入口） | 承接操作活动定义，组织功能表单+概要说明——"出系统需求概要 srh-eng"（系统需求概要定义，03 层） | §八 |
| wft04-eng | 功能表单（组件业务） | 布局组件/构件 / 组件业务 / pa-eng 层收敛项 | 承接系统需求概要，展开功能表单上各组件及其业务——"出系统需求详细 srd-eng"（系统需求详细定义，04 层） | §八 |
| wft05-eng | pa-eng 组件 | 前端组件 / 后端组件 / 平台服务组件 | 消费系统需求详细 srd-eng、FR-BIZ 指标约束包和 NFR 约束包，从组件业务推断三类组件及组件架构——"出组件需求"（平台产品架构，05 层） | §八 |

### 10.5 nfr 路径 Skill

| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-nfr | 非功能需求概要定义 | — | 基于 规范化非功能需求 推断非功能需求概要定义节点和需求清单，对齐 22 分类资产，基于需求清单补充 NFR 需求——"向 NFR 相关方证明我听懂了你的质量诉求" | §九 |
| wft04-nfr | 非功能需求详细定义 | 具体非功能需求 | 消费 bph-nfr → 生成量化指标 + NFR 架构 + 分层约束表 + 页面 NFR + 下游约束包 → 闭环检查与资产落账——"产出可验证的指标和可消费的约束包" | §九 |
...
| 维度 | wft01-eng（规范化的相关方需求→bph-eng） | wft02-eng（bph-eng→bpd-eng） | wft03-eng（bpd-eng→系统需求概要 srh-eng） | wft04-eng（系统需求概要→系统需求详细 srd-eng） | wft05-eng（系统需求详细→pa-eng） |
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（概要说明，PL5，业务流程概要定义末级节点） | 配置信息组（操作活动定义） | 功能表单（概要说明） | 功能表单（组件业务） | pa-eng 组件（前端/后端/平台服务） |
| 输入对象 | FR-ENG 规范化的相关方需求节点 条目（≤10） | bph-eng 节点（≤5） | bpd-eng 节点（A1，≤5） | 系统需求概要 srh-eng（≤5） | 系统需求详细 srd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
...
|------|--------------------------|----------------------------------|
| 末级节点 | 非功能需求概要定义 | 非功能需求详细定义 / 具体非功能需求 |
...
| 04 | `04-eos-business-detailed.md` | 业务流程详细 |
| 05 | `05-eos-system-requirement-summary-architecture.md` | 系统需求概要架构 |
| 06 | `06-eos-system-requirement-detailed.md` | 系统系统需求详细 |
...
| 2026-08-24 | v5.41 | 接力链表 wft04 行 typo 修复（wft04-eng 深审，人类逐项裁决）——§10.6「系统需求概要 srh-eng → srh-eng 系统需求详细需求」→「srh-eng → srd-eng 系统需求详细需求」。 |
