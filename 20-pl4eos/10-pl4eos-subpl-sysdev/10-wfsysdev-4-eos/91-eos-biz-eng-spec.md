...
>
> **版本**：v4.8 | **修订**：2026-07-11 | **说明**：§七~§九 全章重构（产出物链清晰化 + 双表合一 + NFR→eng 单向流 + 平台服务组件）+ 附录A 重构为四部分结构 + §10.4/§10.6 参数对齐 + §十一 新增产品文档结构、读写及反馈（就绪队列、子条目状态、变更感知、HEAD 追踪）+ 附录A 读取/写回协议全量同步 + 全文缺陷修复。
...
| **R3** | 产品正文块内部按构件→WBS→输出文档→节点逐层用 `####`/`#####` 展开 |
...
每个产品数据文档在就绪队列之前包含一个 `## 最近变更` 节，记录 AI 最近对该文档的写回操作。人类打开文档后可快速了解 AI 最近做了什么、影响到哪些节点。
...
- 由 AI 在每次写回时（§11.11）在文件头的 `## 最近变更` 表格中追加一行。
- 保留最近 30 条变更。超出时从最旧的行开始滚动删除。
- 人类不直接修改此表。AI 在写回时自动维护。

| 规则 | 说明 |
|------|------|
| **R7** | 每次写回完成后必须追加一条最近变更记录。变更记录的 `节点 ID` 列必须包含到对应节点的文档内超链接 |
| **R8** | 最近变更记录保留最近 30 条。溢出时删除最旧记录 |

---

### 11.7 就绪队列

就绪队列按 Skill 名称分节列出所有当前可被处理的节点，由各 Skill 写回时自动维护。
...
| 规则 | 说明 |
|------|------|
| **R9** | 就绪队列只列出节点级条目。子条目状态通过父节点 `**子条目状态**` 汇总，不单独列出 |
| **R10** | 同一节点同时满足多个分节条件时，按优先级取最高一节（ort03 > biz > eng > nfr > 反馈 > 退回） |

> 就绪队列的分节由 §10.6 定义的输入合法状态驱动。
...
AI 通过 `git diff`（§11.8）检测到此类变更后，尝试理解意图并标记 `[需确认]`，提交人类审核。
...
| 规则 | 说明 |
|------|------|
| **R11** | 人类优先在 `**确认状态**` 字段中用结构化标注反馈。非结构化反馈经 AI 判定后标记 `[需确认]` |
| **R12** | 子条目标注优先级高于父节点标注。父节点标注多条子条目时，子条目标注各自独立处理 |
| **R13** | 所有标注的处理结果（`[已处理]`）由 AI 在反馈处理后写入，人类无需手动标记 |
...
**入口 A：就绪队列（用于 Step Start 入口判定）**

AI 判定"有什么可以处理"时，读就绪队列：
...
| 规则 | 说明 |
|------|------|
| **R14** | AI 不得遍历索引表以外的区域搜索节点。索引表是发现节点存在的唯一入口 |
| **R15** | 索引表不存储行范围。节点行号通过 `grep -n '@{节点ID}'` 实时定位 |
...
3. 无变更 → 正常流程
   有变更 → 解析变更行号，读变更行附近节点块（上下 5 行）
         → 判定变更类型：
            ...
            - 自由文本 → 尝试理解意图，标记 `[需确认]`
            - 格式/排版 → 忽略
         → 将存在反馈的节点加入就绪队列"待反馈处理"分节
```

| 规则 | 说明 |
|------|------|
| **R16** | `**HEAD @上次 AI 运行**` 由 AI 自动更新为当前提交 hash。人类无需手动修改 |
| **R17** | 若 HEAD 字段不是最近的 AI 提交，AI 在 Step Start 中自动修正 |
| **R18** | 非标反馈经 AI 判定后标记 `[需确认]` |
...
| 操作 | 同时维护 |
|------|---------|
| 新增节点 | 索引表加入节点 ID；就绪队列对应分节加入节点 |
| 修改节点状态 | 就绪队列对应分节加入或移出节点 |
| 删除节点 | 索引表和就绪队列移除节点 ID |
| 新增/修改/删除子条目 | 更新父节点 `**子条目状态**` 汇总值 |
| 人类标注确认状态 | 子条目更新同步父节点 `**子条目状态**`；父节点标注更新 `**确认状态**`；就绪队列"待反馈处理"同步 |
...
| 规则 | 说明 |
|------|------|
| **R19** | 写回必须在本 Skill 同一运行步骤内完成全部更新（索引表 + 就绪队列 + 子条目状态 + 文件头字段） |
| **R20** | 节点 ID 是定位节点的唯一标识。不使用行号，不依赖文件的行序号 |
| **R21** | 每次写回后必须提交。未提交的写回无法被下次 git diff 检测到 |
