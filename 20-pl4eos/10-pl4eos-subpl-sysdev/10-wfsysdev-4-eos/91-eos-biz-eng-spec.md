...
>
> **版本**：v4.4 | **修订**：2026-07-11 | **说明**：§十→附录A（规范性附录）——从正文移至附录，理由：内容为 Skill 框架元约定而非 EOS 领域知识，仅 Skill 作者需按需查阅，不打断 §一~§九正文叙事弧线。全文交叉引用同步更新。
...
| 3 | 判定任务类型与捕获/推断主要节点 | **业务语言**，供人类确认。任务类型三选一：计划（策划→发布）/ 流程（审批流转）/ 工单（派发/处理/验收）。第一个有任务类型的文档必然是计划（R4）。OR 明确描述→原样捕获；OR 只提模式→判定类型标 `[待wft02推断]`；OR 缺失→推断类型标 `[待wft02推断]` |
...
> wft03-biz 的产出是各层**配置需求**（描述在现有或新增引擎上如何配置实现方案），而非直接生成 SR-F 功能表单定义。引擎开发完成后按配置需求进行配置实现。
...
> **与 biz 路径的对称性**：eng 路径的 CU 对应 biz 路径的输出文档——都是各自产品的 PL5 末级节点。wft03-eng 的操作页面 ←→ wft03-biz 的配置需求方案，同属 SR-F 层。
...
> 本章定义 EOS 平台系统设计的 Skill 体系——三路径（biz/eng/nfr）自闭环模型及各 Skill 的职责定位和链路关系。每个 Skill 的内部操作流程和约定见附录A。
...
> 各 Skill 的完整参数（末级节点/输入对象/状态/下游/资产写回/补充材料）定义在其 Skill 文档的第一章和第三章 Step Start 中。全量速查见 §9.5。
