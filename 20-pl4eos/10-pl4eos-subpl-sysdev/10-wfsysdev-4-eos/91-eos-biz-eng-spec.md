...
**CU 设计权规则**：CU 是引擎提供的配置能力单元，其能力边界、配置入口、配置要素、系统处理、运行期输出和 FR-ENG 性能指标仅由 eng 路径设计。**wft01-eng 设计 CU 承接（分解概要末级为配置信息组（概要说明） + 分配配置信息组到 CU 承接，含收敛判定）**；**wft02-eng 详细定义 CU（承接已分配 CU，展开操作活动定义/依赖/状态迁移）**；wft03-eng 将 CU 操作活动组织为功能概要；wft04-eng 展开功能表单上组件业务并标注 PA 层收敛项；wft05-eng 将其展开为软件组件。biz 路径不得新建、修改或详细设计 CU。
...
>
> **与 eng 路径的对称性**（详见 §八）：biz 路径的输出文档对应 eng 路径的 CU——都是各自产品的 PL5 末级节点。wft03-biz 的配置需求方案 ←→ wft03-eng 的功能概要、wft04-eng 的功能详细，同属 SR-F 层。
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明"我听懂了你要的平台能力" | FR-ENG OR 条目 → STR-E 业务概要方案 | 场景业务（主责能力域 + 概要末级节点 + 配置信息组（概要说明，结构判定级） + CU 承接 + 共享依赖） | 业务配置人员确认 → wft02-eng |
| **wft02-eng** | 详细定义已分配的 CU——操作活动定义/依赖/状态迁移 | STR-E 节点 → A1 BP 业务详细方案 | 配置单元 CU（配置信息组 + 操作活动定义：配置操作/系统处理/运行期能力） | wft03-eng |
...
>
> **与 biz 路径的对应关系**（详见 §七）：eng 路径的 CU 对应 biz 路径的输出文档（同为 PL5 细分对象）；wft03-eng 的功能表单是平台软件 SR-F 概要，wft04-eng 的组件业务是平台软件 SR-F 详细，wft03-biz 的功能表单和窗口标签页兼具业务配置需求与配置业务系统 PA 身份。二者同写入 05，但节点类型、使用角色、终态和下游不同。
...
FR-ENG OR 条目（待处理）  ──→  STR-E 平台能力场景业务方案（可以详细定义）
                               末级节点：场景业务（SL5 / PL5）
                               细分：概要末级节点 + 配置信息组（概要说明，结构判定级） + CU 承接
```
...
（可以详细定义 / 待补充详细定义）  ──→         末级节点：配置单元（CU），定义见 §3.3 和 §4.3
                                              细分：配置信息组 + 操作活动定义（配置操作/系统处理/运行期能力）
```
...
| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 场景业务（STR-E） | 主责能力域 / 概要末级节点 / 配置信息组（概要说明，结构判定级） / CU 承接 / 依赖 STR-E | 基于 FR-ENG OR 推断可独立配置、治理和验收的平台能力场景 + 分解概要末级为配置信息组（概要说明） + 分配配置信息组到 CU 承接——"向平台方证明我听懂了"（业务概要定义，01 层） | §八 |
| wft02-eng | 配置单元（CU） | 配置信息组 / 操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式） | 承接已分配 CU，展开操作活动定义/依赖/状态迁移详细定义——"详细定义配置单元"（业务详细定义，02 层） | §八 |
...
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 场景业务（STR-E） | 配置单元（CU） | 功能表单（概要说明） | 功能表单（组件业务） | PA 组件（前端/后端/平台服务） |
