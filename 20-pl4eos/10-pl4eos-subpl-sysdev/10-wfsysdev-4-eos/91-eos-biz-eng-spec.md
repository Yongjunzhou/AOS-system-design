...
>
> **版本**：v4.2 | **修订**：2026-07-10 | **说明**：恢复 PL6 产品架构层级——PL6-A 活动节点（正式文档任务的活动节点序列，跨 wft01~03 逐级展开）+ PL6-B 正文条目（清单类文档的独立条目）。PL 视角 PL0→PL5 扩展为 PL0→PL6。
...
**清单类文档**的正文本质是一个"调度容器"——正文中的每个条目具有独立的名称、独立的定义要素和独立的设计生命周期，是真正有独立业务含义的设计对象。

清单类文档的判定标准：正文条目是否满足以下三个条件——① 每个条目有独立的名称；② 每个条目有独立的定义要素（如指标的口径/维度、看板的布局/指标集）；③ 每个条目的生命周期独立于容器文档（可独立创建/修订/废弃）。

```
PL5 输出文档（清单类：指标清单/看板清单/计划清单等）
  │
  ├── 封面实体（同 §2.2）
...
**适用**：指标清单（每个指标条目）、看板清单（每个看板条目）、计划清单（每条计划条目）。

**不适用**：事务类文档（采购申请单的物料行——运行时交易数据）、内容类文档（会议纪要的议题条目——内容实例）。轻量文档无封面无任务类型，正文即是全部内容，不展开 PL6-B。
...
| **R7** | 正式文档有任务类型时，必须展开 PL6-A 活动节点——wft01 在业务语言层捕获/推断主要节点，wft02 转化为引擎 CU 候选并补全系统节点/异常路径，wft03 生成 CU 配置 |
| **R8** | 清单类文档（指标清单/看板清单/计划清单等）的正文条目展开为 PL6-B——每个条目需满足三个条件：独立名称 + 独立定义要素 + 独立生命周期。事务类文档（采购申请单物料行）和内容类文档（会议纪要议题）不展开 PL6-B |

### 2.6 任务类型与任务关系

**任务类型三选一**（仅正式文档）：每个正式文档可配置零个或一个任务类型——**计划**（自启动，策划→发布）、**流程**（审批流转）、**工单**（派发/处理/验收）。轻量文档无任务类型。
...
**PL6 活动节点/正文条目**是产品架构的真正末级——PL5 之下设 PL6，分为两个子类型：
- **PL6-A 活动节点**：正式文档有任务类型时，任务的活动节点序列构成 PL6-A——wft01 在业务语言层捕获/推断主要节点，wft02 转化为引擎 CU 候选并补全系统节点/异常路径，wft03 生成 CU 配置。跨三个设计阶段逐级展开，是架构节点而非运行时配置
- **PL6-B 正文条目**：仅清单类文档（指标清单/看板清单/计划清单等）展开——正文中的每个条目具有独立名称、独立定义要素和独立生命周期。事务类文档的物料行（运行时交易数据）和内容类文档的议题条目（内容实例）不展开 PL6-B


> **"输出文档"的指代**：本文档及 biz 路径各 Skill 中提及的"输出文档"，均指代文档的**内容本身**而非其容器或文件名。例如"相关方需求文档"指需求条目本身，"采购履约看板"指该看板的定义与布局本身。看板上的每个具体看板/指标为该清单类文档的正文内容条目，在 PL6-B 层展开为独立设计对象。文档的承载表单（如 @engine-kanban）仅为组织容器，不影响每个条目作为独立 PL6 节点的地位。
...
- **功能引擎**：将物理表转换为功能表单并按主子关系（1+N）组装为主子表单，配置行/列级数据访问权限。承载四方面能力：① 表单能力（结构/字段/布局/校验/行为/发布）；② 功能能力（各具体引擎为表单增加的功能）；③ 组装能力（主子表单组装、子表单显示规则匹配、双击下钻）；④ 权限能力（数据行/列对角色/人员的可见、可编辑或不可访问约束）。
  - **产品数据类**（流程/工单/台账）：支撑产品数据文档的开发与管理——流程引擎管理审批流转，工单引擎管理派发/处理/验收/关闭（亦支撑风险与问题管理），台账引擎将物理表转换为台账功能表单并按主子关系装配为主子功能表单（用于设备台账等台账类业务）。封面-正文 1:1 绑定由功能引擎的组装能力统一完成，不设独立文档引擎。
...
**wft02-biz** 将 STR-F 的场景业务展开为 BA 方案——A2 角色场景视角 + Bn 输出产品视角。职责：生成 A2 产品业务架构，推断生成 Bn 产品业务架构；消费 wft01-biz 的任务类型、节点和任务关系（业务语言）→转化为引擎语言；展开 PL6-A 活动节点（补全系统节点/异常路径）并判定 PL6-B 正文条目（清单类文档）；推断输出文档的封面字段/正文结构/正文转化引擎/支持信息引用关系；基于 Bn 构件的 WBS 补充业务需求。D 和 PCA 处理逻辑完全同构。
...
6. 展开 PL6-A 活动节点（仅正式文档有任务类型时）——消费 wft01-biz 的任务类型与主要活动节点（业务语言），转化为引擎 CU 候选（引擎语言）：
   ...
   - 全部活动节点标注 `[推断]`
7. 检测并展开 PL6-B 正文条目——判定输出文档是否为清单类文档（指标清单/看板清单/计划清单等）：每个条目是否有独立名称+独立定义要素+独立生命周期 → 是则逐条目展开为 PL6-B 节点。事务类文档（采购申请单物料行）和内容类文档（会议纪要议题）不展开
8. 任务关系引擎化（仅正式文档）——消费 wft01-biz 的任务关系（业务语言），转化为引擎可执行的触发规则：
   - 文档间流转顺序 → 输出文档序列编排
   - 三角互触发（计划→流程/工单、流程→工单/计划、工单→流程/计划）→ 各引擎的任务间触发规则配置
   - 含触发条件（计划完成/流程到达某节点/工单验收通过）和触发后的任务启动参数

> **业务语言 → 引擎语言的转化**：wft01-biz 用业务方听得懂的话描述活动节点和关系，wft02-biz 将其转化为引擎能执行的 CU 规格，并检测清单类文档展开 PL6-B 正文条目。
