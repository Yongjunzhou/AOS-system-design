...
> nfr 路径将非功能原始需求（NFR 规范化需求）逐级转化为可验证的量化指标和可消费的约束包。与 biz/eng 路径并行但设计对象不同——biz 设计业务场景，eng 设计平台引擎能力，nfr 记录横切业务与平台语境的非功能质量约束。这里的”横切”描述影响语境，不代表存在两个设计消费方；正式 **wft05-eng NFR 约束包**（由 wft04-nfr 产出、供 wft05-eng 消费，下称 NFR 约束包）统一由 wft05-eng 消费。两个 Skill 形成设计接力链：

```
NFR 规范化需求 ──→ wft01-nfr ──→ bph-nfr ──→ wft04-nfr ──→ SysReq-NFR
            角色NFR分类              量化指标+页面 NFR
            定性→结构化              "指标可验证了"
```
...
|-------|---------|------------|-----------------|---------|
| **wft01-nfr** | 向 NFR 相关方证明"我听懂了你的质量诉求" | NFR 规范化需求条目 → bph-nfr 角色非功能需求分类方案 | 角色非功能需求分类 | NFR 相关方确认 → wft04-nfr |
...
**定位**：nfr 路径的第一个 Skill，从零散但规范化的 NFR 规范化需求 到结构化的角色非功能需求分类方案。核心使命是**向 NFR 相关方证明"我听懂了你的质量诉求"**——识别角色语境、判定 22 分类路径、形成分类需求清单，供 wft04-nfr 展开量化设计。

```
输入                                   产出
NFR 规范化需求条目（待处理）  ──→  bph-nfr 角色非功能需求分类方案（可以NFR设计）
                                末级节点：角色非功能需求分类
```
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 推断角色 NFR 分类节点 | 识别 规范化需求 背后的用户角色、业务域/平台能力语境、NFR 一级类目与分类维度，形成 bph-nfr 末级节点 |
...
| 4 | 基于需求清单补充 NFR 需求 | 闭合检查——检出分类缺口、适用范围缺口、验证语义缺口→生成补充 NFR 规范化需求 材料，登记到 材料状态表 |
...
第一轮：三路径并行启动
  ...
  nfr 线：NFR 规范化需求    → wft01-nfr → wft04-nfr 草案（全局/共享约束可先设计，目标级约束待绑定）
...
| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-nfr | 角色非功能需求分类 | — | 基于 NFR 规范化需求 推断角色 NFR 分类节点和需求清单，对齐 22 分类资产，基于需求清单补充 NFR 需求——"向 NFR 相关方证明我听懂了你的质量诉求" | §九 |
...
| 输入对象 | NFR 规范化需求条目（≤10） | bph-nfr 节点（≤5） |
...
| 补充材料 | 补充 NFR 规范化需求 材料 | 上游退回说明 / 指标待确认项 |
