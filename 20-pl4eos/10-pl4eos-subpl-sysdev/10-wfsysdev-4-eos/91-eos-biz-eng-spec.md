...
> **概述**：定义 EOS 平台的 UI 交互模型（含六种页面形态）、输出文档模型（正式/轻量文档、封面-正文 1:1 组装）、引擎分层体系与 CU 配置单元模型、页面组件与构件体系、三类产品（A1/A2/Bn）、资产治理业务，以及三路径（biz/eng/nfr）的职责定位和协作关系。
>
> **版本**：v5.3 | **修订**：2026-07-13 | **说明**：明确正式/轻量是 PL5 封面模式、PL6-B 是正文条目分解，两者正交；彻底删除 wft03-biz 生成 CU 配置的旧语义。
...
页面形态描述页面在窗口中的布局和交互范围，不决定页面属于 biz 或 eng。功能表单、业务节点操作页面和平台操作页面均从以下六种有限形态中选择：
...
| 页面用途 | 所属路径 | 使用角色 | 定义 | 下游身份 |
|---------|---------|---------|------|---------|
| **业务应用页面** | biz | 业务角色 | 承载业务数据和业务活动；包括功能表单与业务节点操作页面 | 05 中的业务配置需求，同时是配置业务系统的 PA 末级节点；状态=`可构件开发` |
| **平台操作页面** | eng | 配置、治理、运行支撑角色 | 承载 CU 配置、校验、发布、装载、监控、回滚和异常处理 | 05 中的平台软件 SR-F；状态=`待产品架构`，由 wft05-eng 消费 |

“操作页面”不得作为无路径限定的架构节点名称。biz 使用“业务节点操作页面”，eng 使用“平台操作页面”；仅在同时覆盖两类页面的 UI 通用规则中使用“页面”作为统称。
...
> **核心结论**：文档不是引擎概念，而是**实体组装模式**——封面实体（DocumentCover 表一行，全系统一张表）+ 正文实体（1:1）+ 支持信息（→其他文档的多对多引用）+ 处理配置。wft02-biz 定义业务活动节点及引擎能力候选；wft03-biz 生成功能表单、业务节点操作页面和引擎能力使用需求，不设计 CU。封面和正文都是普通实体，由实体引擎生成物理表；正文被各引擎按需转化为功能表单，封面由功能引擎的表单能力转化为封面表单，两者由功能引擎的组装能力按 1:1 主子关系组装。因此**不设独立的"文档引擎"**——文档是实体组装模式，而非独立引擎概念。台账类业务由台账引擎（`@engine-ledger`）支撑——将物理表转换为台账功能表单并按主子关系装配为主子功能表单（详见 §3.2）。

### 2.1 封面判定：正式文档与轻量文档

文档是否需要封面，取决于它是否有**独立于正文 CRUD 的生命周期**——需要审批流转、版本追溯、派发验收或独立状态变迁的文档，才有封面。

> **与 PL6-B 的正交关系**：正式/轻量描述的是 PL5 输出文档是否有封面及文档级生命周期；PL6-B 描述的是正文内部是否存在可独立定义和维护的条目类型。轻量文档并不天然产生或排斥 PL6-B：轻量文档正文若只是单一原子记录，其正文行就是 PL5 文档实例，不再重复下沉为 PL6-B；只有正文作为集合容器，且其中的条目类型满足“独立名称、独立定义要素、独立生命周期”时，才展开 PL6-B。PL6-B 条目仍属于父 PL5 正文，不因独立维护而自动升级为正式文档。
...
| **R6** | 有任务类型时，必须展开 PL6-A 活动节点——wft01-biz 在业务语言层识别主要节点；wft02-biz 定义处理角色、业务动作、状态迁移、异常路径和引擎能力候选；wft03-biz 将活动节点转化为业务节点操作页面，并形成对既有引擎/CU的能力使用需求。biz 路径不得设计 CU 本体。正式文档的活动节点附着于封面行，轻量文档的活动节点附着于正文实体 |
...
- **PL6-A 活动节点**：有任务类型时，任务的活动节点序列构成 PL6-A——wft01-biz 在业务语言层识别主要节点，wft02-biz 定义角色、动作、状态迁移、系统节点和异常路径，wft03-biz 生成业务节点操作页面及引擎能力使用需求。正式文档的活动节点附着于封面行，轻量文档的活动节点附着于正文实体。跨三个设计阶段逐级展开，是架构节点而非运行时配置
...
**CU 设计权规则**：CU 是引擎提供的配置能力单元，其能力边界、配置入口、配置要素、系统处理、运行期输出和 FR-ENG 性能指标仅由 eng 路径设计。wft02-eng 正式定义 CU；wft03-eng 将 CU 展开为平台操作页面和操作活动；wft05-eng 将其展开为软件组件。biz 路径不得新建、修改或详细设计 CU。

**引擎能力使用需求**：biz 路径只描述业务应用页面为了实现业务 IPO，需要使用哪些既有引擎/CU能力，以及对应输入、期望输出、业务规则和 FR-BIZ 性能指标。引擎能力使用需求不改变 CU 定义。wft03-biz 对每项使用需求执行能力覆盖判定：已覆盖则引用既有 CU；指标不足或能力缺失则生成 FR-ENG 候选线索，经人类确认和 ort00→ort03 后进入 eng 路径。

**配置单元选配模型**：每个引擎包含若干配置单元（可独立选配），部分配置单元即可让业务运行——引擎提供能力池，配置人员按业务需要从中挑选组合。

**配置单元的功能本质**：CU（配置单元）是在实体页面（§1.2 的六种页面形态之一）上增加一组业务操作能力。业务配置人员打开 CU 对应的平台操作页面，通过页面上的功能按钮编辑各配置要素，提交后系统校验信息的正确性和完备性，通过后保存配置信息。引擎按 CU 定义生成运行期制品（物理表、功能表单、菜单入口），配置人员或业务使用人员即可使用该制品及其功能。

具体而言：
- **实体引擎 CU** → 定义实体及字段，在页面形态①/②/④/⑥上提供增删改查能力。封面实体（DocumentCover 表）和正文实体均由此类 CU 定义
...
> 本章定义业务应用页面与平台操作页面共享的页面组件体系、构件分类，及其与引擎配置单元（CU）的对应关系。页面组件是平台 UI 的结构层次，构件是安装在组件上的可复用功能单元。页面形态是组件的组装模式，组件是构成页面形态的基础单元。

| 对象 | biz 路径权限 | eng 路径权限 |
|------|-------------|-------------|
| 页面形态 | 为业务应用页面选择形态并定义需求 | 为平台操作页面选择形态并完成系统设计 |
| 页面组件 | 描述布局和交互需求 | 设计平台操作页面的页面功能架构 |
...
**与 §1.2 页面形态的正交关系**：§1.2 的六种页面形态是本表组件的**组装模式**——页面形态决定了用哪些组件、如何组合：
...
同一页面组件可出现在不同页面形态中，页面形态决定了组件的组合方式和布局——两者正交。

### 4.2 构件

**构件**是安装在页面组件上的可复用功能单元。同一构件在不同页面组件上提供语义相同、形态适配的功能。构件由引擎按 CU 定义生成并渲染到目标页面组件上。
...
**运行时**：引擎按 CU 定义生成运行期制品——在指定的业务应用页面或平台操作页面组件上渲染构件及其交互行为。
...
→ 形成 FR-ENG 候选线索。候选线索不是正式 OR，不得直接进入 wft01-eng；须经人类确认后作为原始需求材料登记，并完整通过 ort00→ort01→ort02→ort03，形成正式 FR-ENG OR 后，才按 eng 路径（§八）推进——wft01-eng 判定归属引擎 → wft02-eng 设计 CU → wft03-eng 设计平台操作页面与构件 → wft05-eng 展开三类组件。
...
对应设计路径为 **eng 路径**：wft01-eng（场景业务）→ wft02-eng（配置单元）→ wft03-eng（平台操作页面）→ wft05-eng（三类组件）。

> **STR-E 的双重身份**：wft01-eng 产出的 STR-E 在 A2 视角下是流程与IT域中**业务配置人员**的 PL5 场景业务；在 A1 视角下，它对应某个主责能力域中的可独立配置、治理和验收的能力场景。主责能力域可以是引擎，也可以是平台共享能力域。一个能力域可包含多个 STR-E；多个引擎可消费同一共享 STR-E，但不得重复定义共享 CU。CU 是 STR-E 的细分节点。
...
| **wft03-biz** | BA 节点 | 业务应用页面方案——功能表单 + 业务节点操作页面 + 引擎能力使用需求 + FR-BIZ 指标约束包 + 引擎能力缺口线索 | 功能表单 / 业务节点操作页面 | 产出可交付构件开发的配置业务系统 PA，并向 eng 分配指标 |
...
| **产出物** | 场景业务方案（STR-F）——场景业务 + 输出文档序列 + PL6-A 活动节点与 PL6-B 正文条目初步判定 | 业务架构方案（BA）——A2 角色场景（功能表单/主子表单）+ Bn 输出产品（输出文档定义 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 业务配置需求暨配置业务系统 PA——功能表单 + 业务节点操作页面 + 引擎能力使用需求 + FR-BIZ 指标约束包 + 引擎能力缺口线索 |
...
| 7 | 判定并定义 PL6-B 正文条目需求 | Bn | 先判定正文是否为集合容器，再对其中的条目类型执行三独立判定。轻量文档的原子正文行不得重复作为 PL6-B；满足条件的条目逐类定义名称、定义要素和生命周期，并继承正文支撑引擎 |
...
> 将 BA 方案展开为功能表单和业务节点操作页面，并形成引擎能力使用需求与 FR-BIZ 指标分配。功能表单和业务节点操作页面既是 EOS 的业务配置需求，也是配置生成的业务系统 PA 末级节点；biz 路径在 wft03-biz 结束，不再设置 wft05-biz，也不在 07 重复定义。
...
| 5 | 业务节点操作页面序列 | @engine-flow / @engine-task / @engine-workorder | 消费 wft02 的 PL6-A 活动节点需求定义，编排为业务节点操作页面序列（审批页 → 派发页 → 验收页等），定义页面间流转规则、页面交互和 FR-BIZ 指标 |
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明“我听懂了你要的平台能力” | FR-ENG OR 条目 → STR-E 平台能力场景业务方案 | 场景业务（主责能力域 + CU 候选 + 共享依赖） | 业务配置人员确认 → wft02-eng |
| **wft02-eng** | 设计可配置的 CU——名称级 → 配置单元 BA | STR-E 节点 → A1 BA 配置单元方案 | 配置单元 CU（配置入口、配置要素、系统处理、运行期能力） | wft03-eng |
| **wft03-eng** | 把 CU 配置能力翻译成平台操作页面系统需求 | BA 节点（A1）→ SR-F 引擎功能需求 | 平台操作页面（页面功能架构 + 操作活动） | wft05-eng |
| **wft05-eng** | 产出可交付开发的三类组件方案 | 平台操作页面 SR-F + FR-BIZ 指标约束包 + NFR 约束包 → PA 组件需求定义 | PA 组件（前端/后端/平台服务） | 追溯验证 / 构件开发流水线 |
...
>
> **与 biz 路径的对应关系**（详见 §七）：eng 路径的 CU 对应 biz 路径的输出文档（同为 PL5 细分对象）；wft03-eng 的平台操作页面是平台软件 SR-F，wft03-biz 的功能表单和业务节点操作页面兼具业务配置需求与配置业务系统 PA 身份。二者同写入 05，但节点类型、使用角色、终态和下游不同。

---

### 8.1 原始需求处理（wft01-eng）

**将零散的 FR-ENG OR 组织为结构化的平台能力场景业务方案，供业务配置人员确认理解。**

```
输入                              产出
FR-ENG OR 条目（待处理）  ──→  STR-E 平台能力场景业务方案（可以分解分配）
                               末级节点：场景业务（SL5 / PL5）
                               细分：CU 候选（名称级）
```
...
| 4 | 建立追溯链 | FR-ENG OR → STR-E主责能力域 → CU候选/依赖STR-E；共享能力记录提供能力、消费方和复用范围 |
...
| 2 | 装配 A1 产品业务架构 | A1 | 接收 STR-E → 按主责能力域→能力场景→CU 层级展开；共享能力独立建模，消费方只记录依赖 |
...
**将 CU 配置能力定义展开为平台操作页面 SR-F：先推断平台操作页面，再组织页面功能架构，最后定义页面操作活动。**

```
输入                                           产出
BA 节点（A1）                                平台功能 SR-F 需求（待产品架构）
（可以SR设计 / 待补充SR设计）  ──→            末级节点：平台操作页面
                                              细分：页面形态 / 页面组件 / 操作活动
```
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 为 CU 推断平台操作页面 | 基于 CU 的配置入口、配置要素、系统处理和运行期能力，确定配置维护页、配置管理页、发布生效页、运行支撑页、治理追溯页及必要弹窗 |
| 2 | 基于平台操作页面生成功能架构 | 将页面按页面形态、页面组件、TAB、主子关系、跳转、上下文传递、权限和状态组织成页面功能架构 |
| 3 | 为平台操作页面推断操作活动 | 定义页面上的操作活动、入口构件线索、前置条件、系统处理摘要、结果状态、校验规则、异常分支和 PA 层收敛项 |

### 8.4 产品架构开发（wft05-eng）

**将平台操作页面 SR-F、操作活动、页面组件/构件线索、FR-BIZ 指标约束和 NFR 约束展开为 PA 三类组件方案，构件开发流水线可据此进入开发。**（eng 路径跳过 wft04——wft04 为 nfr 路径的系统需求开发 Skill，见 §九。）

```
输入                                              产出
平台操作页面 SR-F + FR-BIZ指标约束包 + NFR约束包     PA 组件需求定义（待追溯验证）
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 基于平台操作页面 SR 及约束推断平台架构 | 判断组件属于引擎专属、页面组件扩展、公共构件、公共平台服务或运行支撑模块；同时消费 FR-BIZ 指标约束包和 wft04-nfr 的 NFR 约束包，形成必要的平台架构决策 |
| 2 | 基于平台操作页面及活动推断前端组件 | 从页面形态、页面组件、构件线索、按钮/右键/辅助栏/弹窗等推断前端容器、页面组件扩展和公共构件 |
...
第二轮：eng/nfr 汇合与收尾
  ...
  汇合条件满足 → wft05-eng 同时消费平台操作页面 SR-F、FR-BIZ 指标约束包和 NFR 约束包 → 平台 PA 组件需求定义
```
...
|--------|------|-----------|---------|--------|
| **业务配置架构及构件需求定义** | biz | wft03-biz | 功能表单 + 业务节点操作页面 + 引擎能力使用需求 + FR-BIZ 指标约束包 + 引擎能力缺口线索 | 构件开发流水线 + wft05-eng（指标约束） |
...
| 依赖关系 | 说明 | 策略 |
|---------|------|------|
| eng 线 wft05-eng ← nfr 线 wft04-nfr | wft05-eng 消费 NFR 约束包 | eng 前三步与 nfr 可并行；首版约束包须在相关 STR-E/CU/平台操作页面 SR-F 可定位后完成目标绑定并确认，wft05-eng 在两侧输入同时就绪后启动 |
...
| wft03-biz | 功能表单 / 业务节点操作页面 | 引擎能力使用需求 | 消费 wft02 的 BA 方案→生成业务应用页面；分配 FR-BIZ 指标到引擎/CU；输出指标约束包和引擎能力缺口线索——"出配置需求暨配置业务系统PA" | §七 |

### 10.4 eng 路径 Skill

| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 场景业务（STR-E） | 主责能力域 / CU 候选 / 依赖 STR-E | 基于 FR-ENG OR 推断可独立配置、治理和验收的平台能力场景——“向平台方证明我听懂了” | §八 |
| wft02-eng | 配置单元（CU） | 配置入口 / 配置要素 / 系统处理 / 运行期能力 | 装配 A2 产品业务架构上下文 + A1 引擎→CU 层级 + CU 配置能力定义 + 依赖关系——"设计配置单元 BA" | §八 |
| wft03-eng | 平台操作页面 | 页面形态 / 页面组件 / 操作活动 | 消费 CU 配置能力定义，推断平台操作页面、页面功能架构和操作活动——"出平台操作页面 SR-F" | §八 |
| wft05-eng | PA 组件 | 前端组件 / 后端组件 / 平台服务组件 | 消费平台操作页面 SR-F、FR-BIZ 指标约束包和 NFR 约束包，推断三类组件及组件架构——"出组件需求" | §八 |
...
|------|----------------------|----------------------|----------------------------|
| 末级节点 | 场景业务（STR-F） | 输出文档（PL5） | 功能表单 / 业务节点操作页面（业务配置需求 + 配置业务系统 PA 双重身份） |
...
|------|----------------------|----------------------|---------------------|---------------------|
| 末级节点 | 场景业务（STR-E） | 配置单元（CU） | 平台操作页面 | PA 组件（前端/后端/平台服务） |
| 输入对象 | FR-ENG OR 条目（≤10） | STR-E 节点（≤5） | BA 节点（A1，≤5） | 平台操作页面 SR-F + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
