...
> **概述**：定义 EOS 平台的 UI 交互模型（含七种页面类型与三层编排链）、输出文档模型（正式/轻量文档、封面-正文 1:1 组装）、引擎分层体系与 CU 配置单元模型、页面组件与构件体系、业务树两族视角树（R／P）、资产治理业务，以及三路径（biz/eng/nfr）的职责定位和协作关系。
>
> **版本**：v5.59 | **修订**：2026-09-10
...
**场景业务**是 bph-biz（业务流程概要架构末级节点）的组织单元。每条场景业务定义一条**端到端业务闭环**——从需求提出到需求被满足并确认的完整链路。
...
**场景业务的类型**由 AI 根据 规范化的相关方需求 语义判定：
...
D 和 PCA 仅在正文数据类型上有差异（产品数据 vs 管理数据），在 wft01/02/03-biz 的处理逻辑上**流程骨架同构**——指处理流程结构（都是场景业务→输出文档序列→每个文档有封面+正文+任务+引用）不因类型分叉。区别在于：
...
PCA 场景业务与 D 类流程骨架同构——由输出文档序列构成，每个输出文档按需配置处理配置（任务类型）。除遵循 §2.5 R4（首个有任务类型的文档为计划，本体为开发/记录类文档）外，PCA 场景还必须包含一个计划清单输出文档（见下文），两者是不同的文档实例。
...
**P0 树（平台／引擎产品的 P 树）**：P0（§5.1）同样沿产品开发视角（PL）展开——平台 → 业务域（流程与IT）→ 产品类型（平台／引擎产品）→ 构件（引擎）→ **CU（配置单元，PL4）→ 配置信息组（PL5，末级节点）**。P0 树无 WBS 层——Pn 树的 PL4=WBS 保留（§5.3），仅 P0 树以 CU 直接占据 PL4 位置。配置信息组是 P0 产品树的**末级节点**（可独立交付/验收的能力单元），对应 Pn 树的 PL5 输出文档——两者同为各产品树的 PL5 末级；CU 是可独立**选配**的配置单元（PL4，配置入口/页面边界），由若干配置信息组构成（选配粒度=CU、能力粒度=配置信息组，两层面正交不冲突）。P0 树到配置信息组为止，无 PL6——操作活动定义（配置操作/系统处理/运行期能力）是配置信息组的内容，非架构节点。
...
**CU 设计权规则**：CU 是引擎提供的配置能力单元，其能力边界、配置入口、配置要素、系统处理、运行期输出和 FR-ENG 性能指标仅由 eng 路径设计。**wft01-eng 设计 CU 承接（名称级识别并分解配置信息组（概要说明） + 分配配置信息组到 CU 承接，含收敛判定）**；**wft02-eng 详细定义 CU（承接已分配 CU，展开操作活动定义/依赖/状态迁移）**；wft03-eng 将 CU 操作活动组织为系统需求概要；wft04-eng 展开功能表单上组件业务并标注 pa-eng 层收敛项；wft05-eng 将其展开为软件组件。biz 路径不得新建、修改或详细设计 CU。**配置信息组（P0 产品树末级节点，PL5）的分解概要由 wft01-eng 设计、操作活动定义由 wft02-eng 展开；biz 路径不得设计。**
...
**从输出文档到引擎支撑**（自顶向下）：bph-biz 架构末级节点是场景业务，场景业务由输出文档序列细化定义。从输出文档出发，按生成方式分两条路径确定支撑引擎链：
...
| CU | 只引用既有能力并形成引擎能力使用需求 | wft02-eng 定义 CU，wft03-eng 组织系统需求概要 |
...
→ 形成 FR-ENG 候选线索。候选线索不是规范化的相关方需求节点，不得直接进入 wft01-eng；须经人类确认后作为原始需求材料登记，并完整通过 ort00→ort01→ort02→ort03，形成FR-ENG 规范化的相关方需求节点 后，才按 eng 路径（§八）推进——wft01-eng 判定归属引擎 → wft02-eng 设计 CU → wft03-eng 组织系统需求概要 → wft04-eng 展开组件业务 → wft05-eng 展开三类组件。
...
## 五、业务树两族：R（角色履职视角）× P（产品开发视角）

EOS 平台的业务配置和运行围绕**两类产品**展开，每类产品有两棵**视角树**——**R 树**（角色履职视角，roles & responsibilities；沿 SL 展开）与 **P 树**（产品开发视角，product or service；沿 PL 展开）。下标索引**所指产品**：`0` ＝ 平台本身，`i ≥ 1` ＝ 基于平台引擎构建的各个业务。简称、树间对应与级语义以 [94-eos-sys-dev-dbs.md](94-eos-sys-dev-dbs.md) §1.1 为唯一权威，本章不复述其口径。

| 树 | 所指产品 | 归属域 | 构件 | 末级节点 |
|----|---------|--------|------|---------|
| **P0** | 平台本身 | 流程与IT | 引擎 | 配置信息组（CU 为 PL4 可独立选配的配置单元） |
| **R0** | 平台本身 | 流程与IT | 业务配置角色（业务配置人员／平台治理角色） | 场景业务 |
| **Rn** | 各业务域 | 各业务域 | 业务运行角色 | 场景业务 |
| **Pn** | 各业务域 | 各业务域 | 产品分解类型（自研件/外购件/复用件/项目等） | 构件的产品数据 |

> 业务树的完整架构定义见 [23 号资产](../../80-pl4eos-2-eosdata/23-eos-output-architecture.md)。
> **节序说明**：本节按**视角**分节（§5.1 P0／§5.2 Rn／§5.3 Pn／§5.4 R0），故 i=0 的两棵不连排；R0 列于末节，以保持既有节号不动（§5.3.1 等锚点被 93／biz02／biz03／02／04 多处引用）。

### 5.1 P0 树（平台／引擎 · 产品开发视角）

P0 是**平台本身的产品开发视角树**（平台引擎产品），归属流程与IT域。构件为引擎（`@engine-entity`、`@engine-flow` 等 12 个），末级节点为配置信息组（CU 为 PL4 可独立选配的配置单元，由配置信息组构成；详见 [25 号资产 §2](../../80-pl4eos-2-eosdata/25-eos-engine-models.md#2-引擎定义)）。
...
对应设计路径为 **eng 路径**：wft01-eng（业务流程概要）→ wft02-eng（业务流程方案）→ wft03-eng（系统需求概要）→ wft04-eng（系统需求方案）→ wft05-eng（平台产品架构/三类组件）。

> **bph-eng 的双重身份**：wft01-eng 产出的 bph-eng 具有双重身份——R0 视角与 P0 视角各取一面（R0 的完整说明见 §5.4）——
> - **R0 视角**：它是流程与IT域中**业务配置人员**的 PL5 场景业务。
> - **P0 视角**：它对应某个主责能力域中的可独立配置、治理和验收的能力场景。
> 
> 主责能力域可以是引擎，也可以是平台共享能力域。一个能力域可包含多个 bph-eng；多个引擎可消费同一共享 bph-eng，但不得重复定义共享 CU。CU 是 bph-eng 的细分节点，配置信息组是 P0 产品树的末级节点（PL5，§3.1 P0 树）。

### 5.2 Rn 树（各业务域 · 业务运行角色履职视角）

Rn 是**各业务域业务运行角色的履职视角树**（角色场景），构件为角色（如采购员、库存管理员），末级节点为场景业务。沿角色场景视角（SL）展开：SL3 角色类型 → SL4 角色职责 → SL5 场景业务 → SL6 功能表单/主子表单。

每棵 Rn 树回答：某角色在哪个菜单入口、按什么顺序使用哪些功能表单/主子表单、完成端到端的职责闭环。场景业务（SL5）与 bph-biz 末级节点 1:1 映射。

对应设计路径为 **biz 路径的前半段**：wft01-biz（业务流程概要，场景业务为枝、文档为末级）→ wft02-biz（业务流程方案，组装 R 产品业务流程 + 表单使用顺序）。

> **流程与IT域的 R 树**即 **R0**（业务配置角色的履职视角），**不属本节**——它不是业务域角色场景，其角色是在平台上履职的人（业务配置人员、平台治理角色）。完整说明见 §5.4。

### 5.3 Pn 树（各业务域 · 产品开发视角）

Pn 是**各业务域的产品开发视角树**（输出产品），沿产品开发视角（PL）展开：PL1 业务域 → PL2 E2E 项目业务 → PL3 构件类型 → PL4 WBS → PL5 业务输出文档。每棵 Pn 树回答：某类输出文档属于哪个业务域/构件类型，需要哪些前序文档和业务规则才能生成。
...
**判据之家（2026-09-04 定）**：面向 23 的资产对齐判据族统一定义于此——**23B 对齐 Q1/Q2/Q3**（Q1=已有、直接引用；Q2=需扩展；Q3=无、新建建议）、**PL4 WBS 归组**（数据依赖链 + 完成耦合硬判据）、**23 一致性三类**（归属冲突 / 误新建 / 误引用）、**缺口推断**（按构件产品数据清单做名称级比对，不依赖任务类型/引擎）。**引用数据**（P 树全貌 / PL 路径 / WBS / 产品数据清单）在 23 资产（`80-pl4eos-2-eosdata/23-eos-output-architecture.md`），不载入判据正文。各 wft/试点人类方案只写本任务的**判定动作**，判据定义引本节、引用数据读 23，不重述判据正文（93 §四.1 判据归属分级）。

**Q 族命名索引**（91 内存在三族同名 Q，引用按全名、勿裸写，避免指错族）：

| Q 族 | 用途 | 判据之家 | 引用数据 |
|------|------|---------|---------|
| **对齐 Q1~Q3**（23B） | 输出文档/方案节点与 23 资产的对齐（引用/扩展/新建） | 本节（§5.3.1） | 23 |
...
### 5.4 R0 树（平台本身 · 业务配置角色履职视角）

R0 是**业务配置角色履职视角树**——它的角色是**在平台上履职的人**，而非业务部门的业务角色。两类角色：

| 角色 | 场景业务 | 架构位置 |
|------|---------|---------|
| **业务配置人员** | bph-eng，归组锚点＝`主责能力域 + 配置/治理目标 + 可独立验收结果`（不是引擎数量） | 与各业务域 bph-biz 同层同构，区别在于场景产出物是**可用的平台能力** |
| **平台治理角色**（业务架构管理员、数据治理员等） | 平台资产治理业务（菜单配置／资产管理／数据治理／权限治理） | bph-biz 已预置在 `02-*.md`（详见 §六） |
...
> **bph-eng 的双重身份**由这对视角给出：同一份 bph-eng，在 R0 视角是业务配置人员的 SL5/PL5 场景业务，在 P0 视角是主责能力域下的能力场景（见 §5.1 注）。
> **角色职责层退化（R0 与 Rn 的结构差异，非遗漏）**：R0 相对 Rn 少「角色职责」一级（SL4）——R0 的「相关方角色」（业务配置角色）之下直接是场景业务；Rn 则因角色类型多而保留 SL3/SL4 两级归纳。

---

## 六、平台资产治理业务

> 平台资产治理是 EOS 平台的**内置场景业务**（**R0 树**的场景业务）：平台自身的管理对象（菜单、架构资产、数据标准、权限）就是平台的业务数据，平台提供对应的引擎和功能表单直接管理这些对象，治理人员使用平台能力治理平台。治理角色即 R0 的两类角色之一（见 §5.4）。bph-biz 已预置在 `02-*.md` 中。
>
...
> 治理 bph-biz 的完整清单以 `02-*.md` 为准，AI 运行时动态读取。上述四类治理业务当前 规范化的相关方需求 承接为空，由人类在 EOS 平台建设过程中逐批增补。
...
|-------|------|------|---------|---------|
| **wft01-biz** | FR-BIZ 规范化的相关方需求条目 | 业务流程概要——bph-biz 场景业务（枝）+ PL5 输出文档序列（概要末梢）+ P 组装（WBS 归组/23 一致性/缺口推断） | 文档（PL5，结构判定级概要说明） | 向业务方证明"我听懂了你的业务" |
| **wft02-biz** | bph-biz 节点 | 业务流程方案——R（角色→场景业务→表单使用顺序）+ P（输出文档详细设计 + PL6-A 活动节点需求 + PL6-B 条目需求） | 输出文档（PL5），细分 PL6-A / PL6-B + 表单使用顺序 | 设计正确的方案——业务语言→架构语言 |
| **wft03-biz** | bpd-biz 节点 | 系统需求概要——⑤ 用例场景功能 + 功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（窗口标签页为页面组件承载） | 产出可交付构件开发的配置业务系统 pa-eng，并向 eng 分配指标 |
...
| **产出物** | 业务流程概要（bph-biz）——场景业务 + 输出文档序列 + P 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务流程方案（BP）——R 角色场景（功能表单/主子表单 + 表单使用顺序）+ P 输出产品（输出文档详细设计 + PL6-A 节点需序）+ P 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统需求概要（业务配置需求暨配置业务系统 pa-eng）——⑤ 用例场景功能（枝梢；功能表单为其业务侧载体，窗口标签页为页面组件承载）+ 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
| **供谁消费** | 业务方确认 → wft02-biz | wft03-biz | 构件开发流水线 + eng 路径（引擎缺口线索） |

> **概要/方案分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要/方案」重述——wft01 业务流程概要（末级=文档，概要说明为**结构判定级**，字段级详设由 wft02-biz 承接）；wft02 业务流程方案（详细条目=**活动 + 正文条目 + 表单使用顺序**）；wft03 系统需求概要（**枝梢＝⑤ 用例场景功能**，叶＝**⑥ IPO功能**只在完整态；功能表单是业务侧 R／P 树 ⑤ 输出文档（枝梢）下任务步骤的载体，窗口标签页为其页面组件承载形式——2026-08-22 深审统一，2026-09-10 按 94 树权威校正末级口径）。两棵树读法——**R 树**（SL3 角色类型→SL4 角色职责→SL5 场景业务）+ **P 树**（PL3 构件类型→PL4 WBS→PL5 输出文档）是**两棵独立的树**，在**输出文档（PL5）**处对位互连、经认亲互指，**不是一棵树的共享节点**；两条链自文档（PL5）起并行展开 → 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bph-biz、BP→bpd-biz、SR-F→srh-biz；追溯/23 不迁。命名「业务流程方案」源于 bpd-biz IPO 承担方案角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = P 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 规范化的相关方需求 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
> **SR 树的末级与功能表单的关系（2026-09-10 定，按 94 树权威校正）**：srh（系统需求）树的**枝梢＝⑤ 用例场景功能、叶＝⑥ IPO功能**（后者只在完整态——系统接收某组字段的值→处理→给出响应）。**功能表单不属于 SR 树**：它是「**用户操作页面视角**」的载体，对应业务侧 R／P 树 **⑤ 业务输出文档（枝梢）下的任务步骤**（该文档的**分解源判据展开项**，见 [94 §2.2](94-eos-sys-dev-dbs.md)），跨视角聚合系统侧的 ④ 系统用例功能 / ⑤ 用例场景功能。两个视角正交——**分类视角**（树的 ③ 类）下一个系统用例功能只归一 类；**页面视角**下同一系统用例功能可按需出现在多张功能表单上。树形/级语义单源见 [94](94-eos-sys-dev-dbs.md)。
>
> **两树分工说明（2026-08-22 深审确认，2026-09-10 按「两棵树」校正）**：**R 树**的归纳（SL3 角色类型→SL4 角色职责）在 wft02、**P 树**的归纳（PL3 构件类型→PL4 WBS）在 wft01——不对称是分工决定的，非遗漏：PL4 WBS 归组与 23B 对齐/23 一致性/缺口推断同属 wft01 P 组装（结构性绑定），SL4 归纳服务 R 场景装配与表单使用顺序（wft02 Step 3），且 R 侧无 23 资产产品数据清单类锚点可做缺口推断。
>
> **26 号文档资产已取消（2026-08-22 深审决策）**：业务文档定义承载于 04/05/06 方案文档，承接锚点在 25 §1.7 业务文档承接。
>
> 业务流程概要架构文档 `02-*.md` 在 wft01-biz 产出 bph-biz 后形成。D 和 PCA 场景的处理逻辑流程骨架同构。
>
> **与 eng 路径的对称性**（详见 §八）：biz 路径的输出文档对应 eng 路径的配置信息组——都是各自产品的 PL5 末级节点；eng 的 CU（PL4）对应 biz 的 WBS（PL4）。wft03-biz 的配置需求方案 ←→ wft03-eng 的系统需求概要、wft04-eng 的系统需求方案，同属系统需求概要层（srh-biz/srh-eng/srd-eng）。

---

### 7.1 原始需求处理（wft01-biz）

> 从零散但规范化的 规范化的相关方需求 到结构化的场景业务方案。核心使命：**向业务方证明"我听懂了你的业务"**——产出供人类确认理解，而非最终设计方案。

```
输入                         产出
FR-BIZ 规范化的相关方需求（待处理）  ──→  业务流程概要 bph-biz 场景业务方案（可以分解分配）
                          末级节点：文档（PL5，结构判定级概要说明）
```
...
| 3 | 判定任务类型与子节点 | 业务语言，供人类确认。判定任务类型（计划/流程/工单）→ 展开 PL6-A 活动节点，捕获 规范化的相关方需求 明示节点。管理类（PCA）场景业务需判定 PL6-B 正文条目。R4：首个有任务类型的文档必然为计划 |
...
| 7 | P 组装 | 23B Q1/Q2/Q3 对齐判定 + PL4 WBS 归组（数据依赖链硬判据）+ 23 一致性检查（归属冲突/误新建/误引用）+ 按构件产品数据清单完整性缺口推断（结构性缺口 → 补充 规范化的相关方需求 回路） |
...
> 将 bph-biz 场景业务展开为完整的 bpd-biz 方案——R（角色怎么工作）+ P（角色产出什么）。核心使命：**设计正确的方案**，完成业务语言 → 架构语言的转化。

```
输入                               产出
bph-biz 节点（含 wft01 P 组装结论）   业务流程方案 bpd-biz 方案（可以srh设计）
（可以分解分配 / 待补充分解分配）  ──→  末级节点：输出文档（PL5）
                                      细分：PL6-A 活动节点 / PL6-B 正文条目 / 表单使用顺序
```

**关键转化——业务语言 → 架构语言**：wft01-biz 用业务方听得懂的话描述活动节点和关系，wft02-biz 将其转化为架构语言描述的 bpd-biz 设计方案——将任务类型映射到对应引擎，逐 PL6-A 活动节点定义处理角色/可执行动作/状态变迁（全量标注 `[推断]`），供 wft03-biz 展开为功能表单/主子表单序列和流程类窗口标签页序列的需求定义：
...
| # | 任务 | 视角 | 说明 |
|---|------|------|------|
| 1 | 装配 R 产品业务流程 | R | 角色 → 场景业务 → 功能表单/主子表单 + 表单使用顺序 |
| 2 | 消费 P 组装结果 | P | 承接 wft01 P 组装结论（PL 路径 + WBS 归组 + 23 一致性，不重复归组/一致性检查），逐输出文档展开详细设计 |
...
| 5 | 定义 PL6-A 活动节点需求 | P | 有任务类型时，消费 wft01 的业务节点（业务语言）→ 逐节点定义处理角色、可执行动作、状态变迁、系统自动节点/异常路径/超时处理，全量标注 `[推断]`。窗口标签页序列编排由 wft03 负责 |
...
> 将 bpd-biz 方案展开为 srh-biz 系统需求（**枝梢＝⑤ 用例场景功能**，叶＝⑥ IPO功能），并形成引擎能力使用需求与 FR-BIZ 指标分配。该产出兼具两重身份——从 EOS 平台视角是业务配置需求，从配置业务系统视角是产品架构节点（**功能表单**为其在业务侧 R／P 树 ⑤ 输出文档（枝梢）下任务步骤上的载体形式）；biz 路径在 wft03-biz 结束，不再设置 wft05-biz，也不在 07 重复定义。

```
输入                               产出
bpd-biz 节点                               功能表单 + 窗口标签页
（可以srh设计 / 待补充srh设计）    ──→  （待构件需求定义）
...
```
FR-ENG 规范化的相关方需求节点 ──→ wft01-eng ──→ bph-eng ──→ wft02-eng ──→ bpd-eng ──→ wft03-eng ──→ srh-eng ──→ wft04-eng ──→ srd-eng ──→ wft05-eng ──→ pa-eng
              业务流程概要          业务流程方案            系统需求概要              系统需求方案               平台产品架构
              （场景+配置信息组概要+承接）（详设：操作活动定义） （功能表单+概要说明）     （组件业务+pa-eng收敛项）      （前后端/平台服务组件）
```
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明"我听懂了你要的平台能力" | FR-ENG 规范化的相关方需求节点 条目 → bph-eng 业务流程概要方案 | 配置信息组（概要说明，PL5 末级，业务流程概要末级节点；bph-eng 架构末级=场景业务 SL5）+ CU 承接 + 依赖 bph-eng（共享依赖） | 业务配置人员确认 → wft02-eng |
| **wft02-eng** | 详细定义已分配的 CU——操作活动定义/依赖/状态迁移 | bph-eng 节点 → P0 bpd-eng 业务流程方案 | 配置信息组（操作活动定义：配置操作/系统处理/运行期能力；CU 为 PL4 选配单元） | wft03-eng |
| **wft03-eng** | 把操作活动定义组织为系统需求概要（⑤ 用例场景功能 + 功能表单 + 概要说明） | bpd-eng 节点（P0）→ srh-eng 系统需求概要需求 | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（页面职责/页面类别/操作活动清单承接 02/页面编排概要） | wft04-eng |
| **wft04-eng** | 展开功能表单上各组件及其业务（系统需求方案） | 系统需求概要 srh-eng → srd-eng 系统需求方案 | 组件业务（布局组件/构件 + 组件业务 + pa-eng 层收敛项） | wft05-eng |
| **wft05-eng** | 产出可交付开发的三类组件方案 | 系统需求方案 srd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 → pa-eng 组件需求定义 | pa-eng 组件（前端/后端/平台服务） | 追溯验证 / 构件开发流水线 |

> **角色**：eng 路径的设计角色为流程与IT域的**业务配置人员**——所有引擎的配置设计均由该角色执行，bph-eng 架构树上该角色层为单值（无需再做分组维度）。流程与IT域的其他角色（业务架构管理员、数据治理员，见 §六）属于平台资产治理角色，不参与 eng 设计路径。
>
> **bph-eng 的双重身份**（详见 §5.1／§5.4）：在 R0 视角下，bph-eng 是业务配置人员的场景业务（SL5/PL5 末级节点）；在 P0 视角下，它对应唯一主责能力域下的独立能力场景。主责类型分为`引擎专属`和`平台共享`，CU 是 bph-eng 的细分节点。
>
> **与 biz 路径的对应关系**（详见 §七）：eng 路径的**配置信息组**对应 biz 路径的输出文档（同为 PL5 末级节点）；CU 对应 biz 的 WBS（同为 PL4）；wft03-eng 的 ⑤ 用例场景功能（功能表单为其业务侧载体）是平台软件 srh-eng 概要，wft04-eng 的组件业务是平台软件 srd-eng 详细，wft03-biz 的 ⑤ 用例场景功能与功能表单/窗口标签页兼具业务配置需求与配置业务系统 pa-eng 身份。二者同写入 05，但节点类型、使用角色、终态和下游不同。

---

### 8.1 原始需求处理（wft01-eng）

**将零散的 FR-ENG 规范化的相关方需求节点 组织为结构化的平台能力场景业务方案（业务流程概要，01 层——概要说明为结构判定级，操作活动定义留 02 展开），供业务配置人员确认理解。**

```
输入                              产出
FR-ENG 规范化的相关方需求节点 条目（待处理）  ──→  bph-eng 平台能力场景业务方案（可以详细定义）
                               P0 产品树末级：配置信息组（PL5，概要说明，结构判定级）
                               bph-eng 架构末级：场景业务（SL5）
                               细分：CU 承接（配置信息组 = 业务流程概要末级节点）
```
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 推断生成平台能力场景业务节点 | 判定主责类型和主责能力域，按配置/治理目标及独立验收结果匹配已有 bph-eng 或新建 |
| 2 | 名称级识别并分解配置信息组（概要说明）+ 分配配置信息组到 CU 承接 | 对称于 bph-biz 下辖的输出文档——场景业务由哪些 CU 组成；按"可以运行的能力"名称级识别并分解为配置信息组（概要说明，**结构判定级**——配置什么/运行期提供什么，不展开操作活动定义——那是 wft02-eng 业务流程方案），逐组分配 CU 承接（含收敛判定复用/新增/拆分/合并 + 对照 25 盘存的骨架判定） |
| 3 | 基于能力可交付性补充平台需求 | 逐 bph-eng 检查配置/治理目标能否形成可独立验收的端到端闭环；检出实质缺口则形成补充 FR-ENG 材料 |
| 4 | 建立追溯链 | FR-ENG 规范化的相关方需求节点 → bph-eng主责能力域 → 配置信息组（概要说明） → CU承接/依赖bph-eng；共享能力记录提供能力、消费方和复用范围 |

### 8.2 业务流程开发（wft02-eng）

**详细定义 wft01-eng 已分配的 CU——业务流程方案（02 层，操作活动定义步骤化三段式：配置操作/系统处理/运行期能力），承接 bph-eng 的 R0 上下文与 P0 装配，展开操作活动定义/依赖/状态迁移。**

```
输入                                           产出
bph-eng 节点                                    P0 配置单元 bpd-eng 业务流程方案（可以srh设计）
...
| # | 任务 | 视角 | 说明 |
|---|------|------|------|
| 1 | 承接 bph-eng 的 R0 上下文 | R0 | 读取 wft01-eng 装配的 R0 信息（业务运行对象、配置/支撑角色、触发事件、业务状态影响）并校验字段完整；只服务 eng 链 CU 推导，不替代 wft02-biz 的 R／P 输出文档 |
| 2 | 承接已分配 CU 并展开 P0 装配 | P0 | 读取 bph-eng 的 CU 承接关系（wft01-eng 已判定收敛/骨架/引擎归属）→ 按主责能力域→能力场景→CU 层级展开详设；共享能力独立建模，消费方只记录依赖 |
| 3 | 定义 CU 操作活动定义 | P0 | 承接 wft01 的配置信息组清单+结构判定级概要说明，逐组展开**操作活动定义**（配置操作=步骤序列每步含对象+约束+联动 / 系统处理=编辑时处理+生效后响应，统一保存机制与载件标注 / 运行期能力=操作处理分支显式化；基准=PDF v1.5.2），定义配置入口与引擎状态迁移；详设层变更（配置能力扩展）以 `[调整]` 标注；承接缺陷退回 `需wft01修订` |
| 4 | 基于 P0 补充引擎需求 | — | 检出详设缺口（操作活动/依赖/状态迁移） → 生成补充 FR-ENG 材料（详设层缺口，登记 材料状态表 A） |

> **R0／P0 双视角**：Task 1 对应 §5.4 所述 R0 树（平台本身的角色履职视角），但在 eng 路径中只做"上下文承接"——R0 信息由 wft01-eng 装配，本 Skill 读取校验后说明 CU 服务于哪个业务运行对象、配置/支撑角色和触发场景；Task 2~3 对应 §5.1 所述 P0 树——描述引擎"怎么被配置"（引擎→CU 层级→配置信息组/操作活动定义）。功能表单本身由 wft03-eng 组织概要、wft04-eng 展开组件业务。
>
> **CU 的页面关系**：CU 在配置时通常对应硬代码功能表单（复合页内的子页面即完整页面，一个 CU 对应一个页面），配置人员编辑配置要素，提交后引擎校验并保存；运行时，引擎按 CU 定义生成或渲染制品、构件及其功能。wft02-eng 只定义操作活动，wft03-eng 才把操作活动组织为系统需求概要。

### 8.3 系统需求概要开发（wft03-eng）

**将 CU 操作活动定义组织为系统需求概要 srh-eng：功能表单+概要说明（结构判定级——页面职责/页面类别/使用角色/承载关系/操作活动清单/页面编排概要），只组织页面，操作活动清单承接 02 标注入口。**

```
输入                                           产出
bpd-eng 节点（P0）                                平台系统需求概要 srh-eng（可以详细定义）
（可以srh设计 / 待补充srh设计）  ──→            枝梢节点：⑤ 用例场景功能（功能表单为其业务侧载体）
                                              细分：页面类别 / 操作活动清单（承接 02）/ 页面编排概要（区域/TAB/弹窗入口）
```
...
| 2 | 组织页面编排概要 | 将功能表单按区域/TAB/弹窗入口、跳转、上下文传递、权限和状态组织成页面编排概要（03 到区域/TAB/弹窗入口级别，布局组件/构件下移 wft04-eng） |
| 3 | 承接操作活动清单 | 将配置信息组操作活动挂载为页面操作活动清单（配置维护/校验处理/发布生效/运行支撑/治理追溯），逐活动标注承接 02 标注入口（不重复步骤） |

### 8.4 系统需求详细开发（wft04-eng）

**将系统需求概要 srh-eng 展开为系统需求方案 srd-eng：判定区域内布局组件/构件、定义组件业务（功能级不写实现）、标注 pa-eng 层收敛项。组件业务是 wft05-eng 推断前后端组件的依据。**

```
输入                                           产出
系统需求概要 srh-eng（05-*.md）                    平台系统需求方案 srd-eng（已详细定义）
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 判定布局组件与构件 | 在页面编排概要（区域/TAB/弹窗入口）基础上，判定区域内布局组件组合（多级横版/列表/单据/辅助栏/弹窗/指标/看板），按展示/交互/数据/反馈四类识别构件 |
...
**将系统需求方案 srd-eng（组件业务）、FR-BIZ 指标约束和 NFR 约束展开为 pa-eng 三类组件方案，构件开发流水线可据此进入开发。**（组件业务是推断前后端组件的依据。）

```
输入                                              产出
系统需求方案 srd-eng + FR-BIZ指标约束包 + NFR约束包     pa-eng 组件需求定义（待追溯验证）
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 基于系统需求方案 srd-eng 及约束推断平台架构 | 判断组件属于引擎专属、布局组件扩展、公共构件、公共平台服务或运行支撑模块；同时消费 FR-BIZ 指标约束包和 wft04-nfr 的 wft05-eng NFR 约束包，形成必要的平台架构决策 |
...
```
规范化非功能需求 ──→ wft01-nfr ──→ bph-nfr ──→ wft04-nfr ──→ SysReq-NFR
            角色NFR分类              量化指标+页面 NFR
            定性→结构化              "指标可验证了"
```
...
|-------|---------|------------|-----------------|---------|
| **wft01-nfr** | 向 NFR 相关方证明"我听懂了你的质量诉求" | 规范化非功能需求条目 → bph-nfr 非功能需求概要方案 | 非功能需求概要 | NFR 相关方确认 → wft04-nfr |
| **wft04-nfr** | 产出可验证的指标和可消费的约束包 | bph-nfr 节点 → SysReq-NFR 架构 + 量化约束 + 页面 NFR | 非功能需求方案 | wft05-eng / 追溯验证 |
...
>
> 业务流程概要架构文档 `02-*.md` 在 wft01-nfr 产出 bph-nfr 后形成。SysReq-NFR 写入 `05-*.md`，06 详细定义在 wft04-nfr 落账时写入 `06-*.md`。NFR 约束全部流向 eng 路径（能力链）——性能/安全/可用性等非功能质量约束由平台的引擎层保障，不进入 biz 路径（业务配置链）。`适用链路=biz` 仅表示约束来源于或影响某个业务语境；设计消费方仍是 wft05-eng，最终由前端、后端或平台服务组件承接。

### 9.1 原始非功能需求处理（wft01-nfr）

**定位**：nfr 路径的第一个 Skill，从零散但规范化的 规范化非功能需求 到结构化的非功能需求概要方案。核心使命是**向 NFR 相关方证明"我听懂了你的质量诉求"**——识别角色语境、判定 22 分类路径、形成分类需求清单，供 wft04-nfr 展开量化设计。

```
输入                                   产出
规范化非功能需求条目（待处理）  ──→  bph-nfr 非功能需求概要方案（可以NFR设计）
                                末级节点：非功能需求概要
```

bph-nfr 的归组锚点为四元组——**用户角色**（提出或承受该非功能诉求的角色/外部系统/保障者）+ **业务/平台语境**（业务域/产品类型/bph-biz或bph-eng线索）+ **NFR 分类维度**（22 资产中的一级类目+分类维度）+ **适用对象线索**（可能约束的 bph-biz/bph-eng/业务域/平台能力/全局对象）。

**任务清单**：

| # | 任务 | 说明 |
|---|------|------|
| 1 | 推断非功能需求概要节点 | 识别 规范化的相关方需求 背后的用户角色、业务域/平台能力语境、NFR 一级类目与分类维度，形成 bph-nfr 末级节点 |
...
**定位**：nfr 路径的第二个 Skill，承接 bph-nfr 中相关方经 ort02 澄清确认的目标值（[通用设计规范 §1.2](../../../00-generalspec/01-generalspec-sysdev.md#12-需求详解) 需求=需求描述+性能指标），展开为系统侧 SysReq-NFR 架构——量化指标 + 分层约束表 + 页面 NFR。核心使命是**产出可验证的指标和可消费的约束包**，供 eng 路径和追溯验证消费。

```
输入                                              产出
bph-nfr 节点                                        SysReq-NFR 架构 + 量化约束 + 页面 NFR
（可以NFR设计 / 待补充NFR设计）  ──→                （bph-nfr → 已NFR设计，SysReq-NFR → 已验证冻结）
                                                    末级节点：非功能需求方案
                                                    细分：具体非功能需求
```

量化指标的六要素：指标名称 / 目标值与单位（目标值由相关方经 ort02 澄清确认，[通用设计规范 §1.2](../../../00-generalspec/01-generalspec-sysdev.md#12-需求详解)）/ 统计口径（时间窗口/样本范围/百分位） / 测量方法（压测/审计检查/日志统计等） / 验收条件 / 来源 bph-nfr 与消费版本。目标值缺失或不可验证时：可依据 22 具体需求模板和同类场景推断则标记 `[需裁决]` 并给出依据；无法推断时保留 `待确认`，不推进写回。

**任务清单**：

| # | 任务 | 说明 |
|---|------|------|
| 1 | 消费 bph-nfr 分类需求清单 | 逐 bph-nfr 检查消费版本——首次消费→全量设计，版本递增→按变更声明执行增量/修订/结构型分流。分类边界或角色语境不足→退回 `需wft01修订` |
...
| 4 | 生成分层约束表 | 逐量化指标判定约束范围（全局 / 业务域 / srh-biz / CU / pa-eng组件 / 页面级）→适用链路（biz / eng / 共享）→约束对象→消费目标→影响级别（阻断型 / 非阻断型 / 待判定）→影响点 |
| 5 | 生成页面 NFR | 影响用户可感知页面行为的指标→绑定七种页面类型之一 + 约束内容 + 量化目标 + 测量方法 + 消费目标。覆盖页面性能/安全/易用性/可访问性/可恢复性/可观测性六个维度 |
| 6 | 装配 wft05-eng NFR 约束包 | 按约束范围组织为 wft05-eng NFR 约束包：全局约束 / 业务域约束 / srh-biz 约束 / 页面级约束 / CU 约束 / pa-eng 组件约束 / 共享约束。约束包只引用分层约束表行 ID 和页面 NFR 条目，不复制正文；分解子指标作为独立约束行装配（标注分解运算+父指标引用）。消费状态/已消费版本由 wft05-eng 消费后回写。NFR 约束仅流向 eng 路径——非功能质量由引擎层保障，biz 路径不消费 |
| 7 | 闭环检查与资产落账 | 七项闭环检查（量化完整/宿主唯一/分层正确/页面可消费/eng一致/22可落账/06可验证）→22 具体需求写回→06 系统需求方案→05/02 状态推进→bph-nfr 消费版本记录 |
...
原始材料 → ort00文本化 → ort01切分 → ort02澄清
          ...
          (bph-biz 场景业务)    (bph-eng 引擎场景)   (bph-nfr 分类)
                │                   │                   │
                ▼                   ▼                   ▼
          wft02-biz-bph2bpd   wft02-eng-bph2bpd   wft04-nfr-str2sr
          (bpd-biz 双视角)          (业务流程方案)     (SysReq-NFR 量化)
                │                   │                   │
                ▼                   ▼                   │
          wft03-biz-bpd2srh    wft03-eng-bpd2srh           │
          (页面组件)            (系统需求概要 srh-eng)          │
                │                   │                   │
                ▼                   ▼                   │
          wft04-eng-srh2srd           │                   │
...
> wft 编号对应瀑布式流程步骤号，非分支编号。每个分支仅在有对应设计活动的步骤出现——如 biz 不涉及 NFR 量化（wft04），eng 从 BP→SR 概要（wft03）→ SR 详细（wft04）→ pa-eng（wft05）。

> **CU 设计权规则**：CU 是引擎提供的配置能力单元，其能力边界、配置入口、配置要素、系统处理、运行期输出和 FR-ENG 性能指标仅由 eng 路径设计。**wft01-eng 设计 CU 承接（名称级识别并分解配置信息组（概要说明） + 分配配置信息组到 CU 承接，含收敛判定）**；**wft02-eng 详细定义 CU（承接已分配 CU，展开操作活动定义/依赖/状态迁移）**；wft03-eng 将 CU 操作活动组织为系统需求概要；wft04-eng 展开功能表单上组件业务；wft05-eng 将其展开为软件组件。biz 路径不得新建、修改或详细设计 CU。
...
第二轮：eng/nfr 汇合与收尾
  eng 线：wft04-eng 的系统需求方案 srd-eng 基线就绪
  nfr 线：wft04-nfr 完成目标绑定并确认首版 wft05-eng NFR 约束包
  汇合条件满足 → wft05-eng 同时消费系统需求方案 srd-eng、FR-BIZ 指标约束包和 wft05-eng NFR 约束包 → 平台 pa-eng 组件需求定义
```
...
| **新输入增量** | 上游产出新一批输入节点（如新 规范化的相关方需求条目、新 bph-biz 节点），状态符合消费条件 | 增量展开新节点，已有方案不受影响 |
...
| 依赖关系 | 说明 | 策略 |
|---------|------|------|
| eng 线 wft05-eng ← nfr 线 wft04-nfr | wft05-eng 消费 NFR 约束包 | eng 前四步与 nfr 可并行；首版约束包须在相关 bph-eng/CU/系统需求方案 srd-eng 可定位后完成目标绑定并确认，wft05-eng 在两侧输入同时就绪后启动 |
...
> **实际建议**：三路径可同时启动第一轮。biz 线最短（3 Skill），eng 线前段（3 Skill）与 nfr 线（2 Skill）可并行；wft04 可先形成全局/共享约束和目标候选，但 CU/页面级约束在相应对象出现后才能正式绑定。eng 系统需求概要/详细（srh-eng/srd-eng）与首版 NFR 约束包均确认后，启动 wft05-eng 收尾。

### 10.3 biz 路径 Skill

| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-biz | 文档（PL5，概要末梢） | PL6-A 活动节点 / PL6-B 正文条目初步判定（结构判定级） | 基于 规范化的相关方需求 推断**业务流程概要**——场景业务（枝）+ 输出文档序列 + **P 组装**（WBS 归组/23 一致性/按构件产品数据清单完整性缺口推断），结构性缺口补充 规范化的相关方需求 回路——"向业务方证明我听懂了" | §七 |
| wft02-biz | 输出文档（PL5） | PL6-A 活动节点 / PL6-B 正文条目 / 表单使用顺序 | 消费 wft01 P 组装，生成 R+P 双视角**业务流程方案**——引擎校验（权威）+ 文档方案 + 活动设计 + 权限 + 任务关系（架构语言）——"设计方案" | §七 |
| wft03-biz | 功能表单（窗口标签页为页面组件承载） | 引擎能力使用需求 | 消费 wft02 业务流程方案 → 生成**系统需求概要**——页面组件；分配 FR-BIZ 指标到引擎/CU；输出 FR-BIZ 指标约束包和 FR-ENG 候选线索——"出配置需求暨配置业务系统pa-eng" | §七 |

### 10.4 eng 路径 Skill

| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 配置信息组（概要说明，PL5，业务流程概要末级节点） | 主责能力域 / 配置信息组（概要说明） / CU 承接 / 依赖 bph-eng（bph-eng 架构末级=场景业务） | 基于 FR-ENG 规范化的相关方需求节点 推断可独立配置、治理和验收的平台能力场景 + 名称级识别并分解配置信息组（概要说明） + 分配配置信息组到 CU 承接——"向平台方证明我听懂了"（业务流程概要，01 层） | §八 |
| wft02-eng | 配置信息组（操作活动定义） | CU（PL4 选配单元）/ 操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式） | 承接已分配 CU，展开操作活动定义/依赖/状态迁移详细定义——"详细定义配置单元"（业务流程方案，02 层） | §八 |
| wft03-eng | 功能表单 | 页面类别 / 操作活动清单（承接 02）/ 页面编排概要（区域/TAB/弹窗入口） | 承接操作活动定义，组织功能表单+概要说明——"出系统需求概要 srh-eng"（系统需求概要，03 层） | §八 |
| wft04-eng | 功能表单（组件业务） | 布局组件/构件 / 组件业务 / pa-eng 层收敛项 | 承接系统需求概要，展开功能表单上各组件及其业务——"出系统需求方案 srd-eng"（系统需求方案，04 层） | §八 |
| wft05-eng | pa-eng 组件 | 前端组件 / 后端组件 / 平台服务组件 | 消费系统需求方案 srd-eng、FR-BIZ 指标约束包和 NFR 约束包，从组件业务推断三类组件及组件架构——"出组件需求"（平台产品架构，05 层） | §八 |

### 10.5 nfr 路径 Skill

| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-nfr | 非功能需求概要 | — | 基于 规范化非功能需求 推断非功能需求概要节点和需求清单，对齐 22 分类资产，基于需求清单补充 NFR 需求——"向 NFR 相关方证明我听懂了你的质量诉求" | §九 |
| wft04-nfr | 非功能需求方案 | 具体非功能需求 | 消费 bph-nfr → 生成量化指标 + NFR 架构 + 分层约束表 + 页面 NFR + 下游约束包 → 闭环检查与资产落账——"产出可验证的指标和可消费的约束包" | §九 |
...
| 维度 | wft01-biz（规范化的相关方需求→bph-biz） | wft02-biz（bph-biz→bpd-biz） | wft03-biz（bpd-biz→配置需求方案） |
|------|----------------------|----------------------|----------------------------|
| 末级节点 | 文档（PL5，结构判定级概要说明） | 输出文档（PL5，细分 PL6-A/PL6-B + 表单使用顺序） | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（窗口标签页为页面组件承载；业务配置需求 + 配置业务系统 pa-eng 双重身份） |
| 输入对象 | FR-BIZ 规范化的相关方需求条目（≤10） | bph-biz 节点（≤5） | bpd-biz 节点（≤5） |
| 输入合法状态 | `待处理` | `可以分解分配` / `待补充分解分配` | `可以srh设计` / `待补充srh设计` |
| 产出状态 | `可以分解分配` | `可以srh设计` | `可构件开发` |
...
| 维度 | wft01-eng（规范化的相关方需求→bph-eng） | wft02-eng（bph-eng→bpd-eng） | wft03-eng（bpd-eng→系统需求概要 srh-eng） | wft04-eng（系统需求概要→系统需求方案 srd-eng） | wft05-eng（系统需求方案→pa-eng） |
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（概要说明，PL5，业务流程概要末级节点） | 配置信息组（操作活动定义） | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（概要说明） | **⑥ IPO功能（叶）**；功能表单为其业务侧载体（组件业务） | pa-eng 组件（前端/后端/平台服务） |
| 输入对象 | FR-ENG 规范化的相关方需求节点 条目（≤10） | bph-eng 节点（≤5） | bpd-eng 节点（P0，≤5） | 系统需求概要 srh-eng（≤5） | 系统需求方案 srd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
| 输入合法状态 | `待处理` | `可以详细定义` / `待补充详细定义` | `可以srh设计` / `待补充srh设计` | `可以详细定义` / `待补充详细定义` | `已详细定义` |
| 产出状态 | `可以详细定义` | `可以srh设计` | `可以详细定义` | `已详细定义` | `待追溯验证` |
...
| 维度 | wft01-nfr（规范化的相关方需求→bph-nfr） | wft04-nfr（bph-nfr→SysReq-NFR） |
|------|--------------------------|----------------------------------|
| 末级节点 | 非功能需求概要 | 非功能需求方案 / 具体非功能需求 |
| 输入对象 | 规范化非功能需求条目（≤10） | bph-nfr 节点（≤5） |
| 输入合法状态 | `待处理` | `可以NFR设计` / `待补充NFR设计` |
| 产出状态 | `可以NFR设计` | bph-nfr → `已NFR设计`，SysReq-NFR → `已验证冻结` |
...
| 02 | `02-eos-business-summary-architecture.md` | 业务流程概要架构 |
| 03 | `03-eos-business-summary-detailed.md` | 业务流程概要详细 |
| 04 | `04-eos-business-detailed.md` | 业务流程方案 |
| 05 | `05-eos-system-requirement-summary-architecture.md` | 系统需求概要架构 |
| 06 | `06-eos-system-requirement-detailed.md` | 系统系统需求方案 |
...
| 字段 | 格式 | 说明 |
|------|------|------|
| `**类型**` | `{层级}.{子类}` | 如 `bpd-biz.L5`、`srh-biz.L3` |
| `**父节点**` | `@{id}` | 父节点 ID |
| `**输入**` | `@{id}` 或自由文本 | 多个用 `/` 分隔 |
| `**输出**` | `@{id}` 或自由文本 | 同上 |
...
> 各文档特有字段（如 bpd-biz 的 `**支撑引擎**`、srh-biz 的 `**验收条件**`/`**量化指标**`）定义在 91 规范各对应章节中。
...
| `@srf-` | srh-biz 节点 | `@srf-proc-apply-001` |
| `@srn-` | SR-NFR 节点 | `@srn-security-001` |
| `@doc-` | 文档/信息载体（23 PL5 输出文档节点 ID；无独立文档资产，定义落 04/05/06，锚点 25 §1.7） | `@doc-purchase-request` |
...
| 要素 | 规则 |
|------|------|
| **标题命名** | 按本 Skill 的末级节点命名（wft01-biz →「业务流程概要设计基础」，wft02-biz →「业务流程详细设计基础」） |
...
bash scripts/read-node.sh <文件> <节点ID>
  → 按节点ID自动识别格式（spr-001 / bph-biz-XXX / @node-xxx / @engine-xxx / ROLE-XXX）
...
graph TD
    ...
    pa-engRALLEL --> CHECK2{② 有待处理反馈？<br/>含人类标注但无 已处理}
...
|---|---------|------|--------------------------|
| ① | **有新输入** | 人类选中了本 Skill 的输入对象，状态符合消费条件 | bph-biz 节点状态为 `可以分解分配` / `待补充分解分配` |
| ② | **有待处理反馈** | 本 Skill 产出的方案中存在人类标注但尚未 `[已处理]` 的条目 | bpd-biz 节点状态为 `待确认方案`，条目含 `[修改]` 未追加 `[已处理]` |
...
| 特化项 | 说明 |
|--------|------|
| 输入对象类型 | 如 bph-biz 节点、bpd-biz 节点、规范化的相关方需求条目 |
...
>
> **共同纪律**：先反馈后新增；各步以「提交基线」维持「diff 只含人类变更」不变式（ort00：Step 3 提交基线；ort01/02/03：Step 4 提交 + Step 6「整体确认」收尾提交；对话反馈即时处理追 `[已处理]`）。详见各 Skill 的反馈检测与提交基线节。
...
> 反馈处理的特化参数（锚点需求、操作条目、推进目标、**清除条件**、级联触发条件、资产对齐级联）由各 Skill 在第三章 Step 2 中定义；其中清除条件定义 `[已处理]` 条目在何种新输入下回到待处理（默认跳过、满足条件清除，见 §A.5.3 Step 1），各 Skill 必须显式定义。
...
> **版本标记与消费追溯**：节点初次生成时版本 = `v1`；每次因上游新输入触发修订，版本号递增。下游 Skill 首次消费时记录 `消费版本：v{N}`，重入时比对版本——若当前版本 > 已消费版本 → 触发变更感知，读取变更影响声明（§A.7），按变更类型处理。**消费版本记录于消费者节点的「来源」字段**（如 srh-biz 的「来源 bpd-biz」、bpd-biz 的「来源 bph-biz」），生产者节点不重复存消费版本——生产者侧下游可见性由资产写回记录（含消费版本列）承担，变更影响声明的「下游已消费节点」按消费者「来源」字段定向查询。**两跳粒度注**：bph-biz→bpd-biz 跳处理粒度是整个 bph-biz（文档序列级），重入靠节点状态路由 + 变更影响声明；bpd-biz→srh-biz 跳处理粒度是节点级（逐 srh-biz 判断基于哪个 bpd-biz 版本），重入靠版本比对。两跳机制不同源于粒度不同，非漏实现。
...
> 各 Skill 特有的影响范围判定表（bpd-biz 变化→srh-biz 受影响范围等）保留在各 Skill 文档中。
...
| 日期 | 版本 | 说明 |
| 2026-09-10 | v5.59 | R0 角色层口径校正（随 94 v1.17）——§5.4 与 §八 两处「bph-eng 架构树上无需角色层做分组维度」改述为「R0 相对 Rn 少「角色职责」一级（SL4）；该角色层在 bph-eng 为单值」。 |
| 2026-09-10 | v5.58 | **树简称随 94 v1.14 对齐**（人类定「严格对齐 94」）——全文 A1/A2/Bn 的树／视角名一律改 **P0／Rn／Pn**：§五 章名改「业务树两族：R（角色履职视角）× P（产品开发视角）」+ 表改四树矩阵 + §5.1/§5.2/§5.3 节名与正文 + **新增 §5.4 R0 树**（由原 §5.2「流程与IT域的 A2 产品」注提成；R0 列末节以保住 §5.3.1 等既有锚点）+ §六「A2 产品」→「R0 树」；§3.1/§3.3「A1 产品树」→「P0 产品树」；§7「A2/Bn 归纳枝」→「R／P 归纳枝」、「Bn 组装」→「P 组装」、§7.3/§8/§10.6 各处视角名（R0 视角／P0 视角／R／P 双视角）同步。**顺带补 v1.13 遗漏**：§7.3 两处「⑤ 用例场景功能…⑥ 任务节点步骤」→「⑤ 输出文档（枝梢）下任务步骤」。**未动**：变更记录历史词；01-pl4eos-spec 的另两套 A1/A2（产品链／产出产品类型）与 wft 链（留待下一轮）。**并案**（人类定「两个视角下就是两棵树」）：§7.3 的「一棵树读法——R／P 不是两条树」改为「**两棵树读法**」——R 树与 P 树是两棵独立的树，在输出文档（PL5）处对位互连、经认亲互指，不是一棵树的共享节点；§7.3「两归纳枝分工说明」→「**两树分工说明**」。 |
| 2026-09-10 | v5.57 | 94 v1.13 校正联动（人类定 2026-09-10）——94 §2.2 末两级并为一级（⑤ 业务输出文档＝枝梢 / ⑥ 活动节点·正文条目＝叶），§八 总锚注的「功能表单对应业务侧 A2/Bn 树的 **⑥ 任务节点**步骤」改述为「**⑤ 业务输出文档（枝梢）下的任务步骤**（该文档的分解源判据展开项）」；§7.3/§10.6 原即 biz 链口径（末级＝PL5），无需改。 |
| 2026-09-10 | v5.56 | 94 结构合并联动（人类定 2026-09-10）——94 原 §二~§七 六章合并为一章六节，§八 总锚注引用的 [94 §3.1/§3.2]（A2/Bn 业务树）→ **[94 §2.2.1/§2.2.2]**（节点内容不变，仅节号随源迁）。 |
| 2026-09-10 | v5.55 | 94 章节编号重排联动（人类定 2026-09-10）——94 章号自「一」起排（原 §〇 → §一，其余顺次后移），§八 总锚注「SR 树的末级与功能表单的关系」引用的 [94 §2.1/§2.2] → **[94 §3.1/§3.2]**（业务侧 A2/Bn 树，节点内容不变，仅节号随源迁）。 |
| 2026-09-10 | v5.54 | **srh 末级口径校正（人类定 2026-09-10，按 94 树权威）**——srh 树的**枝梢＝⑤ 用例场景功能、叶＝⑥ IPO功能**（后者只在完整态）；**功能表单不属于 SR 树**，是「用户操作页面视角」的载体，对应业务侧 A2/Bn 树 **⑥ 任务节点**步骤，跨视角聚合 ④/⑤（两视角正交：分类视角单归属、页面视角多归属）。改十一处：§7.2（wft03-biz 行 + 产出物行 + 概要/方案分工框架注，并新增**总锚注**「SR 树的末级与功能表单的关系」）、§7.3（展开说明 + 树图末级行）、§8.3（wft03-eng 树图 → 枝梢⑤）、§8.4（wft04-eng 树图 → 叶⑥）、§八（wft03-eng 行 + 与 biz 路径对应关系注）、§10.6（biz 表 wft03-biz 行 + eng 表 wft03-eng/wft04-eng 行）。**派生规则**：按 93 概要／完整态——wft03（概要）末级＝⑤ 用例场景功能、wft04（完整态）末级＝⑥ IPO功能；**此派生待人类审视**。**未动**：§7.3「双重身份」句里的"产品架构末级节点"表述、A1 树 配置信息组＝末级（属另一待裁项）。 |
| 2026-09-04 | v5.53 | §5.3 补「5.3.1 资产对齐判据（P 组装）之家」（2026-09-04 判据之家定案，对齐 biz 三试点/93 §四.1）——面向 23 的对齐判据族统一定义于此（23B 对齐 Q1~Q3 / WBS 归组 / 23 一致性三类 / 缺口推断名称级比对），引用数据在 23 不载判据；补 **Q 族命名索引**（对齐 Q1~Q3 ↔ 能力覆盖 Q1~Q4 ↔ 22 分类 Q1~Q3+ 三族分家，防同名指错）。 |
| 2026-08-28 | v5.52 | 层名更名（人类指示 2026-08-28）——业务流程概要定义→业务流程概要、业务流程详细定义→业务流程方案、系统需求概要定义→系统需求概要、系统需求详细定义→系统需求方案（nfr/eng 链同构，含短名/标题后缀）；历史变更记录保留旧词。 |
| 2026-08-27 | v5.51 | s 代号退役（人类指示 2026-08-27）——s01/s02/s03 全链移除，改用具体名称（业务流程概要定义/业务流程详细定义/系统需求概要定义）；树读法位置标注改层名；历史变更记录保留代号。
| 2026-08-27 | v5.50 | 名称统一（人类指示 2026-08-27）——「系统概要定义」→「系统需求概要定义」（biz 链 s03 全称补全，对齐 wft03-biz 产出方标题/文件名 bpd2srh/eng 链 srh 命名节奏）；历史变更记录保留旧词。
| 2026-08-27 | v5.49 | 链级术语清扫（人类指示 2026-08-27）——规范化需求→规范化的相关方需求（术语全称化：需求来源=相关方、加工程度=规范化）。 |
| 2026-08-25 | v5.48 | wft04-nfr 标题正名 + 末级节点改名链级联动（人类定名 2026-08-25）——§9/§9.2/§10.6 中 wft04-nfr 末级节点「非功能需求分类」→「非功能需求详细定义」（4 处：路径分工表/输入输出图/Skill 清单/末级节点对比表）；变更记录历史词保留。 |
| 2026-08-25 | v5.47 | wft04-nfr 深审链级联动（人类逐项裁决 2026-08-25）——§9.2 任务2 分解规则引用「规则见 wft04-nfr §2.4.1」→「§3.2.1」（v5.11 知识层重构残留清扫）。 |
| 2026-08-25 | v5.46 | wft01-nfr 标题正名链级联动（人类定名 2026-08-25）——§9.1 输入术语「NFR 规范化需求」→「规范化非功能需求」（11 处）、产物「角色非功能需求分类」→「非功能需求概要定义」（7 处 + 任务1「角色 NFR 分类节点」→「非功能需求概要定义节点」）；§10.5 nfr 行随迁；变更记录历史词保留。 |
| 2026-08-25 | v5.45 | wft01-nfr 深审链级联动（人类逐项裁决 2026-08-25）——§9.1 任务3 补三准则评审（承接 ort03 候选待写 + Q3/Q3+ 候选，聚类精炼后三准则评审：通过写回 22 疑似、未通过反馈区建议废弃）。 |
| 2026-08-25 | v5.44 | 正式 OR 节点改名链级清扫（人类裁决 2026-08-25）——FR-ENG OR→FR-ENG 规范化需求节点、正式 OR→规范化需求节点、OR 条目→规范化需求条目、节点 ID OR-XXX→spr-XXX、@or-→@spr-（§11.5 ID 前缀表/示例）；链名 OR 预处理→规范化需求预处理。 |
| 2026-08-25 | v5.42 | wft05-eng 深审 C2 链级联动（人类逐项裁决 2026-08-25）——§10.6 eng 路径资产写回列随 srd-eng 文件位置 05→06（06=L4 详细档，对齐 wft03-biz 分层定案）：wft04-eng `05/25`→`06/25`、wft05-eng `05/06/07/23/25`→`06/07/23/25`。 |
| 2026-08-24 | v5.41 | 接力链表 wft04 行 typo 修复（wft04-eng 深审，人类逐项裁决）——§10.6「系统需求概要 srh-eng → srh-eng 系统需求详细需求」→「srh-eng → srd-eng 系统需求方案」。 |
| 2026-08-24 | v5.40 | 输出术语正名（链级联动，对齐 wft01-biz v3.26 / eng 链各 Skill）——业务概要定义→业务流程概要定义、业务详细定义→业务流程详细定义（成对一致，含缩写随迁）；变更记录段保留历史旧词。 |
| 2026-08-24 | v5.39 | 材料状态表联动（wft03 单独轮 B4）——「规范化需求 原料状态表」→「材料状态表」（2 处）；§7.3 正文「pa-eng 末级节点」→「配置业务系统 PA 末级节点」（对齐 wft03 A2）；头部版本号补落后（此前变更记录已到 v5.38）。 |
| 2026-08-24 | v5.38 | 缩写更名（人类定案）——sfh→srh、sfd→srd（sr=系统需求）+ 中文层名统一（系统需求概要/详细定义）；资产名/节点ID/状态名/文件名随迁。 |
| 2026-08-24 | v5.37 | 资产名更名（人类定案，待议项全部定案后执行）——STR-F→bph-biz、STR-E→bph-eng、STR-NFR→bph-nfr、BP→bpd-biz/bpd-eng、SR-F→srh-biz/srh-eng（03 系统需求概要）/srd-eng（04 系统需求详细）、PA→pa-eng（SysReq-NFR 保留）；§7 分工框架「资产名保留」句改写为迁移说明；状态名 可以SR设计→可以srh设计、在PA设计→在pa-eng设计；§11.1 资产清单文件名/说明随迁。 |
| 2026-08-22 | v5.36 | biz 链深审第三轮修复（人类逐项裁决 2026-08-22）——①**消费版本定位置规则**（§A.6.5，B2/B5）：消费版本记录于消费者节点「来源」字段、生产者不重复存、下游可见性由资产写回记录承担、下游已消费列表按来源字段定向查询；补两跳粒度注（STR-F→BP 靠状态路由、BP→SR-F 靠版本比对）；②**§1.4 引擎偏好声明重写**（C1/C2/C3，D 产品数据类优先 / PCA 业务管理类优先 / @engine-task 两场景共用 / 权威校验以任务类型↔引擎映射为准）；③**§1.4/§2.7/§7.1「完全同构」→「流程骨架同构」**（C4，对齐三 Skill 措辞）。 |
| 2026-08-22 | v5.35 | biz 链深审第二轮实锤 1 处置（人类拍板 2026-08-22）——①§7 分工表 wft03 产出物列「功能表单 + 窗口标签页」→「功能表单（窗口标签页为页面组件承载）」；②§4.4 FR-ENG 候选线索字段集需求类型「增强性能」→「扩展能力边界」+ 补 Q2 语义判定注（不涉及数值性能比较，系统性能归 NFR 链）；③覆盖判定语义化本体在 wft03 §3.2（Q1/Q2 改业务指标类型语义对照、删除 CU 性能上限数值比较），§7.3 Task 8 委托「字段集见 wft03-biz §3.2」关系不变。 |
| 2026-08-22 | v5.34 | biz 链深审 C 组 + 取消 26 扩展 25（人类逐项裁决 2026-08-22）——①**取消 26 号文档资产**：§六 6.2 资产表删 26 行（23~28→23/24/25/27/28）、§11.1 文档全景删 26 行、§11.5 `@doc-` 改注（23 PL5 输出文档节点 ID，定义落 04/05/06，锚点 25 §1.7）、§7 分工框架 blockquote 补 26 取消注；业务文档定义承载于 04/05/06，承接锚点 25 §1.7（wft02/wft03 资产写回集 23/24/26/27/28→23/24/27/28）；②**C① 24 资产口径对齐 L0~L6**（§六 6.2 24 行「业务域/业务类型/端到端业务/角色/WBS」→「L1~L6 可复用业务」）；③**C② 末级口径统一**（§7 分工表/§7.3/§10.3/§10.6 末级「功能表单/窗口标签页」→「功能表单（窗口标签页为页面组件承载）」）；④**C⑤ §7 分工框架补两归纳枝分工说明**（PL4 归组在 wft01 与 P 组装结构性绑定、SL4 归纳在 wft02 服务 A2 场景装配）。 |
| 2026-08-22 | v5.33 | eng 链第四轮修复 + NFR 分解设计（人类逐项裁决 2026-08-22）——①§9.2 任务 2 补分解规则摘要引用（四类运算，规则见 wft04-nfr §2.4.1）、任务 6 补分解子指标独立装配 + 消费状态由 wft05-eng 回写；②§11.1 文档清单登记两约束包独立文件（10/11 跨路径契约文档）；③wft05-eng/wft04-nfr/wft06/wft03-biz 同步（A1 §3.4 判据补齐、B1 操作条目、分解处理与消费回写、约束包载体路径）。 |
| 2026-08-22 | v5.32 | eng 链第三轮深审修复（人类逐项裁决 2026-08-22）——①FR-ENG 线索回路/约束包契约：§8.2「补充 规范化需求 材料」→「补充 FR-ENG 材料」、§4.4/§10.3 候选线索字段集引用式（Q4 边界不清纳入触发）、§8.5 Task 1 补 FR-BIZ 约束包消费；②术语统一：「概要末级节点」取消 → 配置信息组（业务概要定义末级节点）、NFR 约束包统一「wft05-eng NFR 约束包」、FR-BIZ 指标统一主名、引擎能力缺口线索 → FR-ENG 候选线索；③§10.6 wft02 资产写回补 01、nfr 参数表补 NFR 约束包交付标注、§3.2 @engine-kanban 对齐分离口径、§A.5.4 补清除条件为 Skill 特化参数强制项。 |
| 2026-08-22 | v5.31 | eng 链架构升级（人类裁决 2026-08-22）——A1 树升级为 4 层（产品→引擎→CU→配置信息组）：配置信息组=P0 产品树末级节点（PL5，可独立交付/验收的能力单元）、CU=PL4（可独立选配的配置单元，由配置信息组构成）、A1 树无 WBS（Bn 树 PL4=WBS 保留）；§3.1 补 A1 PL 树、§3.3 配置信息组定位升级、§5.1 A1 末级=CU→配置信息组、§8/§10.6 末级节点同步（配置信息组对应 biz 文档、CU 对应 WBS）。 |
| 2026-08-22 | v5.30 | §3.3 系统处理对齐 §8.2（人类裁决）——「保存时处理」→「**编辑时处理**」（编辑过程中系统如何校验、写载件与阻断，统一保存机制：设计会话 + 全局保存，见 §8.2），消除 91 内部新旧并存（wft02-eng v10.0 已定案、§8.2 已用新词）；配置信息组说明模板同步。 |
| 2026-08-22 | v5.29 | §11.5 ID 命名前缀表补 `@spr-`（规范化需求 节点，ort03 规范化产出，seq 编号，示例 `@spr-001`）——对齐 ort03 实际产物。 |
| 2026-08-22 | v5.28 | ort 链反馈交互对齐 wft §A.5 双轨（人类确认「ort 应与 wft 保持一致」）——§A.5 预处理链特化改「ort 与 wft 同轨（双轨对话、不用口令表）、ort00 全自动例外」、模式分化表拆三行（ort00 状态扫描·全自动 / ort01 文档编辑+git·双轨 / ort02·ort03 状态扫描·双轨）、共同纪律提交基线编号更新；链级：ort01 v0.60 + ort02 v0.37 + ort03 v0.61（2026-08-22）。 |
| 2026-08-21 | v5.27 | eng 链 5 层「概要/详细」分工框架实施第⑥步（wft01-eng v9.5 + wft02-eng v10.0 + wft03-eng v9.0 + 新增 wft04-eng v1.0 + wft05-eng v10.0 对偶实施，人类裁决插入式重排 2026-08-21）——§8 任务清单重排：§8.1 业务概要定义（wft01-eng，结构判定级）、§8.2 业务详细定义（wft02-eng，操作活动定义步骤化）、§8.3 系统需求概要开发（wft03-eng，功能表单+概要说明）、**新增 §8.4 系统需求详细开发（wft04-eng，组件业务+PA 收敛项）**、原 §8.4→§8.5 产品概要开发（wft05-eng，输入改系统需求详细 SR-F）；§八 章首接力链表四→五个 Skill；§10.1 三路径全景图/执行顺序/CU 设计权规则补 wft04-eng；§10.4 eng 路径 Skill 表加 wft04-eng 行；§10.6 eng 参数表加 wft04-eng 列（wft03 产出状态 `可以详细定义`、wft05 输入状态 `已详细定义`）；链级引用（§3.3 CU 设计权规则/构件表/eng 路径分工/biz 对称性/汇合条件）同步。 |
| 2026-08-21 | v5.26 | biz 链「概要/详细」分工框架叙述完整度补齐（wft01-biz v3.17 + wft02-biz v9.3 + wft03-biz v9.7 对偶实施，人类确认符合性核查后 2026-08-21）——§7.2 分工框架 blockquote 补「命名业务详细定义不破通用规范」兼容句（BP IPO 承担详细角色，通用规范 §1.3 特例）；一棵树读法（A2/P 归纳枝→场景业务→文档→活动/正文条目/表单使用顺序→功能表单）下沉到三个 Skill 概念章；wft01「场景业务（归纳枝）」措辞修正为「汇聚点」。 |
| 2026-08-21 | v5.25 | biz 链「概要/详细」分工框架定案同步（wft01-biz v3.16 + wft02-biz v9.2 对偶实施，人类确认 2026-08-21）——§七 三个 Skill 用「业务概要定义/业务详细定义/系统概要定义」重述分工：wft01 新增 P 组装（WBS 归组 + 23 一致性 + 按构件产品数据清单完整性缺口推断，结构性缺口 规范化需求 回路）、wft02 收缩纯详细层（迁出 WBS 归组/23 一致性/缺口推断）、wft03 末级=功能表单；STR-F/BP/SR-F 资产名保留；层级表联动审查同步——§10.3/§10.6 参数表（末级节点/资产写回/补充材料）、§7.1 任务 7 改 P 组装 + 末级节点=文档、§7.2 末级节点 block 与任务清单（任务 2 消费 P 组装 / 3 引擎候选校验 / 8 设计性缺口）、§5.2/§5.3 设计路径（wft02 详设承接 wft01 P 组装）、附录 A.2.3 标题命名示例。 |
| 2026-08-22 | v5.24 | 分解产物正名链级同步（wft01-eng v9.3 + wft02-eng v9.8 对偶实施，人类确认）——「详细需求条目」→「配置信息组（概要说明）」：§8.1 wft01-eng 细分/任务2/任务4、§8.2 wft02-eng 任务3、§3.3/§10.1 CU 设计权规则、§10.4 参数表、§八 接力链表（wft01 注记补「配置信息组概要」）全部同步。 |
| 2026-08-22 | v5.23 | eng 链设计归属重构（wft01-eng v9.2 + wft02-eng v9.6 对偶实施）——CU 设计权规则改两段（wft01-eng 设计 CU 承接、wft02-eng 详细定义 CU）；§8.1 任务清单加"分解概要末级为详细需求条目 + 分配条目到 CU 承接"；§8.2 收缩为详细定义；§八 接力链表/§10.4/§10.6 同步（STR-E 状态改名 `可以详细定义`/`待补充详细定义`/`已详细定义`，DP1）。 |
