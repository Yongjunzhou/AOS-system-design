...
**P₀ 树（平台／引擎产品的 P 树）**：P₀（§5.1）同样沿产品开发视角（PL）展开——平台 → 业务域（流程与IT）→ 产品类型（平台／引擎产品）→ 构件（引擎）→ **CU（配置单元，PL4）→ 配置信息组（PL5，末级节点）**。P₀ 树无 WBS 层——P_i 树的 PL4=WBS 保留（§5.3），仅 P₀ 树以 CU 直接占据 PL4 位置。配置信息组是 P₀ 产品树的**末级节点**（可独立交付/验收的能力单元），对应 P_i 树的 PL5 输出文档——两者同为各产品树的 PL5 末级；CU 是可独立**选配**的配置单元（PL4，配置入口/页面边界），由若干配置信息组构成（选配粒度=CU、能力粒度=配置信息组，两层面正交不冲突）。P₀ 树到配置信息组为止，无 PL6——操作活动定义（配置操作/系统处理/运行期能力）是配置信息组的内容，非架构节点。
...
**配置单元定义口径**：每个 CU 用以下要素定义。**CU（PL4，可独立选配的配置单元）由配置信息组（PL5，P₀ 产品树末级节点）构成**（每组 = **某组配置字段，这组字段共同提供一个可以运行的能力**，可独立交付/验收），**配置操作、系统处理、运行期能力均以配置信息组为单位说明**；配置入口等留在 CU 级。叙事链为"配置人员操作 → 系统处理 → 业务人员操作"。
...
**三条规则**：① 组的分界按**可以运行的能力**——配置信息组=某组配置字段，这组字段共同提供一个可以运行的能力（运行期产出/驱动一项能力，含业务人员可操作能力与系统侧运行行为两类）；单字段即可提供一项能力时单字段成组，能力不可独立时对应合并；② 运行期能力非必填——某组只提供系统处理、不提供业务互动能力时，显式标注"以系统处理为主"，不硬凑运行期能力；③ 生效后响应一律编号式——"该组配置信息保存后，系统应①…②…③…"，逐条列系统动作。④ **配置信息组是 P₀ 产品树的末级节点（PL5，可独立交付/验收的能力单元），不是 CU 的随意字段分组**——分组的判定以"可独立交付的能力"为边界（§3.1 P₀ 树）。
...
| 树 | 所指产品 | 归属域 | 构件 | 末级节点 |
|----|---------|--------|------|---------|
| **P₀** | 平台本身 | 流程与IT | 引擎 | 配置信息组（CU 为 PL4 可独立选配的配置单元） |
| **R₀** | 平台本身 | 流程与IT | 角色（业务配置人员、平台治理角色） | 场景业务 |
| **R_i** | 各业务域 | 各业务域 | 角色 | 场景业务 |
| **P_i** | 各业务域 | 各业务域 | 产品分解类型（自研件/外购件/复用件/项目等） | 构件的产品数据 |

> 业务树的完整架构定义见 [23 号资产](../../80-pl4eos-2-eosdata/23-eos-output-architecture.md)。
> **节序说明**：本节按**视角**分节（§5.1 P₀／§5.2 R_i／§5.3 P_i／§5.4 R₀），故 i=0 的两棵不连排；R₀ 列于末节，以保持既有节号不动（§5.3.1 等锚点被 93／biz02／biz03／02／04 多处引用）。

### 5.1 P₀ 树（平台／引擎 · 产品开发视角）

P₀ 是**平台本身的产品开发视角树**（平台引擎产品），归属流程与IT域。构件为引擎（`@engine-entity`、`@engine-flow` 等 12 个），末级节点为配置信息组（CU 为 PL4 可独立选配的配置单元，由配置信息组构成；详见 [25 号资产 §2](../../80-pl4eos-2-eosdata/25-eos-engine-models.md#2-引擎定义)）。

每个 CU 定义一个可独立配置、校验、发布的引擎元数据对象，由若干配置信息组（PL5，P₀ 末级节点）构成——配置人员在 CU 配置页面上编辑各组配置要素，提交后引擎校验并保存，运行时按 CU 定义生成运行期制品（物理表/功能表单/菜单入口+构件）。CU 的完整定义见 §3.3 和 §4.3。

对应设计路径为 **eng 路径**：wft01-eng（业务流程概要）→ wft02-eng（业务流程方案）→ wft03-eng（系统需求概要）→ wft04-eng（系统需求方案）→ wft05-eng（平台产品架构/三类组件）。

> **bph-eng 的双重身份**：wft01-eng 产出的 bph-eng 具有双重身份——R₀ 视角与 P₀ 视角各取一面（R₀ 的完整说明见 §5.4）——
> - **R₀ 视角**：它是流程与IT域中**业务配置人员**的 PL5 场景业务。
> - **P₀ 视角**：它对应某个主责能力域中的可独立配置、治理和验收的能力场景。
> 
> 主责能力域可以是引擎，也可以是平台共享能力域。一个能力域可包含多个 bph-eng；多个引擎可消费同一共享 bph-eng，但不得重复定义共享 CU。CU 是 bph-eng 的细分节点，配置信息组是 P₀ 产品树的末级节点（PL5，§3.1 P₀ 树）。

### 5.2 R_i 树（各业务域 · 角色履职视角）

R_i 是**各业务域的角色履职视角树**（角色场景），构件为角色（如采购员、库存管理员），末级节点为场景业务。沿角色场景视角（SL）展开：SL3 角色类型 → SL4 角色职责 → SL5 场景业务 → SL6 功能表单/主子表单。

每棵 R_i 树回答：某角色在哪个菜单入口、按什么顺序使用哪些功能表单/主子表单、完成端到端的职责闭环。场景业务（SL5）与 bph-biz 末级节点 1:1 映射。

对应设计路径为 **biz 路径的前半段**：wft01-biz（业务流程概要，场景业务为枝、文档为末级）→ wft02-biz（业务流程方案，组装 R 产品业务流程 + 表单使用顺序）。

> **流程与IT域的 R 树**即 **R₀**（平台本身的角色履职视角），**不属本节**——它不是业务域角色场景，其角色是在平台上履职的人（业务配置人员、平台治理角色）。完整说明见 §5.4。

### 5.3 P_i 树（各业务域 · 产品开发视角）

P_i 是**各业务域的产品开发视角树**（输出产品），沿产品开发视角（PL）展开：PL1 业务域 → PL2 E2E 项目业务 → PL3 构件类型 → PL4 WBS → PL5 业务输出文档。每棵 P_i 树回答：某类输出文档属于哪个业务域/构件类型，需要哪些前序文档和业务规则才能生成。
...
**与 R_i 的关系**：R_i 和 P_i 是对同一业务的两种互补视角——R_i 描述**角色怎么工作**（谁→在哪个菜单→按什么顺序→使用哪些功能表单→完成职责闭环），P_i 描述**角色产出了什么**（什么产品数据→属于哪个域→经过哪些前序文档→流转到哪里）。两者通过功能表单/主子表单交汇：同一张功能表单，在 R_i 侧是角色的操作界面，在 P_i 侧是产品数据的载体。

对应设计路径为 **biz 路径的后半段**：wft02-biz 展开 P_i 输出产品详细设计（消费 wft01 的 P 组装）→ wft03-biz 生成各层配置需求。
...
R₀ 是**平台本身的角色履职视角树**——它的角色是**在平台上履职的人**，而非业务部门的业务角色。两类角色：
...
> **与 P₀ 的关系**：R₀ 与 P₀ 是**同一个产品（平台本身）的两棵视角树**——R₀ 回答"谁在平台上履职、按什么顺序完成什么闭环"，P₀ 回答"平台的构件（引擎）产出什么产品数据、怎么生成"。二者配对同 §5.2／§5.3 的 R_i／P_i 配对，两族各自与 SL／PL 视角一一对应。
> **bph-eng 的双重身份**由这对视角给出：同一份 bph-eng，在 R₀ 视角是业务配置人员的 SL5/PL5 场景业务，在 P₀ 视角是主责能力域下的能力场景（见 §5.1 注）。
> **角色层退化（R₀ 与 R_i 的结构差异，非遗漏）**：流程与IT域的核心角色类型单一（业务配置人员），故 bph-eng 架构树上无需角色层做分组维度；R_i 则因角色类型多而保留 SL3/SL4 两级归纳。

---

## 六、平台资产治理业务

> 平台资产治理是 EOS 平台的**内置场景业务**（**R₀ 树**的场景业务）：平台自身的管理对象（菜单、架构资产、数据标准、权限）就是平台的业务数据，平台提供对应的引擎和功能表单直接管理这些对象，治理人员使用平台能力治理平台。治理角色即 R₀ 的两类角色之一（见 §5.4）。bph-biz 已预置在 `02-*.md` 中。
>
...
> **概要/方案分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要/方案」重述——wft01 业务流程概要（末级=文档，概要说明为**结构判定级**，字段级详设由 wft02-biz 承接）；wft02 业务流程方案（详细条目=**活动 + 正文条目 + 表单使用顺序**）；wft03 系统需求概要（**枝梢＝⑤ 用例场景功能**，叶＝**⑥ IPO功能**只在完整态；功能表单是业务侧 R／P 树 ⑤ 输出文档（枝梢）下任务步骤的载体，窗口标签页为其页面组件承载形式——2026-08-22 深审统一，2026-09-10 按 94 树权威校正末级口径）。一棵树读法——R 归纳枝（SL3 角色类型→SL4 角色职责）+ P 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bph-biz、BP→bpd-biz、SR-F→srh-biz；追溯/23 不迁。命名「业务流程方案」源于 bpd-biz IPO 承担方案角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = P 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 规范化的相关方需求 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
>
> **两归纳枝分工说明（2026-08-22 深审确认）**：R 归纳枝（SL3→SL4）在 wft02、P 归纳枝（PL3→PL4 WBS）在 wft01——不对称是分工决定的，非遗漏：PL4 WBS 归组与 23B 对齐/23 一致性/缺口推断同属 wft01 P 组装（结构性绑定），SL4 归纳服务 R 场景装配与表单使用顺序（wft02 Step 3），且 R 侧无 23 资产产品数据清单类锚点可做缺口推断。
>
> **26 号文档资产已取消（2026-08-22 深审决策）**：业务文档定义承载于 04/05/06 方案文档，承接锚点在 25 §1.7 业务文档承接。
>
...
| **wft02-eng** | 详细定义已分配的 CU——操作活动定义/依赖/状态迁移 | bph-eng 节点 → P₀ bpd-eng 业务流程方案 | 配置信息组（操作活动定义：配置操作/系统处理/运行期能力；CU 为 PL4 选配单元） | wft03-eng |
| **wft03-eng** | 把操作活动定义组织为系统需求概要（⑤ 用例场景功能 + 功能表单 + 概要说明） | bpd-eng 节点（P₀）→ srh-eng 系统需求概要需求 | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（页面职责/页面类别/操作活动清单承接 02/页面编排概要） | wft04-eng |
...
>
> **bph-eng 的双重身份**（详见 §5.1／§5.4）：在 R₀ 视角下，bph-eng 是业务配置人员的场景业务（SL5/PL5 末级节点）；在 P₀ 视角下，它对应唯一主责能力域下的独立能力场景。主责类型分为`引擎专属`和`平台共享`，CU 是 bph-eng 的细分节点。
>
...
**详细定义 wft01-eng 已分配的 CU——业务流程方案（02 层，操作活动定义步骤化三段式：配置操作/系统处理/运行期能力），承接 bph-eng 的 R₀ 上下文与 P₀ 装配，展开操作活动定义/依赖/状态迁移。**

```
输入                                           产出
bph-eng 节点                                    P₀ 配置单元 bpd-eng 业务流程方案（可以srh设计）
...
| # | 任务 | 视角 | 说明 |
|---|------|------|------|
| 1 | 承接 bph-eng 的 R₀ 上下文 | R₀ | 读取 wft01-eng 装配的 R₀ 信息（业务运行对象、配置/支撑角色、触发事件、业务状态影响）并校验字段完整；只服务 eng 链 CU 推导，不替代 wft02-biz 的 R／P 输出文档 |
| 2 | 承接已分配 CU 并展开 P₀ 装配 | P₀ | 读取 bph-eng 的 CU 承接关系（wft01-eng 已判定收敛/骨架/引擎归属）→ 按主责能力域→能力场景→CU 层级展开详设；共享能力独立建模，消费方只记录依赖 |
| 3 | 定义 CU 操作活动定义 | P₀ | 承接 wft01 的配置信息组清单+结构判定级概要说明，逐组展开**操作活动定义**（配置操作=步骤序列每步含对象+约束+联动 / 系统处理=编辑时处理+生效后响应，统一保存机制与载件标注 / 运行期能力=操作处理分支显式化；基准=PDF v1.5.2），定义配置入口与引擎状态迁移；详设层变更（配置能力扩展）以 `[调整]` 标注；承接缺陷退回 `需wft01修订` |
| 4 | 基于 P₀ 补充引擎需求 | — | 检出详设缺口（操作活动/依赖/状态迁移） → 生成补充 FR-ENG 材料（详设层缺口，登记 材料状态表 A） |

> **R₀／P₀ 双视角**：Task 1 对应 §5.4 所述 R₀ 树（平台本身的角色履职视角），但在 eng 路径中只做"上下文承接"——R₀ 信息由 wft01-eng 装配，本 Skill 读取校验后说明 CU 服务于哪个业务运行对象、配置/支撑角色和触发场景；Task 2~3 对应 §5.1 所述 P₀ 树——描述引擎"怎么被配置"（引擎→CU 层级→配置信息组/操作活动定义）。功能表单本身由 wft03-eng 组织概要、wft04-eng 展开组件业务。
>
...
```
输入                                           产出
bpd-eng 节点（P₀）                                平台系统需求概要 srh-eng（可以详细定义）
...
| 输入对象 | FR-ENG 规范化的相关方需求节点 条目（≤10） | bph-eng 节点（≤5） | bpd-eng 节点（P₀，≤5） | 系统需求概要 srh-eng（≤5） | 系统需求方案 srd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
...
| 日期 | 版本 | 说明 |
| 2026-09-10 | v5.58 | **树简称随 94 v1.14 对齐**（人类定「严格对齐 94」）——全文 A1/A2/Bn 的树／视角名一律改 **P₀／R_i／P_i**：§五 章名改「业务树两族：R（角色履职视角）× P（产品开发视角）」+ 表改四树矩阵 + §5.1/§5.2/§5.3 节名与正文 + **新增 §5.4 R₀ 树**（由原 §5.2「流程与IT域的 A2 产品」注提成；R₀ 列末节以保住 §5.3.1 等既有锚点）+ §六「A2 产品」→「R₀ 树」；§3.1/§3.3「A1 产品树」→「P₀ 产品树」；§7「A2/Bn 归纳枝」→「R／P 归纳枝」、「Bn 组装」→「P 组装」、§7.3/§8/§10.6 各处视角名（R₀ 视角／P₀ 视角／R／P 双视角）同步。**顺带补 v1.13 遗漏**：§7.3 两处「⑤ 用例场景功能…⑥ 任务节点步骤」→「⑤ 输出文档（枝梢）下任务步骤」。**未动**：变更记录历史词；01-pl4eos-spec 的另两套 A1/A2（产品链／产出产品类型）与 wft 链（留待下一轮）。 |
...
| 2026-08-22 | v5.31 | eng 链架构升级（人类裁决 2026-08-22）——A1 树升级为 4 层（产品→引擎→CU→配置信息组）：配置信息组=P₀ 产品树末级节点（PL5，可独立交付/验收的能力单元）、CU=PL4（可独立选配的配置单元，由配置信息组构成）、A1 树无 WBS（Bn 树 PL4=WBS 保留）；§3.1 补 A1 PL 树、§3.3 配置信息组定位升级、§5.1 A1 末级=CU→配置信息组、§8/§10.6 末级节点同步（配置信息组对应 biz 文档、CU 对应 WBS）。 |
