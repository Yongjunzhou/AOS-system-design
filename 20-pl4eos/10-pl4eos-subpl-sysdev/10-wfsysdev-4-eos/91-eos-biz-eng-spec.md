...
>
> **版本**：v5.24 | **修订**：2026-08-21 | **说明**：分解产物正名链级同步（wft01-eng v9.3 + wft02-eng v9.8 对偶实施，人类确认）——「详细需求条目」→「配置信息组（概要说明）」：§8.1 wft01-eng 细分/任务2/任务4、§8.2 wft02-eng 任务3、§3.3/§10.1 CU 设计权规则、§10.4 参数表、§八 接力链表（wft01 注记补「配置信息组概要」）全部同步。上一版 v5.23：eng 链设计归属重构（wft01-eng v9.2 + wft02-eng v9.6 对偶实施）——CU 设计权规则改两段（wft01-eng 设计 CU 承接、wft02-eng 详细定义 CU）；§8.1 任务清单加"分解概要末级为详细需求条目 + 分配条目到 CU 承接"；§8.2 收缩为详细定义；§八 接力链表/§10.4/§10.6 同步（STR-E 状态改名 `可以详细定义`/`待补充详细定义`/`已详细定义`，DP1）。
