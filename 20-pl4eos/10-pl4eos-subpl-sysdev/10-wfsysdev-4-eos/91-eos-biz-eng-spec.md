...
>
> **版本**：v3.15 | **修订**：2026-07-08 | **说明**：通用协议集中化——§8.5 升级为完整 7 步反馈处理流程 + 各 Skill 实例化参数表；§8.6 扩展为通用节点生命周期模型 + 各 Skill 状态名映射；§8.7 扩展为通用变更传播协议 + 声明模板 + 增量清单对照表；新增 §8.8 Skill 通用约定（AI 四标记 + Step End 三区模板 + 三 Skill 实例化总表）。三 Skill 文件同步精简，冗余内容以引用替代。
...
| Skill | 末级节点 | 所属章 |
|-------|---------|--------|
| wft01-eng | 场景业务 | §七 平台能力系统设计 |
| wft02-eng | 配置单元 | §七 平台能力系统设计 |
| wft03-eng | 操作页面 | §七 平台能力系统设计 |
| wft05-eng | 前后端组件 | §七 平台能力系统设计 |
...
biz 路径三 Skill（wft01-biz / wft02-biz / wft03-biz）产出的节点（STR-F / BA / SR-F）遵循统一的生命周期模型：四态主线流转 + 上游变更三条分流 + 下游退回。各 Skill 的 §2.X（生命周期）引用本节并声明本 Skill 的状态名映射。
