...
> 将 bpd-biz 方案展开为功能表单和窗口标签页，并形成引擎能力使用需求与 FR-BIZ 指标分配。功能表单和窗口标签页既是 EOS 的业务配置需求，也是配置生成的业务系统 pa-eng 末级节点；biz 路径在 wft03-biz 结束，不再设置 wft05-biz，也不在 07 重复定义。
...
| 4 | 基于 A1 补充引擎需求 | — | 检出详设缺口（操作活动/依赖/状态迁移） → 生成补充 FR-ENG 材料（详设层缺口，登记 OR 原料状态表 A） |
