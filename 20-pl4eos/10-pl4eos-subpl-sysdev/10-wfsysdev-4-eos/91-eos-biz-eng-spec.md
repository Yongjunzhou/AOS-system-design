...
> **角色层退化（R0 与 Rn 的结构差异，非遗漏）**：流程与IT域的核心角色类型单一（业务配置人员），故 bph-eng 架构树上无需角色层做分组维度；Rn 则因角色类型多而保留 SL3/SL4 两级归纳。
...
> **角色**：eng 路径的设计角色为流程与IT域的**业务配置人员**——所有引擎的配置设计均由该角色执行，bph-eng 架构树上无需角色层做分组维度。流程与IT域的其他角色（业务架构管理员、数据治理员，见 §六）属于平台资产治理角色，不参与 eng 设计路径。
>
