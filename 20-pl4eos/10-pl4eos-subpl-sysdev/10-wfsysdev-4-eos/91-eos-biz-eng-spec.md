...
| **wft02-eng** | 设计可配置的 CU——名称级 → 详细设计 | STR-E 节点 → A1 BA 详细设计 | 配置单元 CU（操作页面：配置页+业务页） | wft03-eng |
| **wft03-eng** | 为每个操作页面配齐构件清单 | BA 节点（A1）→ SR-F 引擎功能需求 | 操作页面（构件） | wft05-eng |
| **wft05-eng** | 产出可交付开发的 PA 组件需求定义 | SR-F 构件清单 + NFR 约束包 → PA 组件需求定义 | PA 组件（前端/后端/平台服务） | 构件开发流水线 |
...
| # | 任务 | 视角 | 说明 |
|---|------|------|------|
| 1 | 装配 A2 产品业务架构 | A2 | 业务配置人员 → STR-E 场景业务 → 操作页面序列。描述业务配置人员从打开引擎配置入口到运行期能力可用的端到端配置工作流 |
| 2 | 装配 A1 产品业务架构 | A1 | 接收 STR-E → 按引擎→CU 层级展开 |
| 3 | 展开 CU 详细设计 | A1 | 配置要素 / 系统处理说明 / 运行期产出 / 引擎状态迁移 |
| 4 | 为每个 CU 设计操作页面 | — | 配置页面 + 业务页面的设计方案 |
| 5 | 基于 A1 补充引擎需求 | — | 检出 CU 缺口 → 生成 FR-ENG 线索或补充 OR 材料 |

> **A2/A1 双视角**：Task 1 对应 §5.2 所述流程与IT域的 A2 产品——描述业务配置人员"怎么工作"（从菜单入口→配置页序列→能力可用）；Task 2~3 对应 §5.1 所述 A1 产品——描述引擎"怎么被配置"（引擎→CU 层级→配置要素）。两者通过操作页面交汇：同一张配置页，在 A2 侧是业务配置人员的操作界面，在 A1 侧是 CU 的配置入口。
>
> **CU 的两类页面**：配置页面为硬代码实现——配置人员编辑 CU 的配置要素，提交后引擎校验并保存；业务页面由引擎按 CU 定义渲染——在操作页面组件上渲染构件能力（见 §4.3）。

### 8.3 系统需求开发（wft03-eng）

**为每个操作页面识别所需构件（展示/交互/数据/反馈类），产出引擎功能 SR-F 需求。对应 biz 路径的 wft03-biz，同属 SR-F 层。**
...
**将构件展开为 PA 组件需求定义——前端组件 + 后端组件 + 平台服务组件，构件开发流水线可据此进入开发。**（eng 路径跳过 wft04——wft04 为 nfr 路径的系统需求开发 Skill，见 §九。）

```
输入                                              产出
SR-F 构件清单 + NFR 约束包                          PA 组件需求定义（待追溯验证）
...
| 3 | 消费 NFR 约束包并制定平台架构决策 | **3a. 加载约束包**——从 wft04-nfr 产出中加载 wft05-eng NFR 约束包，按消费版本分流（首次消费→全量加载，版本递增→按变更声明仅处理增量/修订行）。**3b. 约束→组件映射**——逐约束行判定：CU 级约束 → 对应 CU 的前后端组件设计参数（记入 task 1/2 的组件需求）；PA 组件级约束 → 对应引擎组件的架构参数（并发/缓存/持久化）；全局/业务域/共享约束 → 平台级决策（记入 task 6 的平台服务组件）。**3c. 平台架构决策**——基于映射结果输出四类平台架构：部署架构（节点/集群/容灾）、通信架构（协议/同步异步/熔断降级）、数据架构（存储/副本/备份）、安全架构（认证/授权/审计）。**3d. 闭合验证**——逐约束行检查是否已有对应架构决策覆盖，未覆盖的标记 `[待确认]` 并给出阻塞原因 |
| 4 | 推断前端组件架构 | 组件树 / 路由设计 / 状态管理方案 |
| 5 | 推断后端组件架构 | API 设计 / 服务划分 / 数据访问方案 |
| 6 | 生成平台服务组件需求定义 | 将 task 3c 的四类平台架构决策转化为可交付开发的平台服务组件：**部署拓扑组件**（节点规格定义 / 集群编排 / 自动扩缩容策略 / 容灾切换方案）、**通信中间件组件**（API 网关路由与限流 / 消息队列与主题 / 服务注册发现）、**数据基础设施组件**（主库与只读副本 / 缓存集群 / 分片策略 / 备份恢复）、**安全基础设施组件**（认证授权服务 / 密钥与证书管理 / 审计日志采集与告警）。逐组件标注：所承接的 NFR 约束行 ID、实现策略（自研/外购/云服务）、依赖的其他平台服务组件 |
...
nfr 路径将非功能原始需求（NFR OR）逐级转化为可验证的量化指标和可消费的约束包，最终交付给 eng 路径的 wft05-eng 在 PA 架构设计中落地为平台服务组件。与 biz/eng 路径并行但设计对象不同——biz 设计业务场景"要什么"，eng 设计引擎能力"怎么做"，nfr 设计横切两者的非功能质量约束"做多好"。

```
NFR OR ──→ wft01-nfr ──→ STR-NFR ──→ wft04-nfr ──→ SysReq-NFR ──→ wft05-eng（§八）
            角色NFR分类              量化指标+约束包              PA 平台服务组件
            定性→结构化              "指标可验证了"              "约束落地了"
```
...
| | wft01-nfr | wft04-nfr |
|---|----------|----------|
| **回答什么问题** | 谁在什么场景下关心什么质量属性 | 每个质量属性具体要多好、怎么验证、约束谁 |
| **处理层次** | 相关方需求层（STR-NFR）——定性、角色语境 | 系统需求层（SysReq-NFR）——定量、可验证 |
| **不做什么** | 不量化（数值留给 wft04）；不替代功能设计（功能诉求路由到 biz/eng） | 不消费 biz 路径（NFR 仅流向 eng）；不伪造确定值（信息不足时标 `待确认`） |

**关键概念**：

| 概念 | 说明 |
|------|------|
| **22 号资产** | NFR 分类资产，定义非功能需求的一级类目与分类维度、具体需求模板。是 wft01-nfr 判定分类路径和 wft04-nfr 量化设计、架构组织、资产落账的权威参照。两个 Skill 对 22 的写回见 §10.5 参数表 |
| **NFR 流向** | NFR 约束仅流向 eng 路径——性能/安全/可用性等非功能质量由平台的引擎层保障，不进入 biz 路径 |
| **文档落位** | STR-NFR → `02-*.md`（wft01-nfr 产出后形成）；SysReq-NFR 架构 → `05-*.md`；SysReq-NFR 详细定义 → `06-*.md`（wft04-nfr 落账时写入） |
...
STR-NFR 的末级节点按以下四个维度归组，保证每条 NFR 不丢失角色语境和适用范围线索：

| 锚点维度 | 内容 | 示例 |
|---------|------|------|
| **用户角色** | 提出或承受该非功能诉求的角色/外部系统/保障者 | "采购员""外部供应商""运维保障者" |
| **业务/平台语境** | 业务域、产品类型、STR-F 或 STR-E 线索 | "采购管理域""请假流程 STR-F""表单引擎 STR-E" |
| **NFR 分类维度** | 22 资产中的一级类目 + 分类维度 | "性能/响应时间""安全/访问控制" |
| **适用对象线索** | 可能约束的 STR-F/STR-E/业务域/平台能力/全局对象 | "约束请假流程定义""全局所有页面" |
...
| 2 | 推断 NFR 分类需求清单 | 将 OR 中的质量诉求归纳为分类需求项——定性期望、适用范围线索、来源类型（`OR明示` / `OR隐含` / `同类语境推断` / `资产线索推断`）、待量化线索。**wft01 不将其转为系统量化指标**——量化由 wft04 负责 |
...
每条量化指标必须包含以下六个要素，缺失任一要素则指标不可验证：

| 要素 | 说明 | 示例 |
|------|------|------|
| **指标名称** | 度量对象的唯一标识 | "采购列表页首屏加载时间" |
| **目标值与单位** | 量化阈值及计量单位 | "≤ 2s" |
| **统计口径** | 时间窗口 / 样本范围 / 百分位 | "P95，工作时间（9:00-18:00），30天滚动窗口" |
| **测量方法** | 如何获取指标值 | "压测 / 审计检查 / 日志统计 / APM 埋点" |
| **验收条件** | 判定通过/失败的具体标准 | "P95 ≤ 2s 且 P99 ≤ 3s，连续 7 天达标" |
| **来源与消费版本** | 来源 STR-NFR 节点 + 消费版本号 | "STR-NFR-003，v1.2" |

> 模糊诉求处理原则：可依据 22 具体需求模板和同类场景推断时标记 `[需裁决]` 并给出推断依据；无法推断时保留 `待确认`，不推进写回、不伪造确定值。
...
| 2 | 生成量化指标 | 逐需求项提取待量化线索 → 匹配 22 具体需求模板和同类场景 → 按六要素生成完整指标定义。信息不足时标注 `待确认`，不伪造确定值 |
| 3 | 生成非功能需求架构 | 按 22 分类维度 + 约束对象 + 指标口径匹配或新建 SysReq-NFR 节点，保证每条需求项有唯一系统侧宿主 |
| 4 | 生成分层约束表 | 逐量化指标判定七层约束范围（全局 / 业务域 / SR-F / CU / PA组件 / 操作页面 / 共享）→ 约束对象 → 影响级别（阻断型 / 非阻断型 / 待判定）→ 影响点。分层约束表是 task 6 装配约束包的权威数据源 |
| 5 | 生成操作页面 NFR | 影响用户可感知页面行为的指标 → 绑定六类页面类型之一 + 约束内容 + 量化目标 + 测量方法 + 消费目标。覆盖六个维度：页面性能 / 安全 / 易用性 / 可访问性 / 可恢复性 / 可观测性 |
| 6 | 装配下游约束包 | 将分层约束表（task 4）和操作页面 NFR（task 5）按约束范围组织为 **wft05-eng NFR 约束包**。约束包只引用分层约束表行 ID 和操作页面 NFR 条目，不复制正文。NFR 约束仅流向 eng 路径——非功能质量由引擎层保障，biz 路径不消费 |
| 7 | 闭环检查与资产落账 | 七项检查（量化完整 / 宿主唯一 / 分层正确 / 页面可消费 / 22可落账 / 06可验证 / eng约束包完整）→ 22 具体需求写回 → 06 系统需求详细定义 → 05/02 状态推进 → STR-NFR 消费版本记录 |

---

### 9.3 NFR 约束包 → wft05-eng 交接

wft04-nfr task 6 产出的约束包是 nfr 路径的最终交付物。wft05-eng 的 task 3 加载后分两条路径落地：

```
wft04-nfr task 6
  └─ wft05-eng NFR 约束包（7 类，引用分层约束表行 ID + 操作页面 NFR 条目）
        │
        ▼ wft05-eng task 3：加载 → 映射 → 平台架构决策 → 闭合验证
        │
        ├── CU 级 / PA 组件级约束 ──→ task 1/2/4/5（前后端组件设计参数）
        │
        └── 全局 / 业务域 / 共享 / SR-F / 操作页面约束 ──→ task 6（平台服务组件）
              ├─ 部署拓扑组件（节点规格 / 集群编排 / 扩缩容 / 容灾）
              ├─ 通信中间件组件（API 网关 / 消息队列 / 服务注册发现）
              ├─ 数据基础设施组件（主库与副本 / 缓存集群 / 备份恢复）
              └─ 安全基础设施组件（认证授权 / 密钥管理 / 审计日志）
```

**引用机制**：约束包不复制正文，只引用行 ID。wft05-eng 按行 ID 回溯分层约束表获取完整约束定义，按操作页面 NFR 条目 ID 获取页面级约束。闭合验证时逐行检查是否已有架构决策覆盖。

**消费版本**：首次消费 → wft05-eng 全量加载并做全量设计。wft04-nfr 版本递增后 → wft05-eng 按变更声明仅处理增量/修订行，已消费的约束行不做重复设计。
...
| wft02-biz | 输出文档 | 生成 A2+Bn 产品业务架构；将 wft01 的业务节点转化为 PL6-A 节点需求定义（架构语言）——"设计方案" | §七 |
...
| wft02-eng | 配置单元（CU） | 操作页面（配置页+业务页） | 装配 A2（业务配置人员→STR-E→操作页面序列）+ A1（引擎→CU 层级）+ CU 详细设计 + 操作页面方案——"设计方案" | §八 |
| wft03-eng | 操作页面 | 构件 | 逐操作页面识别所需构件（展示/交互/数据/反馈类），生成 SR-F 构件清单——"配齐构件" | §八 |
| wft05-eng | 构件 | 前端组件 + 后端组件 + 平台服务组件 | 逐构件判定前后端组件需求 + 消费 NFR 约束包→生成平台服务组件 + 推断三层架构——"出组件需求" | §八 |
...
| 输入对象 | FR-ENG OR 条目（≤10） | STR-E 节点（≤5） | BA 节点（A1，≤5） | SR-F 构件清单 + wft05-eng NFR 约束包 |
