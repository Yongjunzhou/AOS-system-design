...
| **PL**（输出产品视角） | PL0→PL6 | 平台 → 业务域 → 产品类型 → 构件 → WBS → 输出文档 → 活动节点（仅流程/工单类） |
| **EL**（引擎视角） | EL1→EL3 | 菜单引擎 → 功能引擎 → 实体引擎 |

> 其余视角层级的完整定义见 `23-eos-output-architecture.md` 和 `24-eos-business-assets.md`。

**PL6 活动节点**：流程类和工单类 PL5 输出文档的内部构成——每个活动节点为该文档生命周期中的一个处理步骤，包含处理动作（谁在这一步做什么）和文档状态（该步骤执行后的文档所处状态）。活动节点序列即输出文档的状态变迁路径（如草稿→已提交→审批中→已通过/已驳回）。非流程/非工单类 PL5 文档无 PL6。活动节点的完整定义见 wft02-biz §2.5。

> **"输出文档"的指代**：本文档及 biz 路径各 Skill 中提及的"输出文档"，均指代文档的**内容本身**而非其容器或文件名。例如"相关方需求文档"指需求条目本身，"订单下达及时率"指该指标的定义与计算口径本身。文档的承载表单（如 @engine-kanban）仅为组织容器，不影响每个条目作为独立 PL5 输出文档的地位。
...
- **功能引擎 CU**（如 @engine-indicator CU）→ 在指标页面上增加指标定义、聚合计算和看板展示能力
...
> **活动节点设计原则**：活动节点是对 STR-F 需求的方案响应，而非对 OR 词汇的机械映射。设计时以 STR-F 场景需求理解为主驱动——该输出文档在业务链中承担什么职责、需要哪些处理步骤方能完成端到端闭环、每个步骤由哪个角色执行并产出什么文档状态。引擎能力边界是约束（超出能力的不修改方案迁就），同类文档模式和行业经验是参考（不替代需求分析），人类反馈与确认是门禁（全部节点标注 `[推断]`，未经确认不得落账）。详见 wft02-biz §3.5.2。
