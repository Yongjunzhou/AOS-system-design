...
**场景业务**是 bpb-biz（业务流程框架架构末级节点）的组织单元。每条场景业务定义一条**端到端业务闭环**——从需求提出到需求被满足并确认的完整链路。
...
**从输出文档到引擎支撑**（自顶向下）：bpb-biz 架构末级节点是场景业务，场景业务由输出文档序列细化定义。从输出文档出发，按生成方式分两条路径确定支撑引擎链：
...
> **bpb-eng 的双重身份**：wft01-eng 产出的 bpb-eng 具有双重身份——R0 视角与 P0 视角各取一面（R0 的完整说明见 §5.4）——
> - **R0 视角**：它是流程与IT域中**业务配置人员**的 PL5 场景业务。
> - **P0 视角**：它对应某个主责能力域中的可独立配置、治理和验收的能力场景。
> 
> 主责能力域可以是引擎，也可以是平台共享能力域。一个能力域可包含多个 bpb-eng；多个引擎可消费同一共享 bpb-eng，但不得重复定义共享 CU。CU 是 bpb-eng 的细分节点，配置信息组是 P0 产品树的末级节点（PL5，§3.1 P0 树）。
...
每棵 Rn 树回答：某角色在哪个菜单入口、按什么顺序使用哪些功能表单/主子表单、完成端到端的职责闭环。场景业务（SL5）与 bpb-biz 末级节点 1:1 映射。
...
| 角色 | 场景业务 | 架构位置 |
|------|---------|---------|
| **业务配置人员** | bpb-eng，归组锚点＝`主责能力域 + 配置/治理目标 + 可独立验收结果`（不是引擎数量） | 与各业务域 bpb-biz 同层同构，区别在于场景产出物是**可用的平台能力** |
| **平台治理角色**（业务架构管理员、数据治理员等） | 平台资产治理业务（菜单配置／资产管理／数据治理／权限治理） | bpb-biz 已预置在 `02-*.md`（详见 §六） |
...
> **bpb-eng 的双重身份**由这对视角给出：同一份 bpb-eng，在 R0 视角是业务配置人员的 SL5/PL5 场景业务，在 P0 视角是主责能力域下的能力场景（见 §5.1 注）。
...
> 平台资产治理是 EOS 平台的**内置场景业务**（**R0 树**的场景业务）：平台自身的管理对象（菜单、架构资产、数据标准、权限）就是平台的业务数据，平台提供对应的引擎和功能表单直接管理这些对象，治理人员使用平台能力治理平台。治理角色即 R0 的两类角色之一（见 §5.4）。bpb-biz 已预置在 `02-*.md` 中。
>
...
> 治理 bpb-biz 的完整清单以 `02-*.md` 为准，AI 运行时动态读取。上述四类治理业务当前 规范化的相关方需求 承接为空，由人类在 EOS 平台建设过程中逐批增补。
...
|-------|------|------|---------|---------|
| **wft01-biz** | FR-BIZ 规范化的相关方需求条目 | 业务流程框架——bpb-biz 场景业务（枝）+ PL5 输出文档序列（框架末梢）+ P 组装（WBS 归组/23 一致性/缺口推断） | 文档（PL5，结构判定级框架说明） | 向业务方证明"我听懂了你的业务" |
| **wft02-biz** | bpb-biz 节点 | 业务流程概要——R（角色→场景业务→表单使用顺序）+ P（输出文档详细设计 + PL6-A 活动节点需求 + PL6-B 条目需求） | 输出文档（PL5），细分 PL6-A / PL6-B + 表单使用顺序 | 设计正确的方案——业务语言→架构语言 |
| **wft03-biz** | bpl-biz 节点 | 系统需求框架——⑤ 用例场景功能 + 功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（窗口标签页为页面组件承载） | 产出可交付构件开发的配置业务系统 pa-eng，并向 eng 分配指标 |
...
| **产出物** | 业务流程框架（bpb-biz）——场景业务 + 输出文档序列 + P 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务流程概要（BP）——R 角色场景（功能表单/主子表单 + 表单使用顺序）+ P 输出产品（输出文档详细设计 + PL6-A 节点需序）+ P 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统需求框架（业务配置需求暨配置业务系统 pa-eng）——⑤ 用例场景功能（枝梢；功能表单为其业务侧载体，窗口标签页为页面组件承载）+ 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
| **供谁消费** | 业务方确认 → wft02-biz | wft03-biz | 构件开发流水线 + eng 路径（引擎缺口线索） |

> **框架/方案分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「框架/方案」重述——wft01 业务流程框架（末级=文档，框架说明为**结构判定级**，字段级详设由 wft02-biz 承接）；wft02 业务流程概要（详细条目=**活动 + 正文条目 + 表单使用顺序**）；wft03 系统需求框架（**枝梢＝⑤ 用例场景功能**，叶＝**⑥ IPO功能**只在完整态；功能表单是业务侧 R／P 树 ⑤ 输出文档（枝梢）下任务步骤的载体，窗口标签页为其页面组件承载形式——2026-08-22 深审统一，2026-09-10 按 94 树权威校正末级口径）。两棵树读法——**R 树**（SL3 角色类型→SL4 角色职责→SL5 场景业务）+ **P 树**（PL3 构件类型→PL4 WBS→PL5 输出文档）是**两棵独立的树**，在**输出文档（PL5）**处对位互连、经认亲互指，**不是一棵树的共享节点**；两条链自文档（PL5）起并行展开 → 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bpb-biz、BP→bpl-biz、SR-F→srb-biz；追溯/23 不迁。命名「业务流程概要」源于 bpl-biz IPO 承担方案角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = P 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 规范化的相关方需求 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
> **26 号文档资产已取消（2026-08-22 深审决策）**：业务文档定义承载于 04/05/06 方案文档，承接锚点在 25 §1.7 业务文档承接。
>
> 业务流程框架架构文档 `02-*.md` 在 wft01-biz 产出 bpb-biz 后形成。D 和 PCA 场景的处理逻辑流程骨架同构。
>
...
```
输入                         产出
FR-BIZ 规范化的相关方需求（待处理）  ──→  业务流程框架 bpb-biz 场景业务方案（可以分解分配）
...
> 将 bpb-biz 场景业务展开为完整的 bpl-biz 方案——R（角色怎么工作）+ P（角色产出什么）。核心使命：**设计正确的方案**，完成业务语言 → 架构语言的转化。

```
输入                               产出
bpb-biz 节点（含 wft01 P 组装结论）   业务流程概要 bpl-biz 方案（可以srb设计）
（可以分解分配 / 待补充分解分配）  ──→  末级节点：输出文档（PL5）
                                      细分：PL6-A 活动节点 / PL6-B 正文条目 / 表单使用顺序
```

**关键转化——业务语言 → 架构语言**：wft01-biz 用业务方听得懂的话描述活动节点和关系，wft02-biz 将其转化为架构语言描述的 bpl-biz 设计方案——将任务类型映射到对应引擎，逐 PL6-A 活动节点定义处理角色/可执行动作/状态变迁（全量标注 `[推断]`），供 wft03-biz 展开为功能表单/主子表单序列和流程类表单标签页序列的需求定义：
...
| 3 | 引擎候选校验 | P | 在 bpl-biz 上下文校验 wft01 引擎候选建议（确认/修正/推翻）——引擎权威在 bpl-biz 校验值 |
...
> 将 bpl-biz 方案展开为 srb-biz 系统需求（**枝梢＝⑤ 用例场景功能**，叶＝⑥ IPO功能），并形成引擎能力使用需求与 FR-BIZ 指标分配。该产出兼具两重身份——从 EOS 平台视角是业务配置需求，从配置业务系统视角是产品架构节点（**功能表单**为其在业务侧 R／P 树 ⑤ 输出文档（枝梢）下任务步骤上的载体形式）；biz 路径在 wft03-biz 结束，不再设置 wft05-biz，也不在 07 重复定义。

```
输入                               产出
bpl-biz 节点                               功能表单 + 窗口标签页
...
```
FR-ENG 规范化的相关方需求节点 ──→ wft01-eng ──→ bpb-eng ──→ wft02-eng ──→ bpl-eng ──→ wft03-eng ──→ srb-eng ──→ wft04-eng ──→ srl-eng ──→ wft05-eng ──→ pa-eng
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明"我听懂了你要的平台能力" | FR-ENG 规范化的相关方需求节点 条目 → bpb-eng 业务流程框架方案 | 配置信息组（框架说明，PL5 末级，业务流程框架末级节点；bpb-eng 架构末级=场景业务 SL5）+ CU 承接 + 依赖 bpb-eng（共享依赖） | 业务配置人员确认 → wft02-eng |
| **wft02-eng** | 详细定义已分配的 CU——操作活动定义/依赖/状态迁移 | bpb-eng 节点 → P0 bpl-eng 业务流程概要 | 配置信息组（操作活动定义：配置操作/系统处理/运行期能力；CU 为 PL4 选配单元） | wft03-eng |
| **wft03-eng** | 把操作活动定义组织为系统需求框架（⑤ 用例场景功能 + 功能表单 + 框架说明） | bpl-eng 节点（P0）→ srb-eng 系统需求框架需求 | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（页面职责/页面类别/操作活动清单承接 02/页面编排框架） | wft04-eng |
...
> **角色**：eng 路径的设计角色为流程与IT域的**业务配置人员**——所有引擎的配置设计均由该角色执行，bpb-eng 架构树上该角色层为单值（无需再做分组维度）。流程与IT域的其他角色（业务架构管理员、数据治理员，见 §六）属于平台资产治理角色，不参与 eng 设计路径。
>
> **bpb-eng 的双重身份**（详见 §5.1／§5.4）：在 R0 视角下，bpb-eng 是业务配置人员的场景业务（SL5/PL5 末级节点）；在 P0 视角下，它对应唯一主责能力域下的独立能力场景。主责类型分为`引擎专属`和`平台共享`，CU 是 bpb-eng 的细分节点。
>
...
```
输入                              产出
FR-ENG 规范化的相关方需求节点 条目（待处理）  ──→  bpb-eng 平台能力场景业务方案（可以详细定义）
                               P0 产品树末级：配置信息组（PL5，框架说明，结构判定级）
                               bpb-eng 架构末级：场景业务（SL5）
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 推断生成平台能力场景业务节点 | 判定主责类型和主责能力域，按配置/治理目标及独立验收结果匹配已有 bpb-eng 或新建 |
| 2 | 名称级识别并分解配置信息组（框架说明）+ 分配配置信息组到 CU 承接 | 对称于 bpb-biz 下辖的输出文档——场景业务由哪些 CU 组成；按"可以运行的能力"名称级识别并分解为配置信息组（框架说明，**结构判定级**——配置什么/运行期提供什么，不展开操作活动定义——那是 wft02-eng 业务流程概要），逐组分配 CU 承接（含收敛判定复用/新增/拆分/合并 + 对照 25 盘存的骨架判定） |
| 3 | 基于能力可交付性补充平台需求 | 逐 bpb-eng 检查配置/治理目标能否形成可独立验收的端到端闭环；检出实质缺口则形成补充 FR-ENG 材料 |
| 4 | 建立追溯链 | FR-ENG 规范化的相关方需求节点 → bpb-eng主责能力域 → 配置信息组（框架说明） → CU承接/依赖bpb-eng；共享能力记录提供能力、消费方和复用范围 |

### 8.2 业务流程开发（wft02-eng）

**详细定义 wft01-eng 已分配的 CU——业务流程概要（02 层，操作活动定义步骤化三段式：配置操作/系统处理/运行期能力），承接 bpb-eng 的 R0 上下文与 P0 装配，展开操作活动定义/依赖/状态迁移。**

```
输入                                           产出
bpb-eng 节点                                    P0 配置单元 bpl-eng 业务流程概要（可以srb设计）
...
| # | 任务 | 视角 | 说明 |
|---|------|------|------|
| 1 | 承接 bpb-eng 的 R0 上下文 | R0 | 读取 wft01-eng 装配的 R0 信息（业务运行对象、配置/支撑角色、触发事件、业务状态影响）并校验字段完整；只服务 eng 链 CU 推导，不替代 wft02-biz 的 R／P 输出文档 |
| 2 | 承接已分配 CU 并展开 P0 装配 | P0 | 读取 bpb-eng 的 CU 承接关系（wft01-eng 已判定收敛/骨架/引擎归属）→ 按主责能力域→能力场景→CU 层级展开详设；共享能力独立建模，消费方只记录依赖 |
...
> **R0／P0 双视角**：Task 1 对应 §5.4 所述 R0 树（平台本身的角色履职视角），但在 eng 路径中只做"上下文承接"——R0 信息由 wft01-eng 装配，本 Skill 读取校验后说明 CU 服务于哪个业务运行对象、配置/支撑角色和触发场景；Task 2~3 对应 §5.1 所述 P0 树——描述引擎"怎么被配置"（引擎→CU 层级→配置信息组/操作活动定义）。功能表单本身由 wft03-eng 组织框架、wft04-eng 展开组件业务。
>
...
```
输入                                           产出
bpl-eng 节点（P0）                                平台系统需求框架 srb-eng（可以详细定义）
...
```
规范化非功能需求 ──→ wft01-nfr ──→ bpb-nfr ──→ wft04-nfr ──→ SysReq-NFR
            角色NFR分类              量化指标+页面 NFR
            定性→结构化              "指标可验证了"
```
...
|-------|---------|------------|-----------------|---------|
| **wft01-nfr** | 向 NFR 相关方证明"我听懂了你的质量诉求" | 规范化非功能需求条目 → bpb-nfr 非功能需求框架方案 | 非功能需求框架 | NFR 相关方确认 → wft04-nfr |
| **wft04-nfr** | 产出可验证的指标和可消费的约束包 | bpb-nfr 节点 → SysReq-NFR 架构 + 量化约束 + 页面 NFR | 非功能需求方案 | wft05-eng / 追溯验证 |
...
>
> 业务流程框架架构文档 `02-*.md` 在 wft01-nfr 产出 bpb-nfr 后形成。SysReq-NFR 写入 `05-*.md`，06 详细定义在 wft04-nfr 落账时写入 `06-*.md`。NFR 约束全部流向 eng 路径（能力链）——性能/安全/可用性等非功能质量约束由平台的引擎层保障，不进入 biz 路径（业务配置链）。`适用链路=biz` 仅表示约束来源于或影响某个业务语境；设计消费方仍是 wft05-eng，最终由前端、后端或平台服务组件承接。
...
```
输入                                   产出
规范化非功能需求条目（待处理）  ──→  bpb-nfr 非功能需求框架方案（可以NFR设计）
                                末级节点：非功能需求框架
```

bpb-nfr 的归组锚点为四元组——**用户角色**（提出或承受该非功能诉求的角色/外部系统/保障者）+ **业务/平台语境**（业务域/产品类型/bpb-biz或bpb-eng线索）+ **NFR 分类维度**（22 资产中的一级类目+分类维度）+ **适用对象线索**（可能约束的 bpb-biz/bpb-eng/业务域/平台能力/全局对象）。

**任务清单**：

| # | 任务 | 说明 |
|---|------|------|
| 1 | 推断非功能需求框架节点 | 识别 规范化的相关方需求 背后的用户角色、业务域/平台能力语境、NFR 一级类目与分类维度，形成 bpb-nfr 末级节点 |
...
**定位**：nfr 路径的第二个 Skill，承接 bpb-nfr 中相关方经 ort02 澄清确认的目标值（[通用设计规范 §1.2](../../../00-generalspec/01-generalspec-sysdev.md#12-需求详解) 需求=需求描述+性能指标），展开为系统侧 SysReq-NFR 架构——量化指标 + 分层约束表 + 页面 NFR。核心使命是**产出可验证的指标和可消费的约束包**，供 eng 路径和追溯验证消费。

```
输入                                              产出
bpb-nfr 节点                                        SysReq-NFR 架构 + 量化约束 + 页面 NFR
（可以NFR设计 / 待补充NFR设计）  ──→                （bpb-nfr → 已NFR设计，SysReq-NFR → 已验证冻结）
                                                    末级节点：非功能需求方案
                                                    细分：具体非功能需求
```

量化指标的六要素：指标名称 / 目标值与单位（目标值由相关方经 ort02 澄清确认，[通用设计规范 §1.2](../../../00-generalspec/01-generalspec-sysdev.md#12-需求详解)）/ 统计口径（时间窗口/样本范围/百分位） / 测量方法（压测/审计检查/日志统计等） / 验收条件 / 来源 bpb-nfr 与消费版本。目标值缺失或不可验证时：可依据 22 具体需求模板和同类场景推断则标记 `[需裁决]` 并给出依据；无法推断时保留 `待确认`，不推进写回。

**任务清单**：

| # | 任务 | 说明 |
|---|------|------|
| 1 | 消费 bpb-nfr 分类需求清单 | 逐 bpb-nfr 检查消费版本——首次消费→全量设计，版本递增→按变更声明执行增量/修订/结构型分流。分类边界或角色语境不足→退回 `需wft01修订` |
...
| 7 | 闭环检查与资产落账 | 七项闭环检查（量化完整/宿主唯一/分层正确/页面可消费/eng一致/22可落账/06可验证）→22 具体需求写回→06 系统需求概要→05/02 状态推进→bpb-nfr 消费版本记录 |
...
原始材料 → ort00文本化 → ort01切分 → ort02澄清
                ...
                │                   │                   │
                ▼                   ▼                   ▼
          wft01-biz-spr2bpb   wft01-eng-spr2bpb   wft01-nfr-or2str
          (bpb-biz 场景业务)    (bpb-eng 引擎场景)   (bpb-nfr 分类)
                │                   │                   │
                ▼                   ▼                   ▼
          wft02-biz-bpb2bpl   wft02-eng-bpb2bpl   wft04-nfr-str2sr
          (bpl-biz 双视角)          (业务流程概要)     (SysReq-NFR 量化)
                │                   │                   │
                ▼                   ▼                   │
          wft03-biz-bpl2srb    wft03-eng-bpl2srb           │
          (页面组件)            (系统需求框架 srb-eng)          │
                │                   │                   │
                ▼                   ▼                   │
          wft04-eng-srb2srl           │                   │
...
| **新输入增量** | 上游产出新一批输入节点（如新 规范化的相关方需求条目、新 bpb-biz 节点），状态符合消费条件 | 增量展开新节点，已有方案不受影响 |
...
| 依赖关系 | 说明 | 策略 |
|---------|------|------|
| eng 线 wft05-eng ← nfr 线 wft04-nfr | wft05-eng 消费 NFR 约束包 | eng 前四步与 nfr 可并行；首版约束包须在相关 bpb-eng/CU/系统需求概要 srl-eng 可定位后完成目标绑定并确认，wft05-eng 在两侧输入同时就绪后启动 |
...
| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 配置信息组（框架说明，PL5，业务流程框架末级节点） | 主责能力域 / 配置信息组（框架说明） / CU 承接 / 依赖 bpb-eng（bpb-eng 架构末级=场景业务） | 基于 FR-ENG 规范化的相关方需求节点 推断可独立配置、治理和验收的平台能力场景 + 名称级识别并分解配置信息组（框架说明） + 分配配置信息组到 CU 承接——"向平台方证明我听懂了"（业务流程框架，01 层） | §八 |
...
| wft04-nfr | 非功能需求方案 | 具体非功能需求 | 消费 bpb-nfr → 生成量化指标 + NFR 架构 + 分层约束表 + 页面 NFR + 下游约束包 → 闭环检查与资产落账——"产出可验证的指标和可消费的约束包" | §九 |
...
| 维度 | wft01-biz（规范化的相关方需求→bpb-biz） | wft02-biz（bpb-biz→bpl-biz） | wft03-biz（bpl-biz→配置需求方案） |
|------|----------------------|----------------------|----------------------------|
| 末级节点 | 文档（PL5，结构判定级框架说明） | 输出文档（PL5，细分 PL6-A/PL6-B + 表单使用顺序） | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（窗口标签页为页面组件承载；业务配置需求 + 配置业务系统 pa-eng 双重身份） |
| 输入对象 | FR-BIZ 规范化的相关方需求条目（≤10） | bpb-biz 节点（≤5） | bpl-biz 节点（≤5） |
...
| 维度 | wft01-eng（规范化的相关方需求→bpb-eng） | wft02-eng（bpb-eng→bpl-eng） | wft03-eng（bpl-eng→系统需求框架 srb-eng） | wft04-eng（系统需求框架→系统需求概要 srl-eng） | wft05-eng（系统需求概要→pa-eng） |
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（框架说明，PL5，业务流程框架末级节点） | 配置信息组（操作活动定义） | **⑤ 用例场景功能（枝梢）**；功能表单为其业务侧载体（框架说明） | **⑥ IPO功能（叶）**；功能表单为其业务侧载体（组件业务） | pa-eng 组件（前端/后端/平台服务） |
| 输入对象 | FR-ENG 规范化的相关方需求节点 条目（≤10） | bpb-eng 节点（≤5） | bpl-eng 节点（P0，≤5） | 系统需求框架 srb-eng（≤5） | 系统需求概要 srl-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
...
| 维度 | wft01-nfr（规范化的相关方需求→bpb-nfr） | wft04-nfr（bpb-nfr→SysReq-NFR） |
|------|--------------------------|----------------------------------|
| 末级节点 | 非功能需求框架 | 非功能需求方案 / 具体非功能需求 |
| 输入对象 | 规范化非功能需求条目（≤10） | bpb-nfr 节点（≤5） |
| 输入合法状态 | `待处理` | `可以NFR设计` / `待补充NFR设计` |
| 产出状态 | `可以NFR设计` | bpb-nfr → `已NFR设计`，SysReq-NFR → `已验证冻结` |
...
| 字段 | 格式 | 说明 |
|------|------|------|
| `**类型**` | `{层级}.{子类}` | 如 `bpl-biz.L5`、`srb-biz.L3` |
| `**父节点**` | `@{id}` | 父节点 ID |
| `**输入**` | `@{id}` 或自由文本 | 多个用 `/` 分隔 |
| `**输出**` | `@{id}` 或自由文本 | 同上 |
...
> 各文档特有字段（如 bpl-biz 的 `**支撑引擎**`、srb-biz 的 `**验收条件**`/`**量化指标**`）定义在 91 规范各对应章节中。
...
| `@bp-` | bpl-biz 节点 | `@bp-proc-apply-001` |
...
bash scripts/read-node.sh <文件> <节点ID>
  → 按节点ID自动识别格式（spr-001 / bpb-biz-XXX / @node-xxx / @engine-xxx / ROLE-XXX）
...
|---|---------|------|--------------------------|
| ① | **有新输入** | 人类选中了本 Skill 的输入对象，状态符合消费条件 | bpb-biz 节点状态为 `可以分解分配` / `待补充分解分配` |
| ② | **有待处理反馈** | 本 Skill 产出的方案中存在人类标注但尚未 `[已处理]` 的条目 | bpl-biz 节点状态为 `待确认方案`，条目含 `[修改]` 未追加 `[已处理]` |
| ③ | **有下游退回节点** | 本 Skill 产出的节点被下游退回，等待修订 | bpl-biz 节点状态为 `需wft02修订` |
...
| 特化项 | 说明 |
|--------|------|
| 输入对象类型 | 如 bpb-biz 节点、bpl-biz 节点、规范化的相关方需求条目 |
...
> **版本标记与消费追溯**：节点初次生成时版本 = `v1`；每次因上游新输入触发修订，版本号递增。下游 Skill 首次消费时记录 `消费版本：v{N}`，重入时比对版本——若当前版本 > 已消费版本 → 触发变更感知，读取变更影响声明（§A.7），按变更类型处理。**消费版本记录于消费者节点的「来源」字段**（如 srb-biz 的「来源 bpl-biz」、bpl-biz 的「来源 bpb-biz」），生产者节点不重复存消费版本——生产者侧下游可见性由资产写回记录（含消费版本列）承担，变更影响声明的「下游已消费节点」按消费者「来源」字段定向查询。**两跳粒度注**：bpb-biz→bpl-biz 跳处理粒度是整个 bpb-biz（文档序列级），重入靠节点状态路由 + 变更影响声明；bpl-biz→srb-biz 跳处理粒度是节点级（逐 srb-biz 判断基于哪个 bpl-biz 版本），重入靠版本比对。两跳机制不同源于粒度不同，非漏实现。
...
> 各 Skill 特有的影响范围判定表（bpl-biz 变化→srb-biz 受影响范围等）保留在各 Skill 文档中。
