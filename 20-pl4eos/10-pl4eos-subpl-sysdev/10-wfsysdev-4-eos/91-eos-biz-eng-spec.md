...
Skill 的 Step 1（设计材料加载）加载产品数据或资产文档时，遵循以下流程：
...
| 规则 | 说明 |
|------|------|
| **R1** | AI 不得跳过第 1~2 步直接读取文件正文。业务域索引是文件内导航的唯一入口 |
...
AI 写入或修改产品数据文档或资产文档中的节点后，必须同步更新文件头部的行索引：

| 操作 | 更新内容 |
|------|---------|
| 新增节点 | 更新域产品索引的 `节点数` + `行范围` + `待裁决`；更新业务域索引的 `节点数` |
| 修改节点 | 若修改导致节点数或行范围变化，同步更新索引对应列 |
| 删除节点 | 域产品索引与业务域索引的 `节点数` 减 1；后续行范围左移；更新受影响的全部行范围 |
| 标注 `[需裁决]` | 域产品索引的 `待裁决` 加 1 |
| 取消 `[需裁决]` | 域产品索引的 `待裁决` 减 1 |

| 规则 | 说明 |
|------|------|
| **R4** | 写回操作必须在本 Skill 的同一运行步骤内完成索引更新。不允许"本次写节点、下次更新索引" |
