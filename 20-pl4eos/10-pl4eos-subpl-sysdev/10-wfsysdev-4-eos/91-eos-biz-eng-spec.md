...
```markdown
### 待 wft01-biz 处理
| 节点 ID | 标题 | 优先级 | 当前状态 | 所属域/角色 | 行范围 |
|---------|------|--------|----------|------------|--------|
| @ba-proc-apply-001 | 创建采购申请 | P0 | 待分解映射 | 采购管理 | 130-145 |
```
...
> 本协议的详细实现指引（写回维护规则、按需扩大规则等）见 §A.3.3。
...
5. Read(file, offset=域起始行, limit=行范围) → 加载域产品索引
6. 按目标产品在域产品索引中查找行范围
7. Read(file, offset=产品起始行, limit=行范围) → 加载产品正文块
```

| 规则 | 说明 |
|------|------|
| **R1** | 就绪队列优先级高于索引导航。只要当前 Skill 有就绪队列分节，必须先读就绪队列 |
| **R2** | 业务域索引常驻当前 Skill 运行的 context，直至该文件不再需要 |
| **R3** | 产品正文块处理完毕后，可从 context 中移除，业务域索引保留 |
...
| 操作 | 更新内容 |
|------|---------|
| 新增节点 | 更新域产品索引的 `节点数` + `行范围` + `待裁决`；更新业务域索引/角色索引的 `节点数`；将节点 ID 加入就绪队列对应分节 |
| 修改节点 | 若修改导致节点数或行范围变化，同步更新索引对应列；若变更 `**当前状态**`，将节点加入或移出就绪队列对应分节 |
| 删除节点 | 域产品索引与业务域索引/角色索引的 `节点数` 减 1；后续行范围左移；更新受影响的全部行范围；从就绪队列中移除该节点 |
...
| 规则 | 说明 |
|------|------|
| **R4** | 写回操作必须在本 Skill 的同一运行步骤内完成所有更新（行索引 + 就绪队列 + 子条目状态）。不允许"本次写节点、下次更新索引" |
| **R5** | 行范围修改影响同一域产品索引中后续所有产品的行范围。AI 必须逐行检查、逐行修正 |
| **R6** | 节点数/待裁决 的计数值必须根据实际新增或删除的节点数精确计算，不得四舍五入或估算 |
