...
>
> **版本**：v4.8 | **修订**：2026-07-11 | **说明**：§七~§九 全章重构（产出物链清晰化 + 双表合一 + NFR→eng 单向流 + 平台服务组件）+ 附录A 重构为四部分结构 + §10.4/§10.6 参数对齐 + §十一 新增产品数据文档结构及读写规范（就绪队列、子条目状态、变更感知、HEAD 追踪）+ 附录A 读取/写回协议全量同步 + 全文缺陷修复。
...
| wft05-eng | PA 组件 | 前端组件 / 后端组件 / 平台服务组件 | 逐构件判定前后端组件需求 + 推断平台/前端/后端三层架构——"出组件需求" | §八 |
...
> **适用范围**：01~07、21~28 号产品数据文档与资产文档。08（追溯矩阵）和 09（验证报告）保留原格式，不适用本章规范。
>
> **本章定位**：91 规范 §七~§十 定义"怎么设计"，本章定义"产出物长什么样"——每个产品数据文档的物理结构、索引机制和节点格式。
...
> `**HEAD @上次 AI 运行**` 由 AI 自动维护。人类无需也不应手动修改此字段；若因人为编辑导致不准确，AI 在 Step Start 中自动修正为距离 HEAD 最近的 AI 提交的 hash。

**自描述索引**：

| 规则 | 说明 |
|------|------|
| **R1** | 文件内必须存在 `## 自描述索引` 标题，其下一行必须是 `索引行数：{N}`（N 为索引表格的总行数，含表头） |
...
3. Read(file, offset=L+1, limit=N)    → 加载业务域索引表     （加载：N 行）
4. 按目标产品在业务域索引中查找域入口锚点
5. 进入域正文块后按节点 ID 列表逐 grep 定位加载产品段
```
...
| 列 | 必填 | 说明 |
|----|------|------|
| 域 | 是 | 业务域名称，与 `## {域名称}` 标题一致 |
| 节点数 | 是 | 该域下全部节点的合计数量 |
| 入口 | 是 | `[→](#{anchor})` 锚点链接 |

> 业务域索引的行数等于业务域数量（约 15 行），常驻 context 约 500 tokens。
...
| 规则 | 说明 |
|------|------|
| **R3** | 每个 `## {域名称}` 必须有锚点 ID，与业务域索引的入口列对应 |
| **R4** | 域产品索引只索引到 E2E 项目（产品）级，不索引到构件 |
| **R5** | 产品正文块内部按构件→WBS→输出文档→节点逐层用 `####`/`#####` 展开 |
...
| 规则 | 说明 |
|------|------|
| **R6** | 跨文件引用通过 `@id`，不包含文件路径 |
| **R7** | `@id` 全局唯一，前缀确保不冲突 |
| **R8** | AI 遇到未知 `@id` 时，按前缀判断归属文件，从该文件业务域索引中查找 |
...
每个产品数据文档必须包含 `## 就绪队列`，按 Skill 名称分节列出所有当前可被处理的节点。就绪队列由各 Skill 在写回时自动维护——新增节点或变更节点状态时同步加入或移出对应分节。
...
| 规则 | 说明 |
|------|------|
| **R9** | 就绪队列只列出节点级条目。子条目的待处理状态通过父节点的 `**子条目状态**` 字段汇总，不单独在就绪队列中列出 |
| **R10** | 就绪队列中的节点不得重复——同一节点同时满足多个分节条件时，按优先级取最高的一节（ort03 > biz > eng > nfr > 反馈 > 退回） |

> 就绪队列的分节由 91 规范 §10.6 定义的输入合法状态驱动。当 §10.6 中的状态值发生变更时，就绪队列的分节条件同步更新。

### 11.10 AI 读取与反馈检测协议

**一、AI 读取文档内容**

AI 读取产品数据或资产文档时，按以下优先级执行：

**步骤 1：读就绪队列（用于 Step Start 入口判定）**

AI 在 Step Start 中判定入口类型时，读取就绪队列获取待处理节点（§11.9）：

```
1. Bash("grep -n '待{Skill名称}' 目标文件")  → 定位就绪分节起始行
2. Read 该就绪分节 → 获取待处理节点 ID 列表
3. 有节点 → 逐节点：Bash("grep -n '@{节点ID}' 目标文件") 定位实际行号
         → Read 节点块
         无需遍历角色索引或业务域索引
```

**步骤 2：索引导航（用于按域/角色/产品发现节点）**

AI 需要浏览或定位特定域/角色下的节点时，按索引表导航：

```
1. Read 自描述索引 → 获取索引行数 N
2. Read 业务域索引/角色索引 → 查找目标域的锚点入口
3. 按锚点 Read 域内索引 → 获取角色/产品级节点 ID 列表
...
| | 步骤 1：就绪队列 | 步骤 2：索引导航 |
|--|-----------------|----------------|
| **用途** | Step Start 发现"有什么可以处理" | 浏览/定位"某个域/角色有什么节点" |
| **驱动** | Skill 重入时自动执行 | 人类询问或 AI 需要跨域引用时执行 |
| **场景** | 新输入/反馈处理/下游退回的入口判定 | 按域浏览、定位引用节点、检查跨域关系 |
| **两者关系** | 互不替代。就绪队列处理入口检测；索引导航处理结构化导航。AI 在 Step Start 中做入口判定时只读就绪队列；在设计步骤中按需使用索引导航 |

| 规则 | 说明 |
|------|------|
| **R11** | AI 不得在常规 Step 中遍历索引表以外的区域搜索节点。索引表是发现节点存在的唯一入口 |
| **R12** | 索引表不存储行范围。节点的实际行号通过 `grep -n '@{节点ID}'` 实时定位，不受人类插入或删除行的影响 |

**二、检测人类反馈内容**

AI 在 Step Start 中通过 git diff 检测自上次运行以来人类是否修改过文件：

```
1. 读取文件头 **HEAD @上次 AI 运行** 字段，获取上次提交 hash H
2. Bash("git diff H..HEAD -- 目标文件")  → 返回 H 到当前 HEAD 的全部变更
3. 无变更 → 进入正常读取流程
   有变更 → 解析变更行号，读取变更行附近（上下 5 行）的节点块
         → 判定变更内容类型：
            - [同意]/[修改]/[驳回] → 按 §A.5 反馈协议处理
            - 其他自由文本 → AI 尝试理解意图，标记 `[需确认]`
            - 无关修改（格式/排版）→ 忽略
         → 将存在反馈的节点加入就绪队列的"待反馈处理"分节
```

| 规则 | 说明 |
|------|------|
| **R14** | `**HEAD @上次 AI 运行**` 由 AI 在每次运行结束时自动更新为当前提交 hash。人类无需手动修改 |
| **R15** | 若 HEAD 字段不是距离 HEAD 最近的 AI 提交，AI 在 Step Start 中自动修正 |
| **R16** | 非标反馈经 AI 判定后标记 `[需确认]`，不走就绪队列的结构化标记规则 |

**三、写回产品数据文档与资产文档**

AI 写入或修改节点后，必须同步更新以下内容：

| 操作 | 更新内容 |
|------|---------|
| 新增节点 | 将节点 ID 写入对应域/角色的索引表；将节点 ID 加入就绪队列对应分节 |
| 修改节点 | 若变更 `**当前状态**`，将节点加入或移出就绪队列对应分节 |
| 删除节点 | 从索引表和就绪队列中移除该节点 ID |
| 新增/修改/删除子条目 | 更新父节点 `**子条目状态**` 汇总值（X 待处理 / Y 已确认 / Z 需裁决） |
| 人类标注 `[同意]`/`[修改]`/`[驳回]` | 子条目标注同步更新父节点 `**子条目状态**`；父节点标注更新 `**确认状态**`；节点进入或移出就绪队列"待反馈处理"分节 |

**写入时的节点定位**：

AI 不依赖行号定位写入位置。写入或修改节点时，使用锚点 ID 定位：

```
1. Bash("grep -n '@{节点ID}' 目标文件")      → 定位节点当前行号
2. Read 节点块 → 确认这是目标节点（验证 ID 和名称）
3. 按行号精确编辑节点块内容                  → Edit 工具替换
4. 若为新节点：找到所属域/角色分组
   Bash("grep -n '## {域名称}' 目标文件")     → 定位域分组起始行
   在域分组末尾插入新节点                    → Write/Edit
```
...
| 规则 | 说明 |
|------|------|
| **R17** | 写回必须在本 Skill 同一运行步骤内完成全部更新（索引表 + 就绪队列 + 子条目状态 + 文件头字段）。不允许"本次写节点、下次更新索引" |
| **R18** | 节点 ID 是定位节点的唯一标识。不使用行号索引，不依赖文件的行序号 |
| **R19** | 每次写回后必须提交。未提交的写回无法被下次 Step Start 的 git diff 检测到变更 |

**四、按需读取规则**

AI 在以下情况可扩大读取范围，超出步骤一/二的常规读取范围：

| 情况 | 操作 |
|------|------|
| 目标节点 ID 在索引表中无法定位 | 全文件搜索目标 ID，确认其是否存在及归属哪个域/角色 |
| 需要跨域检查引用一致性 | 加载目标域/角色的节点块，验证跨域引用关系 |
| 人类明确要求通读 | 按人类要求完整加载指定区域 |

| 规则 | 说明 |
|------|------|
| **R20** | 按需读取完成后，非常驻内容必须从 context 中移除，恢复到角色索引/业务域索引级别的常驻状态 |
| **R21** | 不得以"提前加载"为理由默认加载完整文件正文。索引表之外的内容仅在按需读取条件下加载 |
...
| **R0b** | 非标反馈由 AI 判定后标记 `[需确认]`，不走就绪队列的结构化标记规则 |

**步骤一：读就绪队列（用于 Step Start 入口判定）**

AI 在 Step Start 中判定入口类型时，读取就绪队列获取待处理节点：
...
| 规则 | 说明 |
|------|------|
| **R1** | 就绪队列优先级高于索引导航。只要当前 Skill 有就绪队列分节，必须先读就绪队列 |
...
| 操作 | 更新内容 |
|------|---------|
| 新增节点 | 将节点 ID 加入域产品索引与业务域索引/角色索引；将节点 ID 加入就绪队列对应分节 |
| 修改节点 | 若变更 `**当前状态**`，将节点加入或移出就绪队列对应分节 |
| 删除节点 | 从域产品索引与业务域索引/角色索引中移除节点 ID；从就绪队列中移除该节点 |
| 标注 `[需裁决]` | 域产品索引的 `待裁决` 加 1 |
| 取消 `[需裁决]` | 域产品索引的 `待裁决` 减 1 |
| 新增/修改/删除子条目 | 更新父节点的 `**子条目状态**` 汇总值（X 待处理 / Y 已确认 / Z 需裁决）；子条目待处理状态影响父节点是否进入就绪队列的"待反馈处理"分节 |
| 人类标注 `[同意]`/`[修改]`/`[驳回]` | 子条目标注同步更新父节点 `**子条目状态**`；父节点标注更新父节点 `**确认状态**`；节点进入或移出就绪队列的"待反馈处理"分节 |

| 规则 | 说明 |
|------|------|
| **R4** | 写回操作必须在本 Skill 的同一运行步骤内完成所有更新（索引表 + 就绪队列 + 子条目状态 + 文件头字段）。不允许"本次写节点、下次更新索引" |
| **R5** | 索引表不存储行范围。AI 不依赖行号定位节点，全部通过 `grep -n '@{节点ID}'` 实时定位 |
| **R6** | 新增/删除节点后，必须同步更新索引表中的 `节点数` 和 `待裁决` 的计数值，精确计算 |
| **R7** | 就绪队列的添加和移除必须精确匹配节点当前状态，仅当节点状态符合就绪队列分节条件时加入，不符合时移出 |
...
> **实现细节**：三类入口条件的节点清单不在此处逐一扫描判定，而是通过读取产品数据文档中的`## 就绪队列`对应分节（§11.9）直接获取。就绪队列由各 Skill 在写回时自动维护，减少 AI 在 Step Start 阶段的扫描量。
...
| 11 | 读取协议：引用 §A.3.3——就绪队列优先，索引导航备用，全部通过 grep 实时定位 | §A.3.3 |
...
| 入口 | 怎么知道哪些节点可以处理？ | §11.9 就绪队列 + §A.3.3 步骤零·变更感知 |
