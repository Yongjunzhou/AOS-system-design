...
| **wft03-biz** | BP 节点 | 系统概要定义——功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 | 功能表单 / 窗口标签页 | 产出可交付构件开发的配置业务系统 PA，并向 eng 分配指标 |
...
> **概要/详细分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要定义/详细定义」重述——s01 wft01 业务概要定义（末级=文档，概要说明为**结构判定级**，字段级详设留 s02）；s02 wft02 业务详细定义（详细条目=**活动 + 正文条目 + 表单使用顺序**）；s03 wft03 系统概要定义（末级=功能表单）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名保留，不迁移；追溯/23/26 不迁。命名「业务详细定义」源于 BP IPO 承担详细角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 OR 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
| wft03-biz | 功能表单 / 窗口标签页 | 引擎能力使用需求 | 消费 wft02 业务详细定义 → 生成**系统概要定义**（s03）——页面组件；分配 FR-BIZ 指标到引擎/CU；输出 FR-BIZ 指标约束包和 FR-ENG 候选线索——"出配置需求暨配置业务系统PA" | §七 |
...
|------|----------------------|----------------------|----------------------------|
| 末级节点 | 文档（PL5，结构判定级概要说明） | 输出文档（PL5，细分 PL6-A/PL6-B + 表单使用顺序） | 功能表单 / 窗口标签页（业务配置需求 + 配置业务系统 PA 双重身份） |
...
| 资产写回 | 23（Bn 组装结论：Q 判定 + WBS 归组） | 23/24/26/27/28 | 04/05/06 |
