...
>
> **版本**：v5.22 | **修订**：2026-08-20 | **说明**：§3.3 配置信息组口径对齐 wft02-eng v9.3（张力点②）——组的分界由"相对独立的业务能力"改为"可以运行的能力"（含业务人员可操作能力与系统侧运行行为两类）；配置入口补"一个 CU 对应一个页面（复合页内的子页面即完整页面）"；§8.2 CU 页面关系同款（一 CU 一页面）。上一版 v5.21：反馈检出位置对齐 §11.10（Step Start）——§A.5 反馈通道 + §A.5.4 确认状态字段两处「经 Step 1 变更感知 git diff 检出」→「经 Step Start 变更感知（git diff，§11.10）检出」，消除规范内部矛盾；链级推广 wft01-eng v8.9 / wft02-eng v9.1 / wft02-biz v9.0 同款修复
