...
>
> **版本**：v5.36 | **修订**：2026-08-22 | **说明**：biz 链深审第三轮修复（人类逐项裁决 2026-08-22）——①**消费版本定位置规则**（§A.6.5，B2/B5）：消费版本记录于消费者节点「来源」字段、生产者不重复存、下游可见性由资产写回记录承担、下游已消费列表按来源字段定向查询；补两跳粒度注（STR-F→BP 靠状态路由、BP→SR-F 靠版本比对）；②**§1.4 引擎偏好声明重写**（C1/C2/C3，D 产品数据类优先 / PCA 业务管理类优先 / @engine-task 两场景共用 / 权威校验以任务类型↔引擎映射为准）；③**§1.4/§2.7/§7.1「完全同构」→「流程骨架同构」**（C4，对齐三 Skill 措辞）。上一版 v5.35：biz 链深审第二轮实锤 1 处置（人类拍板 2026-08-22）——①§7 分工表 wft03 产出物列「功能表单 + 窗口标签页」→「功能表单（窗口标签页为页面组件承载）」；②§4.4 FR-ENG 候选线索字段集需求类型「增强性能」→「扩展能力边界」+ 补 Q2 语义判定注（不涉及数值性能比较，系统性能归 NFR 链）；③覆盖判定语义化本体在 wft03 §3.2（Q1/Q2 改业务指标类型语义对照、删除 CU 性能上限数值比较），§7.3 Task 8 委托「字段集见 wft03-biz §3.2」关系不变。上一版 v5.34：biz 链深审 C 组 + 取消 26 扩展 25（人类逐项裁决 2026-08-22）——①**取消 26 号文档资产**：§六 6.2 资产表删 26 行（23~28→23/24/25/27/28）、§11.1 文档全景删 26 行、§11.5 `@doc-` 改注（23 PL5 输出文档节点 ID，定义落 04/05/06，锚点 25 §1.7）、§7 分工框架 blockquote 补 26 取消注；业务文档定义承载于 04/05/06，承接锚点 25 §1.7（wft02/wft03 资产写回集 23/24/26/27/28→23/24/27/28）；②**C① 24 资产口径对齐 L0~L6**（§六 6.2 24 行「业务域/业务类型/端到端业务/角色/WBS」→「L1~L6 可复用业务」）；③**C② 末级口径统一**（§7 分工表/§7.3/§10.3/§10.6 末级「功能表单/窗口标签页」→「功能表单（窗口标签页为页面组件承载）」）；④**C⑤ §7 分工框架补两归纳枝分工说明**（PL4 归组在 wft01 与 Bn 组装结构性绑定、SL4 归纳在 wft02 服务 A2 场景装配）。上一版 v5.33：eng 链第四轮修复 + NFR 分解设计（人类逐项裁决 2026-08-22）——①§9.2 任务 2 补分解规则摘要引用（四类运算，规则见 wft04-nfr §2.4.1）、任务 6 补分解子指标独立装配 + 消费状态由 wft05-eng 回写；②§11.1 文档清单登记两约束包独立文件（10/11 跨路径契约文档）；③wft05-eng/wft04-nfr/wft06/wft03-biz 同步（A1 §3.4 判据补齐、B1 操作条目、分解处理与消费回写、约束包载体路径）。上一版 v5.32：eng 链第三轮深审修复（人类逐项裁决 2026-08-22）——①FR-ENG 线索回路/约束包契约：§8.2「补充 OR 材料」→「补充 FR-ENG 材料」、§4.4/§10.3 候选线索字段集引用式（Q4 边界不清纳入触发）、§8.5 Task 1 补 FR-BIZ 约束包消费；②术语统一：「概要末级节点」取消 → 配置信息组（业务概要定义末级节点）、NFR 约束包统一「wft05-eng NFR 约束包」、FR-BIZ 指标统一主名、引擎能力缺口线索 → FR-ENG 候选线索；③§10.6 wft02 资产写回补 01、nfr 参数表补 NFR 约束包交付标注、§3.2 @engine-kanban 对齐分离口径、§A.5.4 补清除条件为 Skill 特化参数强制项。上一版 v5.31：eng 链架构升级（人类裁决 2026-08-22）——A1 树升级为 4 层（产品→引擎→CU→配置信息组）：配置信息组=A1 产品树末级节点（PL5，可独立交付/验收的能力单元）、CU=PL4（可独立选配的配置单元，由配置信息组构成）、A1 树无 WBS（Bn 树 PL4=WBS 保留）；§3.1 补 A1 PL 树、§3.3 配置信息组定位升级、§5.1 A1 末级=CU→配置信息组、§8/§10.6 末级节点同步（配置信息组对应 biz 文档、CU 对应 WBS）。上一版 v5.30：§3.3 系统处理对齐 §8.2（人类裁决）——「保存时处理」→「**编辑时处理**」（编辑过程中系统如何校验、写载件与阻断，统一保存机制：设计会话 + 全局保存，见 §8.2），消除 91 内部新旧并存（wft02-eng v10.0 已定案、§8.2 已用新词）；配置信息组说明模板同步。上一版 v5.29：§11.5 ID 命名前缀表补 `@or-`（OR 节点，ort03 规范化产出，seq 编号，示例 `@or-001`）——对齐 ort03 实际产物。上一版 v5.28：ort 链反馈交互对齐 wft §A.5 双轨（人类确认「ort 应与 wft 保持一致」）——§A.5 预处理链特化改「ort 与 wft 同轨（双轨对话、不用口令表）、ort00 全自动例外」、模式分化表拆三行（ort00 状态扫描·全自动 / ort01 文档编辑+git·双轨 / ort02·ort03 状态扫描·双轨）、共同纪律提交基线编号更新；链级：ort01 v0.60 + ort02 v0.37 + ort03 v0.61（2026-08-22）。上一版 v5.27：eng 链 5 层「概要/详细」分工框架实施第⑥步（wft01-eng v9.5 + wft02-eng v10.0 + wft03-eng v9.0 + 新增 wft04-eng v1.0 + wft05-eng v10.0 对偶实施，人类裁决插入式重排 2026-08-21）——§8 任务清单重排：§8.1 业务概要定义（wft01-eng，结构判定级）、§8.2 业务详细定义（wft02-eng，操作活动定义步骤化）、§8.3 功能概要开发（wft03-eng，功能表单+概要说明）、**新增 §8.4 功能详细开发（wft04-eng，组件业务+PA 收敛项）**、原 §8.4→§8.5 产品概要开发（wft05-eng，输入改功能详细 SR-F）；§八 章首接力链表四→五个 Skill；§10.1 三路径全景图/执行顺序/CU 设计权规则补 wft04-eng；§10.4 eng 路径 Skill 表加 wft04-eng 行；§10.6 eng 参数表加 wft04-eng 列（wft03 产出状态 `可以详细定义`、wft05 输入状态 `已详细定义`）；链级引用（§3.3 CU 设计权规则/构件表/eng 路径分工/biz 对称性/汇合条件）同步。上一版 v5.26：biz 链「概要/详细」分工框架叙述完整度补齐（wft01-biz v3.17 + wft02-biz v9.3 + wft03-biz v9.7 对偶实施，人类确认符合性核查后 2026-08-21）——§7.2 分工框架 blockquote 补「命名业务详细定义不破通用规范」兼容句（BP IPO 承担详细角色，通用规范 §1.3 特例）；一棵树读法（A2/Bn 归纳枝→场景业务→文档→活动/正文条目/表单使用顺序→功能表单）下沉到三个 Skill 概念章；wft01「场景业务（归纳枝）」措辞修正为「汇聚点」。上一版 v5.25：biz 链「概要/详细」分工框架定案同步（wft01-biz v3.16 + wft02-biz v9.2 对偶实施，人类确认 2026-08-21）——§七 三个 Skill 用「业务概要定义/业务详细定义/系统概要定义」重述分工：wft01 新增 Bn 组装（WBS 归组 + 23 一致性 + 按构件产品数据清单完整性缺口推断，结构性缺口 OR 回路）、wft02 收缩纯详细层（迁出 WBS 归组/23 一致性/缺口推断）、wft03 末级=功能表单；STR-F/BP/SR-F 资产名保留；层级表联动审查同步——§10.3/§10.6 参数表（末级节点/资产写回/补充材料）、§7.1 任务 7 改 Bn 组装 + 末级节点=文档、§7.2 末级节点 block 与任务清单（任务 2 消费 Bn 组装 / 3 引擎候选校验 / 8 设计性缺口）、§5.2/§5.3 设计路径（wft02 详设承接 wft01 Bn 组装）、附录 A.2.3 标题命名示例。上一版 v5.24：分解产物正名链级同步（wft01-eng v9.3 + wft02-eng v9.8 对偶实施，人类确认）——「详细需求条目」→「配置信息组（概要说明）」：§8.1 wft01-eng 细分/任务2/任务4、§8.2 wft02-eng 任务3、§3.3/§10.1 CU 设计权规则、§10.4 参数表、§八 接力链表（wft01 注记补「配置信息组概要」）全部同步。上一版 v5.23：eng 链设计归属重构（wft01-eng v9.2 + wft02-eng v9.6 对偶实施）——CU 设计权规则改两段（wft01-eng 设计 CU 承接、wft02-eng 详细定义 CU）；§8.1 任务清单加"分解概要末级为详细需求条目 + 分配条目到 CU 承接"；§8.2 收缩为详细定义；§八 接力链表/§10.4/§10.6 同步（STR-E 状态改名 `可以详细定义`/`待补充详细定义`/`已详细定义`，DP1）。
