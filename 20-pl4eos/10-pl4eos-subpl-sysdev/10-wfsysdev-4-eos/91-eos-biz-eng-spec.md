...
> **概要/详细分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要定义/详细定义」重述——s01 wft01 业务流程概要定义（末级=文档，概要说明为**结构判定级**，字段级详设留 s02）；s02 wft02 业务流程详细定义（详细条目=**活动 + 正文条目 + 表单使用顺序**）；s03 wft03 系统需求概要定义（末级=**功能表单**，窗口标签页为其页面组件承载形式，2026-08-22 深审统一）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bph-biz、BP→bpd-biz、SR-F→srh-biz；追溯/23 不迁。命名「业务流程详细定义」源于 bpd-biz IPO 承担详细角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 规范化的相关方需求 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
> **两归纳枝分工说明（2026-08-22 深审确认）**：A2 归纳枝（SL3→SL4）在 wft02（s02）、Bn 归纳枝（PL3→PL4 WBS）在 wft01（s01）——不对称是分工决定的，非遗漏：PL4 WBS 归组与 23B 对齐/23 一致性/缺口推断同属 wft01 Bn 组装（结构性绑定），SL4 归纳服务 A2 场景装配与表单使用顺序（wft02 Step 3），且 A2 侧无 23 资产产品数据清单类锚点可做缺口推断。
>
> **26 号文档资产已取消（2026-08-22 深审决策）**：业务文档定义承载于 04/05/06 方案文档，承接锚点在 25 §1.7 业务文档承接。
>
...
|-------|---------|---------|---------|--------|
| wft01-biz | 文档（PL5，概要末梢） | PL6-A 活动节点 / PL6-B 正文条目初步判定（结构判定级） | 基于 规范化的相关方需求 推断**业务流程概要定义**（s01）——场景业务（枝）+ 输出文档序列 + **Bn 组装**（WBS 归组/23 一致性/按构件产品数据清单完整性缺口推断），结构性缺口补充 规范化的相关方需求 回路——"向业务方证明我听懂了" | §七 |
| wft02-biz | 输出文档（PL5） | PL6-A 活动节点 / PL6-B 正文条目 / 表单使用顺序 | 消费 wft01 Bn 组装，生成 A2+Bn 双视角**业务流程详细定义**（s02）——引擎校验（权威）+ 文档方案 + 活动设计 + 权限 + 任务关系（架构语言）——"设计方案" | §七 |
| wft03-biz | 功能表单（窗口标签页为页面组件承载） | 引擎能力使用需求 | 消费 wft02 业务流程详细定义 → 生成**系统需求概要定义**（s03）——页面组件；分配 FR-BIZ 指标到引擎/CU；输出 FR-BIZ 指标约束包和 FR-ENG 候选线索——"出配置需求暨配置业务系统pa-eng" | §七 |
