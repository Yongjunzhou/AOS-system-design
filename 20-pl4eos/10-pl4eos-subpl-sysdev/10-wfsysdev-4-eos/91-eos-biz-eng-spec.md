...
→ 形成 FR-ENG 候选线索。候选线索不是正式 OR，不得直接进入 wft01-eng；须经人类确认后作为原始需求材料登记，并完整通过 ort00→ort01→ort02→ort03，形成正式 FR-ENG OR 后，才按 eng 路径（§八）推进——wft01-eng 判定归属引擎 → wft02-eng 设计 CU → wft03-eng 组织系统需求概要 → wft04-eng 展开组件业务 → wft05-eng 展开三类组件。

biz 路径在设计中发现构件或布局组件缺口时，只输出 FR-ENG 候选线索（字段集以 wft03-biz §3.2 为权威：标识[需求 ID/来源/目标引擎/需求类型] + 能力描述[缺口层级/当前能力/需求能力/指标要求] + 业务影响[业务场景/业务影响]；Q2 性能不足、Q3 能力缺失、Q4 边界不清均生成线索，需求类型分别为扩展能力边界/新增能力/明确边界，Q2 为语义判定不涉及数值性能比较——系统性能归 NFR 链）。页面组件类型及组合属于 biz 页面需求本身；只有当现有布局组件或构件能力无法承载该页面组件需求时，才形成 FR-ENG 候选线索。该线索不得直接写入正式 OR 节点或”待 wft01-eng 处理”分节。
...
> 治理 bph-biz 的完整清单以 `02-*.md` 为准，AI 运行时动态读取。上述四类治理业务当前 OR 承接为空，由人类在 EOS 平台建设过程中逐批增补。
...
|-------|------|------|---------|---------|
| **wft01-biz** | FR-BIZ OR 条目 | 业务流程概要定义——bph-biz 场景业务（枝）+ PL5 输出文档序列（概要末梢）+ Bn 组装（WBS 归组/23 一致性/缺口推断） | 文档（PL5，结构判定级概要说明） | 向业务方证明"我听懂了你的业务" |
...
```
输入                         产出
FR-BIZ OR（待处理）  ──→  业务流程概要定义 bph-biz 场景业务方案（可以分解分配）
...
| 7 | Bn 组装 | 23B Q1/Q2/Q3 对齐判定 + PL4 WBS 归组（数据依赖链硬判据）+ 23 一致性检查（归属冲突/误新建/误引用）+ 按构件产品数据清单完整性缺口推断（结构性缺口 → 补充 OR 回路） |
...
| 8 | 基于 Bn 补充业务需求 | Bn | 详设层设计性缺口（PL6-B/指标/活动节点级）→ 生成补充 OR 材料；结构性缺口已由 wft01 Bn 组装处理 |
...
| 8 | 引擎能力使用与指标分配 | — | 逐业务页面形成引擎能力使用需求，将 FR-BIZ 指标分配到既有引擎/CU并判定覆盖关系；能力或指标不足时生成 FR-ENG 候选线索（字段集见 wft03-biz §3.2），经 OR 预处理链转入 eng 路径 |

---

## 八、平台能力之系统设计

> eng 路径将引擎能力原始需求（FR-ENG OR）逐级转化为可交付开发的 pa-eng 组件需求定义（前端组件 + 后端组件 + 平台服务组件）。与 biz 路径并行但设计对象不同——biz 设计业务场景，eng 设计支撑业务场景的引擎能力。

```
FR-ENG OR ──→ wft01-eng ──→ bph-eng ──→ wft02-eng ──→ bpd-eng ──→ wft03-eng ──→ srh-eng ──→ wft04-eng ──→ srd-eng ──→ wft05-eng ──→ pa-eng
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明"我听懂了你要的平台能力" | FR-ENG OR 条目 → bph-eng 业务流程概要方案 | 配置信息组（概要说明，PL5 末级，业务流程概要定义末级节点；bph-eng 架构末级=场景业务 SL5）+ CU 承接 + 依赖 bph-eng（共享依赖） | 业务配置人员确认 → wft02-eng |
...
**将零散的 FR-ENG OR 组织为结构化的平台能力场景业务方案（业务流程概要定义，01 层——概要说明为结构判定级，操作活动定义留 02 展开），供业务配置人员确认理解。**

```
输入                              产出
FR-ENG OR 条目（待处理）  ──→  bph-eng 平台能力场景业务方案（可以详细定义）
...
| 4 | 建立追溯链 | FR-ENG OR → bph-eng主责能力域 → 配置信息组（概要说明） → CU承接/依赖bph-eng；共享能力记录提供能力、消费方和复用范围 |
...
> nfr 路径将非功能原始需求（NFR OR）逐级转化为可验证的量化指标和可消费的约束包。与 biz/eng 路径并行但设计对象不同——biz 设计业务场景，eng 设计平台引擎能力，nfr 记录横切业务与平台语境的非功能质量约束。这里的”横切”描述影响语境，不代表存在两个设计消费方；正式 **wft05-eng NFR 约束包**（由 wft04-nfr 产出、供 wft05-eng 消费，下称 NFR 约束包）统一由 wft05-eng 消费。两个 Skill 形成设计接力链：

```
NFR OR ──→ wft01-nfr ──→ bph-nfr ──→ wft04-nfr ──→ SysReq-NFR
            角色NFR分类              量化指标+页面 NFR
            定性→结构化              "指标可验证了"
```
...
|-------|---------|------------|-----------------|---------|
| **wft01-nfr** | 向 NFR 相关方证明"我听懂了你的质量诉求" | NFR OR 条目 → bph-nfr 角色非功能需求分类方案 | 角色非功能需求分类 | NFR 相关方确认 → wft04-nfr |
...
```
输入                                   产出
NFR OR 条目（待处理）  ──→  bph-nfr 角色非功能需求分类方案（可以NFR设计）
                                末级节点：角色非功能需求分类
```
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 推断角色 NFR 分类节点 | 识别 OR 背后的用户角色、业务域/平台能力语境、NFR 一级类目与分类维度，形成 bph-nfr 末级节点 |
...
| 4 | 基于需求清单补充 NFR 需求 | 闭合检查——检出分类缺口、适用范围缺口、验证语义缺口→生成补充 NFR OR 材料，登记到 材料状态表 |
...
> OR 按类型分流至三条执行路径，每条路径内的 Skill 形成串行接力链——上游 Skill 产出节点，下游 Skill 消费节点后展开为下一层节点。三路径不是三条都独立通向 pa-eng 的产品链：biz 终止于业务配置需求，eng 形成平台 pa-eng，nfr 形成 SysReq-NFR 与约束包并汇入 wft05-eng。全链路最终产出两套可交付构件开发的成果。各路径 Skill 清单见 §10.3~§10.5，完整参数见 §10.6。
...
第一轮：三路径并行启动
  ...
  eng 线：FR-ENG OR → wft01-eng → wft02-eng → wft03-eng → wft04-eng →（等待 FR-BIZ 指标约束包与 wft05-eng NFR 约束包）
...
| **新输入增量** | 上游产出新一批输入节点（如新 OR 条目、新 bph-biz 节点），状态符合消费条件 | 增量展开新节点，已有方案不受影响 |
...
|-------|---------|---------|---------|--------|
| wft01-biz | 文档（PL5，概要末梢） | PL6-A 活动节点 / PL6-B 正文条目初步判定（结构判定级） | 基于 OR 推断**业务流程概要定义**（s01）——场景业务（枝）+ 输出文档序列 + **Bn 组装**（WBS 归组/23 一致性/按构件产品数据清单完整性缺口推断），结构性缺口补充 OR 回路——"向业务方证明我听懂了" | §七 |
...
| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 配置信息组（概要说明，PL5，业务流程概要定义末级节点） | 主责能力域 / 配置信息组（概要说明） / CU 承接 / 依赖 bph-eng（bph-eng 架构末级=场景业务） | 基于 FR-ENG OR 推断可独立配置、治理和验收的平台能力场景 + 名称级识别并分解配置信息组（概要说明） + 分配配置信息组到 CU 承接——"向平台方证明我听懂了"（业务流程概要定义，01 层） | §八 |
...
| 维度 | wft01-biz（OR→bph-biz） | wft02-biz（bph-biz→bpd-biz） | wft03-biz（bpd-biz→配置需求方案） |
|------|----------------------|----------------------|----------------------------|
| 末级节点 | 文档（PL5，结构判定级概要说明） | 输出文档（PL5，细分 PL6-A/PL6-B + 表单使用顺序） | 功能表单（窗口标签页为页面组件承载；业务配置需求 + 配置业务系统 pa-eng 双重身份） |
| 输入对象 | FR-BIZ OR 条目（≤10） | bph-biz 节点（≤5） | bpd-biz 节点（≤5） |
...
| 补充材料 | 补充 OR 材料（闭合缺口 + Bn 组装结构性缺口） | FR-BIZ OR 材料（详设层设计性缺口） | FR-ENG 候选线索（引擎缺口） |

**eng 路径**：

| 维度 | wft01-eng（OR→bph-eng） | wft02-eng（bph-eng→bpd-eng） | wft03-eng（bpd-eng→系统需求概要 srh-eng） | wft04-eng（系统需求概要→系统需求详细 srd-eng） | wft05-eng（系统需求详细→pa-eng） |
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（概要说明，PL5，业务流程概要定义末级节点） | 配置信息组（操作活动定义） | 功能表单（概要说明） | 功能表单（组件业务） | pa-eng 组件（前端/后端/平台服务） |
| 输入对象 | FR-ENG OR 条目（≤10） | bph-eng 节点（≤5） | bpd-eng 节点（A1，≤5） | 系统需求概要 srh-eng（≤5） | 系统需求详细 srd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
...
| 维度 | wft01-nfr（OR→bph-nfr） | wft04-nfr（bph-nfr→SysReq-NFR） |
|------|--------------------------|----------------------------------|
| 末级节点 | 角色非功能需求分类 | 非功能需求分类 / 具体非功能需求 |
| 输入对象 | NFR OR 条目（≤10） | bph-nfr 节点（≤5） |
...
| 前缀 | 所属 | 示例 |
|------|------|------|
| `@or-` | OR 节点（ort03 规范化产出，seq 编号） | `@or-001` |
...
bash scripts/read-node.sh <文件> <节点ID>
  → 按节点ID自动识别格式（OR-001 / bph-biz-XXX / @node-xxx / @engine-xxx / ROLE-XXX）
...
| 特化项 | 说明 |
|--------|------|
| 输入对象类型 | 如 bph-biz 节点、bpd-biz 节点、OR 条目 |
...
> | **状态扫描 · 双轨** | `eos-ort02-clarify` / `eos-ort03-norm` | 状态表 / 文档标记（ort02 切分文件正文区 `[✓已澄清]` + 表 B；ort03 OR 基线反馈区 `[同意执行]`/`[修改建议]`/`[废弃]` + 表 B/C） | Step 1 状态扫描检出 → 登记表 B `内容摘要` → Step 2 基于记录执行；Step 5 对话反馈即时处理 + `[已处理]`，Step 6「整体确认」提交基线 |
>
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-08-25 | v5.43 | wft05-eng 层名全对齐（人类裁决 2026-08-25）——①§8.5 标题「产品概要开发」→「平台产品架构开发」（wft05-eng，锚点随迁 `#85-平台产品架构开发wft05-eng`）；②§10.1/§10.3/§10.6 及 5 层链表「产品概要(定义)」→「平台产品架构」（wft05 输出层名）；③§11.1 资产表 07 行「产品概要」→「平台产品架构」+ 文件名 `07-eos-product-summary.md`→`07-eos-platform-product-architecture.md`（数据文件随迁）。 |
...
| 2026-08-24 | v5.39 | 材料状态表联动（wft03 单独轮 B4）——「OR 原料状态表」→「材料状态表」（2 处）；§7.3 正文「pa-eng 末级节点」→「配置业务系统 PA 末级节点」（对齐 wft03 A2）；头部版本号补落后（此前变更记录已到 v5.38）。 |
...
| 2026-08-22 | v5.32 | eng 链第三轮深审修复（人类逐项裁决 2026-08-22）——①FR-ENG 线索回路/约束包契约：§8.2「补充 OR 材料」→「补充 FR-ENG 材料」、§4.4/§10.3 候选线索字段集引用式（Q4 边界不清纳入触发）、§8.5 Task 1 补 FR-BIZ 约束包消费；②术语统一：「概要末级节点」取消 → 配置信息组（业务概要定义末级节点）、NFR 约束包统一「wft05-eng NFR 约束包」、FR-BIZ 指标统一主名、引擎能力缺口线索 → FR-ENG 候选线索；③§10.6 wft02 资产写回补 01、nfr 参数表补 NFR 约束包交付标注、§3.2 @engine-kanban 对齐分离口径、§A.5.4 补清除条件为 Skill 特化参数强制项。 |
...
| 2026-08-22 | v5.29 | §11.5 ID 命名前缀表补 `@or-`（OR 节点，ort03 规范化产出，seq 编号，示例 `@or-001`）——对齐 ort03 实际产物。 |
...
| 2026-08-21 | v5.25 | biz 链「概要/详细」分工框架定案同步（wft01-biz v3.16 + wft02-biz v9.2 对偶实施，人类确认 2026-08-21）——§七 三个 Skill 用「业务概要定义/业务详细定义/系统概要定义」重述分工：wft01 新增 Bn 组装（WBS 归组 + 23 一致性 + 按构件产品数据清单完整性缺口推断，结构性缺口 OR 回路）、wft02 收缩纯详细层（迁出 WBS 归组/23 一致性/缺口推断）、wft03 末级=功能表单；STR-F/BP/SR-F 资产名保留；层级表联动审查同步——§10.3/§10.6 参数表（末级节点/资产写回/补充材料）、§7.1 任务 7 改 Bn 组装 + 末级节点=文档、§7.2 末级节点 block 与任务清单（任务 2 消费 Bn 组装 / 3 引擎候选校验 / 8 设计性缺口）、§5.2/§5.3 设计路径（wft02 详设承接 wft01 Bn 组装）、附录 A.2.3 标题命名示例。 |
