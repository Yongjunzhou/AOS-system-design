...
对应设计路径为 **eng 路径**：wft01-eng（业务流程概要）→ wft02-eng（业务流程详细）→ wft03-eng（系统需求概要）→ wft04-eng（系统需求详细）→ wft05-eng（产品概要/三类组件）。
...
FR-ENG OR ──→ wft01-eng ──→ bph-eng ──→ wft02-eng ──→ bpd-eng ──→ wft03-eng ──→ srh-eng ──→ wft04-eng ──→ srd-eng ──→ wft05-eng ──→ pa-eng
              业务流程概要定义          业务流程详细定义            系统需求概要定义              系统需求详细定义               产品概要定义
...
| wft05-eng | pa-eng 组件 | 前端组件 / 后端组件 / 平台服务组件 | 消费系统需求详细 srd-eng、FR-BIZ 指标约束包和 NFR 约束包，从组件业务推断三类组件及组件架构——"出组件需求"（产品概要定义，05 层） | §八 |
...
| 07 | `07-eos-product-summary.md` | 产品概要 |
