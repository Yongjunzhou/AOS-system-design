...
> eng 路径将引擎能力原始需求（FR-ENG OR）逐级转化为可交付开发的前后端组件需求定义。与 biz 路径并行但设计对象不同——biz 设计业务场景，eng 设计支撑业务场景的引擎能力。
...
| **wft05-eng** | 产出可交付开发的组件需求定义 | SR-F 构件清单 → PA 组件需求定义 | 前后端组件（前端+后端） | 构件开发流水线 |
...
**将构件展开为前后端组件需求定义，构件开发流水线可据此进入开发。**（eng 路径跳过 wft04——wft04 为 nfr 路径的系统需求开发 Skill，见 §九。）

```
输入                              产出
SR-F 构件清单                    PA 前后端组件需求定义（待追溯验证）
（待产品架构 / 待补充PA设计）  ──→ 末级节点：前后端组件
                                   细分：前端组件 + 后端组件
```
...
> **组件类型**：前端组件——UI 层的代码实现单元（组件树/路由/状态管理）；后端组件——服务层的代码实现单元（API 设计/服务划分/数据访问）。
...
| wft05-eng | 构件 | 前端组件 + 后端组件 | 逐构件判定前后端组件需求 + 推断平台/前端/后端三层架构——"出组件需求" | §八 |
