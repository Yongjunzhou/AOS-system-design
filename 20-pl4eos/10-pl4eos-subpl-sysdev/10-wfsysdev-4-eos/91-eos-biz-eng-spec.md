...
>
> **版本**：v5.25 | **修订**：2026-08-21 | **说明**：biz 链「概要/详细」分工框架定案同步（wft01-biz v3.16 + wft02-biz v9.2 对偶实施，人类确认 2026-08-21）——§七 三个 Skill 用「业务概要定义/业务详细定义/系统概要定义」重述分工：wft01 新增 Bn 组装（WBS 归组 + 23 一致性 + 按构件产品数据清单完整性缺口推断，结构性缺口 OR 回路）、wft02 收缩纯详细层（迁出 WBS 归组/23 一致性/缺口推断）、wft03 末级=功能表单；STR-F/BP/SR-F 资产名保留；层级表联动审查同步——§10.3/§10.6 参数表（末级节点/资产写回/补充材料）、§7.1 任务 7 改 Bn 组装 + 末级节点=文档、§7.2 末级节点 block 与任务清单（任务 2 消费 Bn 组装 / 3 引擎候选校验 / 8 设计性缺口）、§5.2/§5.3 设计路径（wft02 详设承接 wft01 Bn 组装）、附录 A.2.3 标题命名示例。上一版 v5.24：分解产物正名链级同步（wft01-eng v9.3 + wft02-eng v9.8 对偶实施，人类确认）——「详细需求条目」→「配置信息组（概要说明）」：§8.1 wft01-eng 细分/任务2/任务4、§8.2 wft02-eng 任务3、§3.3/§10.1 CU 设计权规则、§10.4 参数表、§八 接力链表（wft01 注记补「配置信息组概要」）全部同步。上一版 v5.23：eng 链设计归属重构（wft01-eng v9.2 + wft02-eng v9.6 对偶实施）——CU 设计权规则改两段（wft01-eng 设计 CU 承接、wft02-eng 详细定义 CU）；§8.1 任务清单加"分解概要末级为详细需求条目 + 分配条目到 CU 承接"；§8.2 收缩为详细定义；§八 接力链表/§10.4/§10.6 同步（STR-E 状态改名 `可以详细定义`/`待补充详细定义`/`已详细定义`，DP1）。
...
> **概要/详细分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要定义/详细定义」重述——s01 wft01 业务概要定义（末级=文档，概要说明为**结构判定级**，字段级详设留 s02）；s02 wft02 业务详细定义（详细条目=**活动 + 正文条目 + 表单使用顺序**）；s03 wft03 系统概要定义（末级=功能表单）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名保留，不迁移；追溯/23/26 不迁。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 OR 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
