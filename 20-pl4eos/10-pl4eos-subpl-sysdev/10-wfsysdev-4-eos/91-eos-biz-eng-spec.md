...
**P0 树（平台／引擎产品的 P 树）**：P0（§5.1）同样沿产品视角（PL）展开——平台 → 业务域（流程与IT）→ 产品类型（平台／引擎产品）→ 构件（引擎）→ **CU（配置单元，PL4）→ 配置信息组（PL5，末级节点）**。P0 树无 WBS 层——Pn 树的 PL4=WBS 保留（§5.3），仅 P0 树以 CU 直接占据 PL4 位置。配置信息组是 P0 产品树的**末级节点**（可独立交付/验收的能力单元），对应 Pn 树的 PL5 输出文档——两者同为各产品树的 PL5 末级；CU 是可独立**选配**的配置单元（PL4，配置入口/页面边界），由若干配置信息组构成（选配粒度=CU、能力粒度=配置信息组，两层面正交不冲突）。P0 树到配置信息组为止，无 PL6——操作活动定义（配置操作/系统处理/运行期能力）是配置信息组的内容，非架构节点。
