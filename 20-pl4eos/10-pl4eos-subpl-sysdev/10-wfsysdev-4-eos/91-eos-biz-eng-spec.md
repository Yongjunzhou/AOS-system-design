...
biz 路径在设计中发现构件或布局组件缺口时，只输出 FR-ENG 候选线索（字段集以 wft03-biz §3.2 为权威：标识[需求 ID/来源/目标引擎/需求类型] + 能力描述[缺口层级/当前能力/需求能力/指标要求] + 业务影响[业务场景/业务影响]；Q2 性能不足、Q3 能力缺失、Q4 边界不清均生成线索，需求类型分别为增强性能/新增能力/明确边界）。页面组件类型及组合属于 biz 页面需求本身；只有当现有布局组件或构件能力无法承载该页面组件需求时，才形成 FR-ENG 候选线索。该线索不得直接写入正式 OR 节点或”待 wft01-eng 处理”分节。
...
| **产出物** | 业务概要定义（STR-F）——场景业务 + 输出文档序列 + Bn 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务详细定义（BP）——A2 角色场景（功能表单/主子表单 + 表单使用顺序）+ Bn 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统概要定义（业务配置需求暨配置业务系统 PA）——功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
