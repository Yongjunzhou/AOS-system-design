...
| **wft03-biz** | BP 节点 | 系统概要定义——功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + 引擎能力缺口线索 | 功能表单 / 窗口标签页 | 产出可交付构件开发的配置业务系统 PA，并向 eng 分配指标 |
...
| **产出物** | 业务概要定义（STR-F）——场景业务 + 输出文档序列 + Bn 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务详细定义（BP）——A2 角色场景（功能表单/主子表单 + 表单使用顺序）+ Bn 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统概要定义（业务配置需求暨配置业务系统 PA）——功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + 引擎能力缺口线索 |
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明"我听懂了你要的平台能力" | FR-ENG OR 条目 → STR-E 业务概要方案 | 配置信息组（概要，PL5 末级；STR-E 架构末级=场景业务 SL5）+ 概要末级节点 + CU 承接 + 共享依赖 | 业务配置人员确认 → wft02-eng |
...
| **wft05-eng** | 产出可交付开发的三类组件方案 | 功能详细 SR-F + FR-BIZ 指标约束包 + NFR 约束包 → PA 组件需求定义 | PA 组件（前端/后端/平台服务） | 追溯验证 / 构件开发流水线 |
...
| 2 | 分解概要末级为配置信息组（概要说明）+ 分配配置信息组到 CU 承接 | 对称于 STR-F 下辖的输出文档——场景业务由哪些 CU 组成；把概要末级节点按"可以运行的能力"分解为配置信息组（概要说明，**结构判定级**——配置什么/运行期提供什么，不展开操作活动定义——那是 wft02-eng 业务详细定义），逐组分配 CU 承接（含收敛判定复用/新增/拆分/合并 + 对照 25 盘存的骨架判定） |
| 3 | 基于能力可交付性补充平台需求 | 逐 STR-E 检查配置/治理目标能否形成可独立验收的端到端闭环；检出实质缺口则形成补充 FR-ENG 材料 |
| 4 | 建立追溯链 | FR-ENG OR → STR-E主责能力域 → 概要末级 → 配置信息组（概要说明） → CU承接/依赖STR-E；共享能力记录提供能力、消费方和复用范围 |
...
| 4 | 基于 A1 补充引擎需求 | — | 检出详设缺口（操作活动/依赖/状态迁移） → 生成 FR-ENG 线索或补充 OR 材料 |
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 基于功能详细 SR 及约束推断平台架构 | 判断组件属于引擎专属、布局组件扩展、公共构件、公共平台服务或运行支撑模块；同时消费 FR-BIZ 指标约束包和 wft04-nfr 的 NFR 约束包，形成必要的平台架构决策 |
...
> **CU 设计权规则**：CU 是引擎提供的配置能力单元，其能力边界、配置入口、配置要素、系统处理、运行期输出和 FR-ENG 性能指标仅由 eng 路径设计。**wft01-eng 设计 CU 承接（分解概要末级为配置信息组（概要说明） + 分配配置信息组到 CU 承接，含收敛判定）**；**wft02-eng 详细定义 CU（承接已分配 CU，展开操作活动定义/依赖/状态迁移）**；wft03-eng 将 CU 操作活动组织为功能概要；wft04-eng 展开功能表单上组件业务；wft05-eng 将其展开为软件组件。biz 路径不得新建、修改或详细设计 CU。
...
第一轮：三路径并行启动
  ...
  eng 线：FR-ENG OR → wft01-eng → wft02-eng → wft03-eng → wft04-eng →（等待 FR-BIZ 指标约束包与 NFR 约束包）
...
第二轮：eng/nfr 汇合与收尾
  ...
  汇合条件满足 → wft05-eng 同时消费功能详细 SR-F、FR-BIZ 指标约束包和 NFR 约束包 → 平台 PA 组件需求定义
```
...
| wft03-biz | 功能表单 / 窗口标签页 | 引擎能力使用需求 | 消费 wft02 业务详细定义 → 生成**系统概要定义**（s03）——页面组件；分配 FR-BIZ 指标到引擎/CU；输出指标约束包和引擎能力缺口线索——"出配置需求暨配置业务系统PA" | §七 |

### 10.4 eng 路径 Skill

| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 配置信息组（概要，PL5） | 主责能力域 / 概要末级节点 / CU 承接 / 依赖 STR-E（STR-E 架构末级=场景业务） | 基于 FR-ENG OR 推断可独立配置、治理和验收的平台能力场景 + 分解概要末级为配置信息组（概要说明） + 分配配置信息组到 CU 承接——"向平台方证明我听懂了"（业务概要定义，01 层） | §八 |
...
| 补充材料 | 补充 OR 材料（闭合缺口 + Bn 组装结构性缺口） | FR-BIZ OR 材料（详设层设计性缺口） | FR-ENG 线索（引擎缺口） |

**eng 路径**：

| 维度 | wft01-eng（OR→STR-E） | wft02-eng（STR-E→BP） | wft03-eng（BP→功能概要 SR-F） | wft04-eng（功能概要→功能详细 SR-F） | wft05-eng（功能详细→PA） |
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（概要，PL5） | 配置信息组（操作活动定义） | 功能表单（概要说明） | 功能表单（组件业务） | PA 组件（前端/后端/平台服务） |
...
| 资产写回 | 01/02/23/25 | 02/04/23/25 | 04/05/25 | 05/25 | 05/06/07/23/25 |
