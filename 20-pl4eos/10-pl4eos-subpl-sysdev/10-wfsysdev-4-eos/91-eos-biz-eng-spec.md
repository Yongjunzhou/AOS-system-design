...
| **wft03-biz** | bpd-biz 节点 | 系统需求概要——功能表单 + 窗口标签页 + 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 | 功能表单（窗口标签页为页面组件承载） | 产出可交付构件开发的配置业务系统 pa-eng，并向 eng 分配指标 |
...
| **产出物** | 业务流程概要（bph-biz）——场景业务 + 输出文档序列 + Bn 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务流程方案（BP）——A2 角色场景（功能表单/主子表单 + 表单使用顺序）+ Bn 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统需求概要（业务配置需求暨配置业务系统 pa-eng）——功能表单（窗口标签页为页面组件承载）+ 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
| **供谁消费** | 业务方确认 → wft02-biz | wft03-biz | 构件开发流水线 + eng 路径（引擎缺口线索） |

> **概要/方案分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要/方案」重述——wft01 业务流程概要（末级=文档，概要说明为**结构判定级**，字段级详设由 wft02-biz 承接）；wft02 业务流程方案（详细条目=**活动 + 正文条目 + 表单使用顺序**）；wft03 系统需求概要（末级=**功能表单**，窗口标签页为其页面组件承载形式，2026-08-22 深审统一）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bph-biz、BP→bpd-biz、SR-F→srh-biz；追溯/23 不迁。命名「业务流程方案」源于 bpd-biz IPO 承担方案角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 规范化的相关方需求 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
> 将 bpd-biz 方案展开为功能表单和窗口标签页，并形成引擎能力使用需求与 FR-BIZ 指标分配。功能表单和窗口标签页既是 EOS 的业务配置需求，也是配置生成的业务系统 PA 末级节点；biz 路径在 wft03-biz 结束，不再设置 wft05-biz，也不在 07 重复定义。

```
输入                               产出
bpd-biz 节点                               功能表单 + 窗口标签页
（可以srh设计 / 待补充srh设计）    ──→  （待构件需求定义）
                                      末级节点：功能表单（窗口标签页为页面组件承载）
```
...
| **wft03-eng** | 把操作活动定义组织为系统需求概要（功能表单+概要说明） | bpd-eng 节点（A1）→ srh-eng 系统需求概要需求 | 功能表单（页面职责/页面类别/操作活动清单承接 02/页面编排概要） | wft04-eng |
...
>
> **与 biz 路径的对应关系**（详见 §七）：eng 路径的**配置信息组**对应 biz 路径的输出文档（同为 PL5 末级节点）；CU 对应 biz 的 WBS（同为 PL4）；wft03-eng 的功能表单是平台软件 srh-eng 概要，wft04-eng 的组件业务是平台软件 srd-eng 详细，wft03-biz 的功能表单和窗口标签页兼具业务配置需求与配置业务系统 pa-eng 身份。二者同写入 05，但节点类型、使用角色、终态和下游不同。
...
|------|----------------------|----------------------|----------------------------|
| 末级节点 | 文档（PL5，结构判定级概要说明） | 输出文档（PL5，细分 PL6-A/PL6-B + 表单使用顺序） | 功能表单（窗口标签页为页面组件承载；业务配置需求 + 配置业务系统 pa-eng 双重身份） |
...
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（概要说明，PL5，业务流程概要末级节点） | 配置信息组（操作活动定义） | 功能表单（概要说明） | 功能表单（组件业务） | pa-eng 组件（前端/后端/平台服务） |
