...
> **概述**：定义 EOS 平台的 UI 交互模型（含六种页面类型）、引擎分层体系与 CU 配置单元模型、操作页面组件与构件体系、三类产品（A1/A2/Bn）、资产治理业务，以及三路径（biz/eng/nfr）自闭环 Skill 的编制模式和职责定位。
>
> **版本**：v4.0-draft | **修订**：2026-07-09 | **说明**：【规划中·待确认】引擎体系重构——新增文档引擎（有封面的输出文档）/台账引擎（条目化清单）/指标引擎（定义+口径+双模式展示）；移除需求引擎；执行类场景业务输出文档的子节点拆分为活动节点（PL6）和支持信息；管理类场景业务输出文档的内部构成扩展为指标/看板/项目/WBS/任务/风险/问题/进展清单；biz 三 Skill 职责调整。正文以 `[待确认]` 标记未生效内容。
...
两类场景业务的子节点均为输出文档（PL5），但文档的内部结构不同：

**执行类（D）输出文档**的 PL6 为活动节点（处理步骤+状态变迁），辅以支持信息（输入文档/文档规范/合规要素/作业指南/所需资源）。内容+封面由文档引擎生成，活动节点由工单/计划/流程三选一引擎支撑。

**管理类（PCA）输出文档**由管理对象（=封面，台账引擎）加上 PL6 管理子项组成，管理子项包括不限于：指标清单（指标引擎）、看板清单（看板引擎）、项目清单（项目引擎）、WBS清单（WBS引擎）、任务清单（计划引擎）、风险清单（工单引擎）、问题清单（工单引擎）、进展记录（进展引擎）。
...
| **PL**（输出产品视角） | PL0→PL6 | 平台 → 业务域 → 产品类型 → 构件 → WBS → 输出文档 → 活动节点/管理子项 |
| **EL**（引擎视角） | EL1→EL3 | 菜单引擎 → 功能引擎 → 实体引擎 |

> 其余视角层级的完整定义见 `23-eos-output-architecture.md` 和 `24-eos-business-assets.md`。

**PL6** 为 PL5 输出文档的内部构成，按输出文档类型分为两类：

- **执行类输出文档**的 PL6 为**活动节点**——文档生命周期中的处理步骤，包含处理动作和文档状态变迁（如草稿→已提交→审批中→已通过/已驳回）。由工单/计划/流程三选一引擎支撑。
- **管理类输出文档**的 PL6 为**管理子项**——文档的各管理维度，包括不限于指标项（指标引擎）、看板布局（看板引擎）、项目阶段（项目引擎）、WBS节点（WBS引擎）、计划任务（计划引擎）、风险记录（工单引擎）、问题记录（工单引擎）、进展记录（进展引擎）等。

> 支持信息（输入文档/文档规范/合规要素/作业指南/所需资源）不属于 PL6，是输出文档的辅助属性。

> **"输出文档"的指代**：本文档及 biz 路径各 Skill 中提及的"输出文档"，均指代文档的**内容本身**而非其容器或文件名。例如"相关方需求文档"指需求条目本身，"采购履约看板"指该看板的定义与布局本身。看板上的每个指标为该看板的 PL6 内部构成。文档的承载表单（如 @engine-kanban）仅为组织容器，不影响每个条目作为独立 PL5 输出文档的地位。

**输出文档**按场景业务类型分为两类：

- **执行类输出文档**（产品数据类）：有封面（单据头+元数据）和正文内容，内容+封面由文档引擎生成。PL6 为活动节点，由工单/计划/流程引擎（三选一）支撑。
- **管理类输出文档**（业务管理类）：无独立封面，管理对象=封面（台账引擎），内部为条目化清单。PL6 为管理子项，分别由指标/看板/项目/WBS/计划/工单/进展等引擎支撑。
...
| 2 | @engine-document | 文档引擎 | 功能级（产品数据类） | 将物理表转换为有封面（单据头+元数据）的功能表单，管理正文内容与文档属性 |
| 3 | @engine-ledger | 台账引擎 | 功能级（产品数据类） | 将物理表转换为无封面的条目化功能表单，管理对象=台账的定位标识 |
| 4 | @engine-flow | 流程引擎 | 功能级（产品数据类） | 配置流程节点、流转规则、审批行为和状态迁移 |
| 5 | @engine-workorder | 工单引擎 | 功能级（产品数据类） | 配置工单类型、派发规则、处理流程、验收标准和归档策略——亦支撑风险与问题管理 |
...
| 9 | @engine-kanban | 看板引擎 | 功能级（业务管理类） | 配置指标维度、度量口径、聚合规则、展示方式和看板视图布局，为物理表生成指标型+看板型功能表单 |
| 10 | @engine-indicator | 指标引擎 | 功能级（业务管理类） | 定义指标的名称/口径/目标/维度/图示，生成指标结果功能表单（列表模式/图示模式 2×3网格） |
...
- **功能引擎**：将物理表转换为功能表单并按主子关系（1+N）组装为主子表单，配置行/列级数据访问权限。承载四方面能力：① 表单能力（结构/字段/布局/校验/行为/发布）；② 功能能力（各具体引擎为表单增加的功能）；③ 组装能力（主子表单组装、子表单显示规则匹配、双击下钻）；④ 权限能力（数据行/列对角色/人员的可见、可编辑或不可访问约束）。
  - **产品数据类**（文档/台账/流程/工单）：支撑各类产品数据的开发与管理——文档引擎管理有封面输出文档（单据头+正文），台账引擎管理条目化清单（无封面，管理对象=封面标识），流程引擎管理审批流转，工单引擎管理派发/处理/验收/关闭（亦支撑风险与问题管理）
...
具体而言：
- **实体引擎 CU** → 定义实体及字段，在页面类型①/②/④/⑥上提供增删改查能力
- **功能引擎 CU**（如 @engine-document CU）→ 在文档页面上增加封面配置（单据头/元数据）和正文内容管理能力
- **功能引擎 CU**（如 @engine-ledger CU）→ 在台账页面上增加条目化数据管理和对象标识能力
- **功能引擎 CU**（如 @engine-flow CU）→ 在已有实体页面上增加审批流转按钮和流程节点配置能力
- **功能引擎 CU**（如 @engine-workorder CU）→ 在工单页面上增加派发/处理/验收按钮和生命周期管理能力（亦支撑风险与问题管理）
...
#### 内置 CAX
- 执行类输出文档：物理表由文档引擎转换为有封面功能表单（单据头+元数据+正文内容），由文档引擎 CU 管理封面配置和内容组织
- 管理类输出文档：物理表由台账引擎转换为条目化功能表单，管理对象=台账的封面标识，各管理子项由对应引擎（指标/看板/项目/WBS等）生成
- 若存在主子业务关系，按 1+N 组装为主子表单
...
3. **CU 级**：新构件/组件需要新 CU 或 CU 扩展支撑 → 形成 FR-ENG 线索，经 eng 路径（§七）推进
...
> **STR-E 的双重身份**：wft01-eng 产出的 STR-E 节点（引擎场景业务）在 A2 视角下是 PL5 末级节点——即流程与IT域中**业务配置人员**的场景业务（§4.2）；在 A1 视角下，该场景业务对应的引擎是 A1 的 PL3 构件节点。一个引擎对应一个 STR-E（~11 引擎 → ~11 STR-E），引擎的配置业务（从打开配置页到运行期能力可用）即为该 STR-E 的端到端闭环。CU 是 STR-E 的细分节点——与 STR-F 下辖输出文档的层级关系完全对称。
...
> **流程与IT域的 A2 产品**：流程与IT域同样有 A2 产品——**业务配置人员**是该域的核心角色类型（此外还有业务架构管理员、数据治理员，见 §五）。业务配置人员的场景业务（SL5）即 STR-E ——一个引擎的配置业务为一个场景业务（~11 引擎 → ~11 STR-E）。这些 STR-E 与采购/生产/售后等业务域的 STR-F 在架构层级上完全同构：同样是 SL5 末级节点、同样下辖细分节点（CU vs 输出文档）、同样是角色端到端职责闭环。区别在于场景业务的产出物不是业务文档，而是**可用的平台能力**。
...
>
> 末级节点渐进链：场景业务 → 输出文档 → 配置需求方案
>
> 以下各节定义 biz 路径每个 Skill 的末级节点和任务清单。

### 6.1 原始需求处理（wft01-biz）

**wft01-biz** 将非结构化的 FR-BIZ OR 转化为结构化的 STR-F 场景业务方案。职责：基于OR推断定义场景业务节点和子节点，基于端到端补充业务需求。

末级节点：场景业务
1. 推断场景业务节点——判定类型（执行类D / 管理类PCA）、命名、边界定义
2. 推断输出文档序列——逐文档识别并匹配候选引擎
3. 基于E2E补充业务需求——闭合检查（D闭合/PCA闭合），检出缺口→生成补充OR材料
4. 推断输出文档支撑引擎——按文档类型匹配引擎（文档引擎/台账引擎/流程引擎/工单引擎等），全量标注`[推断]`
5. 推断输出文档所属Bn
...
### 6.2 业务架构开发（wft02-biz）

**wft02-biz** 将 STR-F 的场景业务展开为 BA 方案——A2 角色场景视角 + Bn 输出产品视角。职责：生成 A2 产品业务架构，推断生成 Bn 产品业务架构；推断输出文档及子节点的需求及支撑引擎、推断管理类输出文档管理子项的需求及支撑引擎；基于 Bn 构件的 WBS 补充业务需求。
...
3. 推断Bn构件输出文档
4. 基于Bn补充业务需求
5. 定义输出文档PL6方案——执行类：活动节点（处理步骤+状态变迁，由工单/计划/流程三选一引擎支撑）；管理类：管理子项（指标/看板/项目/WBS/任务/风险/问题/进展清单，分别对应引擎）
6. 推断输出文档支撑引擎——区分执行类（文档引擎/流程引擎等）和管理类（台账引擎/指标引擎/看板引擎等）

> **活动节点与看板指标设计原则**：两者均为对 STR-F 需求的方案响应，而非对 OR 词汇的机械映射。
> - **活动节点**（流程/工单类）：以 STR-F 场景需求理解为主驱动——该输出文档在业务链中承担什么职责、需要哪些处理步骤方能完成端到端闭环、每个步骤由哪个角色执行并产出什么文档状态。
> - **看板指标**（看板类）：以 STR-F 场景需求理解为主驱动——该看板在场景中承担什么度量职责、需要哪些指标方能完整反映业务健康状况、每个指标如何度量与展示。
>
> 引擎能力边界是约束（超出能力的不修改方案迁就），同类文档模式和行业经验是参考（不替代需求分析），人类反馈与确认是门禁（全部节点/指标标注 `[推断]`，未经确认不得落账）。详见 wft02-biz §3.5.2~§3.5.3。

### 6.3 系统需求开发（wft03-biz）

**wft03-biz** 将 BA 方案展开为各层配置需求。职责：生成菜单树方案及其配置需求；生成场景业务方案及配置需求；生成输出文档内容配置需求；生成输出文档任务配置需求；生成管理类输出文档管理子项配置需求；基于配置需求补充引擎需求。

末级节点：各层配置需求方案
1. 生成菜单树方案及配置需求——菜单层级结构、末级节点→场景业务映射
2. 生成场景业务方案及配置需求——主子表单序列编排、显示顺序与依赖关系
3. 生成输出文档内容配置需求——执行类：封面（文档引擎）+ 正文内容；管理类：对象标识（台账引擎）+ 条目化清单
4. 生成输出文档任务配置需求——活动节点/管理子项的任务引擎配置（工单/计划/流程三选一）
5. 生成管理类输出文档管理子项配置需求——指标配置（指标引擎）、看板布局（看板引擎）、项目/WBS/任务/风险/问题/进展配置
6. 基于配置需求补充引擎需求——发现引擎能力缺口→生成 FR-ENG 线索
...
### 7.1 原始需求处理（wft01-eng）

**wft01-eng** 将 FR-ENG OR 转化为 STR-E 引擎场景业务方案。

**角色**：业务配置人员（流程与IT域）。该角色是 eng 路径的唯一角色——所有引擎的配置均由业务配置人员执行，因此 STR-E 的架构树上无需角色层做分组维度。

**STR-E = 一个引擎的配置业务**：每个引擎（`@engine-flow`、`@engine-kanban` 等）对应一个 STR-E，即该角色对该引擎的端到端配置业务——从打开配置页面、编辑配置要素、系统校验保存、发布装载、到运行期构件/功能可用的完整闭环。当前约 11 个引擎 → ~11 个 STR-E。

**STR-E 的双重身份**（详见 §4.1）：在 A2 视角下是 PL5 末级节点（流程与IT域的场景业务）；在 A1 视角下该引擎是 PL3 构件节点。
...
### 7.2 业务架构开发（wft02-eng）

**wft02-eng** 将 STR-E 中名称级 CU 候选展开为 A1 配置单元 BA 详细设计。为每个 CU 定义其配置页面和业务页面的设计方案。

**末级节点**：配置单元（CU）。CU 是 A1 产品的架构末级节点（详见 §4.1），以 IPO 定义：Input 为配置要素，Process 为系统处理说明，Output 为运行期业务能力。每个 CU 对应两类页面——配置页面（硬代码实现，配置人员编辑配置要素）+ 业务页面（引擎按 CU 定义在操作页面组件上渲染构件能力，见 §3.3）。
...
### 7.3 系统需求开发（wft03-eng）

**wft03-eng** 将 BA 中的操作页面展开为引擎功能 SR-F 需求——为每个操作页面识别其所需构件。

**末级节点**：操作页面。细分节点为**构件**——安装在操作页面组件上的公共功能单元，可跨页面组件复用（按钮/右键菜单/快捷键/列定义/字段布局/图表类型/钻取等，详见 §3.2）。
...
| **Step 2 · 方案反馈处理** | 引用 91 规范 §8.5 + 本 Skill 的特化规则 + 确认状态格式示例 | 通用协议不重复，仅写本 Skill 的特化部分 |
...
四类增量标记（`[新增]`/`[变更]`/`[确认]`/`[需裁决]`）的定义及使用规则见 §8.8。各 Skill 的 §四 引用 §8.8，并补充本 Skill 的增量清单格式示例（含本层级的完整增量清单），供人类参照。
...
| Skill | 末级节点 | 职责定位 | 所属章 |
|-------|---------|---------|--------|
| wft01-biz | 场景业务 | 基于 OR 推断定义场景业务节点和子节点，基于端到端补充业务需求 | §六 |
| wft02-biz | 输出文档 | 生成 A2+Bn 产品业务架构，推断输出文档及子节点/管理子项的引擎与需求 | §六 |
| wft03-biz | 配置需求方案 | 生成菜单树/场景业务/输出文档内容/任务/管理子项各层配置需求，补充引擎需求 | §六 |

### 8.3 eng 路径 Skill

| Skill | 末级节点 | 细分节点 | 所属章 |
|-------|---------|---------|--------|
| wft01-eng | 场景业务（STR-E） | CU 候选（名称级） | §七 |
| wft02-eng | 配置单元（CU） | 操作页面（配置页+业务页） | §七 |
| wft03-eng | 操作页面 | 构件（按钮/右键菜单/图表/钻取等） | §七 |
| wft05-eng | 构件 | 前端组件 + 后端组件 | §七 |
...
| 操作条目 | 场景职责边界 + PL5 输出文档（含类型判定 D/PCA） | A2/Bn 方案 + 输出文档设计 + PL6（活动节点/管理子项） + 权限 + 对齐结论 | 菜单树/场景业务/输出文档内容/任务/管理子项各层配置需求 + 引擎需求补充 |
| 推进目标 | `可以分解分配` | `可以SR设计` | `待构件需求定义` |
| 需求级联触发 | 场景职责边界变更 / 文档拆分合并 / 文档引擎变更 | 场景职责边界变更 / 输出文档拆分合并 / 文档职责归属变更 / PL6 类型变更 | A2 场景流变更 / Bn 产品数据或实体关系变更 / 配置需求方案变更 |
...
> **版本标记与消费追溯**：节点初次生成时版本 = `v1`；每次因上游新输入触发修订，版本号递增。下游 Skill 首次消费时记录 `消费版本：v{N}`，重入时比对版本——若当前版本 > 已消费版本 → 触发变更感知，读取变更影响声明（§8.7），按变更类型处理。
