...
对应设计路径为 **eng 路径**：wft01-eng（业务概要）→ wft02-eng（业务详细）→ wft03-eng（系统需求概要）→ wft04-eng（系统需求详细）→ wft05-eng（产品概要/三类组件）。
...
对应设计路径为 **biz 路径的前半段**：wft01-biz（业务概要定义，场景业务为枝、文档为末级）→ wft02-biz（业务详细定义，组装 A2 产品业务流程 + 表单使用顺序）。
...
|-------|------|------|---------|---------|
| **wft01-biz** | FR-BIZ OR 条目 | 业务概要定义——bph-biz 场景业务（枝）+ PL5 输出文档序列（概要末梢）+ Bn 组装（WBS 归组/23 一致性/缺口推断） | 文档（PL5，结构判定级概要说明） | 向业务方证明"我听懂了你的业务" |
| **wft02-biz** | bph-biz 节点 | 业务详细定义——A2（角色→场景业务→表单使用顺序）+ Bn（输出文档详细设计 + PL6-A 活动节点需求 + PL6-B 条目需求） | 输出文档（PL5），细分 PL6-A / PL6-B + 表单使用顺序 | 设计正确的方案——业务语言→架构语言 |
...
| **产出物** | 业务概要定义（bph-biz）——场景业务 + 输出文档序列 + Bn 组装（WBS 归组/23 一致性/缺口推断）+ PL6-A 活动节点与 PL6-B 正文条目初步判定（结构判定级） | 业务详细定义（BP）——A2 角色场景（功能表单/主子表单 + 表单使用顺序）+ Bn 输出产品（输出文档详细设计 + PL6-A 节点需求定义 + PL6-B 条目需求定义） | 系统概要定义（业务配置需求暨配置业务系统 pa-eng）——功能表单（窗口标签页为页面组件承载）+ 引擎能力使用需求 + FR-BIZ 指标约束包 + FR-ENG 候选线索 |
| **供谁消费** | 业务方确认 → wft02-biz | wft03-biz | 构件开发流水线 + eng 路径（引擎缺口线索） |

> **概要/详细分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要定义/详细定义」重述——s01 wft01 业务概要定义（末级=文档，概要说明为**结构判定级**，字段级详设留 s02）；s02 wft02 业务详细定义（详细条目=**活动 + 正文条目 + 表单使用顺序**）；s03 wft03 系统概要定义（末级=**功能表单**，窗口标签页为其页面组件承载形式，2026-08-22 深审统一）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bph-biz、BP→bpd-biz、SR-F→srh-biz；追溯/23 不迁。命名「业务详细定义」源于 bpd-biz IPO 承担详细角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 OR 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
```
输入                         产出
FR-BIZ OR（待处理）  ──→  业务概要定义 bph-biz 场景业务方案（可以分解分配）
...
```
输入                               产出
bph-biz 节点（含 wft01 Bn 组装结论）   业务详细定义 bpd-biz 方案（可以srh设计）
...
FR-ENG OR ──→ wft01-eng ──→ bph-eng ──→ wft02-eng ──→ bpd-eng ──→ wft03-eng ──→ srh-eng ──→ wft04-eng ──→ srd-eng ──→ wft05-eng ──→ pa-eng
              业务概要定义          业务详细定义            系统需求概要定义              系统需求详细定义               产品概要定义
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明"我听懂了你要的平台能力" | FR-ENG OR 条目 → bph-eng 业务概要方案 | 配置信息组（概要说明，PL5 末级，业务概要定义末级节点；bph-eng 架构末级=场景业务 SL5）+ CU 承接 + 依赖 bph-eng（共享依赖） | 业务配置人员确认 → wft02-eng |
| **wft02-eng** | 详细定义已分配的 CU——操作活动定义/依赖/状态迁移 | bph-eng 节点 → A1 bpd-eng 业务详细方案 | 配置信息组（操作活动定义：配置操作/系统处理/运行期能力；CU 为 PL4 选配单元） | wft03-eng |
...
**将零散的 FR-ENG OR 组织为结构化的平台能力场景业务方案（业务概要定义，01 层——概要说明为结构判定级，操作活动定义留 02 展开），供业务配置人员确认理解。**
...
| 2 | 名称级识别并分解配置信息组（概要说明）+ 分配配置信息组到 CU 承接 | 对称于 bph-biz 下辖的输出文档——场景业务由哪些 CU 组成；按"可以运行的能力"名称级识别并分解为配置信息组（概要说明，**结构判定级**——配置什么/运行期提供什么，不展开操作活动定义——那是 wft02-eng 业务详细定义），逐组分配 CU 承接（含收敛判定复用/新增/拆分/合并 + 对照 25 盘存的骨架判定） |
...
**详细定义 wft01-eng 已分配的 CU——业务详细定义（02 层，操作活动定义步骤化三段式：配置操作/系统处理/运行期能力），承接 bph-eng 的 A2 上下文与 A1 装配，展开操作活动定义/依赖/状态迁移。**

```
输入                                           产出
bph-eng 节点                                    A1 配置单元 bpd-eng 业务详细方案（可以srh设计）
...
原始材料 → ort00文本化 → ort01切分 → ort02澄清
          ...
          (bpd-biz 双视角)          (业务详细定义)     (SysReq-NFR 量化)
                │                   │                   │
                ▼                   ▼                   │
...
|-------|---------|---------|---------|--------|
| wft01-biz | 文档（PL5，概要末梢） | PL6-A 活动节点 / PL6-B 正文条目初步判定（结构判定级） | 基于 OR 推断**业务概要定义**（s01）——场景业务（枝）+ 输出文档序列 + **Bn 组装**（WBS 归组/23 一致性/按构件产品数据清单完整性缺口推断），结构性缺口补充 OR 回路——"向业务方证明我听懂了" | §七 |
| wft02-biz | 输出文档（PL5） | PL6-A 活动节点 / PL6-B 正文条目 / 表单使用顺序 | 消费 wft01 Bn 组装，生成 A2+Bn 双视角**业务详细定义**（s02）——引擎校验（权威）+ 文档方案 + 活动设计 + 权限 + 任务关系（架构语言）——"设计方案" | §七 |
| wft03-biz | 功能表单（窗口标签页为页面组件承载） | 引擎能力使用需求 | 消费 wft02 业务详细定义 → 生成**系统概要定义**（s03）——页面组件；分配 FR-BIZ 指标到引擎/CU；输出 FR-BIZ 指标约束包和 FR-ENG 候选线索——"出配置需求暨配置业务系统pa-eng" | §七 |

### 10.4 eng 路径 Skill

| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 配置信息组（概要说明，PL5，业务概要定义末级节点） | 主责能力域 / 配置信息组（概要说明） / CU 承接 / 依赖 bph-eng（bph-eng 架构末级=场景业务） | 基于 FR-ENG OR 推断可独立配置、治理和验收的平台能力场景 + 名称级识别并分解配置信息组（概要说明） + 分配配置信息组到 CU 承接——"向平台方证明我听懂了"（业务概要定义，01 层） | §八 |
| wft02-eng | 配置信息组（操作活动定义） | CU（PL4 选配单元）/ 操作活动定义（配置操作/系统处理/运行期能力，步骤化三段式） | 承接已分配 CU，展开操作活动定义/依赖/状态迁移详细定义——"详细定义配置单元"（业务详细定义，02 层） | §八 |
...
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（概要说明，PL5，业务概要定义末级节点） | 配置信息组（操作活动定义） | 功能表单（概要说明） | 功能表单（组件业务） | pa-eng 组件（前端/后端/平台服务） |
...
| 要素 | 规则 |
|------|------|
| **标题命名** | 按本 Skill 的末级节点命名（wft01-biz →「业务概要设计基础」，wft02-biz →「业务详细设计基础」） |
