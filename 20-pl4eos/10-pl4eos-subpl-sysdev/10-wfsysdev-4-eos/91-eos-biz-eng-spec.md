...
| CU | 只引用既有能力并形成引擎能力使用需求 | wft02-eng 定义 CU，wft03-eng 组织功能概要 |
...
→ 形成 FR-ENG 候选线索。候选线索不是正式 OR，不得直接进入 wft01-eng；须经人类确认后作为原始需求材料登记，并完整通过 ort00→ort01→ort02→ort03，形成正式 FR-ENG OR 后，才按 eng 路径（§八）推进——wft01-eng 判定归属引擎 → wft02-eng 设计 CU → wft03-eng 组织功能概要 → wft04-eng 展开组件业务 → wft05-eng 展开三类组件。
...
对应设计路径为 **eng 路径**：wft01-eng（业务概要）→ wft02-eng（业务详细）→ wft03-eng（功能概要）→ wft04-eng（功能详细）→ wft05-eng（产品概要/三类组件）。
...
> **概要/详细分工框架（2026-08-21 定案）**：biz 链三个 Skill 以「概要定义/详细定义」重述——s01 wft01 业务概要定义（末级=文档，概要说明为**结构判定级**，字段级详设留 s02）；s02 wft02 业务详细定义（详细条目=**活动 + 正文条目 + 表单使用顺序**）；s03 wft03 系统概要定义（末级=**功能表单**，窗口标签页为其页面组件承载形式，2026-08-22 深审统一）。一棵树读法——A2 归纳枝（SL3 角色类型→SL4 角色职责）+ Bn 归纳枝（PL3 构件类型→PL4 WBS）汇入场景业务（SL5）→ 文档（PL5）→ 活动/正文条目/表单使用顺序 → 功能表单。STR-F/BP/SR-F 资产名已按 2026-08-24 更名清单迁移：STR-F→bph-biz、BP→bpd-biz、SR-F→sfh-biz；追溯/23 不迁。命名「业务详细定义」源于 bpd-biz IPO 承担详细角色（文档方案/活动/正文条目/表单使用顺序），不破通用规范「BP 仅有架构定义子文档，无详细定义」（[§1.3 特例](../../../00-generalspec/01-generalspec-sysdev.md#13-文档构成)）。**唯一真边界变更 = Bn 组装执行上移 wft01**（PL4 WBS 归组 + 23 一致性检查 + 按构件产品数据清单完整性做名称级缺口推断 + 结构性缺口 OR 回路）——wft02 收缩为纯详细层，引擎/任务类型权威仍在 wft02 校验值。
>
...
>
> **与 eng 路径的对称性**（详见 §八）：biz 路径的输出文档对应 eng 路径的配置信息组——都是各自产品的 PL5 末级节点；eng 的 CU（PL4）对应 biz 的 WBS（PL4）。wft03-biz 的配置需求方案 ←→ wft03-eng 的功能概要、wft04-eng 的功能详细，同属系统功能概要层（sfh-biz/sfh-eng/sfd-eng）。
...
```
输入                               产出
bph-biz 节点（含 wft01 Bn 组装结论）   业务详细定义 bpd-biz 方案（可以sfh设计）
...
```
FR-ENG OR ──→ wft01-eng ──→ bph-eng ──→ wft02-eng ──→ bpd-eng ──→ wft03-eng ──→ sfh-eng ──→ wft04-eng ──→ sfd-eng ──→ wft05-eng ──→ pa-eng
              业务概要定义          业务详细定义            功能概要定义              功能详细定义               产品概要定义
...
| **wft03-eng** | 把操作活动定义组织为功能概要（功能表单+概要说明） | bpd-eng 节点（A1）→ sfh-eng 功能概要需求 | 功能表单（页面职责/页面类别/操作活动清单承接 02/页面编排概要） | wft04-eng |
| **wft04-eng** | 展开功能表单上各组件及其业务（功能详细） | 功能概要 sfh-eng → sfh-eng 功能详细需求 | 组件业务（布局组件/构件 + 组件业务 + pa-eng 层收敛项） | wft05-eng |
| **wft05-eng** | 产出可交付开发的三类组件方案 | 功能详细 sfd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 → pa-eng 组件需求定义 | pa-eng 组件（前端/后端/平台服务） | 追溯验证 / 构件开发流水线 |
...
>
> **与 biz 路径的对应关系**（详见 §七）：eng 路径的**配置信息组**对应 biz 路径的输出文档（同为 PL5 末级节点）；CU 对应 biz 的 WBS（同为 PL4）；wft03-eng 的功能表单是平台软件 sfh-eng 概要，wft04-eng 的组件业务是平台软件 sfd-eng 详细，wft03-biz 的功能表单和窗口标签页兼具业务配置需求与配置业务系统 pa-eng 身份。二者同写入 05，但节点类型、使用角色、终态和下游不同。
...
```
输入                                           产出
bph-eng 节点                                    A1 配置单元 bpd-eng 业务详细方案（可以sfh设计）
...
>
> **CU 的页面关系**：CU 在配置时通常对应硬代码功能表单（复合页内的子页面即完整页面，一个 CU 对应一个页面），配置人员编辑配置要素，提交后引擎校验并保存；运行时，引擎按 CU 定义生成或渲染制品、构件及其功能。wft02-eng 只定义操作活动，wft03-eng 才把操作活动组织为功能概要。

### 8.3 功能概要开发（wft03-eng）

**将 CU 操作活动定义组织为功能概要 sfh-eng：功能表单+概要说明（结构判定级——页面职责/页面类别/使用角色/承载关系/操作活动清单/页面编排概要），只组织页面，操作活动清单承接 02 标注入口。**

```
输入                                           产出
bpd-eng 节点（A1）                                平台功能概要 sfh-eng（可以详细定义）
（可以sfh设计 / 待补充sfh设计）  ──→            末级节点：功能表单
...
### 8.4 功能详细开发（wft04-eng）

**将功能概要 sfh-eng 展开为功能详细 sfd-eng：判定区域内布局组件/构件、定义组件业务（功能级不写实现）、标注 pa-eng 层收敛项。组件业务是 wft05-eng 推断前后端组件的依据。**

```
输入                                           产出
功能概要 sfh-eng（05-*.md）                    平台功能详细 sfd-eng（已详细定义）
...
**将功能详细 sfd-eng（组件业务）、FR-BIZ 指标约束和 NFR 约束展开为 pa-eng 三类组件方案，构件开发流水线可据此进入开发。**（组件业务是推断前后端组件的依据。）

```
输入                                              产出
功能详细 sfd-eng + FR-BIZ指标约束包 + NFR约束包     pa-eng 组件需求定义（待追溯验证）
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 基于功能详细 sfd-eng 及约束推断平台架构 | 判断组件属于引擎专属、布局组件扩展、公共构件、公共平台服务或运行支撑模块；同时消费 FR-BIZ 指标约束包和 wft04-nfr 的 wft05-eng NFR 约束包，形成必要的平台架构决策 |
...
| 4 | 生成分层约束表 | 逐量化指标判定约束范围（全局 / 业务域 / sfh-biz / CU / pa-eng组件 / 页面级）→适用链路（biz / eng / 共享）→约束对象→消费目标→影响级别（阻断型 / 非阻断型 / 待判定）→影响点 |
| 5 | 生成页面 NFR | 影响用户可感知页面行为的指标→绑定七种页面类型之一 + 约束内容 + 量化目标 + 测量方法 + 消费目标。覆盖页面性能/安全/易用性/可访问性/可恢复性/可观测性六个维度 |
| 6 | 装配 wft05-eng NFR 约束包 | 按约束范围组织为 wft05-eng NFR 约束包：全局约束 / 业务域约束 / sfh-biz 约束 / 页面级约束 / CU 约束 / pa-eng 组件约束 / 共享约束。约束包只引用分层约束表行 ID 和页面 NFR 条目，不复制正文；分解子指标作为独立约束行装配（标注分解运算+父指标引用）。消费状态/已消费版本由 wft05-eng 消费后回写。NFR 约束仅流向 eng 路径——非功能质量由引擎层保障，biz 路径不消费 |
...
> **CU 设计权规则**：CU 是引擎提供的配置能力单元，其能力边界、配置入口、配置要素、系统处理、运行期输出和 FR-ENG 性能指标仅由 eng 路径设计。**wft01-eng 设计 CU 承接（名称级识别并分解配置信息组（概要说明） + 分配配置信息组到 CU 承接，含收敛判定）**；**wft02-eng 详细定义 CU（承接已分配 CU，展开操作活动定义/依赖/状态迁移）**；wft03-eng 将 CU 操作活动组织为功能概要；wft04-eng 展开功能表单上组件业务；wft05-eng 将其展开为软件组件。biz 路径不得新建、修改或详细设计 CU。
...
第二轮：eng/nfr 汇合与收尾
  eng 线：wft04-eng 的功能详细 sfd-eng 基线就绪
  nfr 线：wft04-nfr 完成目标绑定并确认首版 wft05-eng NFR 约束包
  汇合条件满足 → wft05-eng 同时消费功能详细 sfd-eng、FR-BIZ 指标约束包和 wft05-eng NFR 约束包 → 平台 pa-eng 组件需求定义
```
...
| 依赖关系 | 说明 | 策略 |
|---------|------|------|
| eng 线 wft05-eng ← nfr 线 wft04-nfr | wft05-eng 消费 NFR 约束包 | eng 前四步与 nfr 可并行；首版约束包须在相关 bph-eng/CU/功能详细 sfd-eng 可定位后完成目标绑定并确认，wft05-eng 在两侧输入同时就绪后启动 |
...
> **实际建议**：三路径可同时启动第一轮。biz 线最短（3 Skill），eng 线前段（3 Skill）与 nfr 线（2 Skill）可并行；wft04 可先形成全局/共享约束和目标候选，但 CU/页面级约束在相应对象出现后才能正式绑定。eng 功能概要/详细（sfh-eng/sfd-eng）与首版 NFR 约束包均确认后，启动 wft05-eng 收尾。
...
| wft03-eng | 功能表单 | 页面类别 / 操作活动清单（承接 02）/ 页面编排概要（区域/TAB/弹窗入口） | 承接操作活动定义，组织功能表单+概要说明——"出功能概要 sfh-eng"（功能概要定义，03 层） | §八 |
| wft04-eng | 功能表单（组件业务） | 布局组件/构件 / 组件业务 / pa-eng 层收敛项 | 承接功能概要，展开功能表单上各组件及其业务——"出功能详细 sfd-eng"（功能详细定义，04 层） | §八 |
| wft05-eng | pa-eng 组件 | 前端组件 / 后端组件 / 平台服务组件 | 消费功能详细 sfd-eng、FR-BIZ 指标约束包和 NFR 约束包，从组件业务推断三类组件及组件架构——"出组件需求"（产品概要定义，05 层） | §八 |
...
| 输入合法状态 | `待处理` | `可以分解分配` / `待补充分解分配` | `可以sfh设计` / `待补充sfh设计` |
| 产出状态 | `可以分解分配` | `可以sfh设计` | `可构件开发` |
...
| 维度 | wft01-eng（OR→bph-eng） | wft02-eng（bph-eng→bpd-eng） | wft03-eng（bpd-eng→功能概要 sfh-eng） | wft04-eng（功能概要→功能详细 sfd-eng） | wft05-eng（功能详细→pa-eng） |
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（概要说明，PL5，业务概要定义末级节点） | 配置信息组（操作活动定义） | 功能表单（概要说明） | 功能表单（组件业务） | pa-eng 组件（前端/后端/平台服务） |
| 输入对象 | FR-ENG OR 条目（≤10） | bph-eng 节点（≤5） | bpd-eng 节点（A1，≤5） | 功能概要 sfh-eng（≤5） | 功能详细 sfd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
| 输入合法状态 | `待处理` | `可以详细定义` / `待补充详细定义` | `可以sfh设计` / `待补充sfh设计` | `可以详细定义` / `待补充详细定义` | `已详细定义` |
| 产出状态 | `可以详细定义` | `可以sfh设计` | `可以详细定义` | `已详细定义` | `待追溯验证` |
...
| 05 | `05-eos-function-summary-architecture.md` | 系统功能概要架构 |
| 06 | `06-eos-function-detailed.md` | 系统功能详细 |
...
| 字段 | 格式 | 说明 |
|------|------|------|
| `**类型**` | `{层级}.{子类}` | 如 `bpd-biz.L5`、`sfh-biz.L3` |
| `**父节点**` | `@{id}` | 父节点 ID |
| `**输入**` | `@{id}` 或自由文本 | 多个用 `/` 分隔 |
| `**输出**` | `@{id}` 或自由文本 | 同上 |
...
> 各文档特有字段（如 bpd-biz 的 `**支撑引擎**`、sfh-biz 的 `**验收条件**`/`**量化指标**`）定义在 91 规范各对应章节中。
...
| `@srf-` | sfh-biz 节点 | `@srf-proc-apply-001` |
...
> **版本标记与消费追溯**：节点初次生成时版本 = `v1`；每次因上游新输入触发修订，版本号递增。下游 Skill 首次消费时记录 `消费版本：v{N}`，重入时比对版本——若当前版本 > 已消费版本 → 触发变更感知，读取变更影响声明（§A.7），按变更类型处理。**消费版本记录于消费者节点的「来源」字段**（如 sfh-biz 的「来源 bpd-biz」、bpd-biz 的「来源 bph-biz」），生产者节点不重复存消费版本——生产者侧下游可见性由资产写回记录（含消费版本列）承担，变更影响声明的「下游已消费节点」按消费者「来源」字段定向查询。**两跳粒度注**：bph-biz→bpd-biz 跳处理粒度是整个 bph-biz（文档序列级），重入靠节点状态路由 + 变更影响声明；bpd-biz→sfh-biz 跳处理粒度是节点级（逐 sfh-biz 判断基于哪个 bpd-biz 版本），重入靠版本比对。两跳机制不同源于粒度不同，非漏实现。
...
> 各 Skill 特有的影响范围判定表（bpd-biz 变化→sfh-biz 受影响范围等）保留在各 Skill 文档中。
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-08-24 | v5.37 | 资产名更名（人类定案，待议项全部定案后执行）——STR-F→bph-biz、STR-E→bph-eng、STR-NFR→bph-nfr、BP→bpd-biz/bpd-eng、SR-F→sfh-biz/sfh-eng（03 功能概要）/sfd-eng（04 功能详细）、PA→pa-eng（SysReq-NFR 保留）；§7 分工框架「资产名保留」句改写为迁移说明；状态名 可以SR设计→可以sfh设计、在PA设计→在pa-eng设计；§11.1 资产清单文件名/说明随迁。 |
...
| 2026-08-21 | v5.27 | eng 链 5 层「概要/详细」分工框架实施第⑥步（wft01-eng v9.5 + wft02-eng v10.0 + wft03-eng v9.0 + 新增 wft04-eng v1.0 + wft05-eng v10.0 对偶实施，人类裁决插入式重排 2026-08-21）——§8 任务清单重排：§8.1 业务概要定义（wft01-eng，结构判定级）、§8.2 业务详细定义（wft02-eng，操作活动定义步骤化）、§8.3 功能概要开发（wft03-eng，功能表单+概要说明）、**新增 §8.4 功能详细开发（wft04-eng，组件业务+PA 收敛项）**、原 §8.4→§8.5 产品概要开发（wft05-eng，输入改功能详细 SR-F）；§八 章首接力链表四→五个 Skill；§10.1 三路径全景图/执行顺序/CU 设计权规则补 wft04-eng；§10.4 eng 路径 Skill 表加 wft04-eng 行；§10.6 eng 参数表加 wft04-eng 列（wft03 产出状态 `可以详细定义`、wft05 输入状态 `已详细定义`）；链级引用（§3.3 CU 设计权规则/构件表/eng 路径分工/biz 对称性/汇合条件）同步。 |
