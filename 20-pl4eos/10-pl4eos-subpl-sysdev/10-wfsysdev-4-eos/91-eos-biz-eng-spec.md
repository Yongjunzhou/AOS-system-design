...
>
> **版本**：v4.8 | **修订**：2026-07-11 | **说明**：§七~§九 全章重构（产出物链清晰化 + 双表合一 + NFR→eng 单向流 + 平台服务组件）+ 附录A 重构为四部分结构 + §10.4/§10.6 参数对齐 + §十一 新增产品数据与资产文档结构规范（就绪队列、子条目状态、变更感知、HEAD 追踪）+ 附录A 读取/写回协议全量同步 + 全文缺陷修复。
...
> **适用范围**：01~07、21~28 号产品数据与资产文档。08（追溯矩阵）和 09（验证报告）保留原格式，不适用本章规范。
>
...
当前 Skill 有对应的就绪队列分节（§11.9）时，优先读取：
...
按索引表定位目标域/角色，再通过 grep 按节点 ID 定位：
...
| **R13** | 就绪队列优先级高于索引导航。只要有对应分节，必须优先读就绪队列 |
...
如果当前 Skill 有对应的就绪队列分节（§11.9），先读就绪队列：
