...
**初始态**：浏览器窗口为**单窗全宽**，顶部多 TAB 对应场景业务下的多个主子表单；选中一行数据双击可下钻。初始态无底部导航面包屑。右侧辅助栏位关闭时显示"编辑"按钮，此时主内容区全宽。

```
┌──────────────────────────────────────────────────────────────────────────────┐
│ [TAB: 主子表单A] [TAB: 主子表单B] [TAB: 主子表单C]                   [编辑] │
│                                                                                │
│ ┌────────────────────────────────────────────────────────────────────────────┐ │
│ │ 主表单（列表/树表）                                                         │ │
│ │ ┌────────────────┬────────────────┬────────────────┬──────────────────┐   │ │
...
| ③ | **单窗单据页** | 顶部 TAB，主区域为单据页（字段+操作按钮），无辅助栏 | 独立录入/编辑/审批页 |
| ④ | **左窗主表+右窗子表+辅助栏** | 分屏模式，左窗为列表/树表，右窗为对应子业务数据，辅助栏在右侧 | 1:N 主子数据管理 |
| ⑤ | **全屏单据页** | 无顶部 TAB，整页为一张单据页 | 大篇幅录入/展示，如合同编辑 |
| ⑥ | **弹窗** | 模态弹出页面，非独立 TAB | 临时焦点操作，如选择数据 |

### 1.3 辅助栏

辅助栏是列表类页面（类型①、②、④）的右侧可展开信息面板，行为在所有页面类型上保持一致：
...
| **Step End · 运行结束输出** | 块引用格式：当前状态 + 下一步 A~E 选项 | 人类下一步操作提示，不写入资产 |
