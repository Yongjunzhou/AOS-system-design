...
>
> **版本**：v5.14 | **修订**：2026-08-15 | **说明**：§A.5 补预处理链特化（试点 eos-ort01-chunk）——反馈走「文档编辑 + git」而非对话模式（git diff 检测 + 基线提交 + 交互只记录不处理）
...
>
> **预处理链特化（试点：eos-ort01-chunk）**：OR 预处理链的 `eos-ort01-chunk` 不走本节对话反馈模式，而走「**文档编辑 + git**」反馈——人类直接编辑文档（方案块/反馈区/切分文件）表达反馈；AI 每次运行开始 `git diff <基线>` 检出变更、判读意图、记录到状态文档表 A `内容摘要`，处理基于记录执行；AI 每轮结束 `scripts/ai-commit.sh` 提交基线（author=EOS-AI + `[AI]` 前缀 + `HEAD @上次AI运行` 记录）。**交互只记录不处理**——对话反馈仅记录到表 A。详见 eos-ort01-chunk 的 Step 1 变更检测与 Step End。其余预处理链 Skill（ort00/02/03）待推广后同步本节。
