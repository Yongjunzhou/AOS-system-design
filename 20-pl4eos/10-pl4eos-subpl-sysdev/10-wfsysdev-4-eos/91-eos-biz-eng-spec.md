...
| 序号 | 页面类型 | 描述 | 适用场景 |
|------|---------|------|---------|
| ① | **单窗列表+辅助栏** | 顶部 TAB，主区域为数据列表（表格），右下方为辅助栏位 | 纯列表管理，如实体数据维护 |
| ② | **单窗树表+辅助栏** | 顶部 TAB，主区域为树形结构表，右下方为辅助栏位 | 树形数据管理，如分类/BOM管理 |
| ③ | **单窗单据页** | 顶部 TAB，主区域为单据页（字段+操作按钮），无辅助栏 | 独立录入/编辑/审批页 |
| ④ | **左窗主表+右窗子表+辅助栏** | 分屏模式，左窗为列表/树表，右窗为对应子业务数据，辅助栏在右下 | 1:N 主子数据管理 |
