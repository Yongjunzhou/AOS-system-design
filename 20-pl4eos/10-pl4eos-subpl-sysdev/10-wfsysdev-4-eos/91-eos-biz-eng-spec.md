...
→ 形成 FR-ENG 候选线索。候选线索不是规范化需求节点，不得直接进入 wft01-eng；须经人类确认后作为原始需求材料登记，并完整通过 ort00→ort01→ort02→ort03，形成FR-ENG 规范化需求节点 后，才按 eng 路径（§八）推进——wft01-eng 判定归属引擎 → wft02-eng 设计 CU → wft03-eng 组织系统需求概要 → wft04-eng 展开组件业务 → wft05-eng 展开三类组件。

biz 路径在设计中发现构件或布局组件缺口时，只输出 FR-ENG 候选线索（字段集以 wft03-biz §3.2 为权威：标识[需求 ID/来源/目标引擎/需求类型] + 能力描述[缺口层级/当前能力/需求能力/指标要求] + 业务影响[业务场景/业务影响]；Q2 性能不足、Q3 能力缺失、Q4 边界不清均生成线索，需求类型分别为扩展能力边界/新增能力/明确边界，Q2 为语义判定不涉及数值性能比较——系统性能归 NFR 链）。页面组件类型及组合属于 biz 页面需求本身；只有当现有布局组件或构件能力无法承载该页面组件需求时，才形成 FR-ENG 候选线索。该线索不得直接写入规范化需求节点或”待 wft01-eng 处理”分节。
...
> 治理 bph-biz 的完整清单以 `02-*.md` 为准，AI 运行时动态读取。上述四类治理业务当前 规范化需求 承接为空，由人类在 EOS 平台建设过程中逐批增补。

---

## 七、业务系统之系统设计

biz 路径将业务原始需求（FR-BIZ 规范化需求）逐级转化为可交付构件开发的业务配置需求方案。三个 Skill 形成设计接力链——每个 Skill 的输出是下一个的输入，语言从业务方听得懂的**业务语言**逐步过渡到构件开发可消费的**平台语言**：

```
FR-BIZ 规范化需求  ──→  wft01-biz  ──→  wft02-biz  ──→  wft03-biz
...
|-------|------|------|---------|---------|
| **wft01-biz** | FR-BIZ 规范化需求条目 | 业务流程概要定义——bph-biz 场景业务（枝）+ PL5 输出文档序列（概要末梢）+ Bn 组装（WBS 归组/23 一致性/缺口推断） | 文档（PL5，结构判定级概要说明） | 向业务方证明"我听懂了你的业务" |
...
> 从零散但规范化的 规范化需求 到结构化的场景业务方案。核心使命：**向业务方证明"我听懂了你的业务"**——产出供人类确认理解，而非最终设计方案。

```
输入                         产出
FR-BIZ 规范化需求（待处理）  ──→  业务流程概要定义 bph-biz 场景业务方案（可以分解分配）
...
| 3 | 判定任务类型与子节点 | 业务语言，供人类确认。判定任务类型（计划/流程/工单）→ 展开 PL6-A 活动节点，捕获 规范化需求 明示节点。管理类（PCA）场景业务需判定 PL6-B 正文条目。R4：首个有任务类型的文档必然为计划 |
| 4 | 推断任务间关系 | 业务语言。文档间流转顺序 + 规范化需求 明示的任务触发关系 |
| 5 | 基于 E2E 补充业务需求 | 闭合检查（D：从需求提出到需求满足；PCA：计划→监控→分析→纠正），检出缺口 → 生成补充 规范化需求 材料 |
| 6 | 推断文档内容支撑引擎 | 按文档类型匹配引擎（台账/工单/指标/看板/项目/WBS/计划/进展等），全量标注 `[推断]` |
| 7 | Bn 组装 | 23B Q1/Q2/Q3 对齐判定 + PL4 WBS 归组（数据依赖链硬判据）+ 23 一致性检查（归属冲突/误新建/误引用）+ 按构件产品数据清单完整性缺口推断（结构性缺口 → 补充 规范化需求 回路） |

**Task 3 中 规范化需求 描述的三种处理情形**：

| 规范化需求 描述情况 | 处理方式 |
|------------|---------|
| 规范化需求 明确描述活动节点 | 原样捕获为 PL6-A 子节点 |
| 规范化需求 只提任务模式（如"需要审批"） | 判定类型，子节点标 `[待wft02推断]` |
| 规范化需求 缺失活动节点 | 推断类型，子节点标 `[待wft02推断]` |
...
| 8 | 基于 Bn 补充业务需求 | Bn | 详设层设计性缺口（PL6-B/指标/活动节点级）→ 生成补充 规范化需求 材料；结构性缺口已由 wft01 Bn 组装处理 |
...
| 8 | 引擎能力使用与指标分配 | — | 逐业务页面形成引擎能力使用需求，将 FR-BIZ 指标分配到既有引擎/CU并判定覆盖关系；能力或指标不足时生成 FR-ENG 候选线索（字段集见 wft03-biz §3.2），经 规范化需求预处理链转入 eng 路径 |

---

## 八、平台能力之系统设计

> eng 路径将引擎能力原始需求（FR-ENG 规范化需求节点）逐级转化为可交付开发的 pa-eng 组件需求定义（前端组件 + 后端组件 + 平台服务组件）。与 biz 路径并行但设计对象不同——biz 设计业务场景，eng 设计支撑业务场景的引擎能力。

```
FR-ENG 规范化需求节点 ──→ wft01-eng ──→ bph-eng ──→ wft02-eng ──→ bpd-eng ──→ wft03-eng ──→ srh-eng ──→ wft04-eng ──→ srd-eng ──→ wft05-eng ──→ pa-eng
...
|-------|---------|------------|-----------------|---------|
| **wft01-eng** | 向平台方证明"我听懂了你要的平台能力" | FR-ENG 规范化需求节点 条目 → bph-eng 业务流程概要方案 | 配置信息组（概要说明，PL5 末级，业务流程概要定义末级节点；bph-eng 架构末级=场景业务 SL5）+ CU 承接 + 依赖 bph-eng（共享依赖） | 业务配置人员确认 → wft02-eng |
...
**将零散的 FR-ENG 规范化需求节点 组织为结构化的平台能力场景业务方案（业务流程概要定义，01 层——概要说明为结构判定级，操作活动定义留 02 展开），供业务配置人员确认理解。**

```
输入                              产出
FR-ENG 规范化需求节点 条目（待处理）  ──→  bph-eng 平台能力场景业务方案（可以详细定义）
...
| 4 | 建立追溯链 | FR-ENG 规范化需求节点 → bph-eng主责能力域 → 配置信息组（概要说明） → CU承接/依赖bph-eng；共享能力记录提供能力、消费方和复用范围 |
...
| # | 任务 | 说明 |
|---|------|------|
| 1 | 推断非功能需求概要定义节点 | 识别 规范化需求 背后的用户角色、业务域/平台能力语境、NFR 一级类目与分类维度，形成 bph-nfr 末级节点 |
| 2 | 推断 NFR 分类需求清单 | 将 规范化需求 中的质量诉求归纳为分类需求项——定性期望、适用范围线索、来源类型（`规范化需求明示` / `规范化需求隐含` / `同类语境推断` / `资产线索推断`）、待量化线索。wft01 只记录 规范化需求 中相关方已确认的目标值（数值/比较词/验证条件），**不将其转为系统量化指标**；系统侧指标设计（统计口径/测量方法/验收条件）由 wft04 负责 |
...
> 规范化需求 按类型分流至三条执行路径，每条路径内的 Skill 形成串行接力链——上游 Skill 产出节点，下游 Skill 消费节点后展开为下一层节点。三路径不是三条都独立通向 pa-eng 的产品链：biz 终止于业务配置需求，eng 形成平台 pa-eng，nfr 形成 SysReq-NFR 与约束包并汇入 wft05-eng。全链路最终产出两套可交付构件开发的成果。各路径 Skill 清单见 §10.3~§10.5，完整参数见 §10.6。
...
```
准备阶段
  规范化需求 归一化（ort/pre Skill）→ 规范化需求条目状态 = 待处理
  规范化非功能需求 同理

第一轮：三路径并行启动
  biz 线：FR-BIZ 规范化需求 → wft01-biz → wft02-biz → wft03-biz → 业务配置需求方案
  eng 线：FR-ENG 规范化需求节点 → wft01-eng → wft02-eng → wft03-eng → wft04-eng →（等待 FR-BIZ 指标约束包与 wft05-eng NFR 约束包）
...
| **新输入增量** | 上游产出新一批输入节点（如新 规范化需求条目、新 bph-biz 节点），状态符合消费条件 | 增量展开新节点，已有方案不受影响 |
...
同一 Skill 可能在同一批 规范化需求 处理期间被调用 2~5 轮（反馈迭代），也可能在几小时到几周的时间跨度内被调用数十次（新输入分批到达、上游变更级联），每次处理一小批节点，逐步累积形成完整的架构方案。
...
| eng 线 ← biz 线 wft03-biz | wft03-biz 产出 FR-ENG 候选线索 | 非阻塞；候选线索经人类确认并完整通过 ort00→ort03 形成FR-ENG 规范化需求节点 后，才能进入 wft01-eng |
...
|-------|---------|---------|---------|--------|
| wft01-biz | 文档（PL5，概要末梢） | PL6-A 活动节点 / PL6-B 正文条目初步判定（结构判定级） | 基于 规范化需求 推断**业务流程概要定义**（s01）——场景业务（枝）+ 输出文档序列 + **Bn 组装**（WBS 归组/23 一致性/按构件产品数据清单完整性缺口推断），结构性缺口补充 规范化需求 回路——"向业务方证明我听懂了" | §七 |
...
| Skill | 末级节点 | 细分节点 | 职责定位 | 所属章 |
|-------|---------|---------|---------|--------|
| wft01-eng | 配置信息组（概要说明，PL5，业务流程概要定义末级节点） | 主责能力域 / 配置信息组（概要说明） / CU 承接 / 依赖 bph-eng（bph-eng 架构末级=场景业务） | 基于 FR-ENG 规范化需求节点 推断可独立配置、治理和验收的平台能力场景 + 名称级识别并分解配置信息组（概要说明） + 分配配置信息组到 CU 承接——"向平台方证明我听懂了"（业务流程概要定义，01 层） | §八 |
...
| 维度 | wft01-biz（规范化需求→bph-biz） | wft02-biz（bph-biz→bpd-biz） | wft03-biz（bpd-biz→配置需求方案） |
|------|----------------------|----------------------|----------------------------|
| 末级节点 | 文档（PL5，结构判定级概要说明） | 输出文档（PL5，细分 PL6-A/PL6-B + 表单使用顺序） | 功能表单（窗口标签页为页面组件承载；业务配置需求 + 配置业务系统 pa-eng 双重身份） |
| 输入对象 | FR-BIZ 规范化需求条目（≤10） | bph-biz 节点（≤5） | bpd-biz 节点（≤5） |
...
| 补充材料 | 补充 规范化需求 材料（闭合缺口 + Bn 组装结构性缺口） | FR-BIZ 规范化需求 材料（详设层设计性缺口） | FR-ENG 候选线索（引擎缺口） |

**eng 路径**：

| 维度 | wft01-eng（规范化需求→bph-eng） | wft02-eng（bph-eng→bpd-eng） | wft03-eng（bpd-eng→系统需求概要 srh-eng） | wft04-eng（系统需求概要→系统需求详细 srd-eng） | wft05-eng（系统需求详细→pa-eng） |
|------|----------------------|----------------------|---------------------|---------------------|---------------------|
| 末级节点 | 配置信息组（概要说明，PL5，业务流程概要定义末级节点） | 配置信息组（操作活动定义） | 功能表单（概要说明） | 功能表单（组件业务） | pa-eng 组件（前端/后端/平台服务） |
| 输入对象 | FR-ENG 规范化需求节点 条目（≤10） | bph-eng 节点（≤5） | bpd-eng 节点（A1，≤5） | 系统需求概要 srh-eng（≤5） | 系统需求详细 srd-eng + FR-BIZ 指标约束包 + wft05-eng NFR 约束包 |
...
| 维度 | wft01-nfr（规范化需求→bph-nfr） | wft04-nfr（bph-nfr→SysReq-NFR） |
|------|--------------------------|----------------------------------|
...
| 编号 | 文件名 | 说明 |
|------|--------|------|
| 01 | `01-eos-specified-requirements.md` | 规范化需求 |
...
| 前缀 | 所属 | 示例 |
|------|------|------|
| `@spr-` | 规范化需求 节点（ort03 规范化产出，seq 编号） | `@spr-001` |
...
| 特化项 | 说明 |
|--------|------|
| 输入对象类型 | 如 bph-biz 节点、bpd-biz 节点、规范化需求条目 |
...
> | **状态扫描 · 双轨** | `eos-ort02-clarify` / `eos-ort03-norm` | 状态表 / 文档标记（ort02 切分文件正文区 `[✓已澄清]` + 表 B；ort03 规范化需求基线反馈区 `[同意执行]`/`[修改建议]`/`[废弃]` + 表 B/C） | Step 1 状态扫描检出 → 登记表 B `内容摘要` → Step 2 基于记录执行；Step 5 对话反馈即时处理 + `[已处理]`，Step 6「整体确认」提交基线 |
>
