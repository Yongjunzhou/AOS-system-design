...
> **文档定位**：EOS 系统设计端到端追溯关系的统一索引文件。本文档不复制 `01~07` 的正文，而是记录 规范化需求、STR、BP、SysReq、平台产品架构 和资产文档之间的关系，支持按源节点、目标节点、关系类型和问题状态进行局部读取。
...
| 层级 | 源文档 | 目标文档 | 关系目标 |
|------|--------|----------|----------|
| 规范化需求 → 业务概要 | `01-eos-specified-requirements.md` | `02-eos-business-summary-architecture.md` | 规范化需求 末级分配到 业务概要架构末级 |
...
| 关系类型 | 含义 | 典型场景 |
|----------|------|----------|
| `derived_from` | 从上游材料或节点推导而来 | 业务概要 derived_from 规范化需求 |
| `refines` | 对上游架构节点做详细化 | 业务概要详细 refines 业务概要 |
| `allocated_to` | 上游需求分配到下游方案节点 | 规范化需求 allocated_to 业务概要 |
...
|--------|----------|------|--------------|--------|------|
| TRACE-SHARD-spr-BPH | 规范化需求 → 业务概要 | 原始需求到相关方需求架构 | `wft01` 验证 规范化需求 分配 | — | `4.1 规范化需求 到业务概要关系块` |
...
| 层间映射 | 检查项 | 期望结果 |
|----------|--------|----------|
| 规范化需求 末级 → 业务概要架构末级 | 每条 规范化需求 末级分配到唯一 业务概要架构末级 | 一条 规范化需求 不应同时分配给多个 业务概要 末级 |
...
|------|------|----------|----------|-----------|
| 2026-08-25 | v2.2 | 补链 | 追溯分片 ID「TRACE-SHARD-OR-BPH」→「TRACE-SHARD-spr-BPH」（OR→规范化需求 改名清扫补遗） | wft01-nfr 深审链级清扫 |
