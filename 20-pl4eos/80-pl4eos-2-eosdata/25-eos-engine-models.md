...
**与设计视角层级的映射**：本文档的引擎支持对象仍按业务本体 `L0~L6` 描述。wft01-biz / wft02-biz 可在设计过程中使用 `SL/PL/ML` 指代来源对象：`SL5 场景业务` 折回 `L4 场景业务`，由场景引擎支撑；`PL5 场景部件` 折回 `L5 场景部件`，由表单/流程/指标/工单等部件级引擎支撑；`PL6 功能元件` 折回 `L6 功能元件`，由实体引擎支撑；`ML1~ML4` 是菜单可达性路径，由菜单引擎维护，末级 `ML4` 映射到 `SL5 / L4 场景业务`。
