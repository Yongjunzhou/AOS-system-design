...
| 统计 | 引擎资产项 13 | 配置单元类型数 22 | 自研 13 | 外购 0 | 自研+外购 0 | 待确认 12 | 已确认 0 | 废弃 0 |
|------|---------|------------------|--------|--------|-------------|----------|----------|--------|
...
**输出文档**是 STR-F 的组成单元。每个输出文档沿三层装配链（详见 [§1.2](#12-引擎层次关系)）逐级生成：其条目化内容对应一个物理表（实体引擎），物理表转换为功能表单并由功能引擎按主子关系组装为主子表单。文档之间有先后依赖和流转关系（如申请单→审批记录→通知→闭环）。输出文档按业务输出物形态判定文档类型、开发方式和候选引擎支撑链，一个文档只对应一种文档类型。

输出文档按生成方式分为两类：

**内置 CAX**（平台内引擎生成）：输出文档的内容由 EOS 功能引擎直接生成，用户在平台上操作引擎界面完成。例如：流程引擎生成审批记录、台账引擎生成台账数据、指标引擎生成指标值。
...
STR-F 架构末级节点是场景业务（详见 [§1.4](#14-场景业务)），场景业务由输出文档序列细化定义——每个输出文档描述一件业务输出物的内容、流转关系和功能需求。输出文档按是否条目化且在 EOS 平台上运行管理，分两条路径推导其支撑引擎链（两种生成方式详见 [§1.4](#14-场景业务及输出文档)）：
...
|------|------|----------|----------|-----------|
| 2026-07-03 | v1.23 | 主子表单引擎→功能引擎合并 + §1 全章审视 | 主子表单引擎合并入功能引擎（三层装配链：实体引擎→功能引擎→菜单引擎）；§1 全章审视——§1 重命名"平台架构总览"、删除附录F（UI重复内容移入§1.5/§1.6）、新增四视角层级体系表（ML/SL/PL/EL）、取消 L0~L6 统一为 SL5、Mermaid 图补全11引擎双边连接、§1.6 重命名"输出文档生成方式"并聚焦生成方式、§1.7 输出文档推导补外置 CAX 分支、§2.7 主子表单引擎整节删除、§2.2 功能引擎扩展吸收组装能力、§2.1 菜单引擎依赖更新、§2.8→§2.7 重编号、附录E A4→A3、B.1/B.2/§4/规则区全量同步 | 人类审视+Codex |
