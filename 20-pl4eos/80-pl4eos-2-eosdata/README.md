...
本目录包含 EOS 企业运营体系的 9 份标准产品数据文档（01~09），按系统设计准则五层结构组织。另有 4 份跨层资产文档（21~23），以及 3 份供输出产品架构台账引用的独立资产清单（24~26）。
...
| 24 | [引擎模型资产清单](24-eos-engine-models.md) | 跨层（引用资产·待实例化） | 23 引用（来源模型） | 骨架 |
| 25 | [业务定义资产清单](25-eos-business-definitions.md) | 跨层（引用资产·待实例化） | 23 引用（业务定义复用） | 骨架 |
| 26 | [文档资产清单](26-eos-document-definitions.md) | 跨层（引用资产·待实例化） | 23 引用（输入/输出文档） | 骨架 |

> **说明**：01~09 为 EOS 标准设计产出，待系统设计迭代填充。21~23 为跨层资产（活跃），24~26 为台账引用的独立资产清单（骨架待实例化）。
