...
本目录包含 EOS 企业运营体系的 9 份标准产品数据文档（01~09），按系统设计准则五层结构组织。另有 5 份跨层资产文档（21~23 发现型资产 + 24~25 定义型资产），以及 1 份供输出产品架构台账引用的文档资产清单（26）。24 号为业务定义清单（A2 详细定义），25 号为引擎模型清单（A1 详细定义），与 23 号台账（架构目录）并列，共同构成完整的输出产品架构资产体系。
...
| 24 | [业务定义清单](24-eos-business-definitions.md) | 跨层（定义型资产·wft02 A2 写回目标） | wft02 | 活跃 |
| 25 | [引擎模型清单](25-eos-engine-models.md) | 跨层（定义型资产·wft02 A1 写回目标） | wft02 | 活跃 |
| 26 | [文档资产清单](26-eos-document-definitions.md) | 跨层（引用资产·待实例化） | 23 引用（输入/输出文档） | 骨架 |

> **说明**：01~09 为 EOS 标准设计产出，待系统设计迭代填充。21~23 为跨层资产（发现型·活跃），24~25 为跨层资产（定义型·活跃），26 为文档资产清单（骨架待实例化）。
