...
本目录包含 EOS 企业运营体系的 9 份标准产品数据文档（01~09），按系统设计准则五层结构组织。另有 4 份跨层资产文档（21~23），以及 1 份供输出产品架构台账引用的文档资产清单（26）。独立引擎模型和业务定义资产清单（24/25）已废弃，其内容并入 23 号台账的 13 列字段中统一管理。
...
| ~~24~~ | ~~[引擎模型资产清单](24-eos-engine-models.md)~~ | ~~跨层（已废弃，内容并入 23）~~ | — | 已废弃 |
| ~~25~~ | ~~[业务定义资产清单](25-eos-business-definitions.md)~~ | ~~跨层（已废弃，内容并入 23）~~ | — | 已废弃 |
| 26 | [文档资产清单](26-eos-document-definitions.md) | 跨层（引用资产·待实例化） | 23 引用（输入/输出文档） | 骨架 |

> **说明**：01~09 为 EOS 标准设计产出，待系统设计迭代填充。21~23 为跨层资产（活跃），24/25 已废弃（其内容并入 23 号台账字段），26 为文档资产清单（骨架待实例化）。
