...
| 中文 | 英文 | 说明 |
|------|------|------|
| **引擎** | Engine | EOS 平台的能力生成器。由 A1（流程与IT域）开发，包含若干可独立选配的**配置单元**（CU，IPO 单元）。业务配置人员（Bn）将业务需求转换为配置信息后，平台使用引擎生成对应业务。引擎分四级：组件级→部件级→单元级→元件级 |
| **业务** | Business | 引擎经配置后生成的可直接运行的实体，供业务人员在 EOS 平台上使用和操作。分四级：端到端业务→功能业务→功能模块→对象及其功能 |
| **配置单元** | Configuration Unit (CU) | 引擎提供的可独立配置的 IPO 单元。部分配置单元即可让业务运行（非全量必配），配置人员按需选配 |
| **组件级引擎** | Assembly Engine | 引擎 L1——场景引擎。生成端到端业务，对应场景菜单 |
| **部件级引擎** | Component Engine | 引擎 L2——菜单引擎。生成功能业务，对应功能菜单 |
| **单元级引擎** | Unit Engine | 引擎 L3——表单引擎、流程引擎。生成功能模块，是功能业务的构成 |
| **元件级引擎** | Part Engine | 引擎 L4——实体引擎。生成对象及其功能，是功能模块的构成 |
| **端到端业务** | End-to-End Business | 由多个功能业务编排组成，在 EOS 平台体现为场景菜单，是用户的顶层操作入口 |
| **功能业务** | Function Business | 由多个功能模块编排组成，在 EOS 平台体现为功能菜单，是用户的二级操作入口 |
| **功能模块** | Function Module | 由多个对象及其功能编排组成，是功能业务的构成部件，不直接体现为菜单 |
| **场景菜单** | Scenario Menu | 端到端业务的用户操作入口 |
| **功能菜单** | Function Menu | 功能业务的用户操作入口 |
...
**更新说明**：v2.2 术语重构——三术语（业务引擎/业务定义/业务实例）→两术语（引擎/业务）；新增引擎四级层次和业务四级编排关系。
