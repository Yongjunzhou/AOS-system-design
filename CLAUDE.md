...
| 子项目 | 位置 | 独立记忆 | 说明 |
|--------|------|---------|------|
| 教材子项目 | `50-research/` | `50-research/10-claude memory/` | 研究报告（`00-系统工程相关研究报告/`）→ 两个产出：教材书（`03-架构师是个怎样的物种/`）+ 培训教材（`02-架构工程师培训材料/`）。子项目导航与记忆隔离协议见 [`50-research/CLAUDE.md`](50-research/CLAUDE.md) |

### 执行路径约定

- **处理主项目（AOS）**：工作目录 = 仓库根，读本 CLAUDE.md + 主记忆（`.claude/memory/`）。**不触碰子项目目录内容**，除非用户明确要求。
- **处理子项目（如教材）**：先 `cd 50-research`（工作目录切到子目录），读 `50-research/CLAUDE.md` + `50-research/10-claude memory/`。**不把子项目内容写入主 `.claude/memory/`**；会话结束把最新状态同步回子项目记忆。
