...
51-AI部署研究/                           # AI 私有部署选型研究子项目（自包含子目录，见 §九 子项目）
52-健康研究报告/                         # 健康主题研究子项目（自包含子目录，见 §九 子项目）
...
| AI 部署研究子项目 | `51-AI部署研究/` | （尚未建立） | **研究型**子项目，企业私域大模型推理平台选型研究。现行 01 号选型研究报告、02 号专用模型训练与机载部署专项研究报告、11 号建设方案建议，落 `01-AI私有部署研究报告/` 与 `11-企业AI基础设施建设方案建议/`。子项目导航与记忆隔离协议见 [`51-AI部署研究/CLAUDE.md`](51-AI部署研究/CLAUDE.md) |
| 健康研究报告子项目 | `52-健康研究报告/` | （尚未建立） | **研究型**子项目，健康主题书稿讲读摘录。现行仅 `走路-被严重低估的生命体征.md` 一篇；用途边界、报告编号组织与后续内容形态待后续确定。子项目导航与记忆隔离协议见 [`52-健康研究报告/CLAUDE.md`](52-健康研究报告/CLAUDE.md) |
...
- 新子项目创建时：建子目录 → 建子 CLAUDE.md（复制隔离协议模板）→ 建 `10-claude memory/`（独立 MEMORY.md + 主记忆）→ 在本章指针区加一行。

---

## 十、工作风格

- **禁止选项式交互**：讨论/确认事项时一律自由散文陈述（背景 + 问题 + 建议 + 理由），不用选项卡片/选项卡界面让用户做多选题；用户自由回答"改/不改/怎么改"，再据其指示执行（2026-08-25 用户定）。教材子项目侧同步记录于 [`50-系统工程研究/10-claude memory/feedback-no-option-interaction.md`](50-系统工程研究/10-claude%20memory/feedback-no-option-interaction.md) 及 [`50-系统工程研究/CLAUDE.md`](50-系统工程研究/CLAUDE.md) 工作风格节。
