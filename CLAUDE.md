...
50-reserch/                         # 教材子项目（自包含子目录，见 §九 子项目）
40-review/                          # 审查与评审（待补充）
90-hold/                            # 暂存归档
80-sessions/                        # 历史会话记录
```
...
| 子项目 | 位置 | 独立记忆 | 说明 |
|--------|------|---------|------|
| 教材子项目 | `50-reserch/` | `50-reserch/记忆/` | 概念研习笔记（01~08）→ 两个产出：教材书（`教材书/`）+ 培训教材（`培训教材/`）。子项目导航与记忆隔离协议见 [`50-reserch/CLAUDE.md`](50-reserch/CLAUDE.md) |

### 执行路径约定

- **处理主项目（AOS）**：工作目录 = 仓库根，读本 CLAUDE.md + 主记忆（`.claude/memory/`）。**不触碰子项目目录内容**，除非用户明确要求。
- **处理子项目（如教材）**：先 `cd 50-reserch`（工作目录切到子目录），读 `50-reserch/CLAUDE.md` + `50-reserch/记忆/`。**不把子项目内容写入主 `.claude/memory/`**；会话结束把最新状态同步回子项目记忆。
