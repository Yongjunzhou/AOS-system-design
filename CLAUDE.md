...
3. **事实记忆用主题名，前缀表封闭**——只允许 `feedback-`（用户给的工作方式指示）、`pending-`（挂账与未决）与 `progress-<对象>`（进行中的工作状态）；其余用主题名本身，不自造前缀。三类各自的记法与读写触发见 [`AGENTS.md` §八](AGENTS.md)。
4. **文件名不带日期**——日期由索引行的标题承载。
5. **语言随性质**——**内容型**（主记忆、事实记忆）随所在区：主项目区用英文 kebab-case，子项目区用中文自然名；**设施型**（记忆根目录名、`feedback-`／`pending-`／`progress-` 前缀项）用英文。
6. **分隔符一律连字符 `-`**。
7. **存量文件名不迁移**；以上各条约束新增。
...
| 名称 | 位置 | 性质 | 独立记忆 | 说明 |
|--------|------|------|---------|------|
| 杂项选题 | `100-subprojects/00-topic-research/` | 杂项 | `claude-memory/` | 未定形选题的暂存与孵化处，**不算子项目**；判据是「该话题是否需要继续深入研究」——需要则移出独立为 `NN-` 子项目。现行内容：运营体系构建、指标与看板、目标价格管理、91 规范第十一章讨论、产品数据条目化树形化及向后分配研究报告（学习材料，不作项目依据）。见 [`CLAUDE.md`](100-subprojects/00-topic-research/CLAUDE.md) |
