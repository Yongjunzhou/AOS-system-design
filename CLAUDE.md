...
50-系统工程研究/                         # 教材子项目（自包含子目录，见 §九 子项目）
61-投资工程研究/                         # 投资工程研究子项目（自包含子目录，见 §九 子项目）
40-review/                          # 审查与评审（待补充）
90-hold/                            # 暂存归档
80-sessions/                        # 历史会话记录
```
...
| 子项目 | 位置 | 独立记忆 | 说明 |
|--------|------|---------|------|
| 教材子项目 | `50-系统工程研究/` | `50-系统工程研究/10-claude memory/` | 研究报告（`00-系统工程相关研究报告/`）→ 两个产出：教材书（`03-架构师是个怎样的物种/`）+ 培训教材（`02-架构工程师培训材料/`）。子项目导航与记忆隔离协议见 [`50-系统工程研究/CLAUDE.md`](50-系统工程研究/CLAUDE.md) |
| 投资工程研究子项目 | `61-投资工程研究/` | `61-投资工程研究/10-claude memory/` | **产出型**子项目，路径「材料准备 → 输出」。当前处**材料准备期**，材料落 `00-投资工程研究报告/`（播客/访谈转写稿、研究稿）；产出形态待后续确定。子项目导航与记忆隔离协议见 [`61-投资工程研究/CLAUDE.md`](61-投资工程研究/CLAUDE.md) |

### 执行路径约定

- **处理主项目（AOS）**：工作目录 = 仓库根，读本 CLAUDE.md + 主记忆（`.claude/memory/`）。**不触碰子项目目录内容**，除非用户明确要求。
- **处理子项目（如教材、投资工程研究）**：先 `cd <子项目目录>`（工作目录切到子目录），读该目录的 `CLAUDE.md` + 其 `10-claude memory/`。**不把子项目内容写入主 `.claude/memory/`**；会话结束把最新状态同步回子项目记忆。
...
- **禁止选项式交互**：讨论/确认事项时一律自由散文陈述（背景 + 问题 + 建议 + 理由），不用选项卡片/选项卡界面让用户做多选题；用户自由回答"改/不改/怎么改"，再据其指示执行（2026-08-25 用户定）。教材子项目侧同步记录于 [`50-系统工程研究/记忆/feedback-no-option-interaction.md`](50-系统工程研究/记忆/feedback-no-option-interaction.md) 及 [`50-系统工程研究/CLAUDE.md`](50-系统工程研究/CLAUDE.md) 工作风格节。
