...
THREE_DIGIT=$(grep -oP '[A-Z]+-[0-9]{3}' "$TARGET" | sort -u | head -20)
DOT_TWO=$(grep -oP '[A-Z]+-[0-9]+\.[0-9]{2}' "$TARGET" | sort -u | head -20)
DOT_ONE=$(grep -oP '[A-Z]+-[0-9]+\.[0-9]\b' "$TARGET" | grep -v '\.[0-9]0\b' | sort -u | head -20)
