...
| 三层工作层面 | Three Work Levels | 第1层自指设计（产品L设计自身）、第2层交付物构建（产品L的构件交付给团队M）、第3层使用工具设计产品M |
| 跨领域通用 | Cross-domain General | 信息处理模式中前9类可在不同业务领域间共享同一信息化平台 |
| 领域特有 | Domain-specific | 信息处理模式中后4类因领域差异需专用系统实现 |
| 自指关系 | Self-Reference | 运营体系包含设计自身的规则和工具——产品L（系统设计流水线）支撑"设计运营体系本身"这项业务 |
...
| 知识组件的双重身份 | Dual Identity of Knowledge Components | 产品M的知识组件（准则/指南/AI辅助/任务定义）同时是产品L的PA构件交付物和产品M的设计工具输入 |
