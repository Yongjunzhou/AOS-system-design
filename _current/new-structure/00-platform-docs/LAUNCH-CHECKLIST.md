...
```bash
# 创建所有任务目录及其子目录
mkdir -p 01-normalization/{guidelines,templates,checklists,skills,workflows,examples}
mkdir -p 02-sr-ba-design/{guidelines,templates,checklists,skills,workflows,examples}
mkdir -p 03-ba-sysreq-design/{guidelines,templates,checklists,skills,workflows,examples}
mkdir -p 04-sr-nfr-design/{guidelines,templates,checklists,skills,workflows,examples}
mkdir -p 05-sysreq-pa-design/{guidelines,templates,checklists,skills,workflows,examples}
mkdir -p 06-traceability-analysis/{guidelines,templates,checklists,skills,workflows,examples}
```
...
### 保留原有目录
- 在迁移完成前，**不要删除**原有的 `01-standards/`、`02-pipeline/`、`03-products/`、`00-platform-docs/` 目录
- 直到第五阶段验证完成后，才能考虑删除
