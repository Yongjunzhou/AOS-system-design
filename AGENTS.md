...
各 Agent 工具自带的记忆机制是另一回事（例如 Codex 写在 `~/.codex/memories/` 下、按用户与机器各存一份，DSH 没有记忆机制），它们与本仓库的记忆**互不相通**。故本仓库**不设镜像、不做复制**：`.claude/memory/` 是唯一存储，任何 Agent 都读写这一处，不另建副本。工具自动加载能力的有无，见「跨 Agent 使用约定」。
...
**开工先读。** 在本仓库做任何事之前，先读 `.claude/memory/MEMORY.md` 索引，再按索引指向展开与本次范围相关的记忆；子项目任务改读该项目目录下 `claude-memory/MEMORY.md`。这一步不依赖工具是否自动加载：自动加载有，算省一次读取；没有，就必须显式读。只读索引不读正文，等于没读——在办的挂账、既有定案与工作方式指示都在正文里。

**收尾写回，四条触发。** 只要本轮出现下列任一情况，就必须在收尾前写回记忆，不等催促：人类做出裁决或定案；出现新的挂账或未决项；完成一轮可交付的修订（含已提交的改动）；会话即将结束。四条都写成可自查的问句："本轮有没有出现新的人为决断？有，就落一条。"

**记哪些、记在哪。** 记忆分三类，各用各的前缀：

| 类型 | 前缀 | 记什么 |
|------|------|--------|
| 工作方式指示 | `feedback-` | 人类给的工作方式与偏好（禁用选项式交互、先发现后修改等） |
| 挂账与未决 | `pending-` | 定了但未落、或启动须点头的一件事 |
| 进行中的工作 | `progress-<对象>` | 某个对象做到哪、下一步从哪继续、待裁与阻塞、改动与提交号 |

`pending-` 记"一件事的未决状态"，`progress-` 记"一个对象进行到哪一步"，两者都会随工作推进而改，收口后可并入正式记忆或删除。

**每条写回都同步索引。** 新增记忆后，在 `MEMORY.md` 加一行：链接 ＋ 一句话摘要（含日期与结论）。索引与文件脱节，下一个人就找不到。

**不另建副本。** 任何 Agent 都不得在别处复制记忆或另建一套并行记忆；工具自带的记忆机制与本目录互不相通，不能替代这里。
